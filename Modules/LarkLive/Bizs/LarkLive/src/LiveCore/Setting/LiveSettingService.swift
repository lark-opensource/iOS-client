...
class LiveSettingService {

    static let logger = LKCommonsLogging.Logger.log(LiveSettingService.self, category: "LarkLive")
    ...
    public func setup() {
        LiveSettingService.logger.info("LiveDataService setup")
        fetchSettings()
    }
    ...
    private func fetchSettings() {
        ...
        ConfigRequestor.requestSettings([.liveSettings, .quicFallbackSettings, .quicLibraAB, .nodeOptimizeConfig]) { [weak self] r in
        
            let fieldGroups = r.value?.fieldGroups
            #if DEBUG
            Self.logger.info("LiveDataService success get fieldGroups: \(fieldGroups)")
            #endif
            
            if let value = fieldGroups?[VCSettingsField.liveSettings.rawValue], let obj = VCSettingsField.liveSettings.decode(value, type: NewLarkLiveSettings.self) {
                self?.liveSettings = obj
                Self.logger.info("LiveDataService Live settings link: \(obj)")
            }
        }
    }
        ...
        if isFeishuHost {
            ...
            for pattern in regexes {
                if let regexExpression = try? NSRegularExpression(pattern: pattern, options: [])  {
                    let range = NSRange(location: 0, length: currentUrlPath.utf16.count)
