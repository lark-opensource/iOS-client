...
class IDPNativeService: IDPLoginServiceProtocol {
    ...
    private var webViewContainerService: IDPNativeContainerService?
...
//    private var needRetry: Bool = true
    ...
    static let logger = Logger.plog(IDPWebViewService.self, category: "SuiteLogin.IDP.IDPNativeService")
...
//        }
    ...
    func loginPageForIDPName(
        ...
        let webViewContainerService = IDPNativeContainerService()
...
extension IDPNativeService {
    ...
    private func handleWeb(_ error: Error) {
        ...
        if let handler = self.webErrorHandler {
            DispatchQueue.main.async {
                handler.vc = PassportNavigator.topMostVC
    ...
    private func handle(_ error: Error) {
        ...
        //showFailView 存在present的逻辑，不延迟的话 topMostVC获取的是错误的，将无法展示alert；后续IDP逻辑重构时一并解决
...
//            }
    ...
    private func showLoading() {
        DispatchQueue.main.async {
            guard let topMostVC = PassportNavigator.topMostVC else {
                IDPWebViewService.logger.error("n_action_idp_topMostVC_nil")
                return
            }
            RoundedHUD.showLoading(on: topMostVC.view)
        }
    }
    ...
    private func hideLoading() {
        DispatchQueue.main.async {
            ...
            RoundedHUD.removeHUD(on: topMostVC.view)
        }
    }
...
extension IDPNativeService: IDPBridgeServiceProtocol {
