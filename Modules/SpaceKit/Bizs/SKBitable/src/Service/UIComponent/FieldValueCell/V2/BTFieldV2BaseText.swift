...
class BTFieldV2BaseText: BTFieldV2Base, BTTextViewDelegate, UITextViewDelegate, BTFieldTextCellProtocol {
    ...
    func resetTypingAttributes() {
        textView.typingAttributes = BTFV2Const.TextAttributes.fieldValue
    }
    ...
    private func updatePlaceHolder() {
        textView.placeholderLabel.attributedText = fieldModel.placeHolderAttrText
        // 刷新placeholder布局
    ...
    func startKeyboardObserver() {
        ...
        keyboard.on(events: [.didShow, .willShow]) { [weak self] options in
    ...
    func handleKeyboardShow(options: Keyboard.KeyboardOptions) {
        // 留给子类实现
        return
    }
}
...
extension BTFieldV2BaseText: UITextPasteDelegate {
