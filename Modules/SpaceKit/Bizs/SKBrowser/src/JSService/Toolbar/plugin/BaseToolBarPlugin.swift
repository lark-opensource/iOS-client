...
extension DocsJSService {
    static let docToolBarJsName = DocsJSService("biz.navigation.setDocToolbar")     //docs工具栏
    static let sheetToolBarJsName = DocsJSService("biz.navigation.setSheetToolbar") // sheet @Docs工具栏
    ///    docToolBarJsNameV2包含了docToolBarJsName&sheetToolBarJsName,
    ///    内部会转成setDocToolbar&setSheetToolbar去适配旧的逻辑
    ///    V3.30开始加入V2接口
    static let docToolBarJsNameV2 = DocsJSService("biz.navigation.setDocToolbarV2")
    ///    mideNote的工具栏 ， V2是专门给ipad适配用的
    static let mindnoteToolBarJsName = DocsJSService("biz.navigation.setMindnoteToolbar")
    static let mindnoteToolBarJsNameV2 = DocsJSService("biz.navigation.setMindnoteToolbarV2")
    //docs iPad工具栏
    static let docToolBarForIpadJsName = DocsJSService("biz.navigation.setDocToolbarForIPad")
}
// swiftlint:disable type_body_length
public final class SKBaseToolBarPlugin: JSServiceHandler {
    ...
    enum ToolType: Int {
        ...
        case EQUATION = 4 //公式
        case MINDNOTE = 5
        case NOTShowToolBar = 6 //特殊的case不展示工具栏，跟公式类似
    }
    ...
    public init(_ config: SKBaseToolBarConfig, _ model: BrowserModelConfig) {
        self.config = config
        self.userResolver = model.userResolver
    ...
    struct InputData: HandyJSON {
        var value: [[String: Any]] = []              //局部样式
        var style: SheetStyleJSON?    //单元格样式
        var realValue: [SheetSegmentBase] = []
        var cellInfo: String = "" //cellID,编辑后回传给web，检测是否在同一个cell上update
        
        mutating func didFinishMapping() {
            var sink = [SheetSegmentBase]()
            for currentRawValue in value {
                let currentSegmentBase = SheetSegmentBase.deserialize(from: currentRawValue) ?? SheetSegmentBase()
                switch currentSegmentBase.type {
                case .text:
                    sink.append(SheetTextSegment.deserialize(from: currentRawValue) ?? SheetTextSegment())
                case .url:
                    sink.append(SheetHyperLinkSegment.deserialize(from: currentRawValue) ?? SheetHyperLinkSegment())
                case .mention:
                    sink.append(SheetMentionSegment.deserialize(from: currentRawValue) ?? SheetMentionSegment())
                case .attachment:
                    sink.append(SheetAttachmentSegment.deserialize(from: currentRawValue) ?? SheetAttachmentSegment())
                case .embedImage:
                    sink.append(SheetEmbedImageSegment.deserialize(from: currentRawValue) ?? SheetEmbedImageSegment())
                case .pano:
                    sink.append(SheetPanoSegment.deserialize(from: currentRawValue) ?? SheetPanoSegment())
                }
            }
            realValue = sink
        }
    ...
    struct ToolbarAttachData: HandyJSON {
        var input: InputData?
    }
    ...
    public func handle(params: [String: Any], serviceName: String) {
        if insertBlockPlugin.canHandle(serviceName) {
            insertBlockPlugin.handle(params: params, serviceName: serviceName)
            return
        }
        ...
        guard let callback = params["callback"] as? String,
            ...
            DocsLogger.info("KeyboardPlugin: \(serviceName) 关闭工具栏", component: LogComponents.toolbar)
            OnboardingManager.shared.targetView(for: [.docToolbarV2BlockTransform,
                                                      .docToolbarV2AddNewBlock,
                                                      .docTodoCenterIntro,
                                                      .docToolbarV2Pencilkit,
                                                      .docInsertTable],
                                                updatedExistence: false)
                currentTool.remove(mode: .toolbar)
                removeAllToolBarView()
                let type = params["type"] as? Int
            if type != ToolType.EQUATION.rawValue, type != ToolType.NOTShowToolBar.rawValue {
                    ...
                    DocsLogger.info("前端调用了只展示键盘不展示工具栏", component: LogComponents.toolbar)
                }
            return
        }
        jsMethod = callback
        ...
        DocsLogger.info("DocsToolbar serviceName:\(serviceName) showingItemID:\(showingItemID ?? "")", component: LogComponents.toolbar)
        ...
        case .docToolBarJsNameV2, .docToolBarForIpadJsName, .mindnoteToolBarJsNameV2:
            ...
            if toolBarInfos.count > 0 {
                ...
                if !(showingItemID?.isEmpty ?? false) {
                    toolBar.willRestoreByKeyboard = false
                }
                if editModeV2(type) != .sheetInput { toolBar.resetSheetStatus() }
                //这个要改 first timer 怎么判断
                if let data = ToolbarAttachData.deserialize(from: params["data"] as? [String: Any]) {
                    let segmentArr = data.input?.realValue
                    let style = data.input?.style
                    let attStr = SheetFieldDataConvert.convertSegmentToAttString(from: segmentArr, cellStyle: SheetCustomCellStyle(style ?? SheetStyleJSON()))
                    let cellID = data.input?.cellInfo
                    let sheetDocsInfo = self.getblockDocsInfo(params: params) //区分syncedBlock权限
                    toolBar.beginSheetEdit(with: attStr,
                                           animated: true,
                                           segmentArr: segmentArr,
                                           sheetCellStyle: style,
                                           sheetCellID: cellID,
                                           sheetDocsInfo: sheetDocsInfo)
                    pluginProtocol?.didReceivedOpenToolBarInfo(firstTimer: false, doubleClick: false)
                    pluginProtocol?.didReceivedInputText(text: attStr != nil)
        ...
        default:
            ...
            if toolBarInfos.count > 0 {
                ...
                if let inputDict = params["input"] as? [String: Any] {
                    toolBar.beginSheetEdit(with: nil, animated: true)   //  不影响
    ...
    func relayoutAttachToolBar() {
        ...
        case .EQUATION, .MINDNOTE, .NOTShowToolBar:
            break
        }
    ...
    func unfoldAllTitleToolBar() {
        ...
        if lastTitleIndex > 0 && h4Index < lastTitleIndex {
            ...
            hnItemInfo.childrenIsShow = showingItemID == BarButtonIdentifier.hn.rawValue
    ...
    func unfoldTitleToolBar() {
        ...
        guard foldIndexs >= 0, foldIndexs < realToolBarInfos.count else { return }
        ...
        if count > 0 && h4Index <= foldIndexs && lastTitleIndex >= foldIndexs {
            //将放不下的H菜单收到Hn的子菜单中
            let hnItemInfo = ToolBarItemInfo(identifier: "hn")
            hnItemInfo.jsMethod = jsMethod
            hnItemInfo.parentIdentifier = BarButtonIdentifier.blockTransform.rawValue
    ...
    func mergeAttachedItemsIntoMainToolbar(for indentifier: String) {
        ...
        if indentifier == BarButtonIdentifier.blockTransform.rawValue {
            //blockTransform在一级工具栏里展开时需要做一下去重，因为blockTransform的children和一级工具栏里都会有checkbox、checklist等item。
            realToolBarInfos = realToolBarInfos.uniqued(by: { $0.identifier })
        }
    }
    ...
    private func updateMainTBViewV2(_ status: [ToolBarItemInfo]) {
        if let view = mainTBPanel {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + DispatchQueueConst.MilliSeconds_1000) {
                OnboardingManager.shared.targetView(for: [.docToolbarV2BlockTransform, .docToolbarV2AddNewBlock, .docToolbarV2Pencilkit], updatedExistence: true)
            }
        }
    }

    private func updateDisplaySubTBView() {
        guard let displayView = subDisplayPanel, displayView.superview != nil else {
            subDisplayPanel = nil
            return
        }
    }
    ...
    private func updateSubTBView(_ status: [ToolBarItemInfo]) {
        for item in status {
            _setSubTBViewWithItemInfo(item)
    ...
    // 获取SubTBView（判断是否重用）
    private func _setSubTBViewWithItemInfo(_ itemInfo: ToolBarItemInfo) {
        // 重用逻辑,需要外部元素注入
        if let view = subTBPanels[itemInfo.identifier], let children = itemInfo.children {
            view.updateStatus(status: DocsSubToolBar.statusTransfer(status: children))
            return
        }
        // 重用逻辑,不需要外部元素注入
        if subTBPanels[itemInfo.identifier] != nil, itemInfo.identifier == BarButtonIdentifier.insertImage.rawValue {
            return
        }
        // 不重用 | 重新构建
        if let newView = config.uiCreater?.updateSubToolBarPanel(itemInfo.children, identifier: itemInfo.identifier, curWindow: self.currentWindow) {
            newView.panelDelegate = self
            if itemInfo.identifier == BarButtonIdentifier.attr.rawValue || itemInfo.identifier == BarButtonIdentifier.textTransform.rawValue {
                subTBPanels[BarButtonIdentifier.attr.rawValue] = newView
                subTBPanels[BarButtonIdentifier.textTransform.rawValue] = newView
            } else {
                subTBPanels[itemInfo.identifier] = newView
            }
        }
    }
    ...
    private func _getSubTBView(_ identifier: String) -> SKSubToolBarPanel? {
        if let view = subTBPanels[identifier] {
            return view
        } else if let view = config.uiCreater?.updateSubToolBarPanel([], identifier: identifier, curWindow: self.currentWindow) {
    ...
    private var currentWindow: UIWindow? {
        self.config.hostView?.window ?? self.userResolver.navigator.mainSceneWindow
    }
}

extension SKBaseToolBarPlugin: SKAttachedTBPanelDelegate {
...
//        }
        ...
        if item.identifier == BarButtonIdentifier.backColor.rawValue
            ...
            if let valueJSON = item.valueJSON, let openPanel = valueJSON["openPanel"] as? Bool, openPanel {
                ...
                if isIPadToolbar {
                    ...
                    let panel = ColorPickerView(colorPickerPanelV2: colorPickerPanelV2, userResolver: userResolver, width: mainTBPanel?.bounds.width ?? 0)
                    pluginProtocol?.requestDisplaySubTBPanel(panel, info: item)
                    attributionView?.setColorPickerUpV2()
                    subDisplayPanel = panel
                } else {
    ...
    private func displayFontColorPickerView(item: ToolBarItemInfo, value: Any? = nil, level: Int) {
        //fix: sheet@doc里多选单元格，使用字体或背景颜色的样式，会直接应用默认的颜色
        if item.identifier == BarButtonIdentifier.foreColor.rawValue {
            let params: [String: Any] = ["id": item.identifier,
                                         "level": level]
            pluginProtocol?.callFunction(DocsJSCallBack(item.jsMethod), params: params, completion: nil)
...
extension SKBaseToolBarPlugin: SKSubTBPanelDelegate {
...
// Todo: 这可以做成一个更抽象的 Delegate，
// 统一管理 SKBaseToolBarPlugin 的子插件的回调
extension SKBaseToolBarPlugin: HighlightPanelPluginDelegate {
...
extension SKBaseToolBarPlugin: InsertBlockPluginDelegate {
    ...
    func setShouldInterceptEvents(to enable: Bool) {
        tool?.setShouldInterceptEvents(to: enable)
    }
}
extension SKBaseToolBarPlugin {
    //展开工具栏对应菜单所需宽度
...
extension SKBaseToolBarPlugin: InsertColorPickerDelegate {
    public func didSelectBlock(id: String) {
        pluginProtocol?.requestDismissViewController(completion: nil)
        DocsMainToolBarV2.hasPresentHighlightPanle = false
...
extension SKBaseToolBarPlugin: BrowserViewLifeCycleEvent {
    ...
    private func refreshLayout() {
        if subDisplayPanel is ColorPickerView {
            attributionView?.colorPickerPanelV2.refreshViewLayout()
        }
    }
}
