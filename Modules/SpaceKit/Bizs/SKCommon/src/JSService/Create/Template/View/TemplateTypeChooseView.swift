...
protocol TemplateTypeChooseViewDelegate: AnyObject {
    func templateTypeChooseView(_ templateTypeChooseView: TemplateTypeChooseView, didClickTypeAt index: Int)
}

/// 一级分类选择
class TemplateTypeChooseView: UIView {
    weak var delegate: TemplateTypeChooseViewDelegate?
    private let blueLineView: UIView = {
        let line = UIView()
        line.layer.cornerRadius = 2
        line.backgroundColor = UDColor.primaryContentDefault
        return line
    }()
    ...
    private func select(at index: Int, animated: Bool, needNotifyDelegate: Bool) {
        ...
        if needNotifyDelegate {
            delegate?.templateTypeChooseView(self, didClickTypeAt: index)
        }
    }
    ...
    private func didClickTypaNameLabel(tap: UITapGestureRecognizer) {
        ...
        select(at: itemView.tag, animated: false, needNotifyDelegate: true)
    ...
    private class ItemView: UIView {
        ...
        override init(frame: CGRect) {
            ...
            label.snp.makeConstraints { make in
                make.edges.equalTo(UIEdgeInsets(top: 0, left: Self.margin, bottom: 0, right: Self.margin))
            }
        }
