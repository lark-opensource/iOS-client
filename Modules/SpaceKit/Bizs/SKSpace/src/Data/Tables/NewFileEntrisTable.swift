...
public final class NewFileEntrisTable {
    var db: Connection?
    let table: Table!
    private let name = Expression<String?>("name")
    private let objtoken = Expression<String>("obj_token")
    private let realToken = Expression<String>("real_token")
    ...
    public let secureLabelName = Expression<String?>(FileListServerKeys.secureLabelName.rawValue)
    public let secretKeyDelete = Expression<Bool?>(FileListServerKeys.secretKeyDelete.rawValue)
    private let objBizType = Expression<Int?>(FileListServerKeys.objBizType.rawValue)
    private let bizNodeToken = Expression<String?>(FileListServerKeys.bizNodeToken.rawValue)
    private let iconInfo = Expression<String?>(FileListServerKeys.iconInfo.rawValue)
    private let favoriteTime = Expression<TimeInterval?>(FileListServerKeys.favoriteTime.rawValue)
    ...
    init(_ docConnection: DocsDBConnectionProvidor = Connection.docs) {
        ...
        table = Table("FileEntries")
        db = docConnection.file
        DocsLogger.info("NewFileEntrisTable init with db \(String(describing: db)) ")
        do {
    ...
    func createTableCMD() -> String {
        return table.create(ifNotExists: true) { tbl in
            tbl.column(objtoken, primaryKey: true)
            tbl.column(realToken)
            tbl.column(name)
            tbl.column(type)
            tbl.column(openTime)
            tbl.column(ownerID)
            ...
            tbl.column(secureLabelName)
            tbl.column(secretKeyDelete)
            tbl.column(objBizType)
            tbl.column(bizNodeToken)
            tbl.column(iconInfo)
            tbl.column(favoriteTime)
        }
    }

    func getFileEntries(_ objTokens: [FileListDefine.ObjToken]) -> [SpaceEntry] {
        var entries = [SpaceEntry]()
        do {
            let query = table.filter(objTokens.contains(objtoken))
            ...
            for record in records {
                ...
                let objTokenOri = DocsTableUtil.getOriColumn(objtoken, record: record)
                guard let typeReal = typeOri, objTokenOri != nil else {
    ...
    private func insert(_ fileEntry: SpaceEntry) {
        do {
            ...
            let insert = table.insert(or: .replace,
                                      ...
                                      self.realToken <- fileEntry.realToken,
                                      ...
                                      self.favoriteTime <- fileEntry.favoriteTime,
                                      ...
                                      self.secureLabelName <- fileEntry.secureLabelName,
                                      self.secretKeyDelete <- fileEntry.secretKeyDelete,
                                      self.objBizType <- fileEntry.objBizType,
                                      self.bizNodeToken <- fileEntry.bizNodeToken,
                                      self.iconInfo <- fileEntry.iconInfo
    ...
    func insert(_ fileEntries: [SpaceEntry]) {
        //频繁的循环插入/删除操作导致内存增加，手动使用releasePool释放
        ...
        do {
            ...
            while currentCount > 0 {
                ...
                try autoreleasepool {
                    ...
                    //删除
                    let matched = table.filter(sliceToDelete.map { $0.objToken }.contains(objtoken))
    ...
    private func parseFileData(_ record: Row, for docType: DocsType) -> SpaceEntry {
        let fileEntry = SpaceEntryFactory.createEntry(type: docType, nodeToken: "", objToken: record[objtoken])
        var node: [String: Any] = [:]
        ...
        node[FileListServerKeys.secureLabelName.rawValue]   = DocsTableUtil.safeGetColumn(column: secureLabelName, record: record)
        node[FileListServerKeys.secretKeyDelete.rawValue]   = DocsTableUtil.safeGetColumn(column: secretKeyDelete, record: record)

        node[FileListServerKeys.objBizType.rawValue]        = DocsTableUtil.safeGetColumn(column: objBizType, record: record)
        node[FileListServerKeys.bizNodeToken.rawValue]      = DocsTableUtil.safeGetColumn(column: bizNodeToken, record: record)
        node[FileListServerKeys.iconInfo.rawValue]          = DocsTableUtil.safeGetColumn(column: iconInfo, record: record)
        node[FileListServerKeys.favoriteTime.rawValue]      = DocsTableUtil.safeGetColumn(column: favoriteTime, record: record)
    ...
    func getFileEntry(by objToken: FileListDefine.ObjToken) -> SpaceEntry? {
        var entries = [SpaceEntry]()
        do {
            guard let records = try db?.prepareRowIterator(table.filter(objtoken == objToken || objToken == realToken)).map({ $0 }) else {
                spaceAssertionFailure()
                DocsLogger.info("db is nil or err occur")
                return nil
            }
            for record in records {
                let docType = DocsType(rawValue: record[type]) 
                let fileEntry = parseFileData(record, for: docType)
                entries.append(fileEntry)
            }
        } catch {
            DocsLogger.error("数据库相关", error: error, component: LogComponents.db)
//            DBErrorStatistics.fileListStatisticsFor(error: error)
        }
        DocsLogger.debug("$DB-SpaceEntry 从DB取出fileEntry：\(entries.count)")
