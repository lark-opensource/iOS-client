//
//  BitableRecentListViewModel.swift
//  SKSpace
//
...
extension BitableRecentListViewModel {
...
public final class BitableRecentListViewModel: SpaceListViewModel {
 
    private let workQueue = DispatchQueue(label: "bitable.recent.vm")
    ...
    private func didSelect(entry: SpaceEntry) {
        ...
        if case let .baseHomeType(c) = homeType {
            ...
            context[SKEntryBody.fromKey] = c.containerEnv == .larkTab ? FileListStatistics.Module.baseHomeLarkTabRecentV4 : FileListStatistics.Module.baseHomeWorkbenchRecentV4
        }
...
extension BitableRecentListViewModel: SpaceFilterSortPanelDelegate {
    ...
    private func updateAfterFilterSortOptionChanged(action: String) {
        ...
        dataModel.refresh().subscribe { [weak self] in
            ...
            guard let self = self else { return }
            DocsLogger.error("bitable.recent.vm --- pull to refresh failed with error", error: error)
...
extension BitableRecentListViewModel: SpaceFilterPanelDelegate {
...
extension BitableRecentListViewModel: SpaceSortPanelDelegate {
...
extension BitableRecentListViewModel: SpaceListItemInteractHandler {
    // 网格模式下，more 按钮
    ...
    func generateSlideConfig(for entry: SpaceEntry) -> SpaceListItem.SlideConfig? {
        ...
        return SpaceListItem.SlideConfig(actions: actions) { [weak self] (cell, action) in
            ...
            default:
                spaceAssertionFailure("bitable.recent.list.vm --- unhandle slide action: \(action)")
                return
            }
        }
    }
...
// SlideAction
extension BitableRecentListViewModel: SpaceListSlideDelegateHelperV2 {
        ...
        if shouldDeleteOriginFile {
            ...
            interactionHelper.delete(item: item)
                .subscribe { [weak self] _ in
                    ...
                    DocsLogger.error("bitable.recent.vm2 --- failed to delete origin file", error: error)
                    guard let self = self else { return }
                    self.actionInput.accept(.hideHUD)
            ...
            dataModel.deleteFromRecentList(objToken: file.objToken, objType: file.docsType)
                .subscribe { [weak self] in
                    ...
                    DocsLogger.error("bitable.recent.vm2 --- failed to delete recent file", error: error)
                    guard let self = self else { return }
                    self.actionInput.accept(.hideHUD)
                    if let docsError = error as? DocsNetworkError, docsError.code == .cacDeleteBlocked {
                        DocsLogger.info("cac delete blocked, should not show tips")
...
// MARK: 列表自动刷新逻辑，待梳理
extension BitableRecentListViewModel: SpaceRefresherListProvider {
    ...
    private func handleRefresher(action: @escaping SpaceListAutoRefresher.RefreshActionHandler, shouldShowRefreshTips: Bool) {
        DocsLogger.info("bitable.recent.vm handling action from auto refresher",
        ...
        let refreshAction = {
            action { [weak self] result in
                ...
                case let .failure(error):
                    DocsLogger.error("bitable.recent.vm auto refresh failed with error", error: error)
