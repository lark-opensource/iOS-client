...
public final class MySpaceViewModel: SpaceListViewModel {

    private let workQueue = DispatchQueue(label: "space.myspace.list.vm")
    ...
    let dataModel: PersonalFileDataModel
    ...
    init(dataModel: PersonalFileDataModel, isShowInDetail: Bool = false) {
    ...
    func prepare() {
        ...
        NotificationCenter.default.rx.notification(.Docs.SpaceSortFilterStateUpdated)
            .filter { [weak self] notification in
                self?.dataModel.token == (notification.userInfo?["listToken"] as? String)
            }
    ...
    func didBecomeActive() {
        isActive = true
        tracker.reportEnter(module: "personal", subModule: nil, srcModule: "home")
    }
...
extension MySpaceViewModel: SpaceSortPanelDelegate {
    private func sortOptionDidChanged() {
        ...
        DocsTracker.reportSpaceHeaderFilterClick(by: filterItem,
                                                 ...
                                                 lastActionName: "",
                                                 eventType: DocsTracker.EventType.spacePersonalPageView,
    ...
    private func updateSortState() {
        ...
        sortSelectOptionRelay.accept(currentSortOption)
...
extension MySpaceViewModel: SpaceFilterPanelDelegate {
...
extension MySpaceViewModel: SpaceListItemInteractHandler {
    // 网格模式下，more 按钮
    ...
    func generateSlideConfig(for entry: SpaceEntry) -> SpaceListItem.SlideConfig? {
        let actions: [SlideAction] = [.delete, .share, .more]
    ...
    private func showMoreVC(for entry: SpaceEntry, sourceView: UIView, forbiddenItems: [MoreItemType]) {
        let listType: SpaceMoreAPI.ListType = dataModel.usingV2API ? .mySpace : .mySpaceV1
...
// SlideAction
extension MySpaceViewModel: SpaceListSlideDelegateHelperV2 {
...
extension MySpaceViewModel {
    ...
    func didSelectDeleteAction(file: SpaceEntry, completion: @escaping ((Bool) -> Void)) {
        ...
        var title: String = BundleI18n.SKResource.Doc_Contract_Remove_Owner_Document_Dialog_Title(file.type.i18Name, file.name)
        if file.isShortCut {
            title = BundleI18n.SKResource.CreationMobile_ECM_DeleteTitle(file.name)
        } else if file.isSingleContainerNode {
            title = BundleI18n.SKResource.CreationMobile_ECM_DeleteConfirmTitle(file.name)
        }
        ...
        if file.isShortCut {
            ...
            content = BundleI18n.SKResource.Doc_Contract_Remove_Owner_Document_Dialog_Content
        }
    ...
    func deleteFile(_ file: SpaceEntry) {
        ...
        if file.type == .folder {
            // 我的空间删除文件夹要用这个接口
            interactionHelper.removeFromFolder(nodeToken: file.objToken, folderToken: nil)
                .subscribe { [weak self] _ in
                    ...
                    guard let self = self else { return }
                    self.actionInput.accept(.hideHUD)
                    DocsLogger.error("space.my-space.list.vm --- failed to delete origin file", error: error)
            ...
            // 删文件用另一个接口
            interactionHelper.delete(item: SpaceItem(objToken: file.objToken, objType: file.docsType))
                .subscribe { [weak self] _ in
                    self?.actionInput.accept(.hideHUD)
                    self?.actionInput.accept(.showHUD(.success(BundleI18n.SKResource.Doc_Facade_DeleteSuccessfullyToastTip)))
                } onError: { [weak self] error in
    ...
    func deleteV2(entry: SpaceEntry) {
        guard isReachable else {
            actionInput.accept(.showHUD(.failure(BundleI18n.SKResource.CreationMobile_Docs_OffLineDelete_Toast)))
            return
        }
        let nodeToken = entry.nodeToken
        let isFolder = entry.type == .folder
        let meta = SpaceMeta(objToken: entry.objToken, objType: entry.type)
        actionInput.accept(.showHUD(.loading))
        interactionHelper.deleteV2(objToken: entry.objToken,
                                   nodeToken: nodeToken,
                                   type: entry.type,
