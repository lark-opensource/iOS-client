...
//  SKECM
//
//  Created by Weston Wu on 2020/12/10.
//
...
public final class QuickAccessViewModel: SpaceListViewModel {

    private let workQueue = DispatchQueue(label: "space.pin.list.vm")
    ...
    func didBecomeActive() {
        ...
        if case .baseHomeType = homeType {
            tracker.reportFileListView(subView: "quick_access")
        }
    }
    ...
    func select(at index: Int, item: SpaceListItemType) {
        ...
        if case let .baseHomeType(ctx) = homeType {
            context[SKEntryBody.fromKey] = ctx.containerEnv == .larkTab ? FileListStatistics.Module.baseHomeLarkTabQuickAccess : FileListStatistics.Module.baseHomeWorkbenchQuickAccess
        }
    ...
    func notifyPullToRefresh() {
        ...
        dataModel.refresh().subscribe { [weak self] in
            ...
            DocsLogger.error("space.quickaccess.list.vm --- pull to refresh failed with error", error: error)
            // show error
            self.serverDataState = .fetchFailed
            self.itemsRelay.accept(self.items)
            return
...
extension QuickAccessViewModel: SpaceListItemInteractHandler {
    // 网格模式下，more 按钮
    ...
    func generateSlideConfig(for entry: SpaceEntry) -> SpaceListItem.SlideConfig? {
        ...
        return SpaceListItem.SlideConfig(actions: actions) { [weak self] (cell, action) in
            ...
            default:
                spaceAssertionFailure("space.pins.list.vm --- unhandle slide action: \(action)")
                return
            }
        }
    }
...
// SlideAction
extension QuickAccessViewModel: SpaceListSlideDelegateHelperV2 {
