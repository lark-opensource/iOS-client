//
//  SpaceListSubSectionLayout.swift
//  SKECM
//
...
public protocol SpaceListSectionAutoLayout: SpaceSectionLayout {
    var sectionLayoutHelper: SpaceListSectionLayoutHelper { get }
}

public extension SpaceListSectionAutoLayout {
    func itemSize(at index: Int, containerWidth: CGFloat) -> CGSize {
        sectionLayoutHelper.itemSize(at: index, containerWidth: containerWidth)
    }

    func sectionInsets(for containerWidth: CGFloat) -> UIEdgeInsets {
        sectionLayoutHelper.sectionInsets(for: containerWidth)
    }

    func minimumLineSpacing(for containerWidth: CGFloat) -> CGFloat {
        sectionLayoutHelper.minimumLineSpacing(for: containerWidth)
    }

    func minimumInteritemSpacing(for containerWidth: CGFloat) -> CGFloat {
        sectionLayoutHelper.minimumInteritemSpacing(for: containerWidth)
    }

    func headerHeight(for containerWidth: CGFloat) -> CGFloat {
        sectionLayoutHelper.headerHeight(for: containerWidth)
    }

    func footerHeight(for containerWidth: CGFloat) -> CGFloat {
        sectionLayoutHelper.footerHeight(for: containerWidth)
    }
}

public protocol SpaceListSectionLayoutHelper: SpaceSectionLayout {}

protocol SpaceSubSectionLayoutDelegate: AnyObject {
    var layoutListState: SpaceListSubSection.ListState { get }
    var layoutDisplayMode: SpaceListDisplayMode { get }
}

class SpaceSubSectionLayoutHelper: SpaceListSectionLayoutHelper {
    ...
    func itemSize(at index: Int, containerWidth: CGFloat) -> CGSize {
        ...
        case let .normal(items):
            ...
            case .grid:
                ...
                case .driveUpload:
                    let cellWidth = containerWidth - 16 - 16 // 减去左右两侧的sectionInsets，在 DriveGridUploadCell 内会延伸到左右16pt
                    return CGSize(width: cellWidth, height: 68 - 12) // 减去顶部的sectionInsets，在 DriveGridUploadCell 内会延伸到顶部16pt
                case .spaceItem:
            ...
            case .list:
                ...
                case .spaceItem:
                    return CGSize(width: containerWidth, height: 68)
                case .driveUpload:
                    return CGSize(width: containerWidth, height: 68)
                }
            }
