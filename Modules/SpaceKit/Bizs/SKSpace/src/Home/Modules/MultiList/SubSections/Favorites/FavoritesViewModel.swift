...
public final class FavoritesViewModel: SpaceListViewModel {

    private let workQueue = DispatchQueue(label: "space.favorites.list.vm")
    ...
    func didBecomeActive() {
        ...
        if case .baseHomeType = homeType {
            tracker.reportFileListView(subView: "favorites")
        }
    ...
    func select(at index: Int, item: SpaceListItemType) {
        ...
        if case let .baseHomeType(ctx) = homeType {
            context[SKEntryBody.fromKey] = ctx.containerEnv == .larkTab ? FileListStatistics.Module.baseHomeLarkTabFavorites : FileListStatistics.Module.baseHomeWorkbenchFavorites
        }
...
extension FavoritesViewModel: SpaceListFilterDelegate {
...
extension FavoritesViewModel: SpaceFilterPanelDelegate {
    ...
    private func updateFilter(selectionIndex: Int) {
        ...
        dataModel.refresh().subscribe { [weak self] in
            ...
            guard let self = self else { return }
            DocsLogger.error("space.favorites.list.vm --- refresh after update filter option failed", error: error)
...
extension FavoritesViewModel: SpaceListItemInteractHandler {
    // 网格模式下，more 按钮
    ...
    func generateSlideConfig(for entry: SpaceEntry) -> SpaceListItem.SlideConfig? {
        ...
        return SpaceListItem.SlideConfig(actions: actions) { [weak self] (cell, action) in
            ...
            default:
                spaceAssertionFailure("space.favorites.list.vm --- unhandle slide action: \(action)")
                return
            }
        }
    }
...
// SlideAction
extension FavoritesViewModel: SpaceListSlideDelegateHelperV2 {
