//
//  SubFolderListViewModelV1.swift
//  SKSpace
//
...
extension SubFolderListViewModelV1 {
    typealias Action = SpaceSection.Action
}

class SubFolderListViewModelV1: PermissionRestrictedFolderListViewModel {

    private let vmIdentifier = "sub-folder-list"
    private let workQueue = DispatchQueue(label: "space.sub-folder-list.list.vm")
    ...
    let dataModel: SubFolderDataModelV1
    ...
    init(dataModel: SubFolderDataModelV1, isShowInDetail: Bool = false) {
    ...
    private func setupEditPermission() {
        dataModel.editPermissionChanged.bind(to: createEnableRelay).disposed(by: disposeBag)
    }
    ...
    private func didSelect(entry: SpaceEntry) {
        ...
        let fromModule = self.isShareFolder ? FileListStatistics.Module.sharedFolder : FileListStatistics.Module.folder
        let body = SKEntryBody(entry)
        let context: [String: Any] = [SKEntryBody.fileEntryListKey: entryLists,
                                      SKEntryBody.fromKey: fromModule]
...
extension SubFolderListViewModelV1: SpaceSortPanelDelegate {
...
extension SubFolderListViewModelV1: SpaceListItemInteractHandler {
    // 网格模式下，more 按钮
    func handleMoreAction(for entry: SpaceEntry) -> ((UIView) -> Void)? {
        createMoreActionHandler(for: entry, forbiddenItems: [])
    }

    private func createMoreActionHandler(for entry: SpaceEntry, forbiddenItems: [MoreItemType]) -> (UIView) -> Void {
    ...
    func folderMoreAction() -> ((UIView) -> Void)? {
        ...
        return { [weak self] view in
            ...
            // 文件夹自己的 more 面板要隐藏 move 按钮
            let handler = self.createMoreActionHandler(for: folderEntry, forbiddenItems: [.moveTo])
            handler(view)
        }
    }
    ...
    func generateSlideConfig(for entry: SpaceEntry) -> SpaceListItem.SlideConfig? {
        let deleteAction: SlideAction = createEnableRelay.value ? .delete : .deleteDisable
    ...
    private func showMoreVC(for entry: SpaceEntry, sourceView: UIView, forbiddenItems: [MoreItemType]) {
        ...
        switch dataModel.folderInfo.folderType {
        case .personal:
            listType = .subFolder(type: .v1Personal)
        case let .share(spaceID, _, _):
            listType = .subFolder(type: .v1Share(spaceID: spaceID))
        }
...
extension SubFolderListViewModelV1: SpaceListSlideDelegateHelperV2 {
    ...
    func handleDelete(for entry: SpaceEntry) {
        didSelectDeleteAction(file: entry, completion: { [weak self] confirm, deleteOrigin in
            guard let self = self, confirm else { return }
            self.deleteFile(entry, deleteOrigin: deleteOrigin)
        })
    }
}

extension SubFolderListViewModelV1 {
    // 回调第一个 Bool 表明是否确认删除操作，第二个 Bool 表示是否需要删除原文件 or 只删除文件夹内引用
    func didSelectDeleteAction(file: SpaceEntry, completion: @escaping ((Bool, Bool) -> Void)) {
        ...
        if let userID = User.current.info?.userID, ownerID == userID, file.type != .folder {
        ...
        if file.type == .folder {
            title = BundleI18n.SKResource.Doc_List_Remove_Folder(fileName)
            content = BundleI18n.SKResource.Doc_List_Remove_Folder_Confirm
        } else {
            title = BundleI18n.SKResource.Doc_List_Remove_Folder_Document_Dialog_Title
            content = BundleI18n.SKResource.Doc_List_Remove_Folder_Document_Dialog_Content
        }
        ...
        dialog.setTitle(text: title, checkButton: canDeleteOriginFile)
        dialog.setContent(text: content, checkButton: canDeleteOriginFile)
        if canDeleteOriginFile {
        ...
        dialog.addDestructiveButton(text: BundleI18n.SKResource.Doc_Facade_Delete, dismissCompletion: { [weak dialog] in
            if canDeleteOriginFile {
                completion(true, dialog?.isChecked ?? false)
            } else {
    ...
    // 文件夹内的删除比较复杂，需要考虑删除文件夹内的引用 or 删除源文件 or 删除文件夹自己
    func deleteFile(_ file: SpaceEntry, deleteOrigin: Bool) {
        ...
        if deleteOrigin {
            ...
            interactionHelper.delete(item: item)
                .subscribe { [weak self] _ in
                    ...
                    guard let self = self else { return }
                    self.actionInput.accept(.hideHUD)
                    DocsLogger.error("space.sub-folder.list.vm --- failed to delete origin file", error: error)
            ...
            if file.type == .folder && tokenToDelete == dataModel.token {
                tokenToDelete = file.objToken
                isDeletingSelf = true
            }
            dataModel.removeFromList(nodeToken: tokenToDelete)
                .subscribe { [weak self] in
                    ...
                    NotificationCenter.default.post(name: SubFolderDataModelV1.subFolderNeedUpdate, object: nil)
                    self.actionInput.accept(.exit)
                } onError: { [weak self] error in
                    guard let self = self else { return }
                    self.actionInput.accept(.hideHUD)
                    DocsLogger.error("space.sub-folder.list.vm --- failed to remove file from folder", error: error)
                    if let docsError = error as? DocsNetworkError, docsError.code == .cacDeleteBlocked {
                        DocsLogger.info("cac delete blocked, should not show tips")
                        return
                    }
