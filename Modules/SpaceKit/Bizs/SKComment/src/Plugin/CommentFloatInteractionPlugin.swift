//
//  CommentFloatInteractionPlugin.swift
//  SKCommon
//
...
final class CommentFloatInteractionPlugin: CommentPluginType {
...
extension CommentFloatInteractionPlugin {
...
//                CommentTracker.retryLoadImage(docsInfo: commentData.docsInfo, isActive: isActive, commentId: commentItem.commentId ?? "")
    ...
    private func handleTapBlank() {
        ...
        case let .newInput(model):
            ...
            // 下掉键盘
            self.scheduler?.reduce(state: .updateFloatTextView(active: false, draftKey: nil))
            // 关掉面板
            scheduler?.reduce(state: .dismiss)
        case .reply, .edit:
            self.scheduler?.dispatch(action: .ipc(.setFloatCommentMode(mode: .browseMode), nil))
        }
    }
    
    func handelClickClose() {
        scheduler?.dispatch(action: .ipc(.removeAllMenu, nil))
        scheduler?.dispatch(action: .api(.closeComment, nil))
        scheduler?.reduce(state: .dismiss)
    }
    ...
    private func handleEdit(_ item: CommentItem) {
        ...
        scheduler?.dispatch(action: .ipc(.setFloatCommentMode(mode: .edit(item)), nil))
    }
