...
def revise_host_project(project_file, target_name, scheme_name)
  Pod::UI.message('- Trying to revise configuration of host project ' \
    ...
    _force_enable_all_testables(project_file, scheme_name)
    project.save
  end
end
...
# Enabled all testable targets for CI check
def _force_enable_all_testables(project_path, scheme_name)
  Pod::UI.message('- Trying to enable all testables for unit test') do
    return unless _need_run_unit_test
    ...
    Pod::UI.message("- Revise xcscheme '#{scheme_path}'")
    scheme = Xcodeproj::XCScheme.new(scheme_path)
    scheme.test_action.testables.each { |testable|
