...
/// 设计文档: https://bytedance.feishu.cn/docs/doccnMtzhc09mnSpsW8qaZHy1oQ#

public protocol SKImagePreviewViewDelegate: NSObjectProtocol {
    /// 单击事件回调
    ...
    /// imageView 相对SKImagePreviewView的frame发生变化：缩放、滚动
...
public final class SKImagePreviewView: UIView, BaseSKImageView {
...
extension SKImagePreviewView: UIGestureRecognizerDelegate {
...
extension SKImagePreviewView {
    ...
    private func loadImage() {
        ...
        resizeImageIfNeed(path) {[weak self] (img, imgPath, imageSize) in
            guard let self = self else { return }
            DispatchQueue.main.async {
                self.loadingIndicatorView.isHidden = true
                self.loadingIndicatorView.stopAnimating()
                guard let image = img,
                      let size = imageSize else {
                        DocsLogger.error("can not load image")
    ...
    /// 如果图片超过屏幕大小会对图片进行缩小，如果图片大小小于屏幕大小，通过此方法在后台解码图片
    private func resizeImageIfNeed(_ path: SKFilePath, complete: @escaping (UIImage?, SKFilePath, CGSize?) -> Void) {
        processQueue.async {[weak self] in
            let image = self?.previewStratery.imageForPreview(imagePath: path)
            let originSize = SKImagePreviewUtils.originSizeOfImage(path: path) ?? image?.size
            complete(image, path, originSize)
        }
    }
        ...
        processQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                if self.tileImageView == nil {
                    self.tileImageView = DriveTileImageView(imagePath: imagePath,
                                                            frame: self.imageView.bounds,
                                                            tileSize: self.tileSize)
                    self.imageView.insertSubview(self.tileImageView!, at: 0)
...
extension SKImagePreviewView: UIScrollViewDelegate {
