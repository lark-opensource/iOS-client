...
for arch in $all_arches; do
	...
	cd build_ios/$arch
	xcodebuild -project ByteRTC.xcodeproj -scheme VolcEngineRTC -configuration $build_config -sdk $sdk -jobs $(sysctl -n hw.ncpu)
...
mkdir -p third_party
cd third_party
cp -r $src_dir/third_party/prebuilt/ios/* .
cp -r $src_dir/third_party/boringssl-src/include boringssl/
all_libs="boya jsoncpp srtp2 protobuf-lite realx_base realx_audio_main realx_network_main realx realx_engine byteaudio_engine byteaudio_static webrtc_p2p"
