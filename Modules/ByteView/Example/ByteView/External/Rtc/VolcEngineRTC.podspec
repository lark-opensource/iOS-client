...
Pod::Spec.new do |s|
  ...
  puts "current dir is #{RtcBuildConfig.current_dir}"

  local_pods = RtcBuildConfig.get_submodules
  puts "local_pods = #{local_pods}"

  # run sh first: download_sources.sh、build_third_party.sh、 prebuild.sh
  cmake_targets = RtcBuildConfig.all_cmake_files
  ...
  pod_target_xcconfig = {
    ...
    'EXPORTED_SYMBOLS_FILE': "#{RtcBuildConfig.rtc_root}/sdk/ios/ByteRtcEngineKit/export-meeting.txt",
...
#  pod_target_xcconfig['OTHER_LDFLAGS'] = "$(inherited) -Wl,-search_paths_first -ObjC -Wl,-rename_section,__TEXT,__text,__BD_TEXT,__text -Wl,-rename_section,__TEXT,__textcoal_nt,__BD_TEXT,__text -Wl,-rename_section,__TEXT,__StaticInit,__BD_TEXT,__text -Wl,-rename_section,__TEXT,__stubs,__BD_TEXT,__stubs -Wl,-rename_section,__TEXT,__picsymbolstub4,__BD_TEXT,__picsymbolstub4 -Wl,-rename_section,__TEXT,__stub_helper,__BD_TEXT,__stub_helper -Wl,-rename_section,__TEXT,__objc_classname,__BD_TEXT,__objc_classname -Wl,-rename_section,__TEXT,__objc_methname,__BD_TEXT,__objc_methname -Wl,-rename_section,__TEXT,__cstring,__BD_TEXT,__cstring -Wl,-segprot,__BD_TEXT,rx,rx -Wl,-rename_section,__TEXT,__const,__COMPRESS,__const -Wl,-rename_section,__TEXT,__objc_methtype,__COMPRESS,__objc_methtype -Wl,-rename_section,__TEXT,__gcc_except_tab,__COMPRESS,__gcc_except_tab -Wl,-segprot,__COMPRESS,r,r"
  ...
  prelink_libs = [
    'opus', 'NICO', 'bytevc0', 'bytevc1dec', 'bytevc1enc', 'boya', 'protobuf-lite', 'jsoncpp', 'srtp2',
    'openh264',
    'realx_base',
    'realx_audio_main',
    'realx_video_main',
  ...
  prelink_libs = prelink_libs.map { |f| "#{RtcBuildConfig.third_party_dir}/#{f}/lib/lib#{f}.a" }
  prelink_libs += [
    'boringssl/lib/libcrypto.a',
    'boringssl/lib/libssl.a',
    'ne10/lib/libNE10.a',
    'zlib/lib/libz.a',
  ].map { |f| "#{RtcBuildConfig.third_party_dir}/#{f}" }
  ...
  pod_target_xcconfig['SYSTEM_HEADER_SEARCH_PATHS'] = [
    ...
    'NICO',
    'zlib',
  ].map { |f| "#{RtcBuildConfig.third_party_dir}/#{f}/include" }.join(' ') + ' $(inherited)'

  header_search_paths = [
    "#{RtcBuildConfig.rtc_root}",
  ]
  [
    'api',
    'src',
  ...
  ].each { |f| header_search_paths.append("#{RtcBuildConfig.realx_root}/#{f}") }
  [
    ...
    'src/sdk/native/rts',
    'src/sdk/objc/rtc',
  ].each { |f| header_search_paths.append("#{RtcBuildConfig.rtc_root}/#{f}") }
  Dir.glob("#{RtcBuildConfig.rtc_root}/rtc/third_party/boost_1_69_0/boost/*/include").each { |f| header_search_paths.append(f) }
  ...
  s.script_phase = {
    ...
    'script': <<-CMD
      ...
      cp -r "#{RtcBuildConfig.current_dir}/framework/Headers" "${BUILT_PRODUCTS_DIR}/#{s.name}.framework/Headers"
    CMD
  }
...
#    'zlibstatic',
  ...
  s.subspec 'Core' do |cs|
    ...
    source_files = cmake_obj['sources'].map { |f| "#{RtcBuildConfig.rtc_dirname}/#{f['path']}" }
  ...
  local_subspecs.each do |target_name|
    s.subspec target_name do |cs|
      ...
      source_files = JSON.parse(File.read(cmake_file))['sources'].map { |f| "#{RtcBuildConfig.rtc_dirname}/#{f['path']}" }
      cs.project_header_files = source_files.filter { |f| f.end_with? '.h' }
      cs.source_files = source_files
    end
  end
