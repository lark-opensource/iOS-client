...
extension EventBasicCellLikeView.Accessory {
    ...
    fileprivate var image: UIImage? {
        ...
        case .type(let type):
            let image: UIImage
            switch type {
            case .next: image = UDIcon.getIconByKey(.rightOutlined, size: EventBasicCellLikeView.Style.iconSize).renderColor(with: .n2)
            case .close: image = UDIcon.getIconByKey(.closeOutlined, size: EventBasicCellLikeView.Style.iconSize).renderColor(with: .n2)
            case .fold: image = UDIcon.getIconByKey(.downOutlined, size: EventBasicCellLikeView.Style.iconSize).renderColor(with: .n2)
            case .unfold: image = UDIcon.getIconByKey(.upOutlined, size: EventBasicCellLikeView.Style.iconSize).renderColor(with: .n2)
