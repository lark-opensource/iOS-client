//
//  VChatContentComponentBinder.swift
//  Action
//
...
class VChatContentComponentBinder<M: CellMetaModel, D: CellMetaModelDependency, C: VChatContentContext>: ComponentBinder<C> {
    ...
    override func update<VM: ViewModel>(with vm: VM, key: String? = nil) {
        guard let vm = vm as? VChatContentViewModel<M, D, C> else {
            assertionFailure()
            return
        }
    ...
    override func buildComponent(key: String? = nil, context: C? = nil) {
        ...
        _component = RichLabelComponent(props: props, style: style, context: context)
    }
}
