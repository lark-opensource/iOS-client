...
module.exports = async function(params, context) {
  const db = larkcloud.db.table('slardar')
  let items = await db.where({"product":"neo"}).find()
  for(let i in items){
    let item = items[i]
    await sendNotice(2848,"crash",item.chat_id)
    await sendNotice(2848,"watch_dog",item.chat_id)
    await sendNew(2848,"crash", item.chat_id)
    await sendNew(2848,"watch_dog", item.chat_id)
    await sendNotice(3058,"crash",item.chat_id)
    await sendNotice(3058,"watch_dog",item.chat_id)
    await sendNew(3058,"crash", item.chat_id)
    await sendNew(3058,"watch_dog", item.chat_id)
  }
}

async function sendNew(aid,crash_type,chat_id){
...
async function sendNotice(aid,crash_type,chat_id){
  let c = await client()
  var divs = []
  ...
  divs.push({
      ...
      "text": {
        "tag": "lark_md",
        "content": `NEO ${getName(aid)}${crash_type} summary`
      }
  })
