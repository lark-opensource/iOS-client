...
/**
 WKUIDelegate 各 UI 方法回调崩溃保护器
 针对window.alert等方法，弹出alert时被异常关闭alert导致无回调进一步导致崩溃，进行防护
 请保证主线程调用
...
final class WKUIDelegateCrashProtection<CompletionHandlerParamsType> {
    ...
    private let logger = Logger.oplog(WKUIDelegateCrashProtection<CompletionHandlerParamsType>.self, category: "WKUIDelegateCrashProtection")
    ...
    deinit {
        ...
        if !hasCalledCompletionHandler {
            ...
            let event = OPMonitor("web_uidelegate_crash_protection")
            event.flush()
        }
