...
final class BundleI18n: LanguageManager {
    ...
    static func LocalizedString(key: String, originalKey: String, lang: Lang? = nil) -> String {
        func fetch() -> String {
            ...
            return self.localizedString(key: key, originalKey: originalKey, bundle: BundleConfig.HelpDeskAutoBundle, moduleName: "HelpDesk", lang: lang) ?? key
            #else
            return self.localizedString(key: key, bundle: BundleConfig.HelpDeskAutoBundle, moduleName: "HelpDesk", lang: lang) ?? key
            #endif
        }
    ...
    final class HelpDesk {
        @inlinable
        static var HelpDesk_Chat_NetworkError: String {
            return LocalizedString(key: "vTk", originalKey: "HelpDesk_Chat_NetworkError")
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func HelpDesk_Chat_NetworkError(lang __lang: Lang? = nil) -> String {
            return LocalizedString(key: "vTk", originalKey: "HelpDesk_Chat_NetworkError", lang: __lang)
        }
    }
}
