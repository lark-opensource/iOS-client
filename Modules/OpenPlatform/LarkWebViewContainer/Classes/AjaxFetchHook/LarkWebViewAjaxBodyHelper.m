#import "LarkWebViewAjaxBodyHelper.h"
#import "LarkWebViewFormDataFile.h"
#import "LarkWebViewURLRequestSerialization.h"

@implementation LarkWebViewAjaxBodyHelper
...
+ (void)setFormData:(NSDictionary *)formDataJson formEnctype:(NSString *)formEnctype toRequest:(NSMutableURLRequest *)request {
    ...
    NSMutableArray<LarkWebViewFormDataFile *> *fileDatas = [NSMutableArray array];
    ...
    for (NSArray *pair in formData) {
        ...
        if ([fileKeys containsObject:key]) {// 说明存储的是个文件数据
            ...
            LarkWebViewFormDataFile *fileData = [LarkWebViewFormDataFile new];
            fileData.key = key;
    ...
    if ([formEnctype isEqualToString:@"multipart/form-data"]) {
        LarkWebViewURLRequestSerialization *serializer = [self urlRequestSerialization];
        [serializer multipartFormRequestWithRequest:request parameters:params constructingBodyWithBlock:^(id<LarkWebViewMultipartFormData>  _Nonnull formData) {
            for (LarkWebViewFormDataFile *fileData in fileDatas) {
...
#pragma mark - LarkWebViewURLRequestSerialization

+ (LarkWebViewURLRequestSerialization *)urlRequestSerialization {
    static LarkWebViewURLRequestSerialization *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [LarkWebViewURLRequestSerialization new];
    });
