...
/// 参数灰度验证器
public struct OpenAPIGrayChecker<T> {

    /// 验证器
    public let checker: OpenAPIValidChecker.Checker<T>
    /// 灰度 featureKey
    public let featureKey: String

    private init(checker: @escaping OpenAPIValidChecker.Checker<T>, featureKey: String) {
        self.checker = checker
        self.featureKey = featureKey
    }

    public static func checker<T>(
        _ checker: @escaping OpenAPIValidChecker.Checker<T>,
        featureKey: String
    ) -> OpenAPIGrayChecker<T> {
        return OpenAPIGrayChecker<T>(checker: checker, featureKey: featureKey)
    }
}

public struct OpenAPIValidChecker {
    ...
    /// 4...5           ---> [4, 5]     ClosedRange
    /// 4.nextUp...5    ---> (4, 5]
    public static func length<R: RangeExpression>(_ range: R) -> (String) -> Bool where R.Bound == Double {
        return { range ~= Double($0.count) }
    }
