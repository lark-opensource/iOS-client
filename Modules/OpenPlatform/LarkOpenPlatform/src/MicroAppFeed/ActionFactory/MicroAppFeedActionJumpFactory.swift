//
//  MicroAppFeedActionJumpFactory.swift
//  LarkFeedPlugin
//
...
final class MicroAppFeedActionJumpFactory: FeedActionBaseFactory {
    ...
    func createActionHandler(model: FeedActionModel, context: FeedCardContext) -> FeedActionHandlerInterface {
        return MicroAppFeedActionJumpHandler(type: type, model: model, context: context)
    }
}

final class MicroAppFeedActionJumpHandler: FeedActionHandler {
    ...
    private func routerToApp(feedPreview: FeedPreview,
        ...
        if let url = URL(string: feedPreview.preview.openAppData.iosSchema) {
            let seqID = feedPreview.preview.openAppData.lastNotificationSeqID
