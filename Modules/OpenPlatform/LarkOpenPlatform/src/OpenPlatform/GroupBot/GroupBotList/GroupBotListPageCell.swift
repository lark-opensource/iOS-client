...
/// 「群机器人」页面的单个Cell
class GroupBotListPageCell: UITableViewCell {
    /// 依赖的viewModel
    private var viewModel: GroupBotListPageCellViewModel?
    /// cell 配置
    struct CellConfig {
        static let cellHeight: CGFloat = 68.0
        static let cellID: String = "GroupBotListPageCell"
    }
    /// 应用图标size
    ...
    ///   - bizScene: 业务场景
    ///   - model: viewModel
    func refresh(viewModel: GroupBotListPageCellViewModel, isLastCellInSection: Bool) {
        self.viewModel = viewModel
        logoView.bt.setLarkImage(with: .avatar(key: viewModel.data.avatar?.key ?? "",
                                               entityID: viewModel.data.botID ?? "",
                                               params: .init(sizeType: .size(Self.logoEdge))))
...
// MARK: 视图展示相关
extension GroupBotListPageCell {
    /// view composition
