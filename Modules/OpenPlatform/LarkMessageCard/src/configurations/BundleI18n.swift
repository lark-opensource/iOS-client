...
final class BundleI18n: LanguageManager {
    ...
    static func LocalizedString(key: String, originalKey: String, lang: Lang? = nil) -> String {
        func fetch() -> String {
            ...
            return self.localizedString(key: key, originalKey: originalKey, bundle: BundleConfig.LarkMessageCardAutoBundle, moduleName: "LarkMessageCard", lang: lang) ?? key
            #else
            return self.localizedString(key: key, bundle: BundleConfig.LarkMessageCardAutoBundle, moduleName: "LarkMessageCard", lang: lang) ?? key
            #endif
        }
    ...
    final class LarkMessageCard {
        ...
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func OpenPlatform_CardForMyAi_PplCntAppend(_ count: Any, lang __lang: Lang? = nil) -> String {
          return OpenPlatform_CardForMyAi_PplCntAppend(count: `count`, lang: __lang)
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        static func OpenPlatform_CardForMyAi_PplCntAppend(count: Any, lang __lang: Lang? = nil) -> String {
            var template = LocalizedString(key: "ENg", originalKey: "OpenPlatform_CardForMyAi_PplCntAppend", lang: __lang)
            template = template.replacingOccurrences(of: "{{count}}", with: "\(`count`)")
            return template
        }
        @inlinable
        ...
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func OpenPlatform_CardForMyAi_PplCntMemberListTtl(_ count: Any, lang __lang: Lang? = nil) -> String {
          return OpenPlatform_CardForMyAi_PplCntMemberListTtl(count: `count`, lang: __lang)
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        static func OpenPlatform_CardForMyAi_PplCntMemberListTtl(count: Any, lang __lang: Lang? = nil) -> String {
        ...
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func OpenPlatform_UniversalCard_ClientMsgExpired(_ day: Any, lang __lang: Lang? = nil) -> String {
          return OpenPlatform_UniversalCard_ClientMsgExpired(day: `day`, lang: __lang)
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        static func OpenPlatform_UniversalCard_ClientMsgExpired(day: Any, lang __lang: Lang? = nil) -> String {
            var template = LocalizedString(key: "dKs", originalKey: "OpenPlatform_UniversalCard_ClientMsgExpired", lang: __lang)
            template = template.replacingOccurrences(of: "{{day}}", with: "\(`day`)")
            return template
        }
        @inlinable
