...
private final class MsgCardImageView: ByteImageView {
...
public final class MsgCardLynxImageView: LynxUIView {
    
    public static let name: String = "msg-card-image"
    
    static let logger = Logger.oplog(MsgCardLynxImageView.self, category: "MsgCardLynxImageView")
    
    @FeatureGatingValue(key: "openplatform.msgcard.image.set.resolution")
    var enableSetResolution: Bool
    
    private lazy var viewModel: MsgCardLynxImageViewModel = MsgCardLynxImageViewModel()
    ...
    lazy private var imageView: ByteImageView = {
        var imageView: MsgCardImageView = MsgCardImageView()
    ...
    lazy private var longImageLabel: UILabel = {    
        ...
        let text = BundleI18n.LarkMessageCard.Lark_Legacy_MsgCard_LongImgTag
    ...
    @objc func clickPreviewImage() {
        Self.logger.info("MsgCardLynxImageView: imageView click")
        guard let images = viewModel.images,
              let imageId = viewModel.imageId,
              let imageKey = images[imageId]?.originKey else {
            Self.logger.error("preview image error, no image")
            return
        }
        MsgCardImageUtils.showPreviewImage(cardContext: viewModel.cardContext, images: images, imageKey: imageKey, previewImages: viewModel.previewImageKeys)
    }
    
    @objc func setProps(props: Any?, requestReset _: Bool) {
        self.viewModel.cleanImageData()
        Self.logger.info("MsgCardLynxImageView: imageView setProps")
        ...
        guard let key = props["key"] as? String,
                let cardContext = cardContextManager.getContext(key: key) else {
            return
        }
        self.viewModel.cardContext = cardContext
        ...
        if let isTranslateElement = props["isTranslateElement"] as? Bool {
            Self.logger.info("MsgCardLynxImageView: set isTranslateElement:\(isTranslateElement)")
            self.viewModel.isTranslateElement = isTranslateElement
        }

        if let imageId = props["image_id"] as? String {
            Self.logger.info("MsgCardLynxImageView: set imageId:\(imageId)")
        ...
        if let resId = props["res_id"] as? String, let image = BundleResources.LarkMessageCard.iconbyName(iconName: resId) {
            Self.logger.info("MsgCardLynxImageView: imageView set local image:\(resId)")
        ...
        if let resId = props["res_id"] as? String, let udIcon = fetchUDIcon(resId: resId) {
            Self.logger.info("MsgCardLynxImageView: imageView set UDIcon:\(resId)")
        ...
        self.viewModel.calculateData { [weak self] in
    ...
    //图片裁剪
     private func cropImage() -> UIImage? {
         ...
         var viewAspectRatio = self.imageView.bounds.width / self.imageView.bounds.height
         ...
         guard image.size.width > 0 && imageCropHeight > 0 else { return nil }
    ...
    private func fetchUDIcon(resId: String) -> UIImage? {
        ...
        case "ud_icon_calendar_outlined":
            return UDIcon.getIconByKey(.calendarOutlined)
    ...
    private func updateImageView() {
        guard let imageId = viewModel.imageId else {
            Self.logger.info("MsgCardLynxImageView: imageId is empty:\(viewModel.imageId.isEmpty)")
            return
        }
        guard let imageProperty: RustPB.Basic_V1_RichTextElement.ImageProperty = viewModel.images?[imageId] else {
            return
        }
        
        if self.enableSetResolution {
            guard self.frame.width != 0, self.frame.height != 0 else {
                Self.logger.info("MsgCardLynxImageView: frame is sizezero")
                return
            }
        }
        DispatchQueue.main.async { [weak self] in
            guard let self = self else {
                Self.logger.info("MsgCardLynxImageView: updateImageView self is nil")
                return
            }
            ...
            if self.enableSetResolution {
                ...
                Self.logger.info("MsgCardLynxImageView: imageView imageStr:\(imageStr)")
                self.imageView.bt.setLarkImage(with: .default(key: imageStr), size: self.frame.size, completion:  {[weak self] (result) in
                    guard let self = self else {
                        Self.logger.info("MsgCardLynxImageView: bt.setLarkImage self is nil")
                        return
                    }
                    ...
                    case .failure(let error):
                        ...
                        Self.logger.error("MsgCardLynxImageView: imageView set image fail,error:\(error)")
                    }
                })
            } else {
                if let imageToken = self.viewModel.imageToken {
                    Self.logger.info("MsgCardLynxImageView: imageView imageKey:\(imageToken)")
                    self.imageView.bt.setLarkImage(with: .default(key: imageToken), completion:  {[weak self] (result) in
                        guard let self = self else {
                            Self.logger.info("MsgCardLynxImageView: bt.setLarkImage self is nil")
                            return
                        }
                ...
                } else if let imageUrl = self.viewModel.imageUrl {
                    Self.logger.info("MsgCardLynxImageView: imageView imageUrl:\(imageUrl)")
                    self.imageView.bt.setLarkImage(with: .default(key: imageUrl), completion:  {[weak self] (result) in
                        guard let self = self else {
                            Self.logger.info("MsgCardLynxImageView: bt.setLarkImage self is nil")
                            return
                        }
   ...
   private func AddLongImageLabelIfNeeded() {
       if viewModel.isLongImage == true {
           Self.logger.info("MsgCardLynxImageView: add longImage tab")
           ...
           longImageLabel.snp.makeConstraints { make in
            ...
            Self.logger.info("MsgCardLynxImageView: remove longImage tab, self:\(self), imageId:\(self.viewModel.imageId)")
            if let superView = longImageLabel.superview {
                longImageLabel.removeFromSuperview()
            }
       }
    }
...
public final class MsgCardLynxImageViewShadowNode: LynxShadowNode, LynxCustomMeasureDelegate {
    
    public static let name: String = "msg-card-image"
    private static let logger = Logger.oplog(MsgCardLynxImageViewShadowNode.self, category: "MsgCardLynxImageViewShadowNode")
    ...
    @objc private func setProps(props: Any?, requestReset _: Bool) {
        guard let props = props as? [String: Any] else {
            assertionFailure("MsgCardLynxImageViewShadowNode receive wrong props type: \(String(describing: props.self))")
            Self.logger.error("MsgCardLynxImageViewShadowNode receive wrong props type: \(String(describing: props.self))")
            return
        }
