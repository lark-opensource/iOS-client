...
public protocol ECONetworkResponseValidator {
    
    /// 将 原始的 Response 序列化为指定数据类型
    /// - Parameters:
    ///   - context: 提供发起请求时的上下文信息供内部使用(createTask 时传入的 context)
    ///   - response: 原始,待序列化的 Response
    func validate(context: ECONetworkServiceContext, response: ECONetworkResponseOrigin) throws
}
