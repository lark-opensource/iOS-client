...
{
"keys":{
"OpenPlatform_AppLink_Check_UserID_FeatureToast":{
"hash":"FtI",
"#vars":1,
"normal_vars":["APP_DISPLAY_NAME"]
},
"OpenPlatform_AppLink_NullFeatureToast":{
"hash":"YKs",
"#vars":0
}
},
...
final class BundleI18n: LanguageManager {
    ...
    static func LocalizedString(key: String, originalKey: String, lang: Lang? = nil) -> String {
        func fetch() -> String {
            ...
            return self.localizedString(key: key, originalKey: originalKey, bundle: BundleConfig.LarkAppLinkSDKAutoBundle, moduleName: "LarkAppLinkSDK", lang: lang) ?? key
            #else
            return self.localizedString(key: key, bundle: BundleConfig.LarkAppLinkSDKAutoBundle, moduleName: "LarkAppLinkSDK", lang: lang) ?? key
            #endif
        }
     ...
     */
    final class LarkAppLinkSDK {
        @inlinable
        static var __OpenPlatform_AppLink_Check_UserID_FeatureToast: String {
            return LocalizedString(key: "FtI", originalKey: "OpenPlatform_AppLink_Check_UserID_FeatureToast")
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        static func OpenPlatform_AppLink_Check_UserID_FeatureToast(lang __lang: Lang? = nil) -> String {
            var template = LocalizedString(key: "FtI", originalKey: "OpenPlatform_AppLink_Check_UserID_FeatureToast", lang: __lang)
            template = template.replacingOccurrences(of: "{{APP_DISPLAY_NAME}}", with: "\(LanguageManager.bundleDisplayName)")
            return template
        }
        @inlinable
        static var OpenPlatform_AppLink_NullFeatureToast: String {
            return LocalizedString(key: "YKs", originalKey: "OpenPlatform_AppLink_NullFeatureToast")
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func OpenPlatform_AppLink_NullFeatureToast(lang __lang: Lang? = nil) -> String {
            return LocalizedString(key: "YKs", originalKey: "OpenPlatform_AppLink_NullFeatureToast", lang: __lang)
        }
    }
}
