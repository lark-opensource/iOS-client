...
private let LKCalendarReactionCommaAttributeName = NSAttributedString.Key(rawValue: "LKCalendarReactionCommaAttributeName")
...
final class ReactionTagLayoutItem {
    ...
    private lazy var iconRect: CGRect = {
        // 尺寸不固定, 需要从资源列表获取
        let width: CGFloat = Cons.contentHeight
    ...
    private lazy var nameX: CGFloat = {
        if FG.rsvpStyleOpt {
            return (rsvpStatusType == .needsAction) ? Cons.hMargin :  Cons.hMargin + iconRect.width + Cons.hPadding
        } else{
            return separatorRect.maxX + Cons.hPadding
        }
    }()
    ...
    var tagBgColor: UIColor = UDColor.calendarRSVPCardReactionTagBgColor
    var separatorColor: UIColor = UIColor.ud.lineDividerDefault
    /// --- TagLayoutItem begin ---
    ...
    let rsvpStatusType: ReplyStatus
    ...
    init?(rsvpStatusType: ReplyStatus,
          justShowCount: Bool,
          userNameAndIds: [(String, String)],
          ownerChatterId: String) {
        /// copy reaction
        self.rsvpStatusType = rsvpStatusType
        self.justShowCount = justShowCount
        ...
        let isOwnerNeedAction: Bool = (rsvpStatusType == .needsAction && userIDs[safeIndex: 0] == ownerChatterId)
        configOwnerNeedActionTag(isOwnerNeedAction: isOwnerNeedAction)
    }
    
    private func configOwnerNeedActionTag(isOwnerNeedAction: Bool) {
        textColor =  isOwnerNeedAction ? UIColor.ud.staticWhite : UIColor.ud.textCaption
        tagBgColor = isOwnerNeedAction ? UIColor.ud.primaryPri500 : UDColor.calendarRSVPCardReactionTagBgColor
    }
    ...
    private func processReactionAppendTextAndWidth(nameCount: Int) -> (String, CGFloat) {
        let totalCount = nameCount

        let text = I18n.Calendar_G_DetailMorePeople(count: totalCount)
    ...
    private func setUserNames() {
        if justShowCount {
            let count = self.userNames.count
            let width = self.width(for: I18n.Calendar_G_DetailMorePeople(count: count), availableWidth: .greatestFiniteMagnitude)
            self.setUserNamesAndWidth(with: I18n.Calendar_G_DetailMorePeople(count: count), width: width)
            return
        }
        ...
        var maxAvailableWidth = self.preferMaxLayoutWidth - self.nameRect.minX - Cons.hMargin - toleranceValue
        if rsvpStatusType == .needsAction { maxAvailableWidth /= 2 }
        self.userNameWidths = []
...
extension ReactionTagLayoutItem {
    ...
    private func checkNeedAppendMore(limitedWidth: CGFloat, ctInfo: CoreTextInformation) -> (findedIndex: Int, needEllipsis: Bool)? {
        ...
        if self.userNames.count > self.canShowUserNameCount {
    ...
    private func buildAttributedString() -> NSAttributedString {
        ...
        if self.userNames.count > 1 {
            ...
            for i in 1..<needCount {
                attrStr.append(NSAttributedString(string: I18n.Calendar_Common_DivideSymbol,
                                                  attributes: [.font: Cons.labelFont,
                                LKCalendarReactionCommaAttributeName: LKCalendarReactionCommaAttributeName.rawValue]))
    ...
    private func buildCoreTextInformation(attrStr: NSAttributedString) -> CoreTextInformation {
        ...
        for runIdx in 0..<lktextLine.runs.count {
            ...
            // comma run
            if runAttrDic.value(forKey: LKCalendarReactionCommaAttributeName.rawValue) != nil {
    ...
    private func generateTextWithIndex(_ index: Int, ctInfo: CoreTextInformation) -> String {
        ...
        let prefixString: String = self.userNames.prefix(self.canShowUserNameCount).joined(separator: I18n.Calendar_Common_DivideSymbol)
        if index == ctInfo.glyphWidthArr.count - 1 {
            return prefixString
        }
...
extension ReactionTagLayoutItem {
    ...
    func reactionTagModel() -> ReactionTagModel {
        ...
        model.rsvpStatusType = self.rsvpStatusType
        return model
    }
}
