//
//  MeetingRoomFormSingleSelectionView.swift
//  Calendar
//
...
final class MeetingRoomFormSingleSelectionView: UIView {
    ...
    private let selectedOptionViewRelay = PublishRelay<MeetingRoomFormSingleSelectionOptionView?>()
    ...
    let question: Rust.CustomizationQuestion

    init(question: Rust.CustomizationQuestion, selectedOptionKey: String?, userInput: String?) {
        ...
        for option in question.options {
            let itemView = MeetingRoomFormSingleSelectionOptionView()
            ...
            if option.isOthers {
                ...
                if option.optionKey == selectedOptionKey {
            ...
            itemView.isSelected = (selectedOptionKey == option.optionKey)
            optionsStackView.addArrangedSubview(itemView)

            itemView.selectedRelay
                .distinctUntilChanged()
                .map { [weak itemView] in $0 ? itemView : nil }
                .bind(to: selectedOptionViewRelay)
                .disposed(by: bag)
        }

        selectedOptionViewRelay
            .distinctUntilChanged()
            .map {
                (question.indexKey, $0.map { [$0.key] } ?? [])
            }
...
private final class MeetingRoomFormSingleSelectionOptionView: UIView {
    ...
    private lazy var indicatorView = UDCheckBox()
