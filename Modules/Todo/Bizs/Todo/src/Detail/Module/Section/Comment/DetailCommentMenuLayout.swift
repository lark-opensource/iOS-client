//
//  DetailCommentMenuLayout.swift
//  Todo
//
//  Created by 张威 on 2021/3/5.
//
...
class DetailCommentMenuLayout: MenuBarLayout {
    private let leadingInset: CGFloat
    private let horizontalCenter: Bool

    init(leadingInset: CGFloat = 10.0, horizontalCenter: Bool = Display.phone) {
    ...
    func calculateUpdate(info: MenuLayoutInfo, downward: Bool, offset: CGPoint) -> CGRect {
        if let origin = info.origin, let location = info.transformTrigerLocation() {
            if downward {
                let origin = CGPoint(x: origin.origin.x + offset.x, y: origin.origin.y + offset.y)
                return CGRect(origin: origin, size: info.menuSize)
            } else {
                var yPos: CGFloat
                if location.y < info.menuVC.view.bounds.height / 2 {
                    yPos = origin.origin.y
                } else {
                    yPos = origin.origin.y + origin.height - info.menuSize.height
                }
                let origin = CGPoint(
