...
def create_image_asset(file_name, name, size_in_pt)
  ...
  %x(
    cairosvg ./temp/imageset_icons/#{file_name}.svg --output-width #{size_2x} --output-height #{size_2x} -o ./#{name}.imageset/#{name}@2x.png
    cairosvg ./temp/imageset_icons/#{file_name}.svg --output-width #{size_3x} --output-height #{size_3x} -o ./#{name}.imageset/#{name}@3x.png
  )

  if has_dm
    %x(
      cairosvg ./temp/imageset_icons/#{file_name}-dm.svg --output-width #{size_2x} --output-height #{size_2x} -o ./#{name}.imageset/#{name}-dm@2x.png
      cairosvg ./temp/imageset_icons/#{file_name}-dm.svg --output-width #{size_3x} --output-height #{size_3x} -o ./#{name}.imageset/#{name}-dm@3x.png
    )
  end
...
def generate_image_assets()
  ...
  Pathname.new("./temp/imageset_icons").children.each do |path|
    threads << Thread.new do
      ...
      create_image_asset(fileName, name, iconSizeMap[name])
  ...
  iconfontHash.keys.each do |iconName|
    ...
    resourceNameMap[camelIconName] = 'UDIcon.iconFontEnable ? "' + iconHashName + '" : ' + iconRawName
  end
