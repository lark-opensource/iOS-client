...
struct JulianDayUtil {

    typealias TimeZoneIdentifier = String
    ...
    static let julianDayFrom1900_01_01: JulianDay = julianDay(fromYear: 1900, month: 1, day: 1)
    /// 2000 年第一天（2000 年 01 月 01 日）的 julianDay
    static let julianDayFrom2000_01_01: JulianDay = julianDay(fromYear: 2000, month: 1, day: 1)
    /// 2100 年第一天（2100 年 01 月 01 日）的 julianDay
    static let julianDayFrom2100_01_01: JulianDay = julianDay(fromYear: 2100, month: 1, day: 1)
    ...
    static let someTimeZoneIdentifiersThatDoNotObserveDaylightSavingTime: Set<String> = [
        "Asia/Shanghai",
        "Asia/Chongqing",
        "Asia/Urumqi",
        "Asia/Taipei",
    ...
    /// - returns: julian day
    static func julianDay(fromYear year: Int, month: Int, day: Int) -> JulianDay {
    ...
    /// - returns: julian day
    static func julianDay(from date: Date, in timeZone: TimeZone) -> JulianDay {
    ...
    /// - parameter timeZone: timeZone
    /// - returns: julian day
    static func julianDay(from timestamp: Timestamp, in timeZone: TimeZone) -> JulianDay {
    ...
    /// - Returns: 星期
    static func weekday(from julianDay: JulianDay) -> EKWeekday {
    ...
    static func yearMonthDay(from julianDay: JulianDay) -> (year: Int, month: Int, day: Int) {
        let J = Int(julianDay)
        let y = 4716
        let v = 3
        let j = 1401
    ...
    static func startOf2000_01_01(in timeZone: TimeZone) -> Timestamp {
    ...
    /// - returns: 对应 day 的第一秒
    static func startOfDay(for julianDay: JulianDay, in timeZone: TimeZone) -> Timestamp {
        let ret: Timestamp
    ...
    static func endOfDay(for julianDay: JulianDay, in timeZone: TimeZone) -> Timestamp {
    ...
    /// - parameter timeZone: first week day
    /// - returns: range of julian day
    static func julianDayRange(inSameMonthAs refJulianDay: JulianDay) -> JulianDayRange {
    ...
    static func date(from julianDay: JulianDay, in timeZone: TimeZone = .current) -> Date {
