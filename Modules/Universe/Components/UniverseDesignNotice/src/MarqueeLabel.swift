...
//                    }
    ...
    */
    ...
    open func labelShouldScroll() -> Bool {
        ...
        guard let text = sublabel.text else {
            return false
        }
        ...
        if !super.adjustsFontSizeToFitWidth {
            ...
            let resizedString = NSMutableAttributedString.init(attributedString: sublabel.attributedText ?? NSAttributedString())
            resizedString.beginEditing()
            // Enumerate all font attributes of attributed string
            resizedString.enumerateAttribute(.font, in: NSRange(0..<(sublabel.attributedText ?? NSAttributedString()).length)) { val, rng, _ in
    ...
    private func scroll(_ scroller: MLAnimation, fader: MLAnimation?) {
        ...
        // Check for IBDesignable
            ...
            if fadeLength > 0.0 {
                ...
                guard let gradientAnimation = gradientAnimation else { return }
    ...
    private func generateScrollAnimation(_ sequence: [MarqueeStep]) -> MLAnimation {
        ...
        guard let scrollSteps = sequence.filter({ $0 is ScrollStep }) as? [ScrollStep] else { return MLAnimation(CAKeyframeAnimation(), 0)}
    ...
    */
    ...
    public func pauseLabel() {
        ...
        maskLayer?.speed = 0.0
        maskLayer?.timeOffset = gradientPauseTime ?? CFTimeInterval()
    }
    ...
    public func unpauseLabel() {
        ...
        let gradientPauseTime = maskLayer?.timeOffset ?? 0
        maskLayer?.speed = 1.0
        maskLayer?.timeOffset = 0.0
        maskLayer?.beginTime = 0.0
        maskLayer?.beginTime = (maskLayer ?? CAGradientLayer()).convertTime(CACurrentMediaTime(), from:nil) - gradientPauseTime
    }
