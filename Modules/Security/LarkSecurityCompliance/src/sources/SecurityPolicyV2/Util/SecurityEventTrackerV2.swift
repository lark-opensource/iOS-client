...
extension SecurityPolicyV2 {
    final class SecurityPolicyEventTrack {
        static func larkSCSFileStrategyResult(resultGroups: [PolicyModel: ValidateResult], function: SecurityAuthFunction, additional: [String: String]) {
            ...
            SecurityPolicy.logger.info("security policy:\(function) validate, security sdk get policy result: \(resultGroups)", additionalData: additional)
        }

        static func larkSCSSecuritySDKResult(resultGroups: [PolicyModel: ValidateResult], function: SecurityAuthFunction, additional: [String: String]) {
            resultGroups.forEach {
                let policyModel = $0.key
                let result = $0.value
                let entityDic = policyModel.entity.asParams()
        ...
        static func larkSCSActionFailPopUpView(businessType: String, actionType: String) {
            ...
            SecurityPolicy.logger.info("security policy: pop intercept dialog with \(businessType), \(actionType)")
            Events.track(servicename, params: params)
        }

        static func larkSCSUnknownAction(actionName: String) {
            let appVersion = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "0"
            SecurityPolicy.logger.error("security policy: unknow action name \(actionName)")
        ...
        static func scsSecurityPolicyHitDelayClearCache(policyModel: PolicyModel, duration: Double) {
            ...
            SecurityPolicy.logger.info("security policy: hit delay clear cache \(policyModel.pointKey.rawValue)")
            SCMonitor.info(business: .security_policy,
        ...
        static func larkStaticCacheMigrate(source: String) {
            SCMonitor.info(business: .security_policy,
                           eventName: "static_cache_migrate_v3",
                           category: ["source": source])
            SecurityPolicy.logger.info("security policy \(source) migrate v3 success")
        }
        
        static func larkStaticCacheCountError(count: Int) {
            SCMonitor.info(business: .security_policy,
                           eventName: "static_cache_count_error",
        ...
        static func scsSecurityPolicyInitVersion() {
            SCMonitor.info(business: .security_policy,
                           ...
                           category: ["version": "V2"])
            SecurityPolicy.logger.info("init SP V2")
        }
    }
