...
extension SecurityPolicy {
    ...
    struct ResultAggregator {
        ...
        private var fallbackAction: Action {
            Action(name: "UNIVERSAL_FALLBACK_COMMON")
        }
        
        let userResolver: UserResolver
        
        let fallbackResultManager: SceneFallbackResultProtocol
        private let settings: Settings
        
        init(resolver: UserResolver) throws {
            userResolver = resolver
            fallbackResultManager = try resolver.resolve(assert: SceneFallbackResultProtocol.self)
            settings = try userResolver.resolve(assert: Settings.self)
        }
        
        func merge(policyModel: PolicyModel, results: [ValidateResultProtocol], interceptHandler: (ValidateResultProtocol) -> Void) -> ValidateResult {
            ...
            guard !results.isEmpty else {
                ...
                interceptHandler(fastPassResponse)
            ...
            if denyResults.isEmpty {
                ...
                if let allowResult = allowResult(results) {
                    interceptHandler(allowResult)
                    return allowResult.validateResult(userResolver: userResolver, logInfos: validateInfos)
                }
                // 获取兜底结果
                let fallbackResult = getClientFallBackResult(policyModel, validateInfo: validateInfos)
                let correctValidateResp = ValidateResponse(effect: fallbackResult.result == .deny ? .deny : .permit,
                                                           actions: [fallbackAction],
                                                           uuid: UUID().uuidString,
                                                           type: .downgrade)
                interceptHandler(correctValidateResp)
                ...
                interceptHandler(denyResult)
                return denyResult.validateResult(userResolver: userResolver, logInfos: validateInfos)
            }
        }
        
        // 获取兜底结果
        private func getClientFallBackResult(_ policyModel: PolicyModel, validateInfo: [ValidateLogInfo]) -> ValidateResult {
            let rawActions = fallbackAction.rustActionModel.rawActionModelString
            guard policyModel.pointKey == .imFileRead,
                ...
                let extra = ValidateExtraInfo(resultSource: .unknown, errorReason: nil, resultMethod: .fallback, isCredible: false, logInfos: [], rawActions: rawActions)
                return ValidateResult(userResolver: userResolver, result: result, extra: extra)
            }
            let result = (fallbackResultManager[senderTenantID]) ? ValidateResultType.allow : ValidateResultType.deny
            let extra = ValidateExtraInfo(resultSource: .unknown, errorReason: nil, resultMethod: .fallback, isCredible: false, logInfos: validateInfo, rawActions: rawActions)
            return ValidateResult(userResolver: userResolver, result: result, extra: extra)
        }
