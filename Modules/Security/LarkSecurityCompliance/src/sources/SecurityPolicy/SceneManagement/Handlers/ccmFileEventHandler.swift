...
class CcmFileEventHandler: SceneEventHandler {
    var sceneContext: SecurityPolicy.SceneContext
    let userResolver: UserResolver
    private var cache: SecurityPolicyCacheProtocol?
    var timer: SCTimer
    let dlpPeriodOfValidity: Int
    let dlpmanager: DLPManagerProtocol?
    ...
    init(resolver: UserResolver, sceneContext: SecurityPolicy.SceneContext) {
        ...
        dlpmanager = try? userResolver.resolve(assert: DLPManagerProtocol.self)
