...
public class FIFOCache: CacheProtocol {
    var type: CacheType {
    ...
    private(set) var count: Int {
        ...
        set {
            ...
            if newValue % 100 == 0 {
                SecurityPolicyEventTrack.scsSecurityPolicyDynamicCapacity(pointKey: cacheKey,
                                                                          current: newValue)
            }
        }
    }
    ...
    func markInvalid() {
        ...
        while index < self.count {
            let key = store.value(forKey: ((index + first) % max).idx) ?? ""
            guard !key.isEmpty,
                  var value: SceneLocalCache = store.value(forKey: key) else {
                index += 1
                continue
            }
            value.markInvalid()
    ...
    func getSceneCacheHeadAndTail() -> String {
        guard let firstKey: String = store.value(forKey: first.idx),
              let currentKey: String = store.value(forKey: current.idx) else { return "" }
        return firstKey + "\n\n" + currentKey
    }
