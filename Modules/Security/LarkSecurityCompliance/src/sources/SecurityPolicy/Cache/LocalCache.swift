...
final class LocalCache {
    ...
    func writeCache<T: Codable>(value: T) {
        Self.writeCacheQueue.async { [weak self] in
            ...
            SPLogger.info("security policy:local_cache: \(self.cacheKey) write local cache")
    ...
    func readCache<T: Codable>() -> T? {
        ...
        guard let result else {
            SPLogger.info("security policy:local_cache: \(cacheKey) local cache is empty")
            return nil
        }
        return result
    }
    ...
    func clear() {
        Self.writeCacheQueue.async { [weak self] in
            guard let self else { return }
            SPLogger.info("security policy:local_cache: \(self.cacheKey) clear local cache")
