# frozen_string_literal: true

#
# Be sure to run `pod lib lint LarkEMM.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
# To learn more about EEScaffold see http://eescaffold.web.bytedance.net
# To learn more about podspec.patch see http://eescaffold.web.bytedance.net/docs/en/podspec/patch

Pod::Spec.new do |s|
  # 修改此文件前请先浏览 *Podspec规范* https://bytedance.feishu.cn/space/doc/doccnZwORNUpwphkrhiTgv#
  s.name             = 'LarkEMM'
  s.version = '5.31.0.5472055'
  s.summary          = 'Lark 安全合规EMM能力组件'
  s.description      = 'Lark 安全合规EMM能力组件'
  s.homepage         = 'https://code.byted.org/lark/ios_security_and_compliance'

  # 界面相关的Pod必填。设置为展示该界面功能的图片地址
  # s.screenshots     = "www.example.com/screenshots_1", "www.example.com/screenshots_2"

  # 责任人，必填。必要时要及时更新该信息
  s.authors = {
    "name": 'wangxijing@bytedance.com'
  }

  s.ios.deployment_target = '12.0'
  s.swift_version = '5.0'

  # 以下2个字段不要修改。EEScaffold会自动修改source字段为生成的zip包地址。
  s.license          = 'MIT'
  s.source           = { git: 'generated_by_eesc.zip', tag: s.version.to_s }

  attributes_hash = s.instance_variable_get('@attributes_hash')
  # setup custom bot 参考教程：https://docs.bytedance.net/doc/fuHCWYbPdHZTGODh1DbiIa#jjJE6r
  # 使用版本机器人，请关闭注释，然后填写你的bot的token到下面
  # attributes_hash['lark_group'] = {
  #  "bot": "TOKEN"
  # }
  attributes_hash['extra'] = {
    # 设置为该Pod所在的Repo的Git地址
    "git_url": 'https://code.byted.org/lark/ios_security_and_compliance'
  }
  
  s.subspec 'Default' do |ss|
    # s.public_header_files = 'Pod/Classes/**/*.h'
    ss.source_files = 'src/**/*.{swift,h,m}'
    ss.exclude_files = ['src/Debug/**/*', 'src/Tests/**/*']
    ss.resource_bundles = {
        'LarkEMM' => ['resources/*.lproj/*', 'resources/*'],
        'LarkEMMAuto' => ['auto_resources/*.lproj/*', 'auto_resources/*', 'R/**/*']
    }

    ss.dependency 'LarkBizAvatar'
    ss.dependency 'LarkLocalizations'
    ss.dependency 'UniverseDesignToast'
    ss.dependency 'LarkContainer'
    ss.dependency 'Swinject'
    ss.dependency 'LarkAssembler'
    ss.dependency 'LarkAccountInterface'
    ss.dependency 'BootManager'
    ss.dependency 'LarkSecurityAudit'
    ss.dependency 'LarkSecurityComplianceInfra'
    ss.dependency 'RxSwift'
    ss.dependency 'RxCocoa'
    ss.dependency 'ServerPB'
    ss.dependency 'LarkAccountInterface'
    ss.dependency 'ByteDanceKit'
    ss.dependency 'EENavigator'
    ss.dependency 'LKCommonsLogging'
    ss.dependency 'LarkSetting'
    ss.dependency 'UniverseDesignDialog'
    ss.dependency 'UniverseDesignCheckBox'
    ss.dependency 'LKLoadable'
    ss.dependency 'AppContainer'
    ss.dependency 'LarkSensitivityControl/API/Pasteboard'
    ss.dependency 'LarkAppLinkSDK'
    ss.dependency 'LarkOpenSetting'
    ss.dependency 'LarkSettingUI'
    ss.dependency 'LarkSecurityComplianceInterface'
    ss.dependency 'LarkExtensions'
  end
  
  s.default_subspec = 'Default'

  s.subspec 'Debug' do |ss|
    ss.source_files = 'src/Debug/**/*'
    ss.xcconfig = {
      "SWIFT_ACTIVE_COMPILATION_CONDITIONS" => 'SECURITY_DEBUG',
      "GCC_PREPROCESSOR_DEFINITIONS" => 'SECURITY_DEBUG=1'
    }
    ss.dependency 'LarkEMM/Default'
  end
  
end
