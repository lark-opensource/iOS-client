...
//            }
        case .advancedSearch(let count):
            ...
            leftLabel.text = BundleI18n.MailSDK.Mail_shared_FilterSearch_Filters_Mobile_Title +  countStr
            leftLabel.isHidden = false
    ...
    private func expandButtonClick() {
        guard let block = self.onClickExpandView, var model = capsuleModel else { return }
        if expandButton.buttonState == .cancel {
    ...
    private func updateExpandButton(state: SearchIntentionCapsuleExpandButton.State) {
        ...
        switch state {
        case .showMore:
            image = UDIcon.expandDownFilled.withRenderingMode(.alwaysOriginal).ud.withTintColor(UIColor.ud.iconN2)
        case .cancel:
        ...
        expandButton.buttonState = state
...
//            }
        case .advancedSearch(let count):
            ...
            title = BundleI18n.MailSDK.Mail_shared_FilterSearch_Filters_Mobile_Button + title
...
final class SearchIntentionCapsuleExpandButton: UIButton {
    ...
    var buttonState: SearchIntentionCapsuleExpandButton.State = .showMore
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        let insets = UIEdgeInsets(top: -10, left: -2, bottom: -10, right: -12)
        if bounds.inset(by: insets).contains(point) {
            return self
        }
        return super.hitTest(point, with: event)
