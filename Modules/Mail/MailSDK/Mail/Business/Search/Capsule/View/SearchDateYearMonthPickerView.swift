...
final class SearchDateYearMonthPickerView: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    private static let defaultMinDate: Date = Date(year: 1900, month: 1, day: 1)
    private static let defaultMaxDate: Date = Date(year: 2099, month: 12, day: 31)
    ...
    private func layoutGradientLayer() {
        ...
        topLayer.colors = [UIColor.ud.bgBody.withAlphaComponent(1.0).cgColor,
                           UIColor.ud.bgBody.withAlphaComponent(0.0).cgColor]
        bottomLayer.colors = [UIColor.ud.bgBody.withAlphaComponent(0.0).cgColor,
                              UIColor.ud.bgBody.withAlphaComponent(1.0).cgColor]
    }
    ...
    public func updateDefaultSelected(year: Int, month: Int) {
        let config = SearchDateYearMonthPickerConfig(minDate: minDate, maxDate: maxDate, defaultSelectedYear: year, defaultSelectedMonth: month)
        if config.isLegal {
            selectedYear = year
            selectedMonth = month
    ...
    private func locateToDefaultSelected() {
        ...
        if yearIndexPath.row >= 0, yearIndexPath.row < yearTableView.numberOfRows(inSection: 0) {
            ...
            yearTableView.scrollToRow(at: yearIndexPath, at: .middle, animated: false)
        ...
        if monthIndexPath.row >= 0, monthIndexPath.row < monthTableView.numberOfRows(inSection: 0) {
            ...
            monthTableView.scrollToRow(at: monthIndexPath, at: .middle, animated: false)
        ...
        if let _year = year, let _month = month {
            ...
            if _year == minDate.year, _month < minDate.month {
                if minDate.month + 1 >= monthTableView.numberOfRows(inSection: 0) {
                    ...
                    monthTableView.scrollToRow(at: IndexPath(row: minDate.month + 1, section: 0), at: .middle, animated: true)
                    return
                }
            }
            if _year == maxDate.year, _month > maxDate.month {
                if maxDate.month + 1 >= monthTableView.numberOfRows(inSection: 0) {
                    ...
                    monthTableView.scrollToRow(at: IndexPath(row: maxDate.month + 1, section: 0), at: .middle, animated: true)
                    return
                }
            }
