...
final class SearchInputFilterViewController: BaseUIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func updateSaveButtonTitle() {
        ...
        self.saveButton.setTitle(BundleI18n.MailSDK.Mail_Alert_Confirm + countStr, for: .normal) //
    ...
    override func viewDidLoad() {
        ...
        self.tableView.rx.observe(CGSize.self, "contentSize")
            ...
            .disposed(by: bag)
        selectedLabel.text = BundleI18n.MailSDK.Mail_shared_FilterSearch_AlreadySelected_Mobile_Text + (self.title ?? "")// "BundleI18n.LarkSearch.Lark_Search_EmailSearch_FiltersCommon_SelectSearchResults_Selected" + (self.title ?? "")
        if scene == .subjectFilter {
            inputTextField.placeholder = BundleI18n.MailSDK.Mail_AdvancedSearchFilter_PlaceholderPlsEnter_KeywordsInSubject
        } else if scene == .excludeFilter {
            inputTextField.placeholder = BundleI18n.MailSDK.Mail_AdvancedSearchFilter_PlaceholderPlsEnter_ExcludeKeywords
        }
