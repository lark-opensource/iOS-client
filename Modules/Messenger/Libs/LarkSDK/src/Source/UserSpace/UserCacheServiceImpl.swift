...
final class UserCacheServiceImpl: UserCacheService, UserResolverWrapper {
    @ScopedInjectedLazy private var userSpaceService: UserSpaceService?
    ...
    init(docsUserCacheService: DocsCacheDependency, userResolver: UserResolver) {
        self.docsUserCacheService = docsUserCacheService
    ...
    func calculateCacheSize() -> Observable<Float> {
        let native = Observable<Float>.create { [weak self] (observer) -> Disposable in
            ...
            if Path.useLarkStorage {
                fileSize = currentUserDirectoryPath.asAbsPath().recursiveFileSize()
            } else {
                fileSize = Path.Old(currentUserDirectoryPath).recursizeFileSize
            }
    ...
    func clearCache() -> Observable<Float> {
        ...
        let nativeClear = Observable<Void>.create { [weak self] (observer) -> Disposable in
            ...
            if Path.useLarkStorage {
                currentUserDirectoryPath.asAbsPath().eachChildren { try? $0.notStrictly.removeItem() }
            } else {
                Path.Old(currentUserDirectoryPath).eachChildren { try? $0.deleteFile() }
            }
            observer.onNext(())
            observer.onCompleted()
            return Disposables.create()
