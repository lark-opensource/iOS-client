...
final class BaseUserAppConfig: UserAppConfig {
    ...
    func resourceAddrWithLanguage(key: String) -> String? {
        ...
        if let settingDict = try? settingService.setting(with: UserSettingKey.make(userKeyLiteral: "client_dynamic_link")), let tempAddr = settingDict[key] {
        ...
        return url.append(parameters: ["lang": LanguageManager.currentLanguage.languageCode ?? "0"]).absoluteString
    }
