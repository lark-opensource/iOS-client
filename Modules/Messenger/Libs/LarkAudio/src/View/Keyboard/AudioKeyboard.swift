...
extension LarkBaseKeyboard.LarkKeyboard {
    ...
    final class AudioGestureKeyboardConfig: UserResolverWrapper {
        let userResolver: UserResolver
        let supportRecognize: Bool
        let audioWithTextEnable: Bool
        let iconColor: UIColor?

        let macInputStyle: Bool
        ...
        let longPressKeyboardAudioToTextHandler: (UILongPressGestureRecognizer) -> RecognizeAudioTextGestureKeyboard
        ...
        init(userResolver: UserResolver,
             ...
             longPressKeyboardAudioToTextHandler: @escaping (UILongPressGestureRecognizer) -> RecognizeAudioTextGestureKeyboard) {
            self.userResolver = userResolver
            self.supportRecognize = supportRecognize
            self.audioWithTextEnable = audioWithTextEnable
            self.iconColor = iconColor
            self.macInputStyle = macInputStyle
    ...
    static func buildNewVoice(userResolver: UserResolver,
                              _ config: AudioGestureKeyboardConfig,
                              getFrom: @escaping () -> UIView?) -> InputKeyboardItem {
        let keyboardItem = AudioKeyboard.keyboard(
        ...
        let keyboardViewBlock = { () -> UIView in
            ...
            let recordTextVM = AudioWithTextRecordViewModel(
            ...
            if config.supportRecognize && config.audioWithTextEnable {
                let recordText = RecordWithTextAudioKeyboardWithLID(userResolver: userResolver,
                                                                    viewModel: recordTextVM,
                                                                    delegate: config.recordTextDelegate)
                recordText.macInputStyle = config.macInputStyle
                items.append(recordText)
            }

            let recordVM = AudioRecordViewModel(userResolver: userResolver, audioRecognizeService: config.recognizeService, uploadIdBlock: config.uploadIdBlock)
            let record = RecordAudioKeyboard(userResolver: userResolver, viewModel: recordVM, delegate: config.recordDelegate)
            items.append(record)

            if config.supportRecognize {
                let recognitionVM = AudioRecognizeViewModel(
                    userResolver: userResolver,
                    audioRecognizeService: config.recognizeService,
                    from: .audioMenu
                )
                let recognition = RecognitionAudioKeyboardWithLID(userResolver: userResolver,
                                                                    viewModel: recognitionVM,
                                                                    delegate: config.recognitionDelegate)
                recognition.macInputStyle = config.macInputStyle
                items.append(recognition)
            }

            let container: AudioCollectionContainerView
            container = AudioKeyboardContainer(items: items)
        ...
        let handleLongPressBlock = { (gesture: UILongPressGestureRecognizer) in
            ...
            if let topWindow = windows.first(where: { (window) -> Bool in
                ...
                if !supportRecognize {
                    ...
                    let recordVM = AudioRecordViewModel(userResolver: userResolver, audioRecognizeService: config.recognizeService, uploadIdBlock: config.uploadIdBlock)
                    recordView = RecordAudioGestureKeyboard(userResolver: userResolver, viewModel: recordVM, gesture: gesture, delegate: config.recordGestureDelegate)
                } else {
                    ...
                    case .audio:
                        ...
                        recordView = RecordAudioGestureKeyboard(userResolver: userResolver, viewModel: recordVM, gesture: gesture, delegate: config.recordGestureDelegate)
                    case .text:
                        let recognizeVM = AudioRecognizeViewModel(
                            userResolver: userResolver,
                            audioRecognizeService: config.recognizeService,
                            from: .audioButton
                        )
                        recordView = RecognizeAudioGestureKeyboard(userResolver: userResolver, viewModel: recognizeVM, gesture: gesture, delegate: config.recognitionGestureDelegate)
                    }
                }
        ...
        let showNetworkErrorIfNeeded: () -> Bool = {
            guard let from = getFrom()?.window else {
                assertionFailure("缺少路由跳转的From window")
                return false
            }
...
public final class AudioKeyboard {
        ...
        case .audioWithText:
            if audioWithTextEnable {
                return KeyboardInfo(
                    ...
                    selectedIcon: Resources.new_voice_with_text_icon_select,
                    unenableIcon: nil,
                    tintColor: tintColor
                )
            } else {
