...
final class AudioRecognizeViewModel: UserResolverWrapper {

    @ScopedInjectedLazy var audioTracker: NewAudioTracker?
    @ScopedInjectedLazy var audioRecordManager: AudioRecordManager?

    fileprivate static let logger = Logger.log(AudioRecognizeViewModel.self, category: "LarkAudio")
    ...
    func startRecognition(language: Lang, useAveragePower: Bool = false) {
        ...
        self.audioRecordManager?.startRecord(
            ...
            if result {
                ...
                AudioRecognizeViewModel.logger.info("start recognition", additionalData: ["uploadid": self.uploadID])
            } else {
                if self.state != .normal {
                    self.state = .normal
                    self.hadFinish = true
                    self.delegate?.audioRecordStartFailed(uploadID: self.uploadID)
                    AudioRecognizeViewModel.logger.error("start recognition failed", additionalData: ["uploadid": self.uploadID])
                }
            }
        }
    }
    ...
    func endRecord() {
        AudioRecognizeViewModel.logger.info("end record", additionalData: ["uploadid": self.uploadID])
        if self.isRecording {
...
extension AudioRecognizeViewModel {
    ...
    fileprivate func handleRecordSuccess(pcmdata: Data, recordTime: TimeInterval) {
        ...
        self.handleRecordStreamData(data: Data(), action: .end, callback: { error in
            if let error = error {
                AudioRecognizeViewModel.logger.error("audio recognize callback failed", error: error)
            }
        })
    }
    ...
    fileprivate func handleRecordStreamDataInQueue(
        ...
        let from = self.from
        AudioRecognizeViewModel.logger.info(
...
extension AudioRecognizeViewModel: RecordAudioDelegate {
    func audioRecordUpdateMetra(_ metra: Float) {
        self.handleUpdateMetra(metra)
    }

    func audioRecordStateChange(state: AudioRecordState) {
        AudioRecognizeViewModel.logger.info("audio record state change", additionalData: ["state": "\(state)"])
        switch state {
        case .cancel:
