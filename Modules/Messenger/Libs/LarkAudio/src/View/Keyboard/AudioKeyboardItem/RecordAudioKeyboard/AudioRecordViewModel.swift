...
protocol AudioRecordViewModelDelegate: AnyObject {
...
final class AudioRecordViewModel: UserResolverWrapper {
    ...
    fileprivate static let logger = Logger.log(AudioRecordViewModel.self, category: "LarkAudio")
    ...
    private func startRecord(recognition: Bool, useAveragePower: Bool = false) {
        ...
        self.audioRecordManager?.startRecord(
            ...
            if result {
                ...
                AudioRecordViewModel.logger.info("start recognition", additionalData: ["uploadid": self.uploadID])
            } else {
                if self.state != .normal {
                    self.state = .normal
                    self.hadFinish = true
                    self.delegate?.audioRecordStartFailed(uploadID: self.uploadID)
                    AudioRecordViewModel.logger.error("start recognition failed", additionalData: ["uploadid": self.uploadID])
                }
            }
        }
    }
    ...
    func endRecord() {
        AudioRecordViewModel.logger.info("end Record", additionalData: ["uploadid": self.uploadID])
    ...
    func cancelRecord() {
        AudioRecordViewModel.logger.info("cancel Record", additionalData: ["uploadid": self.uploadID])
...
extension AudioRecordViewModel {
    ...
    fileprivate func handleRecordSuccess(pcmdata: Data, recordTime: TimeInterval) {
        ...
        if uploadID.isEmpty {
            ...
            self.handleRecordStreamData(data: Data(), isFinish: true, callback: { error in
                if let error = error {
                    AudioRecordViewModel.logger.error("audio record callback failed", error: error)
                }
            })
    ...
    fileprivate func handleRecordStreamData(data: Data, isFinish: Bool, callback: ((Error?) -> Void)?) {
        if isFinish {
            AudioRecordViewModel.logger.info("handle record finish", additionalData: ["uploadid": self.uploadID])
        }
        self.queue.async {
            if isFinish {
                AudioRecordViewModel.logger.info("handle record finish in queue", additionalData: ["uploadid": self.uploadID])
            }
    ...
    fileprivate func handleRecordStreamDataInQueue(opusData: Data, isFinish: Bool, callback: ((Error?) -> Void)?) {
        ...
        AudioRecordViewModel.logger.info(
...
extension AudioRecordViewModel: RecordAudioDelegate {
    func audioRecordUpdateMetra(_ metra: Float) {
        self.handleUpdateMetra(metra)
    }

    func audioRecordStateChange(state: AudioRecordState) {
        AudioRecordViewModel.logger.info("audio record state change", additionalData: ["state": "\(state)"])
        switch state {
        case .cancel:
