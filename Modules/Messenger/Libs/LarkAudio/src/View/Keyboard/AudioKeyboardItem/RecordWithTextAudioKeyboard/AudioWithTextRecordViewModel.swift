...
protocol AudioWithTextRecordViewModelDelegate: AnyObject {
...
final class AudioWithTextRecordViewModel: UserResolverWrapper {

    fileprivate static let logger = Logger.log(AudioWithTextRecordViewModel.self, category: "LarkAudio")
    ...
    func startRecognition(language: Lang, useAveragePower: Bool = false) {
        ...
        self.audioRecordManager?.startRecord(
            ...
            if result {
                ...
                AudioWithTextRecordViewModel.logger.info("start recognition", additionalData: ["uploadid": self.uploadID])
            } else {
                if self.state != .normal {
                    self.state = .normal
                    self.hadFinish = true
                    self.delegate?.audioRecordStartFailed(uploadID: self.uploadID)
                    AudioWithTextRecordViewModel.logger.error("start recognition failed", additionalData: ["uploadid": self.uploadID])
                }
            }
        }
    }
    ...
    func endRecord() {
        AudioWithTextRecordViewModel.logger.info("end record", additionalData: ["uploadid": self.uploadID])
        if self.isRecording {
...
extension AudioWithTextRecordViewModel {
    ...
    fileprivate func handleRecordSuccess(pcmdata: Data, recordTime: TimeInterval) {
        ...
        self.handleRecordStreamData(data: Data(), action: .end, callback: { error in
            if let error = error {
                AudioWithTextRecordViewModel.logger.error("audio recognize callback failed", error: error)
            }
        })
    ...
    fileprivate func handleRecordStreamData(
        ...
        if action != .unknown {
            AudioWithTextRecordViewModel.logger.info("handle record data", additionalData: ["uploadid": self.uploadID, "action": "\(action)"])
        }
        self.queue.async {
            if action != .unknown {
                AudioWithTextRecordViewModel.logger.info("handle record data in queue", additionalData: ["uploadid": self.uploadID, "action": "\(action)"])
            }
    ...
    fileprivate func handleRecordStreamDataInQueue(
        ...
        self.seqID += 1
        AudioWithTextRecordViewModel.logger.info(
...
extension AudioWithTextRecordViewModel: RecordAudioDelegate {
    func audioRecordUpdateMetra(_ metra: Float) {
        self.handleUpdateMetra(metra)
    }

    func audioRecordStateChange(state: AudioRecordState) {
        AudioWithTextRecordViewModel.logger.info("audio record state change", additionalData: ["state": "\(state)"])
        switch state {
        case .cancel:
