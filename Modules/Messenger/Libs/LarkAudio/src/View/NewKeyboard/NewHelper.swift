...
// keyboard实现。VM调用
public protocol AudioSendMessageDelegate: AnyObject {
    // 获取keyboard，用来控制keyboard的隐藏之类的
...
// VM实现。keyboard调用
public protocol AudioRecordPanelProtocol {
    func cleanMaskView()
    func trackAudioRecognizeIfNeeded()
    func cleanAudioRecognizeState()
}

public final class NewAudioKeyboardHelper: NSObject, UITextViewDelegate, EditTextViewTextDelegate, UserResolverWrapper, AudioRecordPanelProtocol {

    static let logger = Logger.log(NewAudioKeyboardHelper.self, category: "Module.Inputs")
    ...
    var recordTextInputView: NewRecordWithTextInputView?
    ...
    // ai相关
    lazy var aiAsrSDK: InlineAIAsrSDK = {
        let config = InlineAIConfig(captureAllowed: true, userResolver: self.userResolver)
        return LarkInlineAIModuleGenerator.createAsrSDK(config: config, customView: audioAIView, delegate: InlineAISDKDelete())
    }()
    let aiCollectionView = AudioShortcutCollectionView()
    ...
    weak var longPressKeyboard: NewRecognizeAudioTextGestureKeyboard?

    var longPressKeyboardAudioToTextHandler: (UILongPressGestureRecognizer) -> NewRecognizeAudioTextGestureKeyboard { { gesture in
        let recordWithTextVM = NewAudioWithTextRecordViewModel(
            userResolver: self.userResolver,
            audioRecognizeService: self.audioRecognizeService,
            from: .audioButton)
        let recordView = NewRecognizeAudioTextGestureKeyboard(
            userResolver: self.userResolver,
            viewModel: recordWithTextVM,
            gesture: gesture, delegate: self)
    ...
    var animationView: RecordAnimationView?
    ...
    lazy var audioAIView: UIView = {
        let view = UIView()
        view.backgroundColor = UDColor.bgBodyOverlay
        return view
    }()

    lazy var LongGestureButtonView: UIView = {
        let view = UIView()
        view.backgroundColor = .clear
        return view
    }()
    let caretView = CaretView(frame: .zero)
    var canShowCaret: Bool = false
        ...
        if !self.chat.isCrypto {
            ...
            RecognizeLanguageManager.shared.typeSubject
                ...
                .subscribe(onNext: { [weak self] (_) in
                    let icons = NewAudioKeyboard.keyboard(
    ...
    public func textViewDidChangeSelection(_ textView: UITextView) {
        ...
        if keyboardView.inputTextView.attributedText.string.isEmpty {
    ...
    func observeRecognizeAudio(uploadID: String) {
        ...
        self.audioRecognizeService?
            ...
            .subscribe(onNext: { [weak self] (result) in
                ...
                NewAudioKeyboardHelper.logger.error("audio recognize failed", error: error)
    ...
    func updateAudioRecognize(text: String, uploadID: String, isPrepare: Bool, finish: Bool, diffIndexSlice: [Int32] = []) {
        ...
        self.caretRange = NSRange(location: lastSelectOffset, length: 0)
        ...
        if finish {
            ...
            if aiCollectionView.superview == nil {
                showAIButton()
            }
        }
        if !showAudioRecognizeLoading {
