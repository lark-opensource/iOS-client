//
//  BurnedContentComponentBinder.swift
//  Action
//
...
final class BurnedContentComponentBinder<M: CellMetaModel, D: CellMetaModelDependency, C: BurnedContentContext>: ComponentBinder<C> {
    ...
    public override func update<VM: ViewModel>(with vm: VM, key: String? = nil) {
        guard let vm = vm as? BurnedContentViewModel<M, D, C> else {
            assertionFailure()
            return
        }
