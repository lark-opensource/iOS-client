//
//  ThreadPostCardContentComponentBinder.swift
//  LarkMessageCore
//
...
import LarkUIKit
public struct MergeForwardPostCardContentConfig {
    public static let contentMaxWidth: CGFloat = 600
}

class MergeForwardPostCardContentBaseComponentBinder<M: CellMetaModel, D: CellMetaModelDependency, C: MergeForwardContentContext>: NewComponentBinder<M, D, C> {
    let mergeForwardViewModel: MergeForwardPostCardContentViewModel<M, D, C>?
    let mergeForwardActionHandler: MergeForwardPostCardContentActionHandler<C>?

    init(
        key: String? = nil,
        context: C? = nil,
        mergeForwardViewModel: MergeForwardPostCardContentViewModel<M, D, C>?,
        mergeForwardActionHandler: MergeForwardPostCardContentActionHandler<C>?
    ) {
...
final class MergeForwardPostCardContentComponentBinder<M: CellMetaModel, D: CellMetaModelDependency, C: MergeForwardContentContext>: MergeForwardPostCardContentBaseComponentBinder<M, D, C> {
    private var props = MergeForwardPostCardContentComponent<C>.Props()
    private lazy var _component: MergeForwardPostCardContentComponent<C> = .init(props: .init(), style: .init(), context: nil)
    public override var component: MergeForwardPostCardContentComponent<C> {
        return _component
    }
    ...
    public override func buildComponent(key: String? = nil, context: C? = nil) {
        ...
        _component = MergeForwardPostCardContentComponent(props: props, style: style, context: context)
    }
}

final class MergeForwardPostCardContentBorderComponentBinder<M: CellMetaModel, D: CellMetaModelDependency, C: MergeForwardContentContext>: MergeForwardPostCardContentBaseComponentBinder<M, D, C> {
    private var props = MergeForwardPostCardContentComponent<C>.Props()
    private lazy var _component: MergeForwardPostCardContentComponent<C> = .init(props: .init(), style: .init(), context: nil)
