//
//  EmailEditFieldCell.swift
//  LarkContact
//
//  Created by SlientCat on 2019/6/7.
//
...
final class EmailEditFieldCell: UITableViewCell, FieldCellAbstractable {
    lazy var editField: UITextField = {
        ...
        field.attributedPlaceholder = NSAttributedString(string: BundleI18n.LarkContact.Lark_Invitation_AddMembersHintInputEmail,
    ...
    private var viewModel: EmailFieldViewModel = .init(state: .failed, isOversea: false) {
        didSet {
    ...
    func bindWithViewModel(viewModel: FieldViewModelAbstractable) {
        guard let viewModel: EmailFieldViewModel = viewModel as? EmailFieldViewModel else { return }
