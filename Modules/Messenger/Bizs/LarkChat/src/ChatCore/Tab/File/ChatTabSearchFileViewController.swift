//
//  ChatTabSearchFileViewController.swift
//  LarkChat
//
...
final class ChatTabSearchFileViewController: BaseUIViewController, UITableViewDataSource, UITableViewDelegate {
    private let viewModel: ChatTabSearchFileViewModel
    ...
    private var currentInChatData = ChatTabSearchFileData(searchQuery: "",
                                                          cellViewModels: [],
                                                          hasMore: false) {
        didSet {
    ...
    init(userResolver: UserResolver, chatId: String, router: ChatTabSearchFileRouter) throws {
        viewModel = try ChatTabSearchFileViewModel(userResolver: userResolver, chatId: chatId, router: router)
    ...
    override func viewDidLoad() {
        ...
        self.title = BundleI18n.LarkChat.Lark_Legacy_FileLabel
        ...
        dataTableView.register(ChatTabSearchFileTableViewCell.self, forCellReuseIdentifier: ChatTabSearchFileTableViewCell.reuseId)
        ...
        FileTabTracker.FileListView()
    }

    private func configViewHidden(state: ChatTabSearchFileState) {
        switch state {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ChatTabSearchFileTableViewCell.reuseId, for: indexPath)
        if let cell = cell as? ChatTabSearchFileTableViewCell {
