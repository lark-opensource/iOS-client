...
final public class ChatTabURLModule: ChatTabSubModule {
    static private let logger = Logger.log(ChatTabURLModule.self, category: "Lark.MessengerAssembly")
    ...
    override public func jumpTab(model: ChatJumpTabModel) {
        ...
        navigator.push(linkUrl, from: model.targetVC)
    }
    ...
    override public func getImageResource(_ metaModel: ChatTabMetaModel) -> ChatTabImageResource {
        let defaultIcon = UDIcon.getIconByKey(.linkCopyFilled, size: CGSize(width: 20, height: 20)).ud.withTintColor(UIColor.ud.iconN3)
        let dic = self.getPayloadDic(metaModel.content?.payloadJson)
        guard let iconDic = dic["icon"] as? [String: Any] else {
            return .image(defaultIcon)
        }
        let useOriginColor: Bool = (iconDic["useOriginColor"] as? Bool) ?? false
        let iconColor: UIColor? = useOriginColor ? nil : UIColor.ud.textLinkNormal
        if let udIconKey = iconDic["udIcon"] as? String, !udIconKey.isEmpty {
            return .image(URLPreviewUDIcon.getIconByKey(udIconKey, iconColor: iconColor, size: CGSize(width: 20, height: 20)) ?? defaultIcon)
        }
        if let key = iconDic["iconKey"] as? String, !key.isEmpty {
            return .key(
                key: key,
                config: ChatTabImageResource.ImageConfig(
                    tintColor: iconColor,
                    placeholder: defaultIcon,
                    imageSetPassThrough: ChatTabImagePassThroughConfig.convertToPBModel(iconDic)
                )
            )
        }
        if let key = iconDic["iconUrl"] as? String, !key.isEmpty {
            return .key(
                key: key,
                config: ChatTabImageResource.ImageConfig(
                    tintColor: iconColor,
    ...
    override public func getClickParams(_ metaModel: ChatTabMetaModel) -> [AnyHashable: Any]? {
        var params: [AnyHashable: Any] = ["tab_type": "web_tab"]
        let dic = self.getPayloadDic(metaModel.content?.payloadJson)
