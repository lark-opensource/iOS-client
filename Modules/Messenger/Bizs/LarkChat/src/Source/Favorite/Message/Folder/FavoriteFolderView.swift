...
// 自己撑开高度，宽度由外界指定
final class FavoriteFolderView: UIView {
    enum Cons {
        ...
        static var iconSize: CGSize {
            let height = titleFont.lineHeight + sizeFont.lineHeight + titleSizeSpacing
            let width = (height / 38) * 30
    ...
    var tapBlock: ((FavoriteFolderView, UIWindow) -> Void)?
    ...
    init() {
        ...
        lu.addTapGestureRecognizer(action: #selector(folderViewTapped))
    }
    ...
    func set(userResolver: UserResolver, icon: UIImage, name: String, size: String, hasPermissionPreview: Bool, dynamicAuthorityEnum: DynamicAuthorityEnum) {
