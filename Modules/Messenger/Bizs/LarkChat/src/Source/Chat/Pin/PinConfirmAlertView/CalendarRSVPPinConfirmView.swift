//
//  CalendarRSVPPinConfirmView.swift
//  LarkChat
//
//  Created by pluto on 2023/3/22.
//
...
// MARK: - CalendarRSVPPinConfirmView
final class CalendarRSVPPinConfirmView: PinConfirmContainerView {
    ...
    override func setPinConfirmContentView(_ contentVM: PinAlertViewModel) {
        ...
        guard let calendarRSVPVM = contentVM as? CalendarRSVPPinConfirmViewModel else {
            return
        }
        title.text = calendarRSVPVM.title
        eventTimeDescription.text = calendarRSVPVM.eventTimeDescription?() ?? ""
    }
}

// MARK: - CalendarSharePinConfirmViewModel
final class CalendarRSVPPinConfirmViewModel: PinAlertViewModel {
    var title: String = ""
    var eventTimeDescription: (() -> String)?
    init?(rsvpEventMessage: Message,
          getSenderName: @escaping (Chatter) -> String,
          eventTimeDescription: @escaping (_ start: Int64, _ end: Int64, _ isAllDay: Bool) -> String) {
        super.init(message: rsvpEventMessage, getSenderName: getSenderName)

        guard let content = rsvpEventMessage.content as? GeneralCalendarEventRSVPContent else {
            return nil
        }
