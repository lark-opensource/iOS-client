...
struct ThreadDetailMetaModel: CellMetaModel {
    let message: Message
    let isPrivateThread: Bool
    let originMergeForwardId: String?
    var getChat: () -> Chat
    init(message: Message,
         isPrivateThread: Bool = false,
...
final class ThreadDetailDataSource: PageContextWrapper {
    var pageContext: LarkMessageBase.PageContext { vmFactory.context }

    let threadMessageIndex = 0
    let replysIndex = 1
    private static let logger = Logger.log(ThreadDetailDataSource.self, category: "Business.ThreadDetail")
    ...
    private let vmFactory: ThreadDetailMessageCellViewModelFactory
        ...
        didSet {
            if let oldValue = oldValue {
                if let index = self.indexReply(messageId: oldValue.id),
                   let messageVM = self.cellViewModels[replysIndex][index] as? ThreadDetailMessageCellViewModel {
                    messageVM.isEditing = false
                } else if let threadMessageCellViewModel = threadMessageCellViewModel as? ThreadDetailMessageCellViewModel {
    ...
    init(
        ...
        vmFactory: ThreadDetailMessageCellViewModelFactory,
        minPosition: Int32 = -1,
        maxPosition: Int32 = -1
    ) {
        self.chat = chat
    ...
    func replaceReplys(
        ...
        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS minMax beforeChange \(self.threadId) \(self.minPosition) \(self.maxPosition) \(messages.count)")
        ...
        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS minMax afterChange \(self.threadId) \(self.minPosition) \(self.maxPosition)")
        if !invisiblePositions.isEmpty {
            ...
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS minMax change by invisiblePositions \(self.threadId) \(invisiblePositions.first ?? 0) \(invisiblePositions.last ?? 0)")
        }
        if !missedPositions.isEmpty {
            ...
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS minMax change by missedPositions \(self.threadId) \(missedPositions.first ?? 0) \(missedPositions.last ?? 0)")
        }
        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS messagesCount \(self.threadId) \(messages.count)")
    ...
    func handle(message: Message) -> HandleMessageScene {
        ...
        if let cellIndex = self.indexReply(message: message) {
            if let messageCellVM = self.cellViewModels[replysIndex][cellIndex] as? ThreadDetailMessageCellViewModel {
                ...
                if (messageCellVM.message.localStatus == .process || messageCellVM.message.localStatus == .fakeSuccess || messageCellVM.message.cid == messageCellVM.message.id), message.localStatus == .success {
                    ...
                    ThreadDetailDataSource.logger.info(
                        "chatTrace threadDetailDS msg sendSuccess: \(self.threadId) \(message.cid) \(message.id) \(message.threadPosition) \(self.maxPosition)"
                    )
                    //位置不变，但lastMsgPos还是要更新的
                    if message.threadPosition > self.maxPosition {
                        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS missedMessages: \(self.threadId) \(message.id) \(message.threadPosition) \(self.maxPosition)")
                    ...
                    ThreadDetailDataSource.logger.info("chatTrace threadDetailDS update: \(self.threadId) \(message.id) \(message.cid) \(message.threadPosition)")
        ...
        if message.threadPosition == self.maxPosition + 1 {//新消息
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS handle newMsg: \(self.threadId) \(message.id) \(message.threadPosition)")
            ...
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS handle missedMessage: \(self.threadId) \(message.id) \(message.threadPosition)")
            self.append(newReply: message)
            return .newMessage
        } else if message.localStatus != .success && self.indexReply(cid: message.cid) == nil {//新的发送态消息(因为时序和引用问题，发的新消息可能直接跳过发送态，直接进入失败态)
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS handle process quasimsg: \(self.threadId) \(message.cid)  \(message.threadPosition)")
            self.append(newReply: message)
            return .newMessage
        } else if message.threadPosition > self.maxPosition { //有新消息，但新消息已经不连续了，后续消息都不会接收
            ThreadDetailDataSource.logger.info("chatTrace threadDetailDS handle discontinuous newMsg: \(self.threadId) \(message.id) \(message.threadPosition)")
            return .none
        }
        return .none
    }
    ...
    func headInsert(messages: [Message], invisiblePositions: [Int32], concurrent: (Int, (Int) -> Void) -> Void) -> Bool {
        guard let dataMinPosition = self.minPosition(messages: messages, invisiblePositions: invisiblePositions),
            ...
            dataMaxPosition >= self.minPosition else {
                ThreadDetailDataSource.logger.info("chatTrace threadDetailDS headInsert: \(self.threadId) 未得到有效交集")
                return false
        }
    ...
    func update(thread: RustPB.Basic_V1_Thread) -> Bool {
        if self.threadId == thread.id,
            let rootCellViewModel = self.cellViewModels[threadMessageIndex].first as? ThreadDetailRootCellViewModel {
    ...
    func update(threadMessage: ThreadMessage) -> Bool {
        ...
        /// 二次编辑翻译
        translateService?.translateMessage(translateParam: MessageTranslateParameter(message: threadMessage.message,
                                                                                    source: .common(id: threadMessage.message.id),
                                                                                    chat: chat()),
                                          isFromMessageUpdate: true)
        return false
    }
    ...
    private func updateThreadMessage(_ messageId: String, doUpdate: (Message) -> Message?) -> Bool {
        var hasChange = false
        if let threadDetailRootVM = self.cellViewModels[threadMessageIndex].first as? ThreadDetailRootCellViewModel,
    ...
    func update(original: (Message) -> Message?) -> Bool {
        ...
        for (index, cellVM) in vms.enumerated() {
            if let cellVM = cellVM as? ThreadDetailRootCellViewModel,
            ...
            } else if let messageVM = cellVM as? ThreadDetailMessageCellViewModel,
...
// MARK: - ThreadDetailDataSource
private extension ThreadDetailDataSource {
    ...
    func minPosition(messages: [Message], invisiblePositions: [Int32]) -> Int32? {
        ...
        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS get minPosition: \(self.threadId) \(result) \(self.minPosition) \(self.maxPosition)")
    ...
    func maxPosition(messages: [Message], invisiblePositions: [Int32]) -> Int32? {
        ...
        ThreadDetailDataSource.logger.info("chatTrace threadDetailDS get maxPosition: \(self.threadId) \(result) \(self.minPosition) \(self.maxPosition)")
