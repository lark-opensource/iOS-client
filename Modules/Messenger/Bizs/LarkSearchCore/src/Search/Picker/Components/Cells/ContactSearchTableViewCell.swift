...
final public class ContactSearchTableViewCell: UITableViewCell, PickerSearchItemCellType {
    ...
    // chatter自定义标签
    lazy var chatterTagBuilder = ChatterTagViewBuilder()
    ...
    // nolint: * 长函数&重复函数-渲染逻辑过于复杂,后续该cell会废弃
    public func setContent(resolver: LarkContainer.UserResolver,
        ...
        if !searchResult.avatarKey.isEmpty {
            personInfoView.contentView.snp.updateConstraints { make in
                make.left.equalToSuperview().offset(16)
            }
            personInfoView.avatarView.setAvatarByIdentifier(searchResult.avatarID ?? "",
                                                            avatarKey: searchResult.avatarKey,
                                                            scene: .Search,
                                                            avatarViewParams: .init(sizeType: .size(48)))
        } else {
            if let backupImage = searchResult.backupImage {
                personInfoView.avatarView.image = backupImage
            } else {
                // 邮件联系人兜底需要展示前两个字符
                personInfoView.avatarView.image = SearchImageUtils.generateAvatarImage(withTitle: searchResult.title.string, length: 2)
            }
        }
        ...
        personInfoView.infoLabel.isHidden = summary.length == 0
