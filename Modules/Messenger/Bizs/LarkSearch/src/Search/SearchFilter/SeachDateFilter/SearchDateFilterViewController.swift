...
final class SearchDateFilterViewController: BaseUIViewController, UIGestureRecognizerDelegate,
    ...
    private let enableSelectFuture: Bool
    ...
    init(startDate: Date?, endDate: Date?, fromView: UIView?, enableSelectFuture: Bool = false) {
        ...
        datePickerView = DatePickerView(startDate: self.startDate, endDate: self.endDate, enableSelectFuture: self.enableSelectFuture, style: .left)
    ...
    func configureContentView() {
        ...
        if let startDate = startDate {
            ...
            leftItemView.set(title: BundleI18n.LarkSearch.Lark_Search_AnyTime)
        }
        ...
        if let endDate = endDate {
            ...
            rightItemView.set(title: BundleI18n.LarkSearch.Lark_Search_AnyTime)
        }
...
extension SearchDateFilterViewController: DateFilterNaviBarDelegate {
    ...
    func naviBarDidClickFinishButton(_ naviBar: DateFilterNaviBar) {
        let startDate = datePickerView.startDate.flatMap { $0.ls.beginDate }
        let endDate = datePickerView.endDate.flatMap { $0.ls.endDate }
...
extension SearchDateFilterViewController: DatePickerViewDelegate {
