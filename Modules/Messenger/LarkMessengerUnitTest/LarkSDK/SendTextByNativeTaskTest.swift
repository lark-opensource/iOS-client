...
class SendTextMsgOnScreenTaskTest: XCTestCase {
    private var task: SendTextMsgOnScreenTask<SendTextMsgOnScreenTaskTest>!
    ...
    override func setUp() {
        ...
        task = SendTextMsgOnScreenTask(context: self)
    }
    ...
    func test_createMsg_result() {
        ...
        task.onEnd { value in
            if case .success(let res) = value, res.message != nil, res.model.cid != nil {
                expectation.fulfill()
            }
        }
...
extension SendTextMsgOnScreenTaskTest: SendTextMsgOnScreenTaskContext {
