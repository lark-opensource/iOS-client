...
SendImageManager.shared
    .sendImage(request: thisRequest)
    .subscribe(onNext: { [weak self] resultStr in
        // 这个resultStr，就是thisUploader返回的str
        根据resultStr判断，展示上传成功等其他提示
    }, onError: { error in
        // 可以根据error的类型，判断是否是「检查+压缩」阶段的报错
        展示上传失败的提示等
    })
