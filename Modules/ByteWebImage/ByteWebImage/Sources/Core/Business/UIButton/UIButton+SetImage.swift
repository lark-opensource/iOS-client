...
extension ImageWrapper where Base: UIButton {
    ...
    public func setImage(_ url: URL?,
        ...
        // 1. URL合法
        guard let url else {
            setInvalidURL(for: state, placeholder: placeholder, callbacks: callbacks)
            return nil
        }
        ...
        if let key = base.image(for: state)?.bt.requestKey, key.isSame(as: tempKey) {
            setExistImageURL(for: state, request: ImageRequest(url: url), callbacks: callbacks)
            return nil
        }
        ...
        // 5. 当前请求是否为该请求
        if let currentRequest = imageSetter?.imageRequest(for: state),
...
extension ImageWrapper where Base: UIButton {
    ...
    public func setImage(_ url: URL?,
        ...
        let callbacks = ImageRequestCallbacks(decrypt: decrypt, progress: progress, completion: completionHandler)
        return self.setImage(url, for: state, alternativeURLs: alternativeURLs, placeholder: placeholder,
                             options: options, callbacks: callbacks)
    }

    @available(*, deprecated, renamed: "setImage(_:for:alternativeURLs:placeholder:options:progress:decrypt:completionHandler:)")
    @discardableResult
    public func setImage(with url: URL?,
        ...
        let callbacks = ImageRequestCallbacks(decrypt: decrypt, progress: progress,
