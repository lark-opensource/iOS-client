//
//  UIButton+LarkImageService.swift
//  ByteWebImage
//
...
private let kByteWebImageSetAnimationKey = "ByteWebImageSetAnimationKey"
...
extension ImageWrapper where Base: UIButton {
    ...
    public func setLarkImage(_ resource: LarkImageResource,
        ...
        // 3. 当前图像是否为该请求图像
        if let key = base.image(for: state)?.bt.requestKey, key.isSame(as: tempKey) {
            setExistImageURL(for: state, request: LarkImageRequest.create(resource: resource, url: url), callbacks: callbacks)
            return nil
        }
        ...
        let newCallbacks = ImageRequestCallbacks(decrypt: callbacks.decrypt, progress: callbacks.progress) { [weak base] result in
            let originRequest = imageSetter?.imageRequest(for: state)
            base?.bt.imageSetter?.setImageRequest(nil, for: state)

            switch result {
            case .success(let imageResult):
                let request = imageResult.request
                if !request.finished, request.image != nil {
                    base?.bt.setUnfinishedImage(imageResult, for: state)
                    return
                }

                base?.bt.setFinishedImage(imageResult, for: state)
        ...
        // 5. 当前请求是否为该请求
        if let currentRequest = imageSetter?.imageRequest(for: state) as? LarkImageRequest,
        ...
        // 6. 设置新图片请求
        cancelImageRequest(for: state)
        resetBeforeNewRequest(for: state, params: params, placeholder: placeholder)
        ...
        setNewImageRequest(request, for: state)
        return request
    }
}
...
extension ImageWrapper where Base: UIButton {
    ...
    public func setLarkImage(_ resource: LarkImageResource,
        ...
        return setLarkImage(resource, for: state, placeholder: placeholder, passThrough: passThrough, options: options, trackInfo: trackInfo, modifier: modifier, file: file, function: function, line: line, callbacks: callbacks)
    }
    ...
    ///   - completion: 图片加载完成回调，可以在此获取到最终图片
    /// - Returns: 图片请求 LarkImageRequest
    @available(*, deprecated, renamed: "setLarkImage(_:for:placeholder:passThrough:options:trackInfo:modifier:file:function:line:progress:decrypt:completion:)")
    @discardableResult
    public func setLarkImage(with resource: LarkImageResource,
        ...
        let callbacks = ImageRequestCallbacks(decrypt: decrypt, progress: progress, completion: completion)
        return setLarkImage(resource, for: state, placeholder: placeholder, passThrough: passThrough, options: options, trackInfo: trackStart, modifier: modifier, file: file, function: function, line: line, callbacks: callbacks)
    }
}
