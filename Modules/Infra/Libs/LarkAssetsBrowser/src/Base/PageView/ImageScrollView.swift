...
//  LarkAssetsBrowser
//
//  Created by Hayden on 2023/4/21.
//
...
open class ImageScrollView: UIScrollView {
    ...
    private var doubleTapZoomScaleFactor: CGFloat {
        maxToMinFactor / 2
    }

    private var maxToMinFactor: CGFloat {
        maximumZoomScale / minimumZoomScale
    }

    open var imageContentMode: ScaleMode = {
        UIDevice.current.userInterfaceIdiom == .pad ? .aspectFit : .widthFill
    }()
    ...
    public var zoomView: UIImageView? {
        didSet {
            ...
            zoomView.removeGestureRecognizer(doubleTapGesture)
            zoomView.addGestureRecognizer(doubleTapGesture)
            if let image = zoomView.image {
                zoomView.bounds = CGRect(origin: .zero, size: image.size)
    ...
    private func setMaxMinZoomScalesForCurrentBounds() {
        ...
        case .aspectFill:
            break
        case .aspectFit:
            maxScale = max(max(xScale, yScale), maxScale)
        case .widthFill:
            maxScale = max(yScale, maxScale)
        case .heightFill:
            maxScale = max(xScale, maxScale)
        }
    ...
    private func doubleTapGestureRecognizer(_ gestureRecognizer: UIGestureRecognizer) {
        ...
        if zoomScale >= minimumZoomScale * doubleTapZoomScaleFactor - 0.01 {
            setZoomScale(minimumZoomScale, animated: true)
        } else {
            let center = gestureRecognizer.location(in: gestureRecognizer.view)
            let zoomRect = zoomRectForScale(doubleTapZoomScaleFactor * minimumZoomScale, center: center)
