...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
    ...
    NSError *error = nil;
    NSString *path = [[[self class] frameworkBundle] pathForResource:@"YTPlayerView-iframe-player"
                                                            ofType:@"html"];

    // lint:disable lark_storage_check - 从 bundle 读数据，不涉及加解密，不进行检查
...
+ (NSBundle *)frameworkBundle {
    ...
    dispatch_once(&predicate, ^{
        NSBundle* bundlePath = [NSBundle bundleForClass:[WKYTPlayerView class]];
        NSURL* bundleURL = [bundlePath URLForResource:@"LarkAssetsBrowser" withExtension: @"bundle"];
