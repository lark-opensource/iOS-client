...
char const ** lk_get_sectiondata_with_header(mach_header_t *header, char *section_name, unsigned long *count) {
...
char const ** lk_get_sectiondata_with_name(char *section_name, unsigned long *count) {
    const uint32_t image_count = _dyld_image_count();
    mach_header_t *header = 0;
    
    if ((header = (mach_header_t *)atomic_load_explicit(&latest_mach_header, memory_order_acquire)) != 0) {
        char const **result = lk_get_sectiondata_with_header(header, section_name, count);
        if (result) {
            return result;
        }
    }
