...
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value size:(int)size fileName:(NSString *)fileName extendedData:(NSData *)extendedData {
    ...
    if (fileName.length == 0) {
        sqlite3_bind_blob(stmt, 4, value.bytes, size, 0);
    } else {
...
- (BOOL)_fileDeleteWithName:(NSString *)filename {
    ...
    BOOL result = [[NSFileManager defaultManager] removeItemAtPath:path error:NULL];
    if (!result) {
...
- (BOOL)saveItemsFromDictionary:(NSDictionary<NSString *, YYKVStorageItem *> *)dictionary {

    char* error;
    int result;
    result = sqlite3_exec(_db, "BEGIN TRANSACTION", NULL, NULL, &error);
    if (error) {
        if (_errorLogsEnabled) NSLog(@"%s line:%d sqlite exec error (%d): %s", __FUNCTION__, __LINE__, result, error);
        sqlite3_free(error);
    }
    [dictionary enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, YYKVStorageItem * _Nonnull obj, BOOL * _Nonnull stop) {
        if (obj.value) {
            [self saveItemWithKey:key value:obj.value filename:obj.filename extendedData:obj.extendedData];
        }
        else {
            [self removeItemForKey:key];
        }
    }];
    result = sqlite3_exec(_db, "COMMIT TRANSACTION", NULL, NULL, &error);
    if (error) {
        if (_errorLogsEnabled) NSLog(@"%s line:%d sqlite exec error (%d): %s", __FUNCTION__, __LINE__, result, error);
        sqlite3_free(error);
    }
...
- (BOOL)removeItemsToFitSize:(int64_t)maxSize {
    if (maxSize == LONG_MAX) return YES;
    if (maxSize <= 0) return [self removeAllItems];

    int64_t total = [self _dbGetTotalItemSize];
...
- (int64_t)getItemsSize {
    return [self _dbGetTotalItemSize];
}

// MARK: file
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value fileName:(NSString *)fileName extendedData:(NSData *)extendedData {
    return [self _dbSaveWithKey:key value:value size:(int)value.length fileName:fileName extendedData:extendedData];
}
...
- (BOOL)saveFileWith:(NSString*)key fileName:(NSString*)fileName size:(int)size extendedData:(NSData*)data {
    return [self _dbSaveWithKey:key value:nil size:size fileName:fileName extendedData:data];
}
...
- (nullable YYKVStorageItem *)getFileOrDataItem:(NSString *)key {
    if (key.length == 0) return nil;
    YYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];

    if (item && (item.filename.length > 0)) {
        [self _dbUpdateAccessTimeWithKey:key];
        ...
        path = [[_path stringByDeletingLastPathComponent] stringByAppendingPathComponent:item.filename];
        if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
            item.filePath = path;
            return item;
        }
...
+ (NSString*)dataFolderName {
    return kDataDirectoryName;
}
