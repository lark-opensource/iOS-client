...
final class SwipeController: NSObject {
    ...
    lazy var panGestureRecognizer: SwipePanGestureRecognizer = {
        let gesture = SwipePanGestureRecognizer(target: self, action: #selector(handlePan(gesture:)))
    ...
    func showActionsView(for orientation: SwipeActionsOrientation) -> Bool {
        guard let actions = delegate?.swipeController(self, editActionsForSwipeableFor: orientation), !actions.isEmpty else { return false }
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        if orientation == .left {
            actionsView.rightAnchor.constraint(equalTo: actionsContainerView.leftAnchor, constant: -(options.actionMargin ?? 0)).isActive = true
        } else {
            actionsView.leftAnchor.constraint(equalTo: actionsContainerView.rightAnchor, constant: options.actionMargin ?? 0).isActive = true
        }
    ...
    func animate(duration: Double = 0.7,
...
extension SwipeController: UIGestureRecognizerDelegate {
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer == panGestureRecognizer,
            let gestureRecognizer = gestureRecognizer as? SwipePanGestureRecognizer {
            let velocity = gestureRecognizer.velocity(in: actionsContainerView)
            let orientation: SwipeActionsOrientation = velocity.x > 0 ? .left : .right
            let options = delegate?.swipeController(
                self,
                editActionsOptionsForSwipeableFor: orientation
            ) ?? SwipeOptions()

            let translation: CGPoint = gestureRecognizer.translation ??
                gestureRecognizer.translation(in: gestureRecognizer.view)

            if let shouldBegin = options.shouldBegin {
                return shouldBegin(translation.x, translation.y)
            }
...
extension SwipeController: SwipeActionsViewDelegate {
    func swipeActionsView(_ swipeActionsView: SwipeActionsView, didSelect action: SwipeAction) {
        perform(action: action, isTapped: true)
    }

    func perform(action: SwipeAction, isTapped: Bool = false) {
        ...
        if action == actionsView.expandableAction, let expansionStyle = actionsView.options.expansionStyle {
            ...
            case .bounce:
                perform(action: action, hide: true, isTapped: isTapped)
            case .fill(let fillOption):
                performFillAction(action: action, fillOption: fillOption, isTapped: isTapped)
            }
        } else {
            perform(action: action, hide: action.hidesWhenSelected, isTapped: isTapped)
        }
    }

    func perform(action: SwipeAction, hide: Bool, isTapped: Bool) {
        ...
        action.handler?(action, indexPath, isTapped)
    }

    func performFillAction(action: SwipeAction, fillOption: SwipeExpansionStyle.FillOptions, isTapped: Bool) {
        guard let swipeable = self.swipeable, let actionsContainerView = self.actionsContainerView else { return }
...
final class SwipePanGestureRecognizer: UIPanGestureRecognizer {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        self.startPoint = self.location(in: self.view)
    }
