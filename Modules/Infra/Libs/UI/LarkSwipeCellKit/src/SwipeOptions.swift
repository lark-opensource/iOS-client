...
public struct SwipeOptions {
    ...
    /// the system automatically uses the default `ScaleAndAlphaExpansion` to show/hide underlying actions.
    ...
    public var actionMargin: CGFloat?

    /// The largest allowable button width.
    ///
    /// - note: By default, the value is set to the table/collection view divided by the number of action buttons minus some additional padding.
    ///  If the value is set to 0, then word wrapping will not occur and the buttons will grow as large as needed to fit the entire title/image.
    ...
    public var buttonWidthStyle: SwipeActionButtonWidthStyle = .average
    ...
    /// The amount of space, in points, between the button image and the button title.
    public var buttonStyle: SwipeActionButtonStyle = .vertical

    /// The callback whether to trigger a lateral gesture
    public var shouldBegin: ((CGFloat, CGFloat) -> Bool)?

    /// Constructs a new `SwipeOptions` instance with default options.
    public init() {}
}

public enum SwipeActionButtonStyle {
    case vertical

    case horizontal
}

public enum SwipeActionButtonWidthStyle {
    case average
...
public enum SwipeTransitionStyle {
    ...
    case custom(SwipeTransitionLayout)
}
