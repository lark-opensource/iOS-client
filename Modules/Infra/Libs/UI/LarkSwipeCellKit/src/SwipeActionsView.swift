...
final class SwipeActionsView: UIView {
    ...
    var maximumImageWidth: CGFloat {
        return actions.reduce(0, { initial, next in max(initial, next.image?.size.width ?? 0) })
    }
    ...
    var preferredWidth: CGFloat {
        switch options.buttonWidthStyle {
        case .auto:
            return contentWidths.reduce(0, +)
        case .average:
    ...
    func addButtons(for actions: [SwipeAction], withMaximum size: CGSize, contentEdgeInsets: UIEdgeInsets) -> [SwipeActionButton] {
        ...
        let buttons: [SwipeActionButton] = actions.map({ action in
            ...
            actionButton.style = options.buttonStyle
        ...
        /// button的style会影响width的计算
        minimumButtonWidth = buttons.reduce(minimum, { initial, next in max(initial, next.preferredWidth(with: options.buttonStyle, maximum: maximum)) })

        buttons.enumerated().forEach { (index, button) in
            let action = actions[index]
            let frame = CGRect(origin: .zero, size: CGSize(width: bounds.width, height: bounds.height))
            let contentWidth = max(minimum, button.preferredWidth(with: options.buttonStyle, maximum: maximum))
            contentWidths.append(contentWidth)
            let wrapperView = SwipeActionButtonWrapperView(frame: frame,
                                                           action: action,
                                                           orientation: orientation,
                                                           contentWidth: options.buttonWidthStyle == .auto ? contentWidth : minimumButtonWidth)
            ...
            button.maximumImageWidth = maximumImageWidth
    ...
    func buttonEdgeInsets(fromOptions options: SwipeOptions) -> UIEdgeInsets {
        let verticalPadding = options.buttonVerticalPadding ?? 8
        let horizontalPadding = options.buttonHorizontalPadding ?? 8
...
final class SwipeActionButtonWrapperView: UIView {
