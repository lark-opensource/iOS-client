...
final class SwipeActionButton: UIButton {
    ...
    var style: SwipeActionButtonStyle = .vertical
    ...
    var alignmentRect: CGRect {
        ...
        let totalHeight = style == .vertical ? (imageHeight + titleHeight + currentSpacing) : max(imageHeight, titleHeight)
    ...
    private var imageWidth: CGFloat {
        get {
            return currentImage == nil ? 0 : maximumImageWidth
        }
    }
    ...
    func preferredWidth(with style: SwipeActionButtonStyle, maximum: CGFloat) -> CGFloat {
    ...
    override func titleRect(forContentRect contentRect: CGRect) -> CGRect {
        ...
        case .horizontal:
            let otherWidth = (currentImage?.size ?? .zero).width + currentSpacing
            var rect = bounds.center(size: titleBoundingRect(with: contentRect.size).size)
            rect.origin.x = rect.minX + otherWidth / 2
            return rect.integral
        }
    }
    ...
    override func imageRect(forContentRect contentRect: CGRect) -> CGRect {
        ...
        case .vertical:
            ...
            return rect
        case .horizontal:
            let otherWidth = titleBoundingRect(with: contentRect.size).width + currentSpacing
            var rect = bounds.center(size: currentImage?.size ?? .zero)
            rect.origin.x = rect.minX - otherWidth / 2
            return rect
        }
    }
