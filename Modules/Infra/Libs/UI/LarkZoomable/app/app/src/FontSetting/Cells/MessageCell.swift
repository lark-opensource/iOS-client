...
class MessageCell: UITableViewCell {
    ...
    enum Cons {
        static var messageColor: UIColor { .black }
        static var selfBubbleColor: UIColor { .systemBlue }
        static var otherBubbleColor: UIColor { UIColor(white: 0.9, alpha: 1) }
        static var bubbleCornerRadius: CGFloat { .fixed(10) }
        static var leftMargin: CGFloat { .fixed(10) }
    ...
    func configure(with model: Message, continuous: Bool, zoom: Zoom) {
        ...
        messageLabel.font = LarkFont.getTitle4(for: zoom)
        messageLabel.textColor = model.isFromSelf ? .white : .black
