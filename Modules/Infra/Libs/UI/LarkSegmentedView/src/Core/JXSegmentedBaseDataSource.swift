...
open class JXSegmentedBaseDataSource: JXSegmentedViewDataSource {
    ...
    open func refreshItemModel(_ segmentedView: JXSegmentedView,
        ...
        if isItemWidthZoomEnabled {
            if (selectedType == .scroll && !isItemTransitionEnabled) ||
                ...
                animator?.progressClosure = {[weak self] (percent) in
                    currentSelectedItemModel.itemWidthCurrentZoomScale = JXSegmentedViewTool.interpolate(from: currentSelectedItemModel.itemWidthSelectedZoomScale,
                                                                                                         to: currentSelectedItemModel.itemWidthNormalZoomScale,
                                                                                                         percent: percent)
                    currentSelectedItemModel.itemWidth = self?.segmentedView(segmentedView,
                                                                             widthForItemAt: currentSelectedItemModel.index,
                                                                             isItemWidthZoomValid: true) ?? 0
                    willSelectedItemModel.itemWidthCurrentZoomScale = JXSegmentedViewTool.interpolate(from: willSelectedItemModel.itemWidthNormalZoomScale,
                                                                                                      to: willSelectedItemModel.itemWidthSelectedZoomScale,
                                                                                                      percent: percent)
                    willSelectedItemModel.itemWidth = self?.segmentedView(segmentedView,
    ...
    open func refreshItemModel(_ segmentedView: JXSegmentedView, leftItemModel: JXSegmentedBaseItemModel, rightItemModel: JXSegmentedBaseItemModel, percent: CGFloat) {
        ...
        if isItemWidthZoomEnabled && isItemTransitionEnabled {
            ...
            leftItemModel.itemWidthCurrentZoomScale = JXSegmentedViewTool.interpolate(from: leftItemModel.itemWidthSelectedZoomScale,
                                                                                      to: leftItemModel.itemWidthNormalZoomScale,
                                                                                      percent: percent)
            leftItemModel.itemWidth = self.segmentedView(segmentedView,
                                                         widthForItemAt: leftItemModel.index,
                                                         isItemWidthZoomValid: true)
            rightItemModel.itemWidthCurrentZoomScale = JXSegmentedViewTool.interpolate(from: rightItemModel.itemWidthNormalZoomScale,
                                                                                       to: rightItemModel.itemWidthSelectedZoomScale,
                                                                                       percent: percent)
            rightItemModel.itemWidth = self.segmentedView(segmentedView,
    ...
    public final func refreshItemModel(_ segmentedView: JXSegmentedView,
