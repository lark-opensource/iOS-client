...
public protocol JXSegmentedViewDataSource: AnyObject {
    ...
    func refreshItemModel(_ segmentedView: JXSegmentedView,
                          _ itemModel: JXSegmentedBaseItemModel,
                          at index: Int, selectedIndex: Int)
...
open class JXSegmentedView: UIView {
    ...
    public private (set) var collectionView: JXSegmentedCollectionView!
    ...
    public var contentEdgeInsetLeft: CGFloat = JXSegmentedViewAutomaticDimension
    /// 整体内容的右边距，默认JXSegmentedViewAutomaticDimension（等于itemSpacing）
    public var contentEdgeInsetRight: CGFloat = JXSegmentedViewAutomaticDimension
    ...
    public override func layoutSubviews() {
        ...
        //部分使用者为了适配不同的手机屏幕尺寸，JXSegmentedView的宽高比要求保持一样。
        //所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。
    ...
    public final func dequeueReusableCell(withReuseIdentifier identifier: String, at index: Int) -> JXSegmentedBaseCell {
        ...
        return cell as? JXSegmentedBaseCell ?? JXSegmentedBaseCell()
    }
    ...
    public func reloadDataWithoutListContainer() {
        ...
        for (index, itemModel) in itemDataSource.enumerated() {
            ...
            itemModel.itemWidth = (dataSource?.segmentedView(self,
                                                             widthForItemAt: index,
                                                             isItemWidthZoomValid: true) ?? 0)
        ...
        if contentScrollView != nil {
            if contentScrollView!.frame.equalTo(CGRect.zero) &&
                ...
                //某些情况系统会出现JXSegmentedView先布局，contentScrollView后布局，就会导致下面指定defaultSelectedIndex失效。
    ...
    public override func observeValue(forKeyPath keyPath: String?,
        ...
        if keyPath == "contentOffset" {
            let contentOffset = change?[NSKeyValueChangeKey.newKey] as? CGPoint ?? CGPoint(x: 0, y: 0)
            if contentScrollView?.isTracking == true || contentScrollView?.isDecelerating == true {
                ...
                if remainderProgress == 0 {
                    //滑动翻页，需要更新选中状态
                    //滑动一小段距离，然后放开回到原位，contentOffset同样的值会回调多次。
                    //例如在index为1的情况，滑动放开回到原位，contentOffset会多次回调CGPoint(width, 0)
                    ...
                    dataSource?.refreshItemModel(self,
                                                 leftItemModel: itemDataSource[baseIndex],
    ...
    private func selectItemAt(index: Int, selectedType: JXSegmentedViewItemSelectedType) {
        ...
        if contentScrollView != nil && (selectedType == .click || selectedType == .code) {
            contentScrollView!.setContentOffset(CGPoint(x: contentScrollView!.bounds.size.width*CGFloat(index), y: 0),
                                                animated: isContentScrollViewClickTransitionAnimationEnabled)
        }
    ...
    private func getItemFrameAt(index: Int) -> CGRect {
        ...
        for i in 0..<index {
            ...
            if itemModel.isTransitionAnimating && itemModel.isItemWidthZoomEnabled {
                ...
                if itemModel.isSelected {
                    itemWidth = (dataSource?.segmentedView(self,
                                                           widthForItemAt: itemModel.index,
                                                           isItemWidthZoomValid: false) ?? 0) * itemModel.itemWidthSelectedZoomScale
                }else {
                    itemWidth = (dataSource?.segmentedView(self,
                                                           widthForItemAt: itemModel.index,
                                                           isItemWidthZoomValid: false) ?? 0) * itemModel.itemWidthNormalZoomScale
                }
            }else {
        ...
        if selectedItemModel.isTransitionAnimating && selectedItemModel.isItemWidthZoomEnabled {
            width = (dataSource?.segmentedView(self,
                                               widthForItemAt: selectedItemModel.index,
                                               isItemWidthZoomValid: false) ?? 0) * selectedItemModel.itemWidthSelectedZoomScale
        }else {
