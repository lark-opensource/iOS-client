...
class UGCoordinator {
    ...
    // 数据
    private var entitys: [UGReachPointEntityInfo] {
        return bizNodes.getImmutableCopy().map { $0.entity }
    }
    // 每次调度的结果
    ...
    func getDisplayResult() {
        ...
        // check if conflict
        let needShowNodes = self.entitys.filter { dagRootNodeIDs.contains($0.reachPointID) }.map { UGBizNode(entity: $0) }
        ...
        if sortedConflictNodes.count > 1 {
            ...
            conflictSolvedNodes = entitys.filter { preshowSolvedRootNodes.contains($0.reachPointID) }.map { UGBizNode(entity: $0) }
        ...
        if !_showingNodes.isEmpty {
            preshowNodes.forEach { preshowNode in
                for index in 0 ..< _showingNodes.count {
                    let showingNode = _showingNodes[index]
                    // 解决对即将出图的节点和正在显示的节点
                    guard isNodeConflict(node1: preshowNode, node2: showingNode) else {
                        return
                    }
                    resolveConflict(preShow: preshowNode, showingNode: showingNode, directedGraph: graph)
                }
            }
        }
        // 获取更新后preshow nodes
        let solvedRootNodes = self.graph.rootNodeIds
        let newShowEntitys = self.entitys
            .filter { solvedRootNodes.contains($0.reachPointID) }

        sendCoordinatorResult(rpState: .show, rpEntitys: newShowEntitys)
        Self.log.debug("[UGCoordinator]: getDisplayResult",
                       additionalData: [
//                        "dagRootNodeIDs": "\(dagRootNodeIDs)",
                        "solvedRootNodes": "\(solvedRootNodes)"
//                        "needShowNodes": "\(needShowNodes)",
//                        "conflictNodes": "\(conflictNodes)",
//                        "newShowEntitys": "\(newShowEntitys)"
                       ])
    }
}
...
extension UGCoordinator {
    ...
    /// 初始化子图
    private func initSubGraph(entitys: [UGReachPointEntityInfo]) -> UGDAGraph? {
        guard !entitys.isEmpty else {
            Self.log.error("[UGCoordinator]: initSubGraph, entitys is empty!")
            return nil
        }
        let nodes = entitys.map { UGDAGNode(ugNode: UGBizNode(entity: $0)) }
    ...
    // 添加子节点
    private func addEntityInfos(entitys: [UGReachPointEntityInfo]) {
        lock.lock()
        defer { lock.unlock() }

        let bizNodes = entitys.map { UGBizNode(entity: $0) }
        let dagNodes = bizNodes.map { UGDAGNode(ugNode: $0) }
        ...
        // 添加数据
        self.bizNodes.append(contentsOf: bizNodes)
        Self.log.debug("[UGCoordinator]: addEntityInfos, newShowEntitys = \(entitys)")
    }

    // 移除子节点
    private func removeEntityInfos(entitys: [UGReachPointEntityInfo]) {
        lock.lock()
        defer { lock.unlock() }
...
extension UGCoordinator {
    ...
    // 触发某个场景
    func onScenarioTrigger(scenarioContext: ScenarioContext) {
        self.addEntityInfos(entitys: scenarioContext.entities)
    }

    // 移除某个场景
    func onScenarioLeave(scenarioContext: ScenarioContext) {
        self.removeEntityInfos(entitys: scenarioContext.entities)
    }

    // 某个引导触达点位变化事件（展示、隐藏、消费）
    func onReachPointEvent(reachPointEvent: ReachPointEvent) {
        lock.lock()
        defer { lock.unlock() }
    ...
    private func onConsumeNode(nodeID: String) {
        guard let node = getResultNodeById(nodeID: nodeID),
              graph.getNodeInGraphByNodeID(nodeID: node.nodeID) != nil else { return }
...
extension UGCoordinator {
    ...
    /// 发送结果信号
    private func sendCoordinatorResult(rpState: ReachPointState, rpEntitys: [UGReachPointEntityInfo]) {
    ...
    private func resolveConflict(preShow: UGBizNode,
