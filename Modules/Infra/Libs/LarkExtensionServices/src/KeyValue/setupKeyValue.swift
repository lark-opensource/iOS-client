...
private final class MMKVWrapper: MMKVType {
    var wrapped: MMKV
...
private var _setupMMKVFlag = false
private var _setupLock = UnfairLock()

public func setupMMKVIfNeeded() {
    _setupLock.lock()
    defer { _setupLock.unlock() }

    guard !_setupMMKVFlag else { return }
    _setupMMKVFlag = true
