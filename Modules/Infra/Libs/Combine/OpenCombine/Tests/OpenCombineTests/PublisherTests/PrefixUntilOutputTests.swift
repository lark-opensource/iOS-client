...
final class PrefixUntilOutputTests: XCTestCase {
    // MARK: should relay until other sends a value
    func testPrefixUntilOutput() {
        let subs1 = CustomSubscription()
        let subs2 = CustomSubscription()
        let pub0 = CustomPublisher(subscription: subs1)
        let pub1 = CustomPublisher(subscription: subs2)
        ...
        // FIXME: something wrong with combine...it seems like Inner of prefix(UntilOutput:) in Combine doesn't conform to protocal "CustomStringConvertible"
        //XCTAssertEqual(tracking.history, [.subscription("PrefixUntilOutput")])
        ...
        XCTAssertEqual(tracking.history[11], .completion(.finished))
    }

    // MARK: should complete when the other complete without a value
    func testPrefixUntilOutputComplete() {
        let subs1 = CustomSubscription()
        let subs2 = CustomSubscription()
        let pub0 = CustomPublisher(subscription: subs1)
        let pub1 = CustomPublisher(subscription: subs2)
        ...
        pub1.send(completion: .failure(.oops))

        for i in 10..<20 {
            _ = pub0.send(i)
        }

        XCTAssertEqual(tracking.history.count, 21)
    }
