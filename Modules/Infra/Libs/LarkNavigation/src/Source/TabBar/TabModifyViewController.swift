//
//  TabModifyViewController.swift
//  LarkNavigation
//
//  Created by phoenix on 2023/9/19.
//
...
public enum TabRenameFrom: String {
    // 重命名主导航应用
    case main
    // 重命名快捷导航应用
    case quick
...
private let kTabModifyTextFieldMaxLength = 300

public final class TabModifyViewController: BaseUIViewController, UDTextFieldDelegate, UserResolverWrapper {
    static let logger = Logger.log(TabRenameViewController.self, category: "LarkNavigation.TabModifyViewController")
    ...
    private var tabItem: AbstractTabBarItem

    private var style: TabbarStyle

    private var confirmCallback: ((AbstractTabBarItem) -> Void)
    ...
    private lazy var navTitleView: UIView = {
        let label = UILabel()
        label.text = BundleI18n.LarkNavigation.Lark_Core_NavbarAppAction_Rename_Button
    ...
    public init(userResolver: UserResolver,
                tabItem: AbstractTabBarItem,
                from: TabRenameFrom,
                style: TabbarStyle,
                confirmCallback: @escaping ((AbstractTabBarItem) -> Void)) {
    ...
    private func reloadData() {
        // 应用的名字
        if let name = tabItem.tab.name {
            titleTextField.text = name
        } else {
            titleTextField.text = ""
        }
        // 应用的图标
        if let icon = tabItem.tab.tabIcon {
    ...
    private func didTapConfirm() {
        ...
        tabItem.tab.extra[RecentRecordExtraKey.displayName] = trimmedText
        tabItem.tab.name = trimmedText
        tabItem.title = trimmedText
        self.confirmCallback(tabItem)
        self.dismiss()
    }
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        let ok = updatedText.count <= kTabModifyTextFieldMaxLength
        if ok {
...
extension TabModifyViewController {
    enum Cons {
