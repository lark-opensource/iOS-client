...
//  Lark
//
//  Created by Li Yuguo on 2018/12/23.
...
public final class SideBarViewController: UIViewController, SideBarAbility {

    override public var supportedInterfaceOrientations: UIInterfaceOrientationMask { .portrait }
    private static let logger = Logger.log(SideBarViewController.self, category: "LarkNavigation.SideBarViewController")

    private lazy var cachedHeightConfig = KVConfig(
        key: KVKeys.Navigation.cachedHeight,
    ...
    // Dependencies
    private let tenantVC: UIViewController
    private let mineViewController: UIViewController
    private let topOffset: CGFloat = Display.iPhoneXSeries ? 60 : 36
    private lazy var disposeBag = KVODisposeBag()

    init(tenantVC: UIViewController, mineViewController: UIViewController) {
        self.tenantVC = tenantVC
    ...
    public override func viewDidLoad() {
        ...
        addChild(tenantVC)
        tenantVC.didMove(toParent: self)
        view.addSubview(tenantVC.view)
        tenantVC.view.snp.makeConstraints { make in
            make.left.top.bottom.equalToSuperview()
            make.width.equalTo(96)
        }

        addChild(self.mineViewController)
        mineViewController.didMove(toParent: self)
        view.addSubview(self.mineViewController.view)
        mineViewController.view.snp.makeConstraints { (make) in
            make.right.equalToSuperview()
            make.bottom.equalTo(self.view.safeAreaLayoutGuide.snp.bottom)
            make.top.equalTo(view.safeAreaLayoutGuide.snp.top).offset(17)
            make.left.equalTo(tenantVC.view.snp.right)
        }

        observePopoverSize()
    }

    public func tenantAddView() -> UIView? {
        (tenantVC as? TenantViewController)?.addView()
    }
    ...
    private func observePopoverSize() {
        // observe change
        if Display.pad {
            mineViewController.observe(\.preferredContentSize, options: .new, changeHandler: { [weak self] _, change in
    ...
    private func adjustPopover(height: NSNumber) {
        ...
        Self.logger.debug("MinePopover SideBarVC preferred height: \(height)")
    }
}
