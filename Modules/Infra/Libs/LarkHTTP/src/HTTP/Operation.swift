...
public enum HTTPOptError: Error {
    case invalidRequest
}
...
open class HTTP {
    ...
    */
    ...
    public var auth:((URLAuthenticationChallenge) -> URLCredential?)? {
        set {
    ...
    public var downloadHandler: ((Response, URL) -> Void)? {
        set {
            guard let resp = DelegateManager.sharedInstance.responseForTask(task) else { return }
    ...
    */
    public func run(_ completionHandler: ((Response) -> Void)? = nil) {
    ...
    */
    ...
    @discardableResult open class func Run(_ url: String, method: HTTPVerb, parameters: HTTPParameterProtocol? = nil, headers: [String:String]? = nil, requestSerializer: HTTPSerializeProtocol = HTTPParameterSerializer(), completionHandler: ((Response) -> Void)? = nil) -> HTTP?  {
...
*/
public final class DelegateManager: NSObject, URLSessionDataDelegate, URLSessionDownloadDelegate {
    ...
    var _taskMap = Dictionary<Int,Response>()
...
open class HTTPQueue {
    ...
    public func add(request: URLRequest) {
        add(http: HTTP(request))
    }
    
    public func add(http: HTTP) {
        var doWork = false
        mutex.lock()
        queue.append(http)
    ...
    public func finished(queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (() -> Void)) {
