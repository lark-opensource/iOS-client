...
extension String {
    ...
    */
    ...
    func asUTF8Data(do handler: (Data) -> Void) {
 ...
 */
public final class HTTPParameterProtocolSettings {
    ...
    public func createPairs(_ key: String?) -> [HTTPPair] {
        ...
        for (k, v) in self {
            if let nestedKey = k as? String {
                let useKey: String = key.map { "\($0)[\(nestedKey)]" } ?? nestedKey
...
extension Data: HTTPParameterProtocol {
    public func createPairs(_ key: String?) -> [HTTPPair] {
        return []
    }
}
...
extension URLRequest {
    ...
    */
    ...
    public mutating func appendParametersAsData(_ data: Data) {
        httpBody = data
    }
    ...
    */
    public mutating func appendParametersAsMultiPartFormData(_ parameters: HTTPParameterProtocol) -> Error? {
        ...
        let multiCRLF = "\r\n"
        "--\(boundary)".asUTF8Data(do: mutData.append(_:))
        for pair in parameters.createPairs(nil) {
            ...
            if let upload = pair.upload {
                ...
                "\(multiCRLF)".asUTF8Data(do: mutData.append(_:))
                if let data = resp.data {
                    multiFormHeader(
                        ...
                        multiCRLF: multiCRLF
                    ).asUTF8Data(do: mutData.append(_:))
                    mutData.append(data)
                } else {
                ...
                str.asUTF8Data(do: mutData.append(_:))
            }
            "\(multiCRLF)--\(boundary)".asUTF8Data(do: mutData.append(_:))
        }
        "--\(multiCRLF)".asUTF8Data(do: mutData.append(_:))
