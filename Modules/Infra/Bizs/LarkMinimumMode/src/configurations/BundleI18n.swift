...
{
"keys":{
"Lark_Legacy_BasicModeErrorOccurredPlsRestart":{
"hash":"uLc",
"#vars":1,
"normal_vars":["APP_DISPLAY_NAME"]
},
"Lark_Legacy_BasicModeErrorOccurredPlsRestartButton":{
"hash":"g84",
"#vars":0
}
},
"name":"LarkMinimumMode",
"short_key":true,
"config":{
...
final class BundleI18n: LanguageManager {
    ...
    static func LocalizedString(key: String, originalKey: String, lang: Lang? = nil) -> String {
        func fetch() -> String {
            ...
            return self.localizedString(key: key, originalKey: originalKey, bundle: BundleConfig.LarkMinimumModeAutoBundle, moduleName: "LarkMinimumMode", lang: lang) ?? key
            #else
            return self.localizedString(key: key, bundle: BundleConfig.LarkMinimumModeAutoBundle, moduleName: "LarkMinimumMode", lang: lang) ?? key
            #endif
        }
     ...
     */
    final class LarkMinimumMode {
        @inlinable
        static var __Lark_Legacy_BasicModeErrorOccurredPlsRestart: String {
            return LocalizedString(key: "uLc", originalKey: "Lark_Legacy_BasicModeErrorOccurredPlsRestart")
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        static func Lark_Legacy_BasicModeErrorOccurredPlsRestart(lang __lang: Lang? = nil) -> String {
            var template = LocalizedString(key: "uLc", originalKey: "Lark_Legacy_BasicModeErrorOccurredPlsRestart", lang: __lang)
            template = template.replacingOccurrences(of: "{{APP_DISPLAY_NAME}}", with: "\(LanguageManager.bundleDisplayName)")
            return template
        }
        @inlinable
        static var Lark_Legacy_BasicModeErrorOccurredPlsRestartButton: String {
            return LocalizedString(key: "g84", originalKey: "Lark_Legacy_BasicModeErrorOccurredPlsRestartButton")
        }
        /// - Parameter lang: 使用指定语言获取文案，同时获取多个不同语言的文案时，注意初始化加载主线程卡死风险
        @inlinable
        static func Lark_Legacy_BasicModeErrorOccurredPlsRestartButton(lang __lang: Lang? = nil) -> String {
            return LocalizedString(key: "g84", originalKey: "Lark_Legacy_BasicModeErrorOccurredPlsRestartButton", lang: __lang)
        }
    }
}
