...
def writeFile()
  ...
  CSV.foreach("SpecialSize.csv", :col_sep => "\t") do |row|
    puts "in special file: #{ row[0] }, #{ row[1] })"
  ...
  # Set up blocklist
  blocklist = ["illustration_specialized-cta-banner-desktop"]
  ...
  Pathname.new('../illustrations').children.each do |path|
    ...
    # empty 资源跳过
    if fileName["empty"]
      puts "skip file: #{ fileName }, because it's an empty resource"
    else
