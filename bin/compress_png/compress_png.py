...
black_list = ['contact_member_invite_gradient_background_light.png',
'contact_member_invite_gradient_background_dark.png']
...
def __compress(proj_dir):
    ...
    for xcassets_dir in glob.glob('{}/**/*.xcassets'.format(proj_dir), recursive=True):
        ...
        for file in glob.glob('{}/**/*.png'.format(xcassets_dir), recursive=True):
            ...
            if os.path.basename(file) in black_list:
                print("图片压缩黑名单：{}".format(file))
                continue
            if file in record:
...
def downloadpngquant():
    ...
    if os.path.exists(pngquantPath):
        ...
        os.system("chmod +x {}pngquant".format(pyPath+"pngquant/"))
        os.system("codesign -f -s - {}pngquant".format(pyPath+"pngquant/"))
