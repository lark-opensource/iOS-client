...
pushq %rbp
movq %rsp, %rbp //now aligned to 16bytes
...
//the 1rs, 2nd arguments are diffferent with caller
movq    0x38(%r12) ,  %rdi
movq    0x30(%r12) ,  %rsi
movq    0xF8(%r12), %rdx // third argument is pointer to ori selector
movq    (%r13), %rcx      // fourth argument is original imp
movq    0x18(%rsp), %r8      // fifth argument is original return address
leaq    onEntry(%rip), %r13
callq   *(%r13)         // call tracing entry routine (saves return address)
...
movq   0x38(%r12) ,  %r14 //first argument %rdi
movq   0x30(%r12) ,  %r15 //second argument %rsi
...
call SYMBOL_NAME(pns_forwarding_trampoline) # 5 bytes
movq %rbp, %rsp
popq    %rbp
retq
.align 4 // align the trampolines at 16 bytes (required for config page lookup and sizing)
