...
Writer::m_ensure_size(size_t size, bool auto_ext) {
    ...
    if (size + m_current_size > m_max_capacity) {
        char buffer_capacity[20] = {0};
        snprintf(buffer_capacity, 20, "%lu", m_max_capacity);
        ZONE_STRING writer_capacity = buffer_capacity;
        void *ptr = malloc_zone_malloc(g_malloc_zone(), sizeof(ZONE_STRING));
        ZONE_STRING *writer_reason_ptr = new(ptr) ZONE_STRING("writer extend: capacity = "+writer_capacity);
        m_err = Error(ErrorType::OverFileSizeLimit, writer_reason_ptr->c_str());
        return ;
    }
...
bool
Writer::write(const void *src, size_t offset, size_t len) {
    size_t new_size = offset + len;
    size_t new_len = round_page(len);
    ...
    char* m_buffer_tmp = (char *)mmap(nullptr, new_len, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd, offset);
    memcpy(m_buffer_tmp, src, len);
    if (m_current_size + m_current_usage < offset + len) {
        m_buffer = m_buffer_tmp;
        m_current_size = offset;
        m_current_usage = len;
        m_cursor = m_buffer + len;
        m_capacity = new_len;
    }
    return true;
}
