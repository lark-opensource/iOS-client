//
// Created by william on 2020-06-05.
//
...
namespace mammon {
    /**
     * Ducker for mixing voice and BGM, reducing the volume of BGM when voice arises, and resume
     * the volume when voice ends
     *
     * @code
     * // set processor
     * Ducker processor;
     ...
     * float* data_voice[2] = {left_channel, right_channel};
     * float* data_bgm[2] = {left_channel, right_channel};
     * bus_array[0] = Bus("master", data_voice, 2, num_samples);
     * bus_array[1] = Bus("reference", data_bgm, 2, num_samples);
     *
     * // process
     ...
     * @endcode
     */
    class Ducker : public Effect {
    public:
        static constexpr const char* EFFECT_NAME = "ducker";
        Ducker(int sample_rate);
        ...
        int preprocess(std::vector<Bus>& bus_array);
