...
///
/// BDMTLPropertyStorageNone       - This property is not included in -description,
///                                -hash, or anything else.
/// BDMTLPropertyStorageTransitory - This property is included in one-off
///                                operations like -copy and -dictionaryValue but
///                                does not affect -isEqual: or -hash.
///                                It may disappear at any time.
/// BDMTLPropertyStoragePermanent  - The property is included in serialization
///                                (like `NSCoding`) and equality, since it can
///                                be expected to stick around.
typedef enum : NSUInteger {
    BDMTLPropertyStorageNone,
    BDMTLPropertyStorageTransitory,
    BDMTLPropertyStoragePermanent,
} BDMTLPropertyStorage;

/// This protocol defines the minimal interface that classes need to implement to
/// interact with BDMantle adapters.
///
/// It is intended for scenarios where inheriting from BDMTLModel is not feasible.
/// However, clients are encouraged to subclass the BDMTLModel class if they can.
///
/// Clients that wish to implement their own adapters should target classes
/// conforming to this protocol rather than subclasses of BDMTLModel to ensure
/// maximum compatibility.
@protocol BDMTLModel <NSObject, NSCopying>
...
- (void)mergeValueForKey:(NSString *)key fromModel:(id<BDMTLModel>)model;

/// Returns the keys for all @property declarations, except for `readonly`
/// properties without ivars, or properties on BDMTLModel itself.
+ (NSSet *)propertyKeys;
...
@interface BDMTLModel : NSObject <BDMTLModel>
...
///
/// The default implementation returns BDMTLPropertyStorageNone for properties that
/// are readonly and not backed by an instance variable and
/// BDMTLPropertyStoragePermanent otherwise.
///
/// Subclasses can use this method to prevent BDMTLModel from resolving circular
/// references by returning BDMTLPropertyStorageTransitory.
///
...
/// Implements validation logic for BDMTLModel.
@interface BDMTLModel (Validation)
