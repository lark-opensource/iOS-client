...
typedef struct {
    ...
    BOOL ready;
} BDEXTSpecialProtocol;

// the full list of special protocols (an array of BDEXTSpecialProtocol structs)
static BDEXTSpecialProtocol * restrict specialProtocols = NULL;
...
// the number of BDEXTSpecialProtocols which have been marked as ready for
...
static void mtl_injectSpecialProtocols (void) {
     ...
     */
    qsort_b(specialProtocols, specialProtocolCount, sizeof(BDEXTSpecialProtocol), ^(const void *a, const void *b){
        // if the pointers are equal, it must be the same protocol
        if (a == b)
            return 0;

        const BDEXTSpecialProtocol *protoA = a;
        const BDEXTSpecialProtocol *protoB = b;

        // A higher return value here means a higher priority
        int (^protocolInjectionPriority)(const BDEXTSpecialProtocol *) = ^(const BDEXTSpecialProtocol *specialProtocol){
            int runningTotal = 0;
...
BOOL mtl_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // construct a new BDEXTSpecialProtocol structure and add it to the first
        // empty space in the array
        specialProtocols[specialProtocolCount] = (BDEXTSpecialProtocol){
            .protocol = protocol,
