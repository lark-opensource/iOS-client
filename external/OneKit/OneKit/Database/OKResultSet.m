...
+ (instancetype)resultSetWithStatement:(OKStatement *)statement usingParentDatabase:(OKDatabase*)aDB {
    ...
    return OKDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    OKDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return OKDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"OKDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
