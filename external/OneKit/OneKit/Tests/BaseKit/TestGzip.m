...
@interface BDGZIPUnitTests : XCTestCase

@end


@implementation BDGZIPUnitTests
...
- (void)testOutputEqualsInput
{
    ...
    //compress
    NSData *compressedData = [inputData ok_dataByGZipCompressingWithError:nil];

    //decode
    NSData *outputData = [compressedData ok_dataByGZipDecompressingDataWithError:nil];
...
- (void)testUnzipNonZippedData
{
    ...
    //decode
    NSData *outputData = [inputData ok_dataByGZipDecompressingDataWithError:nil];
    NSString *outputString = [[NSString alloc] initWithData:outputData encoding:NSUTF8StringEncoding];
...
{
    NSData *data = [[NSData data] ok_dataByGZipCompressingWithError:nil];
    XCTAssertEqual(data.length, 0);

    data = [[NSData data] ok_dataByGZipDecompressingDataWithError:nil];
    XCTAssertEqual(data.length, 0);
}
...
- (void)testDecompressionPerformance
{
    ...
    NSData *compressedData = [inputData ok_dataByGZipCompressingWithError:nil];
    [self measureBlock:^{
        __unused NSData *outputData = [compressedData ok_dataByGZipDecompressingDataWithError:nil];
    }];
}
