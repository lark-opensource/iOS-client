...
@implementation ACCPollStickerConfig
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.gestureCanStartCallback = ^BOOL(__kindof ACCBaseStickerView<ACCGestureResponsibleStickerProtocol> * _Nonnull contentView, UIGestureRecognizer * _Nonnull gesture) {
            if ([gesture isKindOfClass:[UIPanGestureRecognizer class]] && (contentView.gestureActiveState & ACCStickerGestureStatePinch || contentView.gestureActiveState & ACCStickerGestureStateRotate)) {
                return NO;
            }
            return YES;
        };
...
- (NSArray<ACCStickerBubbleConfig *> *)bubbleActionList
{
    ...
    config.callback = ^(ACCStickerBubbleAction actionType) {
        if (actionType == ACCStickerBubbleActionBizEdit) {
            @strongify(self);
            ACCBLOCK_INVOKE(self.editPoll);
        }
    };
    [bubbleActionList addObject:config];
