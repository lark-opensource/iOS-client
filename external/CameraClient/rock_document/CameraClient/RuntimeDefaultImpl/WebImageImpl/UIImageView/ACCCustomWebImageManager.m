...
@protocol ACCResizableImageConfig <HTSService>
...
@protocol ACCReuseExternalImageConfig <HTSService>
...
@implementation ACCCustomWebImageConfig
...
@interface ACCCustomWebImageManager ()
...
static dispatch_queue_t ACCImageManagerUpdateQueue = nil;
static dispatch_queue_t ACCReuseCacheImageInfoUpdateQueue = nil;

@implementation ACCCustomWebImageManager
...
{
    static ACCCustomWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        ...
        ACCImageManagerUpdateQueue = dispatch_queue_create("ACCCustomWebImageManagerUpdateQueue", DISPATCH_QUEUE_CONCURRENT);
        ACCReuseCacheImageInfoUpdateQueue = dispatch_queue_create("ACCReuseCacheImageInfoUpdateQueue", DISPATCH_QUEUE_SERIAL);
    });
    return manager;
}

- (void)registerCustomWebImage:(Class<ACCCustomWebImageProtocol>)customWebImageManager
{
    dispatch_barrier_async(ACCImageManagerUpdateQueue , ^{
        ACCCustomWebImageConfig *configModel = [customWebImageManager getWebImageConfig];
...
- (void)getCustomBDWebImageManager:(ACCCustomWebImageConfig *)configModel cacheName:(NSString *)cacheName
{
...
- (BDImageCache *)getBDImageCache:(ACCCustomWebImageConfig *)configModel cacheName:(NSString *)cacheName
{
    BDImageCache *cache = nil;
    NSString *cachePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory,
                                                               NSUserDomainMask, YES) firstObject];
    cachePath = [cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/images", cacheName]];
    cache = [[BDImageCache alloc] initWithName:cachePath];
    BDImageCacheConfig *cacheConfig = [[BDImageCacheConfig alloc] init];
    cacheConfig.memoryCountLimit = configModel.memoryCountLimit;
    cacheConfig.memorySizeLimit = configModel.memorySizeLimit;
    cacheConfig.memoryAgeLimit = configModel.memoryAgeLimit;
...
- (void)updateCustomWebImageConfig:(ACCCustomWebImageConfig *)configModel cacheName:(NSString *)cacheName
{
...
- (void)updateBDWebImageConfig:(ACCCustomWebImageConfig *)configModel cacheName:(NSString *)cacheName
{
...
- (nullable ACCCoverImageCacheModel *)getCacheImageInfoWithIdentificationKey:(NSString *)identifier
{
    if (![APPContextIMP(ACCReuseExternalImageConfig) enableReuseExternalImage]) {
        return nil;
    }
    NSString *identificationKey = [NSString stringWithFormat:@"reuse_image_for_feed_play_%@",identifier];
    ACCCoverImageCacheModel *cacheInfo = DynamicCast([self.reuseCacheImageInfoDictionary objectForKey:identificationKey], ACCCoverImageCacheModel);
    return cacheInfo;
}

- (void)stagingImageInfo:(NSString *)imageURLString cacheName:(NSString *)cacheName identificationKey:(NSString *)identifier
{
    if (![APPContextIMP(ACCReuseExternalImageConfig) enableReuseExternalImage] || BTD_isEmptyString(imageURLString) || BTD_isEmptyString(identifier)) {
        return ;
    }
    dispatch_async(ACCReuseCacheImageInfoUpdateQueue, ^{
        NSString *identificationKey = [NSString stringWithFormat:@"reuse_image_for_feed_play_%@",identifier];
        ACCCoverImageCacheModel *model = [[ACCCoverImageCacheModel alloc] initWithCacheImageURLString:imageURLString cacheName:cacheName];
...
@implementation ACCCoverImageCacheModel
...
@implementation ACCWebImageManagerTools

+ (void)removeAllMemoryCache
{
    [[ACCCustomWebImageManager sharedInstance] removeCustomMemoryCache];
    // Default - BD
    BDImageCache *BDImageCache = nil;
    NSString *imageCacheName = [APPContextIMP(ACCResizableImageConfig) getCustomBDWebImageCacheName];
...
{
    [[ACCCustomWebImageManager sharedInstance] removeCustomDiskCache];
...
+ (NSUInteger)totalDiskCost
{
    ...
    totalCost += [[ACCCustomWebImageManager sharedInstance] totalCustomDiskCost];
    // Default - BD (Custom + Default)
    BDImageCache *BDImageCache = nil;
    NSString *imageCacheName = [APPContextIMP(ACCResizableImageConfig) getCustomBDWebImageCacheName];
    if (!BTD_isEmptyString(imageCacheName)) {
