...
@implementation ACCNLEEditorBuilder
...
- (ACCEditSessionWrapper *)buildEditSession
{
    ...
    [nleVideoData moveResourceToDraftFolder:nleVideoData.draftFolder];
    [self.editor acc_commitAndRender:nil];
    
    [self.nle resetPlayerWithViews:@[self.mediaContainerView]];
    
    self.publishModel.repoCutSame.cutSameNLEModel = [[self.editor getModel] deepClone];
...
- (void)resetEditSessionWithPublishModel:(AWEVideoPublishViewModel *)publishModel
{
    ...
    if (self.publishModel.repoCutSame.cutSameNLEModel) {
        // 要把cutsame里面业务的贴纸信息回填到nle缓存
        [self restoreStickerUserInfoToNLE];
    }
}
    ...
    if (needAdaptScreen) {
        ...
        if (outputSize) {
            ...
            if (self.publishModel.repoCutSame.isNewCutSameOrSmartFilming) {
                // cutsame template should use AspectRatio
                aspectFill = NO;
            }
