...
@class BDCTIndicatorView;
typedef void (^DismissHandler)(BOOL isUserDismiss);

typedef NS_ENUM(NSInteger, BytedCertIndicatorViewStyle) {
    BytedCertIndicatorViewStyleImage = 0,  //icon+文字样式
    BytedCertIndicatorViewStyleWaitingView //转等待提示+文字样式
};
 ...
 *  @param shouldRemoveWaitingView 如果指示器类型是BytedCertIndicatorViewStyleWaitingView，是否需要停止并移除
 */
- (void)updateIndicatorWithText:(nullable NSString *)updateIndicatorText
        shouldRemoveWaitingView:(BOOL)shouldRemoveWaitingView;

/**
 *  更新指示器iconImage，同时BytedCertIndicatorViewStyle强制更新为BytedCertIndicatorViewStyleImage
 *
...
static inline BOOL BytedCertShown(UIView *_Nonnull view) {
    return !view.hidden;
}
