...
#define MKDIR(DIRNAME) mkdir(DIRNAME, S_IRWXU|S_IRWXG|S_IRWXO)
...
static int mkpath(char *path) {
  ...
  for (p = path + len; *p && len < MAX_PATH; p++) {
    ...
    defined(__MINGW32__)
      ...
      const wchar_t *wc = char2wchar(npath);
      if (_wmkdir(wc) == -1) {
        if (errno != EEXIST) {
          return -1;
        }
  ...
  for (i = 0; i < n; ++i) {
    ...
    defined(__MINGW32__)
      ...
      if (xattr > 0) {
          int modResult = 0;
#if defined(_WIN32) || defined(__WIN32__) || defined(_MSC_VER) || defined(__MINGW32__)
          const wchar_t *wc = char2wchar(path);
          modResult = _wchmod(wc, (mode_t)xattr);
#else
          modResult = chmod(path, (mode_t)xattr);
#endif
          if (modResult < 0) {
            goto out;
          }
