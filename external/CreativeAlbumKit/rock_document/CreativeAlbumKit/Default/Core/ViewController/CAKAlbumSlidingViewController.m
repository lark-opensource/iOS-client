//
//  CAKAlbumSlidingViewController.m
...
@interface UIScrollView (CAKAlbumSlidingScroll)

- (NSInteger)cak_currentPageIndex;

@end

@implementation UIScrollView (CAKAlbumSlidingScroll)
...
@implementation CAKAlbumSlidingViewController
...
- (void)setTabbarView:(CAKAlbumSlidingTabBarView *)tabbarView
{
...
- (void)reloadViewControllers
{
    ...
    for (NSInteger i = 0; i < count; i++) {
        [self.viewControllers acc_addObject:[NSNull null]];
    }
...
- (void)updateSelectedIndexWithScrollView:(UIScrollView *)scrollView
{
    ...
    if (self.animatingDestinationIndex != index && index == self.selectedIndex) { // 手滑动过程中又取消了
    ...
    [self updateScrollIndexControllerWithTransitionType:CAKAlbumSlidingVCTransitionTypeScroll index:index];
}
...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [self updateScrollIndexControllerWithTransitionType:CAKAlbumSlidingVCTransitionTypeTapTab index:index];
...
- (void)updateScrollIndexControllerWithTransitionType:(CAKAlbumSlidingVCTransitionType)transitionType index:(NSInteger)index
{
    ...
    if (self.tabbarView) {
        if ([self.tabbarView respondsToSelector:@selector(setSelectedIndex:)]) {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    NSInteger nextIndex = [scrollView cak_currentPageIndex];
    if (velocity.x < 0) {
        nextIndex++;
    } else {
        nextIndex--;
    ...
    if (nextIndex >= 0 && nextIndex < self.viewControllers.count) {
        NSInteger curIndex = [scrollView cak_currentPageIndex];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(slidingViewController:willTransitionToViewController:transitionType:)]) {
            [self.delegate slidingViewController:self willTransitionToViewController:[self currentVCAtIndex:nextIndex] transitionType:CAKAlbumSlidingVCTransitionTypeScroll];
        }
    }
}
...
- (void)_makeViewControllerVisibleAtIndex:(NSInteger)index transitionType:(CAKAlbumSlidingVCTransitionType)transitionType
{
    if (index >= self.viewControllers.count || index < 0) {
        return;
    }
    if (transitionType == CAKAlbumSlidingVCTransitionTypeScroll) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(slidingViewController:willTransitionToViewController:transitionType:)]) {
            [self.delegate slidingViewController:self willTransitionToViewController:[self currentVCAtIndex:index] transitionType:CAKAlbumSlidingVCTransitionTypeTapTab];
        }
        [self _makeControllersDidAppear];
    }
...
- (void)_makeControllersWillAppearAtPreIndex:(NSInteger)preIdx currentIndex:(NSInteger)currentIdx
{
    ...
    if (preIdx != currentIdx) {
        ...
        if (oldVC && [self.childViewControllers containsObject:oldVC]) {
            ...
            [self.transitionDisappearArray acc_addObject:oldVC];
        }
    }
    ...
    if (newVC && self.isViewAppearing) {
        ...
        [self.transitionAppearArray acc_addObject:newVC];
    }
}
...
- (void)vcHasCompleteAppear:(UIViewController *)controller
{
    ...
    if (controller) {
        [self.appearCompleteArray acc_addObject:controller];
    }
}
...
{
    if (controller) {
        [self.viewControllers acc_replaceObjectAtIndex:index withObject:controller];
...
{
    if (viewController) {
        [self.viewControllers acc_insertObject:viewController atIndex:0];
...
{
    if (viewController) {
        [self.viewControllers acc_addObject:viewController];
        [self addChildViewController:viewController];
...
{
    return [CAKAlbumSlidingScrollView class];
}

- (NSInteger)currentScrollPage
{
    return self.contentScrollView.cak_currentPageIndex;
}
...
- (void)enumerateChildViewControllers:(UIViewController *)currentVC usingBlock:(void (^)(UIViewController *childVC))block
{
    ...
    while (containerArray.count > 0) {
        ...
        [containerArray acc_removeObjectAtIndex:0];
        if (vc) {
