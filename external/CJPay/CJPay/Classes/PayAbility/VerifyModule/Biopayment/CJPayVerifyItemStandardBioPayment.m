//
//  CJPayVerifyItemStandardBioPayment.m
...
@interface CJPayVerifyItemStandardBioPayment()
...
@property (nonatomic, assign) BOOL isNeedLoading; //验证指纹时是否需要loading
@property (nonatomic, strong) CJPayEvent *event;

@end

@implementation CJPayVerifyItemStandardBioPayment
...
- (void)bioPayWithResponse:(CJPayBDCreateOrderResponse *)response
    ...
    NSDictionary *nopwdDic = @{ @"nopwd_disable_reason": CJString(response.payInfo.verifyDowngradeReason)};
    ...
    [CJPayTouchIdManager showTouchIdWithLocalizedReason:localizedReason
        ...
        [self event:@"wallet_fingerprint_verify_page_verify_result"
             params:@{@"result" : @"0",
                      @"error_code" : @(LAErrorUserFallback),
                      @"error_message" : @"fallback",
                      @"error_cn_message" : @"输入密码",
                      @"bio_type" : [CJPayTouchIdManager currentBioType]}];
...
- (void)handleEvent:(CJPayEvent *)event {
    ...
    if ([event.data isKindOfClass:NSDictionary.class]) {
        ...
        self.isNeedLoading = [dict cj_boolValueForKey:@"is_need_loading"];
    }
}
...
- (BioPaymentAction)p_bioPaymentAction {
    ...
    } else if ([self.manager.response.payInfo.cashierTags containsObject:@"BioFailedPwdVerifyOptimizationV1"]) {
...
- (BOOL)p_shouldShowRetainVCWithRedoBlock:(void (^)(void))redoBlock {
    ...
    UIViewController *sourceVC = [self.manager.homePageVC topVC];
