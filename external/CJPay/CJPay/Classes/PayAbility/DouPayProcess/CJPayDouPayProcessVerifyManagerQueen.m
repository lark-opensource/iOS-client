//
//  CJPayDouPayProcessVerifyManagerQueen.m
//  CJPaySandBox
//
...
@interface CJPayDouPayProcessVerifyManagerQueen ()
...
@implementation CJPayDouPayProcessVerifyManagerQueen
...
- (void)beforeConfirmRequest {
    ...
    if ([self.verifyManager isKindOfClass:CJPayDouPayProcessVerifyManager.class] && [CJPaySettingsManager shared].currentSettings.isHitEventUploadSampled) {
        CJPayDouPayProcessVerifyManager *douPayVerifyManager = (CJPayDouPayProcessVerifyManager *)self.verifyManager;

        NSString *checkType = [self p_checkTypeMapString:[[douPayVerifyManager lastConfirmVerifyItem] checkType]];
        NSMutableDictionary *timestampInfo = [[douPayVerifyManager.extParams cj_dictionaryValueForKey:@"timestamp_info"] mutableCopy];
    ...
    [self trackCashierWithEventName:@"wallet_cashier_confirm_loading"
                             params:@{}];
    [self trackVerifyWithEventName:@"wallet_rd_main_process" params:@{
        @"process_name" : @"发起确认支付流程",
        @"process_source" : @"标准化流程",
        @"ext_params" : @{
            @"is_bind_card" : self.verifyManager.isBindCardAndPay ? @"1" : @"0",
            @"real_check_type": CJString([self.verifyManager.lastWakeVerifyItem checkType]),
            @"real_handle_check_type": CJString([self.verifyManager.lastHandleVerifyItem checkType]),
            @"real_confirm_check_type": CJString([self.verifyManager.lastConfirmVerifyItem checkType])
        }
    }];
}
...
- (void)afterConfirmRequestWithResponse:(CJPayOrderConfirmResponse *)orderResponse {
    [self trackVerifyWithEventName:@"wallet_cashier_confirm_error_info"
                            params:@{@"error_code": CJString(orderResponse.code),
                                     ...
                                     @"is_bindcard_and_pay": self.verifyManager.isBindCardAndPay ? @"1" : @"0"
                                   }];
...
- (void)beforQueryResult {
    ...
    if ( [CJPaySettingsManager shared].currentSettings.isHitEventUploadSampled) {
        NSString *checkType = [self p_checkTypeMapString:[[self.verifyManager lastConfirmVerifyItem] checkType]];
        [self.verifyManager.verifyManagerQueen trackVerifyWithEventName:@"wallet_rd_custom_scenes_time" params:@{
            @"scenes_name" : @"电商",
            @"check_type" : CJString(checkType),
            @"sub_section" : @"确认支付耗时",
...
//    NSString *preMethod = CJString(response.payInfo.businessScene);
    CJPayDefaultChannelShowConfig *currentShowConfig = [self.verifyManager.homePageVC curSelectConfig];
    NSString *preMethod = CJString(currentShowConfig.businessScene);
    if (currentShowConfig.isCombinePay) {
        if (currentShowConfig.type == BDPayChannelTypeBankCard) {
            preMethod = @"Pre_Pay_Balance_Bankcard";
        } else if (currentShowConfig.type == BDPayChannelTypeAddBankCard) {
...
//        }
//    }
    NSString *isColdLaunch = [self.verifyManager.bizParams cj_stringValueForKey:@"is_cold_launch" defaultValue:@""];
    NSString *prepayId = [self.verifyManager.bizParams cj_stringValueForKey:@"prepayid" defaultValue:@""];
    if (!Check_ValidString(prepayId)) {
        prepayId = [self.verifyManager.bizParams cj_stringValueForKey:@"prepay_id" defaultValue:@""];
    }
    NSMutableDictionary *mutableDic = [@{
        ...
        @"issue_check_type": CJString([self.verifyManager issueCheckType]),
        @"real_check_type":CJString([self.verifyManager.lastWakeVerifyItem checkType]),
        @"is_sdk_standard" : self.verifyManager.isStandardDouPayProcess ? @"1" : @"0",
        @"is_cold_launch" : CJString(isColdLaunch),
    ...
    if ([self.verifyManager isKindOfClass:CJPayDouPayProcessVerifyManager.class]) {
        CJPayDouPayProcessVerifyManager *douPayVerifyManager = (CJPayDouPayProcessVerifyManager *)self.verifyManager;
        [mutableDic addEntriesFromDictionary:[douPayVerifyManager.extParams cj_dictionaryValueForKey:@"track_info"]];
    }
