...
#define kLongPressMinimumDuration 0.15 // Time in seconds the fingers must be held down for long press gesture.
#define kLongPressCheckMinimumDuration 0.2
...
NSString *const YYTextViewIsInLongPressSelectionRects = @"YYTextViewIsInLongPressSelectionRects";
...
@interface YYTextView () <UIScrollViewDelegate, UIAlertViewDelegate, YYTextDebugTarget, YYTextKeyboardObserver> {
    ...
    NSMutableArray *_menusItems;
    BOOL _handledLongPressed;
...
- (void)_updateSelectionView {
    if (_allowLongPressSelectionAlwaysAll) {
    ...
    _selectionView.color = _allowLongPressSelectionAlwaysAll ? _selectionViewBackgroundColor : [self _defaultTintColor];
    _selectionView.useAlpha = !_allowLongPressSelectionAlwaysAll;
    _selectionView.selectionRects = allRects;

    if (_allowLongPressSelectionAlwaysAll) {
        _selectionView.startGrabber.hidden = YES;
        _selectionView.endGrabber.hidden = YES;
    }
...
- (void)removeMySelectionRects {
    [_maskViews enumerateObjectsUsingBlock: ^(UIView *v, NSUInteger idx, BOOL *stop) {
        [v removeFromSuperview];
    }];
    [_maskViews removeAllObjects];
}

- (void)setDragAreaOffset:(CGFloat)dragAreaOffset {
    _dragAreaOffset = dragAreaOffset;
    _selectionView.dragAreaOffset = dragAreaOffset;
}


- (void)setMySelectionRects:(nullable YYTextBorder *)border range:(NSRange)range {
    [self setMySelectionRects:border range:range lineSeperate:YES];
}

- (void)setMySelectionRects:(nullable YYTextBorder *)border range:(NSRange)range lineSeperate:(BOOL)seperate {
    NSArray *selectionRects = [_innerLayout selectionRectsForRange:[YYTextRange rangeWithRange:range] lineSeperate:seperate];
    [selectionRects enumerateObjectsUsingBlock: ^(YYTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        ...
        if (rect.size.width > 0 && rect.size.height > 0) {
            CGRect markFrame = UIEdgeInsetsInsetRect(rect, border.insets);
            UIView *mark = [[UIView alloc] initWithFrame:markFrame];
            mark.backgroundColor = border.fillColor;
            mark.layer.borderColor = border.strokeColor.CGColor;
            mark.layer.borderWidth = border.strokeWidth;
            mark.layer.cornerRadius = border.cornerRadius;

            if (border.bottomLineStrokeWidth > 0) {
                if (border.bottomLineType == YYTextBottomLineTypeStraightLine) {
                    UIView *bottomLine = [[UIView alloc] init];
                    bottomLine.backgroundColor = border.bottomLineStrokeColor == nil ? [UIColor blackColor] : border.bottomLineStrokeColor;
                    [mark addSubview:bottomLine];
                    bottomLine.frame = CGRectMake(0, markFrame.size.height - border.bottomLineStrokeWidth, markFrame.size.width, border.bottomLineStrokeWidth);
                } else {
                    CAShapeLayer *shapeLayer = [CAShapeLayer layer];
                    [shapeLayer setStrokeColor:[border.bottomLineStrokeColor CGColor]];
                    [shapeLayer setLineWidth:border.bottomLineStrokeWidth];
                    [shapeLayer setLineJoin:kCALineJoinRound];
                    [shapeLayer setLineDashPattern: @[@3, @3]];

                    CGPoint beginPoint = CGPointMake(0, markFrame.size.height - 1);
                    CGPoint endPoint = CGPointMake(markFrame.size.width, markFrame.size.height - 1);
                    ...
                    CGPathRelease(path);
                    [[mark layer] addSublayer:shapeLayer];
                }
            }
            [self insertSubview:mark belowSubview:_containerView];
            [_maskViews addObject:mark];
        }
    }];
}
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        ...
        if (needDraw) {
            ...
            format.opaque = NO;
            format.scale = 0;
            UIGraphicsImageRenderer *render = [[UIGraphicsImageRenderer alloc] initWithBounds:self.bounds format:format];
...
- (void)_showMagnifierCaret {
    if (!_allowShowMagnifierCaret) return;
...
- (void)_startLongPressTimer {
    ...
    _longPressCheckTimer = [NSTimer timerWithTimeInterval:kLongPressCheckMinimumDuration
                                              target:[YYTextWeakProxy proxyWithTarget:self]
                                            selector:@selector(_longPressCheck)
                                            userInfo:nil
                                             repeats:NO];
    [[NSRunLoop currentRunLoop] addTimer:_longPressCheckTimer forMode:NSRunLoopCommonModes];
}

- (void)_longPressCheck {
    if (_handledLongPressed == NO) {
        if (_textTapEndAction) {
            ...
            _textTapEndAction(self, _innerText, range, rect);
        }
    }
}
...
- (void)_initTextView {
    ...
    _menusItems = [NSMutableArray array];
    _text = @"";
    _attributedText = [NSAttributedString new];
    _allowSelectionDot = YES;
    _allowShowMagnifierCaret = YES;
    _allowLongPressSelection = YES;
    _allowLongPressSelectionAlwaysAll = NO;
    ...
    _selectionViewBackgroundColor = [self _defaultTintColor];
...
- (void)tintColorDidChange {
    if ([self respondsToSelector:@selector(tintColor)]) {
        ...
        if (_allowLongPressSelectionAlwaysAll) {
            _selectionView.color = _selectionViewBackgroundColor;
        } else {
...
- (void)_handleTouchCallback:(BOOL)isEnd {
    NSRange range = NSMakeRange(NSNotFound, 0);
    CGRect rect = CGRectNull;
    CGPoint point = [self _convertPointToLayout:_touchBeganPoint];
    YYTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    if (isEnd) {
        ...
        if (_textTapBeganAction) {
            _textTapBeganAction(self, _innerText, range, rect);
        }
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            if ([_selectionView isGrabberContainsPoint:point]) { // track grabber
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:YYTextViewIsInLongPressSelectionRects object:@[@(0), self]];
                [self _updateSelectionView];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:YYTextViewIsInLongPressSelectionRects object:@[@(_selectionView.selectionRects.count), self]];
}
