    ...
    [selectionRects enumerateObjectsUsingBlock: ^(YYTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        ...
        if (r.containsStart || r.containsEnd) {
            ...
            if (rect.size.width > 0 && rect.size.height > 0) {
                ...
                mark.alpha = _useAlpha ? kMarkAlpha : 1.0;
...
- (BOOL)isEndGrabberContainsPoint:(CGPoint)point {
    ...
    // 扩大拖动范围
    startRect.size.width = endRect.size.width - _dragAreaOffset;
    startRect.size.height = endRect.size.height - _dragAreaOffset;

    endRect.size.width = endRect.size.width + _dragAreaOffset;
    endRect.size.height = endRect.size.height + _dragAreaOffset;
