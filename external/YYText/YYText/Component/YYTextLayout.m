...
- (NSArray<YYTextSelectionRect *> *)selectionRectsForRange:(YYTextRange *)range {
    return [self selectionRectsForRange:range lineSeperate:NO];
}

- (NSArray *)selectionRectsForRange:(YYTextRange *)range lineSeperate:(BOOL)seperate {
    range = [self _correctedRangeWithEdge:range];
                ...
                if (isVertical) {
                    ...
                    if (seperate) {
                        for (NSUInteger l = startLineIndex + 1; l < endLineIndex; l++) {
                            ...
                            r.origin.y = CGRectGetMinY(line.bounds);
                            r.size.height = line.bounds.size.height;
