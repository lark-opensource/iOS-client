...
/** It's used to get weather current video resolution is TTVideoEngineResolutionTypeABRAuto
...
/**
 * initialize without UIKit, allow to invoked in any thread,
 * must invoke the function below before set up anything or use.
 * + (TTVideoEnginePlayerViewWrapper *)viewWrapperWithType:(TTVideoEnginePlayerType)type;
 */
- (instancetype)initWithType:(TTVideoEnginePlayerType)type async:(BOOL)async NS_DESIGNATED_INITIALIZER;
/**
 set play api version and authorization
 
 @param apiVersion play apiVersion
 @param auth Authorization
 */
- (void)setPlayAPIVersion:(TTVideoEnginePlayAPIVersion)apiVersion auth:(nullable NSString *)auth;
