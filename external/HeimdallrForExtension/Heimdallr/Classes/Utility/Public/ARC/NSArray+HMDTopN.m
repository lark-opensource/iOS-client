...
+ (NSArray *)hmd_heapTopNWithArray:(NSArray *)array topN:(NSUInteger)topN usingComparator:(NSComparator NS_NOESCAPE)cmptr {
    ...
    for (NSInteger i = topN; i < array.count; i++) {
        if (cmptr([heap hmd_objectAtIndex:1], [array hmd_objectAtIndex:i]) == NSOrderedAscending) {
            heap[1] = [array hmd_objectAtIndex:i];
            hmd_heapTopNHeapFormat(heap, topN, 1, cmptr);
        }
    }
...
- (void)hmd_heapTopNAddObject:(id)object {
    ...
    if (self.count == (self.hmd_topN + 1)) {
        [self addObject:object];
        hmd_heapTopNBuildHeap(self, self.hmd_topN, self.hmd_cmptr);
    }

    if (self.hmd_cmptr([self hmd_objectAtIndex:1], object) == NSOrderedAscending) {
        self[1] = object;
        hmd_heapTopNHeapFormat(self, self.hmd_topN, 1, self.hmd_cmptr);
    }
...
- (NSArray *)hmd_topNArray {
    ...
    NSArray *array = [self subarrayWithRange:NSMakeRange(1, MIN(self.hmd_topN, self.count))];
    return [array copy];
}
