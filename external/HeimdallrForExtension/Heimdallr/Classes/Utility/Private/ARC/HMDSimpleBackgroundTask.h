...
// backgroundTask 的功能非常简单，因为在应用进入到后台的时刻，它就会暂停工作嘛，
// 然后在 backgroundTask 里的 task 就有尽可能多的额外的工作时间，然后当你的工作完成之后
// 记得调用一下 completeHandle 就好咯

#import <Foundation/Foundation.h>

NS_ASSUME_NONNULL_BEGIN

typedef void (^HMDSimpleBackgroundTaskBlock)(void(^completeHandle)(void));

@interface HMDSimpleBackgroundTask : NSObject

+ (void)detachBackgroundTaskWithName:(NSString *)name
                                task:(HMDSimpleBackgroundTaskBlock)task;

// ExpireTime 依赖于 NSTimer 意味着你的代码不能在主线程进行 backgroundTask
// 任何 dispatch_async detachNewThread 都可以 记得最后调用 completeHandle 就行
// 就是在这个 task 里，你再调用一次 dispatch_async 把你的工作挂到非主线程
+ (void)detachBackgroundTaskWithName:(NSString *)name
