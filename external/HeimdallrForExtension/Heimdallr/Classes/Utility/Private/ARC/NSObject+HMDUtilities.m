...
+ (NSDictionary *)hmd_properties
{
    ...
    for(i = 0; i < outCount; i++)
    {
        ...
        if(propName)
        {
            ...
            GHNSObjectProperty *propertyType = [self ghPropertyWithProperty:property];
            if (propertyType) {
    ...
    @synchronized(cache) {
        [cache hmd_setObject:dictionary forKey:className];
    }
    return dictionary;
}

+ (GHNSObjectProperty *)ghPropertyWithProperty:(objc_property_t)property
{
    return [[GHNSObjectProperty alloc] initWithProperty:property class:self];
}
@end
