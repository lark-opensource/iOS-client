...
- (void)setup {
    ...
    if(injectedInfo != nil) {
        SEL scene_selector = @selector(userID);
        if([injectedInfo respondsToSelector:scene_selector])
            [injectedInfo addObserver:self
                           forKeyPath:NSStringFromSelector(scene_selector)
                              options:NSKeyValueObservingOptionInitial|NSKeyValueObservingOptionNew
                              context:NULL];
...
-(void)removeObserver {
    ...
    if(injectedInfo != nil) {
        SEL scene_selector = @selector(userID);
        if([injectedInfo respondsToSelector:scene_selector])
            [injectedInfo removeObserver:self forKeyPath:NSStringFromSelector(scene_selector)];
