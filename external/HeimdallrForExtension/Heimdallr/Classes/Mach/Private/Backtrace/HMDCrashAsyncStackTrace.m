...
hmd_async_stack_record_t *hmd_async_stack_trace_current_thread(void) {
    ...
    return hmd_async_stack_record_with_pthread(async_shared_stack_record_list(), pthread_self());
}
...
hmd_async_stack_record_t *hmd_async_stack_trace_pthread(pthread_t thread) {
    ...
    return hmd_async_stack_record_with_pthread(async_shared_stack_record_list(), thread);
}
...
hmd_async_stack_record_t *hmd_async_stack_trace_mach_thread(thread_t thread) {
    ...
    return hmd_async_stack_record_with_mach_thread(async_shared_stack_record_list(),thread);
}
...
static hmd_async_stack_record_t *record_current_stack() {
    
    mark_start
    
    hmd_async_stack_record_t *record = (hmd_async_stack_record_t *)calloc(1,sizeof(hmd_async_stack_record_t));
    if (record == NULL) {
    ...
    if (atomic_load_explicit(&enable_multiple_async_stack_trace,memory_order_acquire)) {
        ...
        if (pre_record) {
            for (int i = pre_record->skip_length; i < pre_record->length; i++) {
                if (record->length >= HMD_MAX_ASYNC_STACK_LENGTH) {
                    break;
                }
...
static dispatch_block_t DISPATCH_IS_CALLING(dispatch_block_t block){
    ...
    dispatch_block_t new_block = ^{
        void *node = insert_stack_record(record);
        block();
        record->valid = false;
        remove_stack_record_node(node);
    };
...
static void __DISPATCH_IS_CALLING_work_invoke(hmd_async_stack_record_context *context) {
    if (context) {
        if (context->work) {
            void *node = insert_stack_record(context->record);
...
//        if (strlen(hmd_executable_path) == 0) {
//            NSCAssert(NO, @"hmd_executable_path is null");
//            return;
//        }
//
...
void hmd_enable_async_stack_trace(void) {
    ...
    if (!hmd_is_address_sanitizer() && !hmd_is_thread_sanitizer()) {
        ...
        if (ret == 0) {
            ...
            DEBUG_LOG("creat pthread key err, %d", ret);
        }
        mark_end
    }
}
