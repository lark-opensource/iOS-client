/*!@header hmd_mach_exception_protection_definition.h
 * @abstract Mach 异常防护的参数，改变异常防护的行为
 *
 * @discussion [ 业务使用方 ] 使用请参考注释，若提及不可使用，或者请与 Heimdallr 开发讨论的话，请严格准守
 *             [ Heimdallr 维护者 ] 该文档需要和 HMDMachRecoverDeclaration.h 和 HMDCrashPreventMachExceptionDefinition.h
 *                           文件参数相同对应，请 double check 它们的关系
 */
...
typedef enum : uint64_t {
    ...
    HMDMachRecoverOptionScopeCheckType_disableCheck       = 1 << 13,    // ⚠️ 使用该参数需要与 Heimdallr 开发沟通 (已内部无效化处理)
...
struct HMDMachRecoverContext {
    uint64_t context_size;          // 请传递参数 .context_size = sizeof(HMDMachRecoverContext)
    uint64_t reserved0;             // checksum
    uint64_t reserved1;             // option
    uint64_t reserved2;             // scope
    uint64_t reserved3;             // reserved
    uint64_t reserved4;             // reserved
    uint64_t backtrace_depth;
    uint64_t scope_length;
    
    /* HMDMachRecoverContext_version 0 end */
    
} HMD_PACKED HMD_ALIGNED(8);

typedef struct HMDMachRecoverContext  HMDMachRecoverContext;
typedef struct HMDMachRecoverContext *HMDMachRecoverContextRef;

#ifndef GCC_FORCE_NO_OPTIMIZATION
#define GCC_FORCE_NO_OPTIMIZATION  asm("");
#endif
