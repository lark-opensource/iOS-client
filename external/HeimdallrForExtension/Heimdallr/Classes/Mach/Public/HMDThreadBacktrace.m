...
HMDThreadBacktrace *construct_with_hmd_thread_backtrace(hmdbt_backtrace_t *bt) {
    ...
    hmd_async_stack_record_t safe_pre_bt;
    if (pre_bt != NULL) {
        if (hmd_async_read_memory((hmd_vm_address_t)pre_bt, &safe_pre_bt, sizeof(safe_pre_bt)) == HMD_ESUCCESS) {
            if (safe_pre_bt.valid) {
                has_pre_stack = true;
                for (int i=safe_pre_bt.skip_length; i < safe_pre_bt.length; i++){
                    HMDThreadBacktraceFrame *frame = [[HMDThreadBacktraceFrame alloc] init];
                    frame.address = (uintptr_t)(safe_pre_bt.backtrace[i]);
    ...
    if(bt->name != NULL) {
        ...
        if (has_pre_stack && strlen(safe_pre_bt.thread_name) > 0) {
            name = [NSString stringWithFormat:@"%@(pre %@)", name, [NSString stringWithUTF8String:safe_pre_bt.thread_name]];
        }
        backtrace.name = name;
    }
