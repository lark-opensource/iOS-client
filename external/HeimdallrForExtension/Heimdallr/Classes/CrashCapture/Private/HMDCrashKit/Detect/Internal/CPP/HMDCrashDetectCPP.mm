...
// terminate handle
static void hmd_cpp_terminate_handle(void);
static std::terminate_handler original_handle;
...
void HMDCrashDetect_cpp_start(void) {
    if (std::get_terminate() == hmd_cpp_terminate_handle) {
        SDKLog_warn("cpp terminate handle already setted");
        return;
    }
        
    original_handle = std::set_terminate(hmd_cpp_terminate_handle);
    SDKLog("cpp detector launch complete");
}

void HMDCrashDetect_cpp_end(void) {
    if (original_handle) {
        std::set_terminate(original_handle);
...
static void hmd_cpp_terminate_handle(void) {
    ...
    if (original_handle != hmd_cpp_terminate_handle) {
...
bool HMDCrashDetect_cpp_check(void)
{
    ...
    if (handler != hmd_cpp_terminate_handle) {
