...
#include "HMDCrashImages.h"
#include "HMDCrashExceptionExtend.h"
#include "HMDCrashDirectory_LowLevel.h"

#define QUERY_THREAD_NAME_SIZE 256
#define kGWPASanLength 1024

#define HMD_MAX_FRAME_COUNT 100000

typedef void (hmd_crashdetect_finish_callback)(char **crash_data, uint64_t crash_time, uint64_t fault_address, thread_t current_thread, thread_t crash_thread);
hmd_crashdetect_finish_callback *hmd_crash_detect_finish_callback;
...
static void hmdCrashWriteGwpasan(struct hmdcrash_detector_context *crash_detector_context);

void WEAK_FUNC handle_coredump(struct hmdcrash_detector_context *crash_detector_context, struct hmd_crash_env_context *envContextPointer) {}
bool WEAK_FUNC detect_gwp_asan(uintptr_t fault_address, char *str, int strLen, uintptr_t *allocateTrace, int *allocateTraceLen, uintptr_t *deallocateTrace, int *deallocateTraceLen) {
    return false;
}
...
static void extra_crash_info_callback(hmd_async_image_t *image, int index, bool *stop, void *ctx)
{
    hmd_async_mem_range crash_info = image->macho_image.crash_info;
    if (crash_info.size >= sizeof(crash_info_t)) {
        crash_info_t *info = (crash_info_t *)crash_info.addr;
        ...
        for (int i = 0; i<sizeof(addr)/sizeof(uintptr_t); i++) {
            ...
            if (err == HMD_ESUCCESS) {
                if (strlen(buffer) > 0) {
                    bool *is_first_crash_info = ctx;
                    if (is_first_crash_info) {
                        HMDCrashWriteExtraCrashInfo(buffer,*is_first_crash_info);
                        *is_first_crash_info = false;
                    }
                }
            }
...
static void write_stack_record(struct hmd_machine_context *crash_context)
{
...
static void write_address_analyze(struct hmd_machine_context *crash_context) {
...
void write_extra_crash_info() {
    if (HMDCrashOpenExtraCrashInfoFile()) {
        HMDCrashBeginExtraCrashInfo();
        
        bool is_first_crash_info = true;
        hmd_async_enumerate_image_list(extra_crash_info_callback, &is_first_crash_info);
        
        HMDCrashEndExtraCrashInfo();
        HMDCrashCloseExtraCrashInfoFile();
    }
    HMDCrashStopDetect();
}

void dump_memory(struct hmdcrash_detector_context *crash_detector_context, struct hmd_crash_env_context *envContextPointer)
{
    handle_coredump(crash_detector_context, envContextPointer);
}
...
void hmd_crash_handler(struct hmd_crash_env_context *envContextPointer, struct hmdcrash_detector_context *crash_detector_context)
{
    ...
    write_stack_record(crash_machine_context);
    ...
    HMDCrashWriteDynamicData();
    
    write_address_analyze(crash_machine_context);
    ...
    if (hmd_crash_detect_finish_callback) {
        char *crash_data = NULL;
        hmd_crash_detect_finish_callback(&crash_data, (uint64_t)(crash_detector_context->crash_time * 1000), (uint64_t)crash_detector_context->fault_address, envContextPointer->current_thread, crash_thread);
        HMDCrashWriteScriptStack(crash_data);
    }
    
    if (hmd_crash_has_extra_dynamic_data_callback()) {
        SDKLog("write realtime dynamic data");
        HMDCrashWriteExtraDynamicData((uint64_t)(crash_detector_context->crash_time * 1000), (uint64_t)crash_detector_context->fault_address, envContextPointer->current_thread, crash_thread);
    }
    
    close_exception();
    
    SDKLog("all expcetion data complete :>");
    
//    begin write binary image
    if (hmd_crash_switch_state(HMDCrashSwitchWriteImageOnCrash)) {
        writeImageOnCrash();
    }
    
    hmdCrashWriteGwpasan(crash_detector_context);
    
    dump_memory(crash_detector_context, envContextPointer);
    
    hmdmc_resumeEnvironment(envContextPointer);
    
    if (hmd_crash_switch_state(HMDCrashSwitchExtendFD)) {
        hmd_fetch_current_fds();
    } else {
        remove_exception_fd();
    }
    ...
    if (crash_detector_context->crash_type == HMDCrashTypeFatalSignal) {
        HMDCrashStopDetect();
    } else {
        HMDCrashStopDetectPortion(HMDCrashTypeFatalSignal);
    }
}

static void hmdCrashWriteGwpasan(struct hmdcrash_detector_context *crash_detector_context) {
    if (!((crash_detector_context->crash_type == HMDCrashTypeMachException ) || (crash_detector_context->crash_type == HMDCrashTypeFatalSignal))) {
        return;
    }
    // save gwp_asan trace
    char asanStr[kGWPASanLength];
    uintptr_t allocateTrace[64];
    uintptr_t deallocateTrace[64];
    int allocateLength = 64;
    int deallocateLength = 64;
    bool detectGWPASanCrash = detect_gwp_asan(crash_detector_context->fault_address, asanStr, kGWPASanLength, (uintptr_t *)allocateTrace, &allocateLength, (uintptr_t *)deallocateTrace, &deallocateLength);
    if (!detectGWPASanCrash) {
        return;
    }
    HMDCrashWriteGWPASanInfo(asanStr, allocateTrace, allocateLength, deallocateTrace, deallocateLength);
}
...
static void write_thread_backtrace(hmd_stack_cursor *stackCursor) {
    ...
    while(stackCursor->advanceCursor(stackCursor))
    {
        ...
        if((backtrace_count += 1) >= STACK_OVERFLOW_COUNT) {
            ...
            SDKLog("most likely stack overflow happend!");
