...
- (NSMutableDictionary *)columnNameToIndexMap {
    if (!_columnNameToIndexMap) {
        ...
        for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
            const char *c = sqlite3_column_name([_statement statement], columnIdx);
            if (c) {
                [_columnNameToIndexMap setObject:[NSNumber numberWithInt:columnIdx]
                forKey:[[NSString stringWithUTF8String:c] lowercaseString]];
            } else {
                FMDBLog(@"Warning: There seem to be error columns in this map.");
            }
        }
    }
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        FMDBLog(@"Warning: There seem to be no columns in this set.");
    }
...
- (NSDictionary*)resultDictionary {
    ...
    if (num_cols > 0) {
        ...
        for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
            const char *c = sqlite3_column_name([_statement statement], columnIdx);
            if (c) {
                NSString *columnName = [NSString stringWithUTF8String:c];
                id objectValue = [self objectForColumnIndex:columnIdx];
                [dict setObject:objectValue forKey:columnName];
            } else {
                FMDBLog(@"Warning: There seem to be error columns in this set.");
            }
        }
...
- (int)internalStepWithError:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        FMDBLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        FMDBLog(@"Database busy");
        if (outErr) {
        ...
        FMDBLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        // wtf?
        FMDBLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    FMDBLog(@"Warning: I could not find the column named '%@'.", columnName);
