...
+ (SInt32)FMDBVersion {
    ...
    dispatch_once(&once, ^{
        ...
        for (NSUInteger i = 0; i < 3; i++) {
            ...
            if (component > 15) {
                FMDBLog(@"FMDBVersion is invalid: Please use FMDBUserVersion instead.");
                component = 15;
            }
...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
    ...
    return YES;
#else
    FMDBLog(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    FMDBLog(@"Closing leaked statement");
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            FMDBLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
- (int)busyRetryTimeout {
    FMDBLog(@"%s:%d", __FUNCTION__, __LINE__);
    FMDBLog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    FMDBLog(@"%s:%d", __FUNCTION__, __LINE__);
    FMDBLog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    ...
    if (!query) {
        FMDBLog(@"API misuse, -[FMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        FMDBLog(@"error on rekey: %d", rc);
        FMDBLog(@"%@", [self lastErrorMessage]);
    }
...
- (BOOL)goodConnection {
    ...
    if ([rs next]) {
        FMDBLog(@"SQLCipher version: %@", rs.resultDictionary[@"cipher_version"]);
...
- (void)warnInUse {
    FMDBLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        FMDBLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                FMDBLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                FMDBLog(@"DB Query: %@", sql);
                FMDBLog(@"DB Path: %@", _databasePath);
            }
...
- (BOOL)bindStatement:(sqlite3_stmt *)pStmt WithArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                FMDBLog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                if (rc != SQLITE_OK) {
                    FMDBLog(@"Error: unable to bind (%d, %s", rc, sqlite3_errmsg(_db));
                    sqlite3_finalize(pStmt);
                    pStmt = 0x00;
                    _isExecutingStatement = NO;
                    return false;
                ...
                FMDBLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    FMDBLog(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    FMDBLog(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
        FMDBLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        pStmt = 0x00;
        _isExecutingStatement = NO;
        return false;
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        FMDBLog(@"Error inserting batch: %s", errmsg);
    }
    if (errmsg) {
        sqlite3_free(errmsg);
    }
...
#else
    FMDBLog(@"sqlite3_wal_checkpoint_v2 unavailable before sqlite 3.7.6. Ignoring checkpoint mode: %d", mode);
