...
bool CacheBlob::insert(const std::string& filename, const uint8_t* data, int length) {
...
void CacheBlob::output() {
  if (mode_ == kAppending && append_vec_ == nullptr)
    return;
  bool appending =
      mode_ == kAppending && append_vec_ != nullptr;

  FILE* file_out = appending ?
      fopen(target_path_.c_str(), "ab")
      : fopen(target_path_.c_str(), "wb");
  if (file_out) {
    if (appending) {
      for (auto it : *append_vec_)
        write_cache_unit(file_out, it);
    } else {
      fwrite(&MAGIC, DOUBLE_SIZE, 1, file_out);
      for (auto &it : cache_map_)
        write_cache_unit(file_out, it.second);
    }
    fclose(file_out);
...
void CacheBlob::dump_status(void *p) {
  ...
  std::sort(heat_ranking_.begin(),
                heat_ranking_.end(), CachedData::compare);
  status_vec->push_back(
      std::pair<std::string, int>("Total", total_query_));
  status_vec->push_back(
      std::pair<std::string, int>("Missed", missed_query_));
  status_vec->push_back(
      std::pair<std::string, int>("Expired", expired_query_));
  status_vec->push_back(
      std::pair<std::string, int>("Updated",
                                  mode_ == kAppending
                                  && append_vec_ == nullptr ? 0 : 1));
  status_vec->push_back(
      std::pair<std::string, int>("Size", current_size_));
  status_vec->push_back(
      std::pair<std::string, int>("Heat Ranking, total ", heat_ranking_.size()));
...
bool CacheBlob::get_enough_space(int data_size) {
  ...
  if (current_size_ + data_size <= max_capacity_)
    return true;
