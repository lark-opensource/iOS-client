...
class Reference : public RefBase {
 ...
 private:
  ...
  // into the engine.
  static void FinalizeCallback(const v8::WeakCallbackInfo<SecondPassCallParameterRef>& data) {
  ...
  static void SecondPassCallback(const v8::WeakCallbackInfo<SecondPassCallParameterRef>& data) {
    SecondPassCallParameterRef* parameter = data.GetParameter();
    Reference* reference = *parameter;
    delete parameter;
    if (reference == nullptr) {
...
// TODO support v8 Object.getOwnPropertyDescriptor
napi_status napi_get_own_property_descriptor(napi_env env, napi_value obj, napi_value prop, napi_value* result) {
  return napi_clear_last_error(env);                                                    
}
...
napi_status napi_run_script(napi_env env, const char* script, size_t length,
  ...
  if (filename) {
    ...
    v8::ScriptOrigin so(origin_string);
...
napi_status napi_run_script_cache(napi_env env, const char* script, size_t length,
                            const char* filename, napi_value* result) {
  NAPI_PREAMBLE(env);
  ...
  bool need_mkcache = false;
  {
    ...
    v8::ScriptOrigin so(origin_string);
    {
      ...
      if (data != nullptr) {
        // The following v8::ScriptCompiler::Source will release this cached_data.
        ...
        LOG_TIME_START();
        maybe_script =
            v8::ScriptCompiler::Compile(context, &src,
...
#endif  // #ifndef _NAPI_V8_EXPORT_SOURCE_ONLY_
