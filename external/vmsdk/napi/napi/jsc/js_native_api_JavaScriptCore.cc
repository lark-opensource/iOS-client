...
class Function {
 ...
 private:
  ...
  static void Initialize(
      JSContextRef ctx, JSObjectRef object, NativeInfo* info) {
 ...
 public:
  static JSObjectRef Create(napi_env env) {
    ...
    std::call_once(once_flag, []() {
      ...
      wrapperClassDef.className = "Wrapper";
...
// TODO support jsc unhandled_rejection_exception
napi_status napi_get_unhandled_rejection_exception(napi_env env,
                                                   napi_value* result) {
  return napi_clear_last_error(env);
}

// TODO add jsc Object.getOwnPropertyDescriptor(obj, prop)
