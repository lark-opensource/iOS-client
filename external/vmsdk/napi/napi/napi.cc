...
void Env::OutputCodeCache() {
  NAPI_ENV_CALL(output_code_cache, _env, 0);
}
...
Value Env::RunScriptCache(const char* utf8script, size_t length,
  ...
  napi_value result;
  napi_status status =
      NAPI_ENV_CALL(run_script_cache, _env, utf8script, length, filename, &result);
  if (status != napi_ok) {
    return Value();
  }
...
Object Object::GetOwnPropertyDescriptor(napi_env env, Value obj, Value prop) {
  ...
  CheckStatus(env, status,
              "failed to call napi_get_own_property_descriptor");
  return Object(env, value);
}
...
#define TypedArrayImpl(CLAZZ, NAPI_TYPE, C_TYPE)                            \
  ...
  CLAZZ CLAZZ::New(napi_env env, size_t elementLength,                      \
    ...
    return CLAZZ(                                                           \
        ...
        reinterpret_cast<C_TYPE*>(reinterpret_cast<uint8_t*>(               \
          arrayBuffer.Data()) + bufferOffset));                             \
  }                                                                         \
                                                                            \
