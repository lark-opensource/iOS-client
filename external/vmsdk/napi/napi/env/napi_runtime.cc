...
struct napi_runtime__ {
  ...
  #ifdef ENABLE_CODECACHE
  bool StoreCodeCache(const std::string& filename, const uint8_t* data, int length) {
    if (blob_ != nullptr) {
  ...
  void GetCodeCache(const std::string& filename,
...
class Mutex {
 public:
  ...
  Mutex(Args&&... args) : val_(std::forward(args)...) {}
...
class ErrorScope {
 public:
  ErrorScope(napi_env env) : env_(env){};
...
napi_status napi_output_code_cache(napi_env env, unsigned int place_holder) {
  // Considering that with very very small possibility that this task be completed
  // if it's posted to napi's worker thread, A synchonous file operation is performed.
  LOG_TIME_START();
  env->rt->WriteCache();
