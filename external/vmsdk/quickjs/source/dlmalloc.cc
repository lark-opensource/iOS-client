...
/*
  ...
  thus be inaccurate.
*/
DLMALLOC_EXPORT struct mallinfo dlmallinfo(mstate m);
#endif /* NO_MALLINFO */
...
*/
DLMALLOC_EXPORT void dlmalloc_stats(mstate m);
...
// TODO: enable_primjs_runtime will goto this.
...
__attribute__((always_inline)) void lepus_dlmalloc_abort(mstate m,
                                                         mchunkptr p) {
#if defined(ANDROID) || defined(__ANDROID__)
  if (p != NULL) {
    __android_log_print(ANDROID_LOG_FATAL, "VMSDK_DLMALLOC",
...
#define CORRUPTION_ERROR_ACTION(m) lepus_dlmalloc_abort(m, NULL)
#endif /* CORRUPTION_ERROR_ACTION */

#ifndef USAGE_ERROR_ACTION
#define USAGE_ERROR_ACTION(m, p) lepus_dlmalloc_abort(m, p)
...
#if defined(ANDROID) || defined(__ANDROID__)
    __android_log_print(
        ANDROID_LOG_ERROR, "VMSDK_DLMALLOC",
        "====insert_small_chunk  (m)->smallmap:%u, idx2bit(idx):%u\n",
        (m)->smallmap, idx2bit(idx));
#endif
...
#if defined(ANDROID) || defined(__ANDROID__)
      __android_log_print(ANDROID_LOG_ERROR, "VMSDK_DLMALLOC",
...
#define internal_free(m, mem) \
  if (m == gm)                \
    ...
    mspace_free(m, mem);
#else /* MSPACES */
#define internal_malloc(m, b) dlmalloc(m, b)
#define internal_free(m, mem) dlfree(m, mem)
#endif /* MSPACES */
#endif /* ONLY_MSPACES */
...
#define PR_SET_VMA_ANON_NAME 0
    prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, mm, mmsize, "quickjs_heap");
#endif
    if (mm != CMFAIL) {
...
static __attribute__((unused)) void dump_segment_internal(mstate m) {
  msegmentptr sp = &m->seg;
  while (sp != NULL) {
    PRINT("start:%x, end:%x \n", (uintptr_t)sp->base,
          (uintptr_t)(sp->base + sp->size));
    // traverse_segment(sp);
    sp = sp->next;
  }
  PRINT(
      "top:%x, topsize:%zu,dv:%x, dvsize:%zu, footprint:%zu, max_footprint:%zu "
      "\n",
      (uintptr_t)m->top, m->topsize, (uintptr_t)m->dv, m->dvsize, m->footprint,
      m->max_footprint);
  PRINT("---------\n");
}
void dump_segment(mstate m) { dump_segment_internal(m); }
...
static inline void set_prctlinfo(int granularity, char* ret) {
#if defined(ANDROID) || defined(__ANDROID__)
#define PR_SET_VMA 0x53564d41
#define PR_SET_VMA_ANON_NAME 0
  prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, ret, granularity, "quickjs_heap");
#endif
}
...
#if defined(__x86_64__) || defined(__aarch64__)
#define MMAP_GRANULARITY 96 * 1024
  if (req_size >= 32 * 1024) {
    char* ret = (char*)CALL_MMAP(req_size);
    if (ret == MAP_FAILED) return CMFAIL;
    int granulirity = req_size;
    set_prctlinfo(granulirity, ret);
    return ret;
  }
  ...
  if (cur_size == 0 || cur_size < req_size) {
    ...
    char* ret = (char*)CALL_MMAP(MMAP_GRANULARITY);
    int granulirity = MMAP_GRANULARITY;
    if (ret == MAP_FAILED) {
      release_unused_segments(m);
      ret = (char*)CALL_MMAP(MMAP_GRANULARITY);
      if (ret == MAP_FAILED) return CMFAIL;
    }
    set_prctlinfo(granulirity, ret);
...
static void* sys_alloc(mstate m, size_t nb) {
  ...
  if (nb >= 32 * 1024 /*mparams.mmap_threshold*/ && m->topsize != 0) {
    void* mem = mmap_alloc(m, nb);
    if (mem != 0) {
      PRINT("dlmalloc mmap size:%zu \n", nb);
      return mem;
    } else {
      PRINT("dlmalloc mmap failed \n");
    }
  }
...
static __attribute__((unused)) void traverse_segment(msegmentptr seg) {
  PRINT("--------traverse_chunk  start---------- \n");
  mchunkptr first_chunk = align_as_chunk(seg->base);
  size_t psize = chunksize(first_chunk);
  PRINT(" chunksize: %zu, first_chunk:%zu, base:%zu, size:%zu, cinuse:%zu \n",
        psize, (uintptr_t)first_chunk, (uintptr_t)seg->base, seg->size,
        cinuse(first_chunk));
  mchunkptr next = chunk_plus_offset(first_chunk, psize);
  int cnt = 0;
  while ((uintptr_t)next < (uintptr_t)(seg->base + seg->size - TOP_FOOT_SIZE)) {
    psize = chunksize(next);
    PRINT(" chunksize: %zu, next:%zu, cinuse:%zu \n", psize, (uintptr_t)next,
          cinuse(next));
    next = chunk_plus_offset(next, psize);
    if (cnt++ > 1000) {
      PRINT("too more.....\n");
      break;
    }
...
static __attribute__((unused)) void release_physical_mem(mstate m) {
  ...
  for (size_t i = 0; i < back_idx; i++) {
    ...
    void* release_start =
        (uint8_t*)page_align((uintptr_t)ck) + malloc_getpagesize;
    size_t release_size =
...
void destroy_dlmalloc_instance(mstate m) {
  ...
  while (sp != NULL) {
    next = sp->next;
    if (sp->size && CALL_MUNMAP(sp->base, sp->size) != 0) {
#if defined(ANDROID) || defined(__ANDROID__)
      __android_log_print(ANDROID_LOG_ERROR, "VMSDK_DLMALLOC",
...
static int sys_trim(mstate m, size_t pad) {
  ...
  if (pad < MAX_REQUEST && is_initialized(m)) {
    ...
    if (m->topsize > pad) {
      ...
      // printf("unit:%zu, extra:%f (M) \n", unit, extra/1024.0/1024);
      msegmentptr sp = segment_holding(m, (char*)m->top);

      if (extra != 0 && sp->size >= extra &&
...
//#include <android/log.h>
#if !ONLY_MSPACES
void* dlmalloc(mstate gm, size_t bytes) {
  // dump_segment(gm);
  /*
     Basic algorithm:
...
postaction:
  POSTACTION(gm);
  PRINT("dlmalloc, addr:%p, size:%zu, mstate:%p, chunksize:%zu \n", mem, bytes,
        gm, ppsize);
  return mem;
...
void dlfree(mstate fm, void* mem) {
  /*
     Consolidate freed chunks with preceeding or succeeding bordering
     free chunks, if they exist, and then place in a bin.  Intermixed
     with special cases for top, dv, mmapped chunks, and usage errors.
  */
  PRINT("dlfree, addr:%p, mstate:%p, tid:%d\n", mem, fm, gettid());
...
#endif
void* dlrealloc(mstate gm, void* oldmem, size_t bytes) {
  void* mem = 0;
  if (oldmem == 0) {
    mem = dlmalloc(gm, bytes);
  } else if (bytes >= MAX_REQUEST) {
    MALLOC_FAILURE_ACTION;
  }
...
#if !NO_MALLINFO
struct mallinfo dlmallinfo(mstate m) {
  return internal_mallinfo(m);
}
#endif /* NO_MALLINFO */

#if !NO_MALLOC_STATS
void dlmalloc_stats(mstate m) { internal_malloc_stats(m); }
#endif /* NO_MALLOC_STATS */
...
size_t dlmalloc_usable_size_debug(void* mem) {
  return ((hdr_t*)((char*)mem - sizeof(hdr_t)))->size;
}

size_t dlmalloc_usable_size_debug_protect(/*mstate m, */ void* mem) {
  return *(uint64_t*)((uint8_t*)mem - 16);
}
#endif
#endif /* !ONLY_MSPACES */
