...
NSString * const BDUGActivityTypePostToWechat             = @"com.BDUG.UIKit.activity.PostToWechat";
static NSString *const BDUGWechatSystemActivityType = @"com.tencent.xin.sharetimeline";

@interface BDUGWechatActivity () <BDUGWechatShareDelegate>

@property (nonatomic,copy) BDUGActivityCompletionHandler completion;

@end

@implementation BDUGWechatActivity
...
{
    return BDUGActivityTypePostToWechat;
}
...
- (NSString *)activityImageName
{
    if ([self.contentItem respondsToSelector:@selector(activityImageName)] && [self.contentItem activityImageName]) {
        ...
        return @"BDUGShareWechatResource.bundle/weixin_allshare";
    }
}
...
{
    self.contentItem = (BDUGWechatContentItem *)contentItem;
...
- (void)beginShareActionWithItemModel:(BDUGShareDataItemModel *)itemModel {
    ...
    BDUGWechatContentItem *contentItem = [self contentItem];
    ...
    } activityTypeString:BDUGWechatSystemActivityType completion:^(id<BDUGActivityProtocol> activity, NSError *error, NSString *desc) {
    ...
    //wechatShareType优先级 > defaultShareType
    switch (contentItem.wechatShareType) {
        case BDUGWechatShareTypeFile: {
            if (!contentItem.fileURL || !contentItem.fileName) {
                //invalid content
                NSError *error = [BDUGShareError errorWithDomain:BDUGWechatShareErrorDomain code:BDUGShareErrorTypeInvalidContent userInfo:nil];
                [self weChatShare:nil sharedWithError:error customCallbackUserInfo:nil];
            } else if ([contentItem.fileURL isFileURL]) {
                //直接分享本地文件
                [wechatShare sendFileWithFileName:contentItem.fileName fileURL:contentItem.fileURL thumbImage:contentItem.thumbImage];
            } else {
                //先下载再分享。
                [BDUGShareFileManager getFileFromURLStrings:@[contentItem.fileURL.absoluteString] fileName:contentItem.fileName downloadProgress:nil completion:^(NSError *error, NSString *filePath) {
                    [wechatShare sendFileWithFileName:contentItem.fileName fileURL:[NSURL fileURLWithPath:filePath] thumbImage:contentItem.thumbImage];
                }];
            }
        }
            return;
            break;
        case BDUGWechatShareTypeMiniProgram: {
            //分享小程序到微信。
            [wechatShare sendMiniProgramToScene:BDUGWechatShareSceneSession thumbnailImage:contentItem.thumbImage title:contentItem.title description:contentItem.desc miniProgramUserName:contentItem.miniProgramUserName miniProgramPath:contentItem.miniProgramPath webPageURLString:contentItem.webPageUrl launchMiniProgram:contentItem.launchMiniProgram];
        }
            return;
            break;
        default:
    ...
    switch (contentItem.defaultShareType) {
        case BDUGShareText: {
            [wechatShare sendTextToScene:BDUGWechatShareSceneSession withText:contentItem.title customCallbackUserInfo:contentItem.callbackUserInfo];
        }
            break;
        case BDUGShareImage: {
            ...
            info.completeBlock = ^(BDUGVideoShareStatusCode statusCode, NSString *desc, BDUGVideoImageShareContentModel *resultModel) {
                ...
                if (statusCode == BDUGVideoShareStatusCodeSuccess && desc.length > 0) {
                    [wechatShare sendImageToScene:BDUGWechatShareSceneSession withImage:resultModel.resultImage customCallbackUserInfo:contentItem.callbackUserInfo];
        ...
        case BDUGShareWebPage: {
            [wechatShare sendWebpageToScene:BDUGWechatShareSceneSession withWebpageURL:contentItem.webPageUrl thumbnailImage:contentItem.thumbImage imageURL:contentItem.imageUrl title:contentItem.title description:contentItem.desc customCallbackUserInfo:contentItem.callbackUserInfo];
        }
            break;
        case BDUGShareVideo: {
            [wechatShare sendVideoToScene:BDUGWechatShareSceneSession withVideoURL:contentItem.videoURL thumbnailImage:contentItem.thumbImage title:contentItem.title description:contentItem.desc customCallbackUserInfo:contentItem.callbackUserInfo];
        }
            break;
        default:{
