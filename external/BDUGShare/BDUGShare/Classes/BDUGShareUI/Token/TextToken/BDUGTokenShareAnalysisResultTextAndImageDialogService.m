//
//  BDUGTokenShareAnalysisResultTextAndImageDialogService.m
//  Article
//
...
@interface BDUGTokenShareAnalysisTextAndImageContentView : BDUGTokenShareAnalysisContentViewBase
...
@implementation BDUGTokenShareAnalysisTextAndImageContentView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        NSString *imageName = @"picture_group_icon";
        _messageImageView.image = [UIImage imageNamed:imageName inBundle:BDUGTokenShareBundle.resourceBundle compatibleWithTraitCollection:nil];
...
- (void)refreshContent:(BDUGTokenShareAnalysisResultModel *)resultModel {
    ...
    if (resultModel.mediaType == BDUGTokenShareDialogTypePhotos && resultModel.picCount > 0) {
        self.messageBackgroundView.hidden = NO;
        self.messageView.text = [NSString stringWithFormat:@"%ld图",resultModel.picCount];
...
- (void)refreshFrame {
    ...
    if (!self.messageBackgroundView.hidden) {
        ...
        self.messageImageView.btd_left = 6;
        self.messageImageView.btd_centerY = self.messageBackgroundView.btd_height / 2;
        self.messageView.frame = CGRectIntegral(self.messageView.frame);
        self.messageView.btd_left = self.messageImageView.btd_right + 2;
        self.messageView.btd_centerY = self.messageImageView.btd_centerY;
...
#pragma mark - BDUGTokenShareAnalysisResultTextAndImageDialogService

@implementation BDUGTokenShareAnalysisResultTextAndImageDialogService

NSString * const kBDUGTokenShareAnalysisResultTextAndImageDialogKey = @"kBDUGTokenShareAnalysisResultTextAndImageDialogKey";
                  ...
                  actionModel:(BDUGTokenShareServiceActionModel *)actionModel
{
    ...
    BDUGTokenShareAnalysisTextAndImageContentView *contentView = [[BDUGTokenShareAnalysisTextAndImageContentView alloc] initWithFrame:CGRectMake(0, 0, 300, 264)];
