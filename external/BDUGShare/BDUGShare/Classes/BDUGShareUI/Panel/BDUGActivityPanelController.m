...
#import "BDUGActivityPanelController.h"
#import <ByteDanceKit/ByteDanceKit.h>
#import "BDUGActivityPanelDefine.h"
#import <BDUGShare/BDUGActivityProtocol.h>
#import <BDUGShare/BDUGShareAdapterSetting.h>
//全屏相关。
//#import "TTSharePanelTransformMessage.h"
#import <Aspects/Aspects.h>
#import <BDUGShare/UIColor+UGExtension.h>
...
#pragma mark - BDUGActivityPanelThemedButton

@interface BDUGActivityPanelThemedButton : UIButton
...
@implementation BDUGActivityPanelThemedButton
...
@interface BDUGActivityPanelControllerWindow : UIWindow

@property (strong, nonatomic) BDUGActivityPanelController *panel;

@end

@implementation BDUGActivityPanelControllerWindow

@end

#pragma mark - BDUGActivityPanelRootViewController

@interface BDUGActivityPanelRootViewController : UIViewController
...
@implementation BDUGActivityPanelRootViewController
...
@interface BDUGActivityPanelController () {
    CGFloat _itemWidth;
}
...
@property (strong, nonatomic) BDUGActivityPanelControllerWindow *backWindow;
@property (strong, nonatomic) BDUGActivityPanelRootViewController *rootViewController;
...
@implementation BDUGActivityPanelController

#pragma mark -- Life cycle

GAIA_FUNCTION(BDUGShareInitializeGaiaKey)() {
    [[BDUGShareAdapterSetting sharedService] setPanelClassName:NSStringFromClass([BDUGActivityPanelController class])];
}
...
- (void)createComponents {
    ...
    self.rootViewController = [[BDUGActivityPanelRootViewController alloc] init];
    ...
    //Back window
    self.backWindow = [[BDUGActivityPanelControllerWindow alloc] init];
...
//    SAFECALL_MESSAGE(TTSharePanelTransformMessage, @selector(message_sharePanelIfNeedTransformWithBlock:), message_sharePanelIfNeedTransformWithBlock:self.fullScreenTransformHandlerBlock);
    ...
    [self.items enumerateObjectsUsingBlock:^(NSArray * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [self.items[idx] enumerateObjectsUsingBlock:^(id  _Nonnull jobj, NSUInteger jidx, BOOL * _Nonnull jstop) {
            ...
            BDUGActivityPanelThemedButton *itemView = [self itemViewWithIndex:indexPath item:self.items[idx][jidx]];
...
- (BDUGActivityPanelThemedButton *)itemViewWithIndex:(NSIndexPath *)indexPath item:(id<BDUGActivityProtocol>)item {
    BDUGActivityPanelThemedButton *view = nil;
    CGRect frame;
    ...
    if (self.items.count == 1 && amount < 4) {
        ...
        view = [[BDUGActivityPanelThemedButton alloc] initWithFrame:frame item:item itemIndexPath:indexPath];
    }else {
...
- (void)selectedDigIconButtonAction:(BDUGActivityPanelThemedButton *)sender {
...
- (void)buttonClickAction:(BDUGActivityPanelThemedButton *)sender {
    id<BDUGActivityProtocol> activity = self.items[sender.itemIndexPath.section][sender.itemIndexPath.row];
...
- (void)show {
    ...
    [UIView animateWithDuration:0.48 animations:^{
        self.cancelButton.btd_bottom = self.maskView.btd_height - 8 - bottomSafeAreaInset;
        self.panelView.btd_bottom = self.cancelButton.btd_top - 6;
