...
- (BOOL)addInfo:(id<STHookInfo>)info {
    ...
    BOOL flag = NO;
    NSUInteger position = info.options&StingerPositionFilter;
    switch (position) {
        case STOptionBefore:
...
- (BOOL)removeInfo:(id<STHookInfo>)info {
    ...
    BOOL success = NO;
    dispatch_semaphore_wait(_semaphore, DISPATCH_TIME_FOREVER);
    NSUInteger index = [_beforeInfos indexOfObjectIdenticalTo:info];
    if (index != NSNotFound) {
        [_beforeInfos removeObjectAtIndex:index];
        success = YES;
    } else if (_insteadInfo == info) {
        _insteadInfo = nil;
        success = YES;
    } else {
        index = [_afterInfos indexOfObjectIdenticalTo:info];
        if (index != NSNotFound) {
            [_afterInfos removeObjectAtIndex:index];
            success = YES;
        }
    }
