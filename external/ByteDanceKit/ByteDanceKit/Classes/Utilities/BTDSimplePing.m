...
- (void)didFailWithError:(NSError *)error {
    id<BTDSimplePingDelegate>  strongDelegate;
...
- (NSData *)packetWithPingData:(NSData *)data {
    ...
    payload = data;
    if (payload == nil) {
        payload = [[NSString stringWithFormat:@"%484zd bottles of beer on the wall", (ssize_t) 99 - (size_t) (self.nextSequenceNumber % 100) ] dataUsingEncoding:NSASCIIStringEncoding];
        assert(payload != nil);
...
static void STSocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge BTDSimplePing *) info;
    assert([obj isKindOfClass:[BTDSimplePing class]]);
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                ...
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        ...
                        free(s);
                        if (self.addressStyle != BTDSimplePingAddressStyleICMPv6) {
                    ...
                    case AF_INET6: {
                        ...
                        self.IPAddress = [NSString stringWithFormat:@"%s", s];
                        free(s);
                        if (self.addressStyle != BTDSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
