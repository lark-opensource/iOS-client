...
{
    UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0.0);
    [self btd_drawInRect:rect radius:radius];
    UIImage * scaledImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return scaledImage;
}
...
+ (CGFloat *)_btd_getColorComponentsInArray:(NSArray<UIColor *> *)colorArray {
    ...
    CGFloat *compoents = malloc(sizeof(CGFloat) * 4 * count);
    
    for (int i = 0; i < count; i++) {
        const CGFloat * colorComponents = CGColorGetComponents(colorArray[i].CGColor);
        size_t num_s = CGColorGetNumberOfComponents(colorArray[i].CGColor);
        
        int index = 4 * i;
        if (num_s == 2) {
            compoents[index] = compoents[index + 1] = compoents[index + 2] = colorComponents[0];
            compoents[index + 3] = colorComponents[1];
        } else if(num_s == 4) {
            compoents[index] = colorComponents[0];
            compoents[index + 1] = colorComponents[1];
            compoents[index + 2] = colorComponents[2];
            compoents[index + 3] = colorComponents[3];
        }
    }
...
+ (CGFloat *)_btd_getLocationComponentsInArray:(NSArray<NSNumber *> *)locationArray andCount:(NSInteger)count {
     ...
     */
    CGFloat *compoents = malloc(sizeof(CGFloat) * count);
    if (locationArray.count == 0) {
        compoents[0] = 0;
        compoents[count - 1] = 1;
        float interval = 1.0f / (count - 1);
        for (int i = 1; i < count - 1; i++) {
            compoents[i] = interval * i;
        }
    } else {
        for (int i = 0; i < count; i++) {
            compoents[i] = i < locationArray.count ? locationArray[i].floatValue : locationArray.lastObject.floatValue;
            /// 避免创建CGGradientRef时crash
            compoents[i] = compoents[i] > 1 ? 1 : compoents[i];
            compoents[i] = compoents[i] < 0 ? 0 : compoents[i];
        }
    }
...
+ (UIImage *)btd_cutImage:(UIImage *)img withRect:(CGRect)rect
{
    ...
    CGImageRef newImagRef = CGImageCreateWithImageInRect(sourceImageRef, CGRectMake(rect.origin.x*img.scale, rect.origin.y*img.scale, rect.size.width*img.scale, rect.size.height*img.scale));
    if (newImagRef == NULL) {
        return img;
    }
