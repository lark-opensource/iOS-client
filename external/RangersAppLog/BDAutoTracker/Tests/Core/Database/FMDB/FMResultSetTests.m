...
@interface BDAutoTrackResultSetTests : FMDBTempDBTests

@end

@implementation BDAutoTrackResultSetTests
...
- (void)testNextWithError_WithoutError
{
    ...
    BDAutoTrackResultSet *resultSet = [self.db executeQuery:@"SELECT * FROM testTable WHERE key=1"];
...
- (void)testNextWithError_WithMisuseError
{
    ...
    BDAutoTrackResultSet *resultSet = [self.db executeQuery:@"SELECT * FROM testTable WHERE key=9"];
    XCTAssertNotNil(resultSet);
