...
@interface BDAutoTrackDatabaseTests : FMDBTempDBTests

@end

@implementation BDAutoTrackDatabaseTests
...
- (void)testOpenWithVFS
{
    ...
    BDAutoTrackDatabase *db = [[BDAutoTrackDatabase alloc] initWithPath:@":memory:"];
...
{
    BDAutoTrackResultSet *ps = [self.db executeQuery:@"pragma journal_mode=delete"];
...
- (void)testSelectULL
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select a from ull"];
    while ([rs next]) {
...
{
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select rowid,* from test where a = ?", @"hi"];
...
{
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select rowid, a, b, c from test"];
...
- (void)testBusyRetryTimeout
{
    ...
    BDAutoTrackDatabase *newDB = [BDAutoTrackDatabase databaseWithPath:self.databasePath];
    [newDB open];
    
    BDAutoTrackResultSet *rs = [newDB executeQuery:@"select rowid,* from test where a = ?", @"hi'"];
...
- (void)testCaseSensitiveResultDictionary
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from cs"];
    while ([rs next]) {
...
- (void)testBoolInsert
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from btest"];
    while ([rs next]) {
...
- (void)testNamedParametersCount
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from namedparamcounttest"];
...
- (void)testBlobs
{
    ...
    if (safariCompass) {
        ...
        BDAutoTrackResultSet *rs = [self.db executeQuery:@"select b from blobTable where a = ?", @"safari's compass"];
        XCTAssertTrue([rs next]);
...
- (void)testNullValues
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from t2"];
    while ([rs next]) {
...
{
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from t3"];
    while ([rs next]) {
        ...
        BDAutoTrackResultSet *rs2 = [self.db executeQuery:@"select a from t3 where a = ?", [NSNumber numberWithInt:newVal]];
        [rs2 next];
...
- (void)testNSNullInsertion
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from nulltest"];
...
- (void)testNullDates
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from datetest"];
...
- (void)testLotsOfNULLs
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from nulltest2"];
...
- (void)testUTF8Strings
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from utest where a = ?", @"/übertest"];
    XCTAssertTrue([rs next]);
    [rs close];
    XCTAssertFalse([self.db hasOpenResultSets], @"Shouldn't have any open result sets");
    XCTAssertFalse([self.db hadError], @"Shouldn't have any errors");
...
- (void)testArgumentsInArray
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from testOneHundredTwelvePointTwo where b > ?" withArgumentsInArray:[NSArray arrayWithObject:[NSNumber numberWithInteger:1]]];
...
- (void)testColumnNamesContainingPeriods
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select t4.a as 't4.a', t4.b from t4;"];
...
- (void)testFormatStringParsing
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQueryWithFormat:@"select * from t5 where a = %s and a = %@ and b = %d", "text", @"text", 42];
    XCTAssertNotNil(rs);
...
- (void)testFormatStringParsingWithSizePrefixes
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQueryWithFormat:@"select * from t55 where a = %s order by 2", "a"];
    XCTAssertNotNil(rs);
...
- (void)testFormatStringParsingWithNilValue
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQueryWithFormat:@"select * from tatwhat"];
    XCTAssertNotNil(rs);
    XCTAssertTrue([rs next]);
    XCTAssertTrue([rs columnIndexIsNull:0]);
...
{
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from test where a=?" withArgumentsInArray:@[]];
    XCTAssertNil(rs);
}
...
- (void)testDatabaseAttach
{
    ...
    BDAutoTrackDatabase *dbB = [BDAutoTrackDatabase databaseWithPath:@"/tmp/attachme.db"];
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from attack.attached"];
    XCTAssertNotNil(rs);
    XCTAssertTrue([rs next]);
    [rs close];
}
...
- (void)testNamedParameters
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from namedparamtest"];
...
{
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"pragma database_list"];
    int counter = 0;
    while ([rs next]) {
        counter++;
...
- (void)testStatementCachingWorks
{
    ...
    for (int i = 1; i <= 2; i++ ) {
        ...
        BDAutoTrackResultSet* rs1 = [self.db executeQuery: @"SELECT rowid, * FROM testStatementCaching WHERE value = ?", @1]; // results in 2 rows...
        XCTAssertNotNil(rs1);
        ...
        BDAutoTrackResultSet* rs2 = [self.db executeQuery:@"SELECT rowid, * FROM testStatementCaching WHERE value = ?", @2]; // results in 1 row
        XCTAssertNotNil(rs2);
...
- (void)testDateFormat
{
    void (^testOneDateFormat)(BDAutoTrackDatabase *, NSDate *) = ^( BDAutoTrackDatabase *db, NSDate *testDate ){
        ...
        BDAutoTrackResultSet *rs = [db executeQuery:@"SELECT test FROM test_format"];
        XCTAssertNotNil(rs);
        XCTAssertTrue([rs next]);
    ...
    NSDateFormatter *fmt = [BDAutoTrackDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
- (void)testColumnNameMap
{
    ...
    BDAutoTrackResultSet *ars = [self.db executeQuery:@"select * from colNameTest"];
    XCTAssertNotNil(ars);
...
- (void)testCustomFunction
{
    ...
    int rowCount = 0;
    BDAutoTrackResultSet *ars = [self.db executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
    while ([ars next]) {
        rowCount++;
...
- (void)testCharAndBoolTypes
{
    ...
    BDAutoTrackResultSet *rs = [self.db executeQuery:@"select * from charBoolTest"];
    XCTAssertNotNil(rs);
...
- (void)testOpenTwice
{
    BDAutoTrackDatabase *db = [[BDAutoTrackDatabase alloc] init];
    [db open];
...
- (void)testInvalid
{
    ...
    BDAutoTrackDatabase *db = [[BDAutoTrackDatabase alloc] initWithPath:path];
...
- (void)testCloseOpenResultSets
{
    ...
    BDAutoTrackResultSet *rs = [db executeQuery:@"select bar from foo"];
