...
- (void)presetAggregation
{
    ...
    BDroneMonitorAggregation *eventUsageAggregation = [BDroneMonitorAggregation countAggregation];
    [eventUsageAggregation addAggregateField:@[@"dims_0",@"err_code",@"err_underlying_code"]];
    [self presetAggregation:eventUsageAggregation forMetrics:@"event_process" category:@"event"];
...
{
    //
    static NSString *metrics = @"event_process";
    static NSString *category = @"event";
    
    NSMutableDictionary *dimensions = [NSMutableDictionary dictionary];
        
    if (error) {
        [dimensions setValue:@(error.code) forKey:@"err_code"];
        NSError *underlyingError = error.userInfo[NSUnderlyingErrorKey];
        if (underlyingError) {
            [dimensions setValue:@(underlyingError.code) forKey:@"err_underlying_code"];
        }
    }
    
    [dimensions setValue:@(state) forKey:@"dims_0"];
    [self trackMetrics:metrics value:count category:category dimensions:dimensions processId:processId];
