                ...
                id delegateBlock = ^void (id delegateSelf, CLLocationManager *manager, NSArray<CLLocation *> *locations) {
                    ...
                    dispatch_async(bd_serialQueue, ^{
                        @try {
                            ...
                            [BDAutoTrackApplication updateAutoTrackGPSLocation:BDAutoTrackGeoCoordinateSystemWGS84 longitude:coordinate.longitude latitude:coordinate.latitude];
                        } @catch (NSException *exception) {
                            RL_WARN(nil, @"locationManager didUpdateLocations error: %@", exception.reason);
                        }
//                        NSLog(@">>>> inject gps - lng: %f, lat: %f", coordinate.longitude, coordinate.latitude);
                    });
