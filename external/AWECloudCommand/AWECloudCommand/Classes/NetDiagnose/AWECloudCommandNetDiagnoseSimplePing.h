...
typedef NS_ENUM(NSInteger, AWECloudCommandSimplePingAddressStyle) {
    AWECloudCommandSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    AWECloudCommandSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    AWECloudCommandSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@property (nonatomic, weak, readwrite, nullable) id<AWECloudCommandSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) AWECloudCommandSimplePingAddressStyle addressStyle;
 ...
 *      stop the object before you get the callback).  These callbacks are currently deCloudCommandred
 ...
 *      deCloudCommandred to all ICMP sockets.
 *
...
struct AWECloudCommandICMPHeader {
    uint8_t     type;
    uint8_t     code;
    uint16_t    checksum;
    uint16_t    identifier;
...
typedef struct AWECloudCommandICMPHeader AWECloudCommandICMPHeader;

__Check_Compile_Time(sizeof(AWECloudCommandICMPHeader) == 8);
__Check_Compile_Time(offsetof(AWECloudCommandICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(AWECloudCommandICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(AWECloudCommandICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(AWECloudCommandICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(AWECloudCommandICMPHeader, sequenceNumber) == 6);

enum {
    AWECloudCommandICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    AWECloudCommandICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    AWECloudCommandICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    AWECloudCommandICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
