...
struct AWECloudCommandIPv4Header {
    uint8_t     versionAndHeaderLength;
    uint8_t     differentiatedServices;
    uint16_t    totalLength;
    uint16_t    identification;
    uint16_t    flagsAndFragmentOffset;
    uint8_t     timeToCloudCommand;
    uint8_t     protocol;
...
typedef struct AWECloudCommandIPv4Header AWECloudCommandIPv4Header;

__Check_Compile_Time(sizeof(AWECloudCommandIPv4Header) == 20);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, timeToCloudCommand) == 8);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(AWECloudCommandIPv4Header, destinationAddress) == 16);
...
@interface AWECloudCommandNetDiagnoseSimplePing ()
...
@implementation AWECloudCommandNetDiagnoseSimplePing
...
- (void)didFailWithError:(NSError *)error {
    id<AWECloudCommandSimplePingDelegate>  strongDelegate;
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    AWECloudCommandICMPHeader *            icmpPtr;
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if (!self.hostAddress) {
        strongDelegate = self.delegate;
        [self didFailWithError:[NSError errorWithDomain:@"AWECloudCommandSimplePing" code:-1 userInfo:@{}]];
        return;
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:AWECloudCommandICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:AWECloudCommandICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    NSUInteger                  result;
    const struct AWECloudCommandIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(AWECloudCommandIPv4Header) + sizeof(AWECloudCommandICMPHeader))) {
        ipPtr = (const AWECloudCommandIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
            ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(AWECloudCommandICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct AWECloudCommandICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);
        ...
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == AWECloudCommandICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    BOOL                    result;
    const AWECloudCommandICMPHeader *      icmpPtr;
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == AWECloudCommandICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    AWECloudCommandNetDiagnoseSimplePing *    obj;
    
    obj = (__bridge AWECloudCommandNetDiagnoseSimplePing *) info;
    assert([obj isKindOfClass:[AWECloudCommandNetDiagnoseSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != AWECloudCommandSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != AWECloudCommandSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
