//
//  AWECloudBackgroundTaskUtility.m
//  Heimdallr
//
...
#define AWESimpleBackgroundTask_ExpireTime_NO_EXPIRE -1.0

static pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER;

@implementation AWECloudBackgroundTaskUtility {
    NSString *_name;
    volatile UIBackgroundTaskIdentifier _identifer;
    volatile BOOL _isEnded;
}

+ (NSMutableArray<AWECloudBackgroundTaskUtility *> *)currentBackgroundTask {
    static NSMutableArray<AWECloudBackgroundTaskUtility *> *array;
...
+ (void)detachBackgroundTaskWithName:(NSString *)name
                          expireTime:(NSTimeInterval)expireTime
                                task:(AWESimpleBackgroundTaskBlock)customWork {
    ...
    if(name != nil && customWork != nil) {
        AWECloudBackgroundTaskUtility *task = [[AWECloudBackgroundTaskUtility alloc] initWithName:name];
        pthread_mutex_lock(&mtx);
        NSMutableArray<AWECloudBackgroundTaskUtility *> *allBGTasks = [AWECloudBackgroundTaskUtility currentBackgroundTask];
        ...
        if(identifier != UIBackgroundTaskInvalid) {
            ...
            if(expireTime != AWESimpleBackgroundTask_ExpireTime_NO_EXPIRE && expireTime > 0.0) {
...
+ (void)detachBackgroundTaskWithName:(NSString *)name
                                task:(AWESimpleBackgroundTaskBlock)customWork {
    [AWECloudBackgroundTaskUtility detachBackgroundTaskWithName:name expireTime:AWESimpleBackgroundTask_ExpireTime_NO_EXPIRE task:customWork];
}

+ (void)endBackgroundTaskWithName:(NSString *)name {
    __block AWECloudBackgroundTaskUtility *thisTask;
    pthread_mutex_lock(&mtx);
    NSMutableArray<AWECloudBackgroundTaskUtility *> *allBGTasks = [AWECloudBackgroundTaskUtility currentBackgroundTask];
    [allBGTasks enumerateObjectsUsingBlock:^(AWECloudBackgroundTaskUtility * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)completeBackgroundTask {
    ...
    if(__atomic_compare_exchange_n(&_isEnded, &expected, YES, NO, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
        [[UIApplication sharedApplication] endBackgroundTask:_identifer];
        pthread_mutex_lock(&mtx);
        NSMutableArray<AWECloudBackgroundTaskUtility *> *allBGTasks = [AWECloudBackgroundTaskUtility currentBackgroundTask];
...
- (BOOL)isEqual:(id)object {
    if(object != nil &&
       [object isKindOfClass:AWECloudBackgroundTaskUtility.class] &&
       [_name isEqualToString:((AWECloudBackgroundTaskUtility *)object)->_name])
        return YES;
    return NO;
}
