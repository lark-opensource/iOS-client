...
@implementation BDLynxPostDataHttpRequestSerializer
...
- (TTHttpRequest *)setRequestBody:(TTHttpRequest *)request
  ...
  if (![NSJSONSerialization isValidJSONObject:parameters]) {
    LLogError(@"parameters is not a valid json");
    return nil;
  }
  NSData *postData = [NSJSONSerialization dataWithJSONObject:parameters
                                                     options:NSJSONWritingPrettyPrinted
                                                       error:&error];
  if (error != nil) {
    LLogError(@"postData JSON serialization failed");
    return nil;
  }
