...
- (instancetype)initWithFamilyName:(NSString *)familyName andSrc:(NSString *)src {
  if (self = [super init]) {
    ...
    _srcArray = [LynxFontFace parseSrc:src];
...
+ (NSArray *)parseSrc:(NSString *)src {
  ...
  for (NSString *cur in srcArr) {
    if (curStr == nil) {
      ...
      curStr = [[curStr stringByAppendingFormat:@","] stringByAppendingFormat:@"%@", cur];
    }
...
- (NSString *)registerFontWithData:(NSData *)data
  ...
  if (!regResult) {
    ...
    if (nsError.code == kCTFontManagerErrorAlreadyRegistered) {
      ...
      [[lynxView getLynxContext]
          reportModuleCustomError:[NSString stringWithFormat:@"font %@ AlreadyRegistered, src:%@",
                                                             newFontName, cacheKey]];
    } else {
      ...
      [[lynxView getLynxContext]
          reportModuleCustomError:[NSString stringWithFormat:@"Failed to load font: %@, src:%@",
                                                             errorDescription, cacheKey]];
...
- (void)handleFontData:(NSData *)data
              ...
              lynxView:(LynxView *)rootView
          withCacheKey:(NSString *)cacheKey {
  if ([data length] > 0) {
    item.dataFontName = [self registerFontWithData:data withCacheKey:cacheKey lynxView:rootView];
  } else {
    [self onRequestErrorClearCache:cacheKey];
    NSString *errorDetail = [NSString stringWithFormat:@"Load Font-Face Failed: %@", item.src];
    NSDictionary *errorDic = @{@"src" : item.src, @"type" : @"font", @"error_msg" : errorDetail};
    ...
    [[rootView getLifecycleDispatcher] lynxView:rootView didRecieveError:err];
  }
}
...
- (void)requestFontfaceItem:(LynxFontSrcItem *)item
  ...
  if (fontResProvider != nil && item.src != nil) {
    if (![self onRequestStartForItem:item cache:cacheKey fontFaceObserver:fontFaceObserver]) {
      return;
    }
    LynxResourceRequest *request = [[LynxResourceRequest alloc] initWithUrl:item.src];
    [fontResProvider request:request
                  onComplete:^(LynxResourceResponse *_Nonnull response) {
                    [weakSelf handleFontData:response.data
                                    withItem:item
                                    lynxView:weakFontFaceContext.rootView
                                withCacheKey:cacheKey];
  ...
  if (fetcher != nil &&
      ...
      item.src != nil) {
    NSURL *url = [[NSURL alloc] initWithString:item.src];
    if ([self onRequestStartForItem:item cache:cacheKey fontFaceObserver:fontFaceObserver]) {
    ...
    for (NSUInteger i = 0; i < count; ++i) {
      ...
      if (item.type == LynxFontSrcLocal) {
        ...
        font = [self findFontWithSize:fontSize
                               weight:fontWeight
                                style:fontStyle
                       fontFamilyName:item.src
                             fontInfo:info];
      } else if (item.type == LynxFontSrcUrl) {
        if (item.dataFontName == nil) {
          NSString *cacheKey = [self cachedKey:item.src];
          @synchronized(_registedFontMap) {
...
- (UIFont *)generateFontWithSize:(CGFloat)fontSize
  ...
  if (forceApplyItalic || forceApplyBold) {
    ...
    NSMutableDictionary<UIFontDescriptorAttributeName, id> *attributes =
        [fontDescriptor.fontAttributes mutableCopy];
    if (forceApplyItalic) {
