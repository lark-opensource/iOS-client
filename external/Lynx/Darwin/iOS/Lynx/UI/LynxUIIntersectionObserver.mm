...
- (instancetype)initWithOptions:(NSDictionary*)options
  ...
  self = [self initWithManager:manager observerId:-1 componentId:-1 options:options];
  if (self) {
    _manager = manager;
    _enableNewIntersectionObserver = manager.enableNewIntersectionObserver;
...
- (void)checkForIntersectionWithTarget:(LynxUIObservationTarget*)target
  ...
  if (needNotifyJS) {
    if (_isCustomEventObserver) {
      ...
      if (_manager.context) {
        _manager.context->proxy_->CallJSIntersectionObserver(_observerId, target.jsCallbackId,
                                                             [entry toDictionary]);
      }
    }
  }
...
- (void)onLynxEvent:(LynxEventType)type event:(LynxEvent*)event {
  ...
  // throttle to 30 fps
  //    if (currentTimestamp - lastTimeStamp_ < 0.032) {
  //        return;
  //    }
