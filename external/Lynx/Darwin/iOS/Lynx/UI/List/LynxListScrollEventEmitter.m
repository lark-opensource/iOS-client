...
NSString *const kLynxEventScroll = @"scroll";
NSString *const kLynxEventScrollToUpper = @"scrolltoupper";
NSString *const kLynxEventScrollToLower = @"scrolltolower";
...
- (void)sendScrollEvent:(UIScrollView *)scrollView force:(BOOL)force {
  ...
  NSString *eventType = self.enableScrollEvent ? kLynxEventScroll : nil;
    ...
    if (deltaX > 0) {
      ...
      if (_enableScrollToLowerEvent) {
        ...
        if (isLower || isReadyToLower) {
          eventType = kLynxEventScrollToLower;
        }
      }
    }
    ...
    if (deltaX < 0) {
      ...
      if (_enableScrollToUpperEvent) {
        ...
        if (isUpper || isReadyToUpper) {
          eventType = kLynxEventScrollToUpper;
        }
      }
    }
  } else {
  ...
  if ([eventType isEqualToString:kLynxEventScroll]) {
...
- (NSString *)fetchScrollEvent:(UIScrollView *)scrollView {
  ...
  switch (currentPosition) {
    case LynxListScrollPositionTop:
      event = self.scrollPosition == LynxListScrollPositionTop ? kLynxEventScroll
                                                               : kLynxEventScrollToUpper;
      break;
    case LynxListScrollPositionBottom:
      event = self.scrollPosition == LynxListScrollPositionBottom ? kLynxEventScroll
                                                                  : kLynxEventScrollToLower;
      break;
    case LynxListScrollPositionMid:
      event = kLynxEventScroll;
      break;
    default:
      break;
  }

  if (self.lastEvent == event && event == kLynxEventScroll &&
