...
- (LynxUI *)uiAtIndexPath:(NSIndexPath *)indexPath {
  ...
  LynxUI *ui = [rootView.templateRender.uiOwner findUIBySign:uiSign];
  return ui;
}
...
- (void)loadListInfo:(NSDictionary *)diffResult
  ...
  for (NSUInteger i = 0; i < _currentItemKeys.count; i++) {
    ...
    CGFloat estimatedHeight = [components[@"estimated_height"][i] doubleValue];
    CGFloat estimatedHeightPX = [components[@"estimated_height_px"][i] doubleValue];
...
- (void)updateListActionInfo:(NSDictionary *)noDiffResult {
  ...
  if (updateAction != nil) {
    for (id component in updateAction) {
      ...
      [_currentItemKeys insertObject:itemKey atIndex:fromPos];
      [_reuseIdentifiers insertObject:reuseType atIndex:fromPos];
