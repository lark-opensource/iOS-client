...
@property(nonatomic) NSMutableDictionary<NSString *, LynxWeakProxy *> *exposedLynxUIMap;
...
- (void)setEnableCheckExposureOptimize:(BOOL)enableCheckExposureOptimize {
  // The switch is jointly controlled by pageConfig and Lynx AB Trail.
  if ([LynxEnv getBoolExperimentSettings:@"enable_check_exposure_optimize"]) {
...
- (CGRect)frameOfUIInWindow:(LynxUI *)ui {
  CGRect frameOfUIInWindow = [ui.view convertRect:ui.view.bounds toView:nil];
...
- (CGRect)borderOfExposureScreen:(LynxUI *)ui {
  if ([ui enableExposureUIMargin]) {
    ...
    CGRect borderOfExposureScreen = [UIScreen mainScreen].bounds;
...
- (BOOL)uiInWindow:(LynxUI *)ui {
  ...
  */
  ...
  [parentAry enumerateObjectsUsingBlock:^(LynxUI *parent, NSUInteger idx, BOOL *stop) {
    CGRect frameOfParent = [parent.view convertRect:parent.view.bounds toView:nil];
  ...
  CGRect frameOfRootUIInWindow = [_rootUI.lynxView convertRect:_rootUI.lynxView.bounds toView:nil];
  CGRect borderOfExposureScreen = [self borderOfExposureScreen:ui];
  return uiInWindow &&
         [self checkIntersect:frameOfUIInWindow
...
- (BOOL)isLynxViewChanged {
  CGRect lynxViewNewFrame = [_rootUI.lynxView convertRect:_rootUI.lynxView.bounds toView:nil];
...
- (void)exposureHandler:(CADisplayLink *)sender {
  ...
  for (LynxWeakProxy *ui in [_exposedLynxUIMap allValues]) {
    if (ui.target != nil) {
      LynxUI *lynxUI = ui.target;
      if ([self uiInWindow:lynxUI]) {
        LynxUIExposureDetail *detail = [[LynxUIExposureDetail alloc] initWithUI:lynxUI];
...
- (void)sendEvent:(NSMutableSet<LynxUIExposureDetail *> *)uiSet eventName:(NSString *)eventName {
  if ([uiSet count] != 0) {
    ...
    if ([_rootUI.lynxView.templateRender enableJSRuntime] ||
      ...
      if (params.count > 0) {
        [_rootUI.lynxView sendGlobalEvent:eventName withParams:@[ params ]];
      }
    } else {
      // Just use for Air 1.0, forward compatible is required
      for (LynxUIExposureDetail *detail in uiSet) {
        LynxUI *ui = [_rootUI.lynxView.templateRender.uiOwner findUIBySign:detail.sign];
        if (ui != nil) {
...
- (NSDictionary<NSString *, NSString *> *)createParam:(LynxUIExposureDetail *)detail {
  NSDictionary *dict = @{
    @"exposure-id" : detail.exposureID,
    @"exposureID" : detail.exposureID,
    @"exposure-scene" : detail.exposureScene,
    @"exposureScene" : detail.exposureScene,
    @"internal-signature" : detail.internalSignature,
    @"sign" : [NSNumber numberWithInteger:detail.sign],
...
- (BOOL)addLynxUI:(LynxUI *)ui {
  if (ui.exposureID || ui.internalSignature) {
    if ([_exposedLynxUIMap count] == 0) {
      [self addExposureToRunLoop];
    }
    // combine the exposure-id, exposure-scene and internal-signature as Key, for example:
    // "Scene1_1_LynxUI_XXX"
    NSString *key = [NSString stringWithFormat:@"%@_%@_%ld_%@", ui.exposureScene, ui.exposureID,
                                               (long)ui.sign, ui.internalSignature];
    [_exposedLynxUIMap setObject:[LynxWeakProxy proxyWithTarget:ui] forKey:key];
    return YES;
  }
  return NO;
}
...
- (void)removeLynxUI:(LynxUI *)ui {
  if (ui.exposureID || ui.internalSignature) {
    NSString *key = [NSString stringWithFormat:@"%@_%@_%ld_%@", ui.exposureScene, ui.exposureID,
                                               (long)ui.sign, ui.internalSignature];
    LynxWeakProxy *weakUI = [_exposedLynxUIMap objectForKey:key];
    if (weakUI) {
