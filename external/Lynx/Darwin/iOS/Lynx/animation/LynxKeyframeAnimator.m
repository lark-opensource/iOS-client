...
- (void)recordPauseTime {
  if (_pauseTime == kTimeNotInit) {
    _pauseTime = [_ui.view.layer convertTime:[[SyncedTimeHelper shareInstance] getSyncedTime]
                                   fromLayer:nil];
  }
}
- (CFTimeInterval)getPauseDuration {
  ...
  CFTimeInterval pauseDuration =
      [_ui.view.layer convertTime:[[SyncedTimeHelper shareInstance] getSyncedTime] fromLayer:nil] -
      _pauseTime;
  _pauseTime = kTimeNotInit;
...
- (BOOL)isAnimationExpired:(LynxAnimationInfo*)info {
  ...
  return ([_ui.view.layer convertTime:[[SyncedTimeHelper shareInstance] getSyncedTime]
                            fromLayer:nil] -
              _keyframeStartTime >=
          allDuration);
}
...
- (void)applyAnimationInfo:(LynxAnimationInfo*)info {
  ...
  for (NSString* key in sortedKeys) {
    ...
    if (_keyframeStartTime == kTimeNotInit) {
      _keyframeStartTime =
          [self.ui.view.layer convertTime:[[SyncedTimeHelper shareInstance] getSyncedTime]
                                fromLayer:nil];
    }
  ...
  if (![self isAnimationExpired:info]) {
    ...
    // According to https://bytedance.feishu.cn/docs/doccnAcziEwE4bMoQkntIKw2Rgb#, only send start
    // event when state is IDLE.
