...
- (NSUInteger)hash {
  return (*(NSUInteger *)&_width) ^ (*(NSUInteger *)&_height) ^ _widthMode ^ _heightMode ^
         _textOverflow ^ _whiteSpace ^ _maxLineNum ^ (_breakByChar ? 1 : 0) ^
         (_enableTailColorConvert ? 1 : 0) ^ (_enableTextRefactor ? 1 : 0) ^
         (_enableNewClipMode ? 1 : 0);
}
