...
typedef struct {
  NSTimeInterval duration;  // frame duration
  UInt32 frameCount;        // number of frames dropped
} LynxFPSFrame;
...
static void LynxFPSRawMetricsAddFrame(LynxFPSRawMetrics *metrics, LynxFPSFrame frame) {
  ...
  if (frame.frameCount > 1) {
    ...
    metrics->drop1Duration += frame.duration;

    if (frame.frameCount > 3) {
      metrics->drop3Count += 1;
      metrics->drop3Duration += frame.duration;

      if (frame.frameCount > 7) {
        metrics->drop7Count += 1;
        metrics->drop7Duration += frame.duration;
      }
    }
  }
}
...
- (void)displayLinkDidUpdate:(CADisplayLink *)displayLink {
  ...
  if (likely(_lastTimestamp != 0.0)) {
    LynxFPSFrame frame = (LynxFPSFrame){0, 0};
    frame.duration = currentTimestamp - _lastTimestamp;
    frame.frameCount = frame.duration / desiredFrameInterval;
