...
#ifndef CANVAS_2D_LITE_NANOVG_INCLUDE_NANOVG_H_
#define CANVAS_2D_LITE_NANOVG_INCLUDE_NANOVG_H_
...
enum NVGpaintType : unsigned short {
  NVG_PAINT_DEFAULT = 0,
  NVG_PAINT_LINEAR_GRADIENT,
  NVG_PAINT_BOX_GRADIENT,
  NVG_PAINT_RADIAL_GRADIENT,
  NVG_PAINT_IMAGE_PATTERNS,
};
...
enum NVGwinding {
  NVG_NONE_WINDING = 0,    // Not force change winding
...
enum NVGTextStyle {
  NVG_NONE = 1, // alias for FONS_GLYPH_BITMAP_OPTIONAL
  NVG_FILL = 2, // alias for FONS_GLYPH_BITMAP_REQUIRED
  NVG_STROKE = 4, // alias for FONS_GLYPH_BITMAP_STROKE
};
...
enum NVGimageFlags {
  // NVG_IMAGE_GENERATE_MIPMAPS	= 1<<0,     // Generate mipmaps during creation of the image. // unsupporteded
  NVG_IMAGE_REPEATX = 1 << 1,        // Repeat image in X direction.
  NVG_IMAGE_REPEATY = 1 << 2,        // Repeat image in Y direction.
  NVG_IMAGE_FLIPY = 1 << 3,        // Flips (inverses) image in Y direction when rendered.
  NVG_IMAGE_PREMULTIPLIED = 1 << 4,        // Image data has premultiplied alpha.
  NVG_IMAGE_NEAREST = 1 << 5,        // Image interpolation is Nearest instead Linear
  NVG_IMAGE_TRANSFORM = 1 << 6,        // for cases when image pattern used, as it may be affected by canvas transform
  NVG_IMAGE_SMOOTHINGIN = 1 << 7,		// whether enable bicubic sampler
};
...
// get the stroke width of the stroke style
...
// Sets dash array or clear if (count == 0 or floats == nullptr)
...
// other points for Gradient
struct GradientItems {
  uint32_t count;
  struct {
	float pos;
	uint32_t color;
...
// All colors and positions are in items.
...
// Creates and returns a radial gradient. Parameters (cx,cy), (cx2,cy2) specify the center, inr and outr specify
...
NVGpaint nvgImagePattern(NVGcontext *ctx, float ox, float oy, float ex, float ey,
						 float angle, int image, int flags);
...
// Fills the current path with current fill style and whether even odd mode.
void nvgFillEx(NVGcontext *ctx, bool evenOdd);

// Fills the current path with current stroke style.
void nvgStroke(NVGcontext *ctx);

// Clip using the current path with whether even odd mode.
void nvgClip(NVGcontext *ctx, bool evenOdd);

// Clip is set currently
...
// Auto load system font of specified name
...
struct NVGpath {
  ...
  NVGvertex *stroke;
  int nstroke;
  short winding, realWinding;
  int convex;
  bool antiAlias; // If there is inclination or bending, anti aliasing is recommended
};
...
struct NVGparams {
  ...
  int (*renderPutTexture)(void *uptr, int textureId, int w, int h, int imageFlags);
...
// Sets the font weight, style and variant
void nvgFontWSV(NVGcontext *ctx, unsigned short packagedWSV);
// image pattern transform
void nvgSetImagePatternTransform(NVGcontext *ctx, NVGpaint *paint, const float *matrix);

enum NVGPointCheckOpt {
  NVG_POINTCK_EVENODD = 0x01,
  NVG_POINTCK_STROKE = 0x02,
};
bool nvgIsPointInConvexPolygon(NVGcontext *ctx, float x, float y, short opts);

struct NVGtextMetricsInfo {
  float fontAscent, fontDescent, textBounds[4];
  int missingGlyphCount;
};
typedef struct NVGtextMetricsInfo NVGtextMetricsInfo;
float nvgTextMetricsInfo(NVGcontext *ctx,
						 float x,
						 float y,
						 const char *string,
						 const char *end,
						 NVGtextMetricsInfo *info);
...
#endif  // CANVAS_2D_LITE_NANOVG_INCLUDE_NANOVG_H_
