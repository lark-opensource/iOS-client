...
// aurium
#define RTC_LOG(a)
#include "resampler_webrtc_aurum.h"
...
//        << ", outFreq = " << outFreq;
      // check more frequence
      const int toCheckDiv = 441;
      if ((outFreq % toCheckDiv) == 0) {
          outFreq = (toCheckDiv - 1) * (outFreq / toCheckDiv);
      }
      if (ComputeResamplerMode(inFreq, outFreq, &mode) != 0) {
          return -1;
      }
  ...
  case kResamplerMode6To11:
      ...
      state3_ = malloc(sizeof(WebRtcSpl_State8khzTo22khz));
      WebRtcSpl_ResetResample8khzTo22khz(
                                         static_cast<WebRtcSpl_State8khzTo22khz*>(state3_));
      break;
  case kResamplerMode12To11:
      state1_ = malloc(sizeof(WebRtcSpl_State48khzTo16khz));
      WebRtcSpl_ResetResample48khzTo16khz(
           static_cast<WebRtcSpl_State48khzTo16khz*>(state1_));
...
int Resampler::ComputeResamplerMode(int in_freq_hz,
  ...
  // aurium
  } else if ((reduced_in_freq == 80) && (reduced_out_freq == 147)) {
      *mode = kResamplerMode6To11; // 24000 -> 44100
  } else if ((reduced_in_freq == 160) && (reduced_out_freq == 147)) {
      *mode = kResamplerMode12To11; // 48000 -> 44100
  // aurium_end
  } else {
    return -1;
  }
...
//      outLen = lengthIn;
  ...
  case kResamplerMode12To11:
      ...
      if (maxLen < (lengthIn * 11 / 12)) {
          return -1;
      }
      
      // tmp_mem = static_cast<int32_t*>(malloc((496 + 98) * sizeof(int32_t)));
      tmp = static_cast<int16_t*>(malloc((sizeof(int16_t) * lengthIn) / 3));
      // 12:4
      for (size_t i = 0; i < lengthIn; i += 480) {
          WebRtcSpl_Resample48khzTo16khz(
      ...
      for (size_t i = 0; i < lengthIn; i += 80) {
          WebRtcSpl_Resample8khzTo22khz(
                tmp + i, samplesOut + (i * 11) / 4,
                static_cast<WebRtcSpl_State8khzTo22khz*>(state2_), tmp_mem + 496);
      }
      outLen = (lengthIn * 11) / 4;
      // free(tmp_mem);
      free(tmp);
  ...
  case kResamplerMode6To11:
      ...
      lengthIn *= 2;
      // tmp_mem = static_cast<int32_t*>(malloc((496 + 98) * sizeof(int32_t)));
      tmp_2 = static_cast<int16_t*>(malloc((sizeof(int16_t) * lengthIn) / 3));
      ...
      for (size_t i = 0; i < lengthIn; i += 80) {
          WebRtcSpl_Resample8khzTo22khz(
                tmp_2 + i, samplesOut + (i * 11) / 4,
                static_cast<WebRtcSpl_State8khzTo22khz*>(state3_), tmp_mem + 496);
      }
      outLen = (lengthIn * 11) / 4;
      // free(tmp_mem);
      free(tmp);
