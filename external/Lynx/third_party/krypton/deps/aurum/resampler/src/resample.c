...
#include "resample_by_2_internal_aurum.h"
                                    ...
                                    WebRtcSpl_State16khzTo22khz* state, int32_t* tmpmem)
{
    ...
    for (k = 0; k < SUB_BLOCKS_16_22; k++)
    {
        ...
        in += 160 / SUB_BLOCKS_16_22 << 1;
        out += 220 / SUB_BLOCKS_16_22 << 1;
    }
}
                                   ...
                                   WebRtcSpl_State8khzTo22khz* state, int32_t* tmpmem)
{
    ...
    for (k = 0; k < SUB_BLOCKS_8_22; k++)
    {
        ...
        in += 160 / SUB_BLOCKS_8_22;
        out += 440 / SUB_BLOCKS_8_22;
    }
}
                                      ...
                                      int32_t K)
{
    ...
    for (m = 0; m < K; m++)
    {
        ...
        // sum and accumulate filter coefficients and input samples
        WebRtcSpl_DotProdIntToShort(&In[0], &In[22], kCoefficients32To22[0], &Out[1 << 1], &Out[10 << 1]);

        // sum and accumulate filter coefficients and input samples
        WebRtcSpl_DotProdIntToShort(&In[2], &In[20], kCoefficients32To22[1], &Out[2 << 1], &Out[9 << 1]);

        // sum and accumulate filter coefficients and input samples
        WebRtcSpl_DotProdIntToShort(&In[3], &In[19], kCoefficients32To22[2], &Out[3 << 1], &Out[8 << 1]);

        // sum and accumulate filter coefficients and input samples
        WebRtcSpl_DotProdIntToShort(&In[5], &In[17], kCoefficients32To22[3], &Out[4 << 1], &Out[7 << 1]);

        // sum and accumulate filter coefficients and input samples
        WebRtcSpl_DotProdIntToShort(&In[6], &In[16], kCoefficients32To22[4], &Out[5 << 1], &Out[6 << 1]);
