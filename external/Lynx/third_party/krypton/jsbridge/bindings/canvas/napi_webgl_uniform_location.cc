...
namespace {
const uint64_t kWebGLUniformLocationClassID = reinterpret_cast<uint64_t>(&kWebGLUniformLocationClassID);
const uint64_t kWebGLUniformLocationConstructorID = reinterpret_cast<uint64_t>(&kWebGLUniformLocationConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLUniformLocation>;
typedef Value (NapiWebGLUniformLocation::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLUniformLocation::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLUniformLocation::NapiWebGLUniformLocation(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
WebGLUniformLocation* NapiWebGLUniformLocation::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLUniformLocation::Wrap(std::unique_ptr<WebGLUniformLocation> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLUniformLocation>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLUniformLocation::Init(std::unique_ptr<WebGLUniformLocation> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLUniformLocation::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLUniformLocationClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLUniformLocation", props));
  env.SetInstanceData<Napi::Class>(kWebGLUniformLocationClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLUniformLocation::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLUniformLocationConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLUniformLocationConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLUniformLocation::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLUniformLocation")) {
    return;
  }
  target.Set("WebGLUniformLocation", Constructor(env));
}
