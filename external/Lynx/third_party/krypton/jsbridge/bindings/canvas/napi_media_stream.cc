...
namespace {
const uint64_t kMediaStreamClassID = reinterpret_cast<uint64_t>(&kMediaStreamClassID);
const uint64_t kMediaStreamConstructorID = reinterpret_cast<uint64_t>(&kMediaStreamConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiMediaStream>;
typedef Value (NapiMediaStream::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiMediaStream::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiMediaStream::NapiMediaStream(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
MediaStream* NapiMediaStream::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiMediaStream::Wrap(std::unique_ptr<MediaStream> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiMediaStream>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiMediaStream::Init(std::unique_ptr<MediaStream> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiMediaStream::SetBeautifyParamMethod(const CallbackInfo& info) {
  DCHECK(impl_);

  if (info.Length() < 4) {
    ExceptionMessage::NotEnoughArguments(info.Env(), InterfaceName(), "SetBeautifyParam", "4");
    return Value();
  }

  auto arg0_whiten = NativeValueTraits<IDLFloat>::NativeValue(info, 0);
  if (info.Env().IsExceptionPending()) {
    return info.Env().Undefined();
  }

  auto arg1_smoothen = NativeValueTraits<IDLFloat>::NativeValue(info, 1);
  if (info.Env().IsExceptionPending()) {
    return info.Env().Undefined();
  }

  auto arg2_enlargeEye = NativeValueTraits<IDLFloat>::NativeValue(info, 2);
  if (info.Env().IsExceptionPending()) {
    return info.Env().Undefined();
  }

  auto arg3_slimFace = NativeValueTraits<IDLFloat>::NativeValue(info, 3);
  if (info.Env().IsExceptionPending()) {
    return info.Env().Undefined();
  }

  impl_->SetBeautifyParam(arg0_whiten, arg1_smoothen, arg2_enlargeEye, arg3_slimFace);
  return info.Env().Undefined();
}

// static
Napi::Class* NapiMediaStream::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kMediaStreamClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Methods
  AddInstanceMethod(props, "setBeautifyParam", &NapiMediaStream::SetBeautifyParamMethod);

  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "MediaStream", props));
  env.SetInstanceData<Napi::Class>(kMediaStreamClassID, clazz);
  return clazz;
}

// static
Function NapiMediaStream::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kMediaStreamConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kMediaStreamConstructorID, ref);
  return ref->Value();
}

// static
void NapiMediaStream::Install(Napi::Env env, Object& target) {
  if (target.Has("MediaStream")) {
    return;
  }
  target.Set("MediaStream", Constructor(env));
}
