...
// clang-format off
#ifndef JSBRIDGE_BINDINGS_CANVAS_NAPI_EVENT_H_
#define JSBRIDGE_BINDINGS_CANVAS_NAPI_EVENT_H_
...
class NapiEvent : public BridgeBase {
 public:
  NapiEvent(const Napi::CallbackInfo&, bool skip_init_as_base = false);

  Event* ToImplUnsafe();

  static Napi::Object Wrap(std::unique_ptr<Event>, Napi::Env);

  void Init(std::unique_ptr<Event>);

  // Attributes
  Napi::Value TypeAttributeGetter(const Napi::CallbackInfo&);
  Napi::Value TargetAttributeGetter(const Napi::CallbackInfo&);
 ...
 private:
  std::unique_ptr<Event> impl_;
};

}  // namespace canvas
}  // namespace lynx

#endif  // JSBRIDGE_BINDINGS_CANVAS_NAPI_EVENT_H_
