...
namespace {
const uint64_t kBoundRectClassID = reinterpret_cast<uint64_t>(&kBoundRectClassID);
const uint64_t kBoundRectConstructorID = reinterpret_cast<uint64_t>(&kBoundRectConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiBoundRect>;
typedef Value (NapiBoundRect::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiBoundRect::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiBoundRect::NapiBoundRect(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
void NapiBoundRect::Init(const CallbackInfo& info) {
  auto impl = BoundRect::Create();
  Init(std::move(impl));
}

BoundRect* NapiBoundRect::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiBoundRect::Wrap(std::unique_ptr<BoundRect> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiBoundRect>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiBoundRect::Init(std::unique_ptr<BoundRect> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiBoundRect::XAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetX());
}

Value NapiBoundRect::YAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetY());
}

Value NapiBoundRect::WidthAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetWidth());
}

Value NapiBoundRect::HeightAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetHeight());
}

Value NapiBoundRect::TopAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetTop());
}

Value NapiBoundRect::RightAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetRight());
}

Value NapiBoundRect::BottomAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetBottom());
}

Value NapiBoundRect::LeftAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetLeft());
}

// static
Napi::Class* NapiBoundRect::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kBoundRectClassID);
  if (clazz) {
    return clazz;
  }
  ...
  AddAttribute(props, "x",
               &NapiBoundRect::XAttributeGetter,
               nullptr
               );
  AddAttribute(props, "y",
               &NapiBoundRect::YAttributeGetter,
               nullptr
               );
  AddAttribute(props, "width",
               &NapiBoundRect::WidthAttributeGetter,
               nullptr
               );
  AddAttribute(props, "height",
               &NapiBoundRect::HeightAttributeGetter,
               nullptr
               );
  AddAttribute(props, "top",
               &NapiBoundRect::TopAttributeGetter,
               nullptr
               );
  AddAttribute(props, "right",
               &NapiBoundRect::RightAttributeGetter,
               nullptr
               );
  AddAttribute(props, "bottom",
               &NapiBoundRect::BottomAttributeGetter,
               nullptr
               );
  AddAttribute(props, "left",
               &NapiBoundRect::LeftAttributeGetter,
               nullptr
               );
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "BoundRect", props));
  env.SetInstanceData<Napi::Class>(kBoundRectClassID, clazz);
  return clazz;
}

// static
Function NapiBoundRect::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kBoundRectConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kBoundRectConstructorID, ref);
  return ref->Value();
}

// static
void NapiBoundRect::Install(Napi::Env env, Object& target) {
  if (target.Has("BoundRect")) {
    return;
  }
  target.Set("BoundRect", Constructor(env));
}
