...
namespace {
const uint64_t kWebGLRenderbufferClassID = reinterpret_cast<uint64_t>(&kWebGLRenderbufferClassID);
const uint64_t kWebGLRenderbufferConstructorID = reinterpret_cast<uint64_t>(&kWebGLRenderbufferConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLRenderbuffer>;
typedef Value (NapiWebGLRenderbuffer::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLRenderbuffer::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLRenderbuffer::NapiWebGLRenderbuffer(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
NapiWebGLRenderbuffer::~NapiWebGLRenderbuffer() {
  impl_->Dispose();
}

WebGLRenderbuffer* NapiWebGLRenderbuffer::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLRenderbuffer::Wrap(std::unique_ptr<WebGLRenderbuffer> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLRenderbuffer>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLRenderbuffer::Init(std::unique_ptr<WebGLRenderbuffer> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLRenderbuffer::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLRenderbufferClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLRenderbuffer", props));
  env.SetInstanceData<Napi::Class>(kWebGLRenderbufferClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLRenderbuffer::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLRenderbufferConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLRenderbufferConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLRenderbuffer::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLRenderbuffer")) {
    return;
  }
  target.Set("WebGLRenderbuffer", Constructor(env));
}
