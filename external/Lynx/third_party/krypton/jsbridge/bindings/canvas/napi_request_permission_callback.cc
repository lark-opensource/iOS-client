...
namespace canvas {
namespace {
  const uint64_t kNapiRequestPermissionCallbackClassID = reinterpret_cast<uint64_t>(&kNapiRequestPermissionCallbackClassID);
}

void NapiRequestPermissionCallback::Invoke(bool arg0) {
  bool valid;
  ...
  HolderStorage *storage = reinterpret_cast<HolderStorage*>(env.GetInstanceData(kNapiRequestPermissionCallbackClassID));
  DCHECK(storage);

  auto cb = storage->PopHolder(reinterpret_cast<uintptr_t>(this));

  Napi::Value arg0_accepted;
  arg0_accepted = Napi::Boolean::New(env, arg0);

  // The JS callback object is stolen after the call.
  piper::CallbackHelper::Invoke(std::move(cb), result_, exception_handler_, { arg0_accepted });
}

NapiRequestPermissionCallback::NapiRequestPermissionCallback(Napi::Function callback) {
  Napi::Env env = callback.Env();
  HolderStorage *storage = reinterpret_cast<HolderStorage*>(env.GetInstanceData(kNapiRequestPermissionCallbackClassID));
  if (storage == nullptr) {
    storage = new HolderStorage();
    env.SetInstanceData(kNapiRequestPermissionCallbackClassID, storage, [](napi_env env, void* finalize_data,
...
Napi::Env NapiRequestPermissionCallback::Env(bool *valid) {
  if (valid != nullptr) {
    *valid = false;
  }
