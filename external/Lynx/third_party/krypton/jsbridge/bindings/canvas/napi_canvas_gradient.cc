...
namespace {
const uint64_t kCanvasGradientClassID = reinterpret_cast<uint64_t>(&kCanvasGradientClassID);
const uint64_t kCanvasGradientConstructorID = reinterpret_cast<uint64_t>(&kCanvasGradientConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiCanvasGradient>;
typedef Value (NapiCanvasGradient::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiCanvasGradient::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiCanvasGradient::NapiCanvasGradient(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
CanvasGradient* NapiCanvasGradient::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiCanvasGradient::Wrap(std::unique_ptr<CanvasGradient> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiCanvasGradient>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiCanvasGradient::Init(std::unique_ptr<CanvasGradient> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiCanvasGradient::AddColorStopMethod(const CallbackInfo& info) {
  DCHECK(impl_);
  piper::ExceptionState exception_state(info.Env());

  if (info.Length() < 2) {
    ExceptionMessage::NotEnoughArguments(info.Env(), InterfaceName(), "AddColorStop", "2");
    return Value();
  }

  auto arg0_offset = NativeValueTraits<IDLDouble>::NativeValue(info, 0);
  if (info.Env().IsExceptionPending()) {
    return info.Env().Undefined();
  }

  auto arg1_color = NativeValueTraits<IDLString>::NativeValue(info, 1);

  impl_->AddColorStop(exception_state, arg0_offset, std::move(arg1_color));
  return info.Env().Undefined();
}

// static
Napi::Class* NapiCanvasGradient::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kCanvasGradientClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Methods
  AddInstanceMethod(props, "addColorStop", &NapiCanvasGradient::AddColorStopMethod);

  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "CanvasGradient", props));
  env.SetInstanceData<Napi::Class>(kCanvasGradientClassID, clazz);
  return clazz;
}

// static
Function NapiCanvasGradient::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kCanvasGradientConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kCanvasGradientConstructorID, ref);
  return ref->Value();
}

// static
void NapiCanvasGradient::Install(Napi::Env env, Object& target) {
  if (target.Has("CanvasGradient")) {
    return;
  }
  target.Set("CanvasGradient", Constructor(env));
}
