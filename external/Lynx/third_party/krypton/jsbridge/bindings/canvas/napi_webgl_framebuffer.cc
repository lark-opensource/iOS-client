...
namespace {
const uint64_t kWebGLFramebufferClassID = reinterpret_cast<uint64_t>(&kWebGLFramebufferClassID);
const uint64_t kWebGLFramebufferConstructorID = reinterpret_cast<uint64_t>(&kWebGLFramebufferConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLFramebuffer>;
typedef Value (NapiWebGLFramebuffer::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLFramebuffer::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLFramebuffer::NapiWebGLFramebuffer(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
NapiWebGLFramebuffer::~NapiWebGLFramebuffer() {
  impl_->Dispose();
}

WebGLFramebuffer* NapiWebGLFramebuffer::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLFramebuffer::Wrap(std::unique_ptr<WebGLFramebuffer> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLFramebuffer>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLFramebuffer::Init(std::unique_ptr<WebGLFramebuffer> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLFramebuffer::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLFramebufferClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLFramebuffer", props));
  env.SetInstanceData<Napi::Class>(kWebGLFramebufferClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLFramebuffer::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLFramebufferConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLFramebufferConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLFramebuffer::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLFramebuffer")) {
    return;
  }
  target.Set("WebGLFramebuffer", Constructor(env));
}
