...
namespace {
const uint64_t kCanvasPatternClassID = reinterpret_cast<uint64_t>(&kCanvasPatternClassID);
const uint64_t kCanvasPatternConstructorID = reinterpret_cast<uint64_t>(&kCanvasPatternConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiCanvasPattern>;
typedef Value (NapiCanvasPattern::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiCanvasPattern::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiCanvasPattern::NapiCanvasPattern(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
CanvasPattern* NapiCanvasPattern::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiCanvasPattern::Wrap(std::unique_ptr<CanvasPattern> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiCanvasPattern>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiCanvasPattern::Init(std::unique_ptr<CanvasPattern> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiCanvasPattern::SetTransformMethod(const CallbackInfo& info) {
  DCHECK(impl_);
  piper::ExceptionState exception_state(info.Env());

  if (info.Length() <= 0) {
    impl_->SetTransform(exception_state);
  ...
  impl_->SetTransform(exception_state, std::move(arg0_transform));
  return info.Env().Undefined();
}

// static
Napi::Class* NapiCanvasPattern::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kCanvasPatternClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Methods
  AddInstanceMethod(props, "setTransform", &NapiCanvasPattern::SetTransformMethod);

  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "CanvasPattern", props));
  env.SetInstanceData<Napi::Class>(kCanvasPatternClassID, clazz);
  return clazz;
}

// static
Function NapiCanvasPattern::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kCanvasPatternConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kCanvasPatternConstructorID, ref);
  return ref->Value();
}

// static
void NapiCanvasPattern::Install(Napi::Env env, Object& target) {
  if (target.Has("CanvasPattern")) {
    return;
  }
  target.Set("CanvasPattern", Constructor(env));
}
