...
namespace {
const uint64_t kCanvasContextClassID = reinterpret_cast<uint64_t>(&kCanvasContextClassID);
const uint64_t kCanvasContextConstructorID = reinterpret_cast<uint64_t>(&kCanvasContextConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiCanvasContext>;
typedef Value (NapiCanvasContext::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiCanvasContext::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiCanvasContext::NapiCanvasContext(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
CanvasContext* NapiCanvasContext::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiCanvasContext::Wrap(std::unique_ptr<CanvasContext> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiCanvasContext>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiCanvasContext::Init(std::unique_ptr<CanvasContext> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiCanvasContext::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kCanvasContextClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "CanvasContext", props));
  env.SetInstanceData<Napi::Class>(kCanvasContextClassID, clazz);
  return clazz;
}

// static
Function NapiCanvasContext::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kCanvasContextConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kCanvasContextConstructorID, ref);
  return ref->Value();
}

// static
void NapiCanvasContext::Install(Napi::Env env, Object& target) {
  if (target.Has("CanvasContext")) {
    return;
  }
  target.Set("CanvasContext", Constructor(env));
}
