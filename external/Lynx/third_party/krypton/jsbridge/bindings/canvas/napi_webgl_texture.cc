...
namespace {
const uint64_t kWebGLTextureClassID = reinterpret_cast<uint64_t>(&kWebGLTextureClassID);
const uint64_t kWebGLTextureConstructorID = reinterpret_cast<uint64_t>(&kWebGLTextureConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLTexture>;
typedef Value (NapiWebGLTexture::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLTexture::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLTexture::NapiWebGLTexture(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
NapiWebGLTexture::~NapiWebGLTexture() {
  impl_->Dispose();
}

WebGLTexture* NapiWebGLTexture::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLTexture::Wrap(std::unique_ptr<WebGLTexture> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLTexture>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLTexture::Init(std::unique_ptr<WebGLTexture> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLTexture::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLTextureClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLTexture", props));
  env.SetInstanceData<Napi::Class>(kWebGLTextureClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLTexture::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLTextureConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLTextureConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLTexture::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLTexture")) {
    return;
  }
  target.Set("WebGLTexture", Constructor(env));
}
