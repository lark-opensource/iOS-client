...
namespace {
const uint64_t kWebGLBufferClassID = reinterpret_cast<uint64_t>(&kWebGLBufferClassID);
const uint64_t kWebGLBufferConstructorID = reinterpret_cast<uint64_t>(&kWebGLBufferConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLBuffer>;
typedef Value (NapiWebGLBuffer::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLBuffer::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLBuffer::NapiWebGLBuffer(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
NapiWebGLBuffer::~NapiWebGLBuffer() {
  impl_->Dispose();
}

WebGLBuffer* NapiWebGLBuffer::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLBuffer::Wrap(std::unique_ptr<WebGLBuffer> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLBuffer>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLBuffer::Init(std::unique_ptr<WebGLBuffer> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLBuffer::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLBufferClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLBuffer", props));
  env.SetInstanceData<Napi::Class>(kWebGLBufferClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLBuffer::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLBufferConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLBufferConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLBuffer::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLBuffer")) {
    return;
  }
  target.Set("WebGLBuffer", Constructor(env));
}
