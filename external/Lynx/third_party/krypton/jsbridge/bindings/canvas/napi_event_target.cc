...
namespace {
const uint64_t kEventTargetClassID = reinterpret_cast<uint64_t>(&kEventTargetClassID);
const uint64_t kEventTargetConstructorID = reinterpret_cast<uint64_t>(&kEventTargetConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiEventTarget>;
typedef Value (NapiEventTarget::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiEventTarget::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiEventTarget::NapiEventTarget(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
void NapiEventTarget::Init(const CallbackInfo& info) {
  auto impl = EventTarget::Create();
  Init(std::move(impl));
}

EventTarget* NapiEventTarget::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiEventTarget::Wrap(std::unique_ptr<EventTarget> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiEventTarget>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiEventTarget::Init(std::unique_ptr<EventTarget> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiEventTarget::ResetEventListenerStatusMethod(const CallbackInfo& info) {
  DCHECK(impl_);

  if (info.Length() < 2) {
    ExceptionMessage::NotEnoughArguments(info.Env(), InterfaceName(), "ResetEventListenerStatus", "2");
    return Value();
  }

  auto arg0_type = NativeValueTraits<IDLString>::NativeValue(info, 0);

  auto arg1_status = NativeValueTraits<IDLBoolean>::NativeValue(info, 1);

  impl_->ResetEventListenerStatus(std::move(arg0_type), arg1_status);
  return info.Env().Undefined();
}

// static
Napi::Class* NapiEventTarget::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kEventTargetClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Methods
  AddInstanceMethod(props, "resetEventListenerStatus", &NapiEventTarget::ResetEventListenerStatusMethod);

  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "EventTarget", props));
  env.SetInstanceData<Napi::Class>(kEventTargetClassID, clazz);
  return clazz;
}

// static
Function NapiEventTarget::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kEventTargetConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kEventTargetConstructorID, ref);
  return ref->Value();
}

// static
void NapiEventTarget::Install(Napi::Env env, Object& target) {
  if (target.Has("EventTarget")) {
    return;
  }
  target.Set("EventTarget", Constructor(env));
}
