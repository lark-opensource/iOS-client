...
namespace {
const uint64_t kWebGLProgramClassID = reinterpret_cast<uint64_t>(&kWebGLProgramClassID);
const uint64_t kWebGLProgramConstructorID = reinterpret_cast<uint64_t>(&kWebGLProgramConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLProgram>;
typedef Value (NapiWebGLProgram::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLProgram::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLProgram::NapiWebGLProgram(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
NapiWebGLProgram::~NapiWebGLProgram() {
  impl_->Dispose();
}

WebGLProgram* NapiWebGLProgram::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLProgram::Wrap(std::unique_ptr<WebGLProgram> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLProgram>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLProgram::Init(std::unique_ptr<WebGLProgram> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
// static
Napi::Class* NapiWebGLProgram::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLProgramClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLProgram", props));
  env.SetInstanceData<Napi::Class>(kWebGLProgramClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLProgram::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLProgramConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLProgramConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLProgram::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLProgram")) {
    return;
  }
  target.Set("WebGLProgram", Constructor(env));
}
