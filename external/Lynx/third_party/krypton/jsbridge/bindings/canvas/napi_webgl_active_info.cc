...
#include "canvas/webgl/webgl_active_info.h"
...
namespace {
const uint64_t kWebGLActiveInfoClassID = reinterpret_cast<uint64_t>(&kWebGLActiveInfoClassID);
const uint64_t kWebGLActiveInfoConstructorID = reinterpret_cast<uint64_t>(&kWebGLActiveInfoConstructorID);

using Wrapped = piper::NapiBaseWrapped<NapiWebGLActiveInfo>;
typedef Value (NapiWebGLActiveInfo::*InstanceCallback)(const CallbackInfo& info);
typedef void (NapiWebGLActiveInfo::*InstanceSetterCallback)(const CallbackInfo& info, const Value& value);
...
NapiWebGLActiveInfo::NapiWebGLActiveInfo(const CallbackInfo& info, bool skip_init_as_base)
    : BridgeBase(info) {
...
WebGLActiveInfo* NapiWebGLActiveInfo::ToImplUnsafe() {
  return impl_.get();
}

// static
Object NapiWebGLActiveInfo::Wrap(std::unique_ptr<WebGLActiveInfo> impl, Napi::Env env) {
  DCHECK(impl);
  auto obj = Constructor(env).New({Napi::External::New(env, nullptr, nullptr, nullptr)});
  ObjectWrap<NapiWebGLActiveInfo>::Unwrap(obj)->Init(std::move(impl));
  return obj;
}

void NapiWebGLActiveInfo::Init(std::unique_ptr<WebGLActiveInfo> impl) {
  DCHECK(impl);
  DCHECK(!impl_);
...
Value NapiWebGLActiveInfo::SizeAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetSize());
}

Value NapiWebGLActiveInfo::TypeAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return Number::New(info.Env(), impl_->GetType());
}

Value NapiWebGLActiveInfo::NameAttributeGetter(const CallbackInfo& info) {
  DCHECK(impl_);

  return String::New(info.Env(), impl_->GetName());
}

// static
Napi::Class* NapiWebGLActiveInfo::Class(Napi::Env env) {
  auto* clazz = env.GetInstanceData<Napi::Class>(kWebGLActiveInfoClassID);
  if (clazz) {
    return clazz;
  }
  ...
  // Attributes
  AddAttribute(props, "size",
               &NapiWebGLActiveInfo::SizeAttributeGetter,
               nullptr
               );
  AddAttribute(props, "type",
               &NapiWebGLActiveInfo::TypeAttributeGetter,
               nullptr
               );
  AddAttribute(props, "name",
               &NapiWebGLActiveInfo::NameAttributeGetter,
               nullptr
               );
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "WebGLActiveInfo", props));
  env.SetInstanceData<Napi::Class>(kWebGLActiveInfoClassID, clazz);
  return clazz;
}

// static
Function NapiWebGLActiveInfo::Constructor(Napi::Env env) {
  FunctionReference* ref = env.GetInstanceData<FunctionReference>(kWebGLActiveInfoConstructorID);
  if (ref) {
    return ref->Value();
  }
  ...
  env.SetInstanceData<FunctionReference>(kWebGLActiveInfoConstructorID, ref);
  return ref->Value();
}

// static
void NapiWebGLActiveInfo::Install(Napi::Env env, Object& target) {
  if (target.Has("WebGLActiveInfo")) {
    return;
  }
  target.Set("WebGLActiveInfo", Constructor(env));
}
