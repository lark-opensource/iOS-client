...
#define PERFETTO_XLOG(level, fmt, ...)                                        \
  do {                                                                        \
    async_safe_format_log((ANDROID_LOG_DEBUG + level), "perfetto",            \
                          "%s:%d " fmt, ::lynx::perfetto::base::Basename(__FILE__), \
                          __LINE__, ##__VA_ARGS__);                           \
  } while (0)
#elif defined(PERFETTO_DISABLE_LOG)
#define PERFETTO_XLOG(...) ::lynx::perfetto::base::ignore_result(__VA_ARGS__)
#else
#define PERFETTO_XLOG(level, fmt, ...)                                      \
  ::lynx::perfetto::base::LogMessage(level, ::lynx::perfetto::base::Basename(__FILE__), \
...
#define PERFETTO_LOG(fmt, ...) \
  PERFETTO_XLOG(::lynx::perfetto::base::kLogInfo, fmt, ##__VA_ARGS__)
#else  // PERFETTO_BUILDFLAG(PERFETTO_VERBOSE_LOGS)
#define PERFETTO_LOG(...) ::lynx::perfetto::base::ignore_result(__VA_ARGS__)
#endif  // PERFETTO_BUILDFLAG(PERFETTO_VERBOSE_LOGS)

#define PERFETTO_ILOG(fmt, ...) \
  PERFETTO_XLOG(::lynx::perfetto::base::kLogImportant, fmt, ##__VA_ARGS__)
#define PERFETTO_ELOG(fmt, ...) \
  PERFETTO_XLOG(::lynx::perfetto::base::kLogError, fmt, ##__VA_ARGS__)
...
#define PERFETTO_DLOG(fmt, ...) \
  PERFETTO_XLOG(::lynx::perfetto::base::kLogDebug, fmt, ##__VA_ARGS__)
...
#define PERFETTO_DLOG(...) ::lynx::perfetto::base::ignore_result(__VA_ARGS__)
#define PERFETTO_DPLOG(...) ::lynx::perfetto::base::ignore_result(__VA_ARGS__)
...
#define PERFETTO_DFATAL(...) ::lynx::perfetto::base::ignore_result(__VA_ARGS__)
