...
#ifndef ANIMAX_LAYER_SHAPE_LAYER_H_
#define ANIMAX_LAYER_SHAPE_LAYER_H_
...
class ShapeLayer : public BaseLayer {
 public:
  ShapeLayer(std::shared_ptr<LayerModel>& layer_model,
  ...
  BlurEffectModel* GetBlurEffect() override;
  DropShadowEffectModel* GetDropEffect() override;

 private:
  CompositionLayer* composition_layer_ = nullptr;
  std::unique_ptr<ContentGroup> content_group_;
  std::shared_ptr<ShapeGroupModel> shape_group_model_;
};

}  // namespace animax
}  // namespace lynx

#endif  // ANIMAX_LAYER_SHAPE_LAYER_H_
