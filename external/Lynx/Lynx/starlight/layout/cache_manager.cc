...
inline bool CheckCache(const OneSideConstraint& current_constraint,
  ...
  const auto CheckPercentageBase = [&is_horizontal, &target]() -> bool {
    if (is_horizontal && target->DoesLayoutDependOnHorizontalPercentBase()) {
      return false;
    }
    if (!is_horizontal && target->DoesLayoutDependOnVerticalPercentBase()) {
      return false;
    }
    return true;
  };
  if (last_constraint.Mode() == SLMeasureModeIndefinite) {
    ...
    if (current_constraint.Mode() == SLMeasureModeIndefinite) {
      ...
      if (FloatsLargerOrEqual(last_constraint.Size(),
                              current_constraint.Size()) &&
          FloatsLargerOrEqual(current_constraint.Size(), last_computed_size)) {
        return true;
      }
      return false;
    } else {
