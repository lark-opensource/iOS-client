...
void CalcStartOffset(LayoutObject* absolute_or_fixed_item,
  ...
  const ComputedCSSStyle* item_style = absolute_or_fixed_item->GetCSSStyle();
...
Constraints GetContainingBlockForAbsolute(
...
Constraints GetAbsoluteOrFixedItemSizeAndMode(
    ...
    const Constraints& content_bound) {
  const Constraints absolute_box =
      GetContainingBlockForAbsolute(container, content_bound);
...
void CalcAbsoluteOrFixedPosition(
  ...
  const auto containing_block =
      GetContainingBlockForAbsolute(container, container_constraints);
