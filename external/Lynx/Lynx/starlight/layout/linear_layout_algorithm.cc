...
void LinearLayoutAlgorithm::DetermineItemSize() {
  ...
  std::vector<float> bases(item_size, 0.f);
  ...
  for (size_t idx = 0; idx < item_size; ++idx) {
    ...
    if (container_->GetBaselineFlag()) {
      if (inflow_items_[idx]->HasBaseline()) {
        baseline_ = std::max(baseline_, inflow_items_[idx]->GetBaseline() +
                                            margin[CrossFront()]);
      } else {
        baseline_ =
            std::max(baseline_, cross_size_[idx] + margin[CrossFront()]);
      }
    }
  }
}
...
void LinearLayoutAlgorithm::LayoutWeightedChildren(
    const std::vector<float>& base_sizes) {
  const float weight_sum = container_style_->GetLinearWeightSum();
  const size_t item_size = inflow_items_.size();
  std::vector<float> hypothetical_size(item_size, 0.f);
  ElasticLayoutUtils::ComputeHypotheticalSizes(inflow_items_, base_sizes, *this,
                                               hypothetical_size);
  ElasticLayoutUtils::ElasticInfos infos(
      inflow_items_, base_sizes, hypothetical_size, true, *this, 0, item_size);
...
void LinearLayoutAlgorithm::CrossAxisAlignment(LayoutObject* item) {
  const ComputedCSSStyle* item_style = item->GetCSSStyle();
...
void LinearLayoutAlgorithm::InitializeAlgorithmEnv() {
  main_size_.resize(inflow_items_.size(), -1);
  cross_size_.resize(inflow_items_.size(), -1);
}
...
LinearLayoutGravityType LinearLayoutAlgorithm::GetComputedLinearLayoutGravity(
  ...
  if (std::ceil(
          std::strtof(container_->GetLayoutConfigs().target_sdk_version.c_str(),
                      nullptr) *
          10) >= 22 &&
...
void LinearLayoutAlgorithm::HandleScrollView() {
  ...
  if ((container_->GetTag().IsEqual("scroll-view") ||
       container_->GetTag().IsEqual("x-scroll-view")) &&
      MainFront() == kRight) {
    ...
    if (shift_length > 0) {
      for (LayoutObject* item : inflow_items_) {
        item->SetBoundLeftFrom(
            item->GetBoundLeftFrom(BoundType::kMargin, BoundType::kContent) +
                shift_length,
...
void LinearLayoutAlgorithm::SetContainerBaseline() {
  if (IsHorizontal()) {
    for (LayoutObject* item : inflow_items_) {
      ...
      if (IsLayoutGravityDefault(layout_gravity)) continue;

      float cross_offset = 0.f;

      if (item->HasBaseline()) {
        cross_offset = item->GetBaseline();
      } else {
        cross_offset =
            item->GetMarginBoundHeight() - item->GetLayoutMarginBottom();
      }
