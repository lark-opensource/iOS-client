...
int GetAlignPhysicalStart(const ComputedCSSStyle& css, Dimension dimension) {
...
int GetAlignPhysicalEnd(const ComputedCSSStyle& css, Dimension dimension) {
  if (dimension == kVertical) {
...
inline bool IsCenterAlign(const ComputedCSSStyle& css, Dimension dimension) {
  if (dimension == kVertical) {
...
using DependencyGetter =
    std::function<void(size_t idx, const ComputedCSSStyle* style,
                       std::set<size_t>& item_dependencies,
                       std::vector<std::set<size_t>>& reverse_dependencies)>;

void SortDependencies(const Items& items,
                      const DependencyGetter& dependency_getter,
                      std::vector<size_t>& order) {
  std::set<size_t> unsorted;
  std::vector<std::set<size_t>> dependencies(items.size());
  std::vector<std::set<size_t>> reverse_dependencies(items.size());
...
void RelativeLayoutAlgorithm::UpdateChildrenSize() {
  const auto& MeasureWithOrder = [this](const std::vector<size_t>& orders,
...
void RelativeLayoutAlgorithm::InitializeAlgorithmEnv() {
  layout_results_.resize(inflow_items_.size());
  proposed_position_.resize(inflow_items_.size(),
                            DirectionValue<float>({0.f, 0.f, 0.f, 0.f}));
  vertical_order_.resize(inflow_items_.size());
  if (!container_style_->GetRelativeLayoutOnce()) {
    horizontal_order_.resize(inflow_items_.size());
  }
...
LayoutUnit RelativeLayoutAlgorithm::GetPositionConstraints(
  ...
  const ComputedCSSStyle& css = *(obj.GetCSSStyle());
...
void RelativeLayoutAlgorithm::GenerateIDMap() {
  for (size_t idx = 0; idx < inflow_items_.size(); ++idx) {
    ...
    if (id != DefaultCSSStyle::SL_DEFAULT_RELATIVE_ID) {
      id_map_[id] = idx;
    }
  }
}
...
void RelativeLayoutAlgorithm::Sort() {
  if (container_style_->GetRelativeLayoutOnce()) {
    const auto& dependency_getter =
        [this](size_t idx, const ComputedCSSStyle* style,
               std::set<size_t>& item_dependencies,
               std::vector<std::set<size_t>>& reverse_dependencies) {
...
void RelativeLayoutAlgorithm::ComputePosition(
    const ComputedCSSStyle& css, Dimension dimension,
...
void RelativeLayoutAlgorithm::RecomputeProposedPosition(
    const std::vector<size_t>& orders, Dimension dimension, Direction start) {
