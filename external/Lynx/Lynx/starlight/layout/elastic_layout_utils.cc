...
float CalculateRemainingSpace(
    ...
    std::vector<float>& computed_item_sizes, const std::vector<bool>& freeze) {
  ...
  for (size_t idx = elastic_info.start_idx_; idx < elastic_info.end_idx_;
    ...
    // for other items, use their outer flex base size.
      ...
      // flex标准中对于flex_base_size不会进行clamp content box size,
      // 但YOGA中会使用content box > 0 的值来作为base size
...
bool ResolveOneLine(
    ...
    ElasticInternalVariables& internal_variables,
    ElasticLayoutUtils::ElasticFactorGetter elastic_factor_getter,
    std::vector<float>& computed_item_sizes, std::vector<bool>& freeze,
    float free_space) {
  // 9.7-4 Loop
  float total_violations = 0;
  std::vector<size_t> min_violations;
  std::vector<size_t> max_violations;
...
float ElasticLayoutUtils::ComputeElasticItemSizes(
  ...
  std::vector<bool> freeze(end - start, false);
...
float ElasticLayoutUtils::ComputeHypotheticalSizes(
  ...
  if (targets.size() != elastic_bases.size() ||
    ...
    NOTREACHED() << "Array sizes mismatch when computing hypothetical sizes";
