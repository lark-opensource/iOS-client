...
void Scanner::ParseString(Token& token) {
  ...
  token = Token(line_, column_, Token_String,
                lynx::lepus::StringImpl::Create(buffer));
}
...
bool Scanner::ParseRegExp(Token& token) {
  ...
  token = Token(line_, column_, Token_RegExp,
                lynx::lepus::StringImpl::Create(pattern),
                lynx::lepus::StringImpl::Create(flags));
  return true;
}
...
void Scanner::ParseId(Token& token) {
  ...
  if (!IsKeyWord(buffer, token_type)) {
    token = Token(line_, column_, Token_Id,
                  lynx::lepus::StringImpl::Create(buffer));
    return;
  }
...
void Scanner::SetSdkVersion(std::string& sdk_version) {
