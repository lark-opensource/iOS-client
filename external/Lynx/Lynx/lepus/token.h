...
struct Token {
  ...
  Token() : str_(""), line_(0), column_(0), token_(Token_EOF) {}
  ...
  Token(int line, int column, int token,
        lynx::base::scoped_refptr<StringImpl> str)
      : str_(str), line_(line), column_(column), token_(token) {}

  Token(int line, int column, int token,
        lynx::base::scoped_refptr<StringImpl> pattern,
        lynx::base::scoped_refptr<StringImpl> flags)
      : line_(line), column_(column), token_(token) {
