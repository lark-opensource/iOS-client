...
#ifndef LYNX_LEPUS_SYNTAX_TREE_H_
#define LYNX_LEPUS_SYNTAX_TREE_H_
...
class ObjectLiteralAST : public ASTree {
 public:
  ObjectLiteralAST(std::unordered_map<String, std::unique_ptr<ASTree>> property)
      : property_(std::move(property)) {}
  ~ObjectLiteralAST() {}
  AST_ACCEPT_VISITOR

  const std::unordered_map<String, std::unique_ptr<ASTree>>& property() {
    return property_;
  }

 private:
  // Now only support string as key
  std::unordered_map<String, std::unique_ptr<ASTree>> property_;
};
...
#endif  // LYNX_LEPUS_SYNTAX_TREE_H_
