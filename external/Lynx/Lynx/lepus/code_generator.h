...
struct BlockGenerate {
  ...
  std::unordered_map<String, std::pair<int64_t, int64_t>>
      closure_variables_outside_;
  std::unordered_map<String, std::pair<int64_t, int64_t>> closure_variables_;
...
struct FunctionGenerate {
  ...
  std::vector<String> function_params_;
...
class CodeGenerator : public Visitor {
 ...
 private:
  ...
  void InsertVariable(const String& name, long register_id);
  long SearchVariable(const String& name);
  long SearchVariable(const String& name, FunctionGenerate* current);
  long SearchGlobal(const String& name);
  long SearchBuiltin(const String& name);
  long ManageUpvalues(const String& name);
  std::pair<long, long> ManageUpvaluesNew(const String& name);
  ...
  long GetUpvalueArrayIndex(const lynx::lepus::String& name);
  ...
  VMContext* context_;
  base::scoped_refptr<StringImpl> current_function_name_;
  ...
  std::unordered_map<std::pair<lynx::lepus::String, uint64_t>, long, pair_hash>
      upvalue_array_;
  std::vector<std::unordered_map<std::pair<lynx::lepus::String, uint64_t>, long,
                                 pair_hash>>
      context_stack_;
...
#endif  // LYNX_LEPUS_CODE_GENERATOR_H_
