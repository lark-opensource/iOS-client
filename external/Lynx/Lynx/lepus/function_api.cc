...
static void getArrayNumber(const Value* param, std::string& str) {
  if (param->IsArray() && param->Array()->size() > 0) {
    auto array0 = param->Array()->get(0);
    if (array0.IsString()) {
      str = array0.String()->str();
      ...
      getArrayNumber(&(param->Array()->get(0)), str);
    }
  }
}

static Value ParseInt(Context* context) {
  ...
  if (context->GetParam(0)->IsString()) {
    str = context->GetParam(0)->String()->str();
  } else if (context->GetParam(0)->IsNumber()) {
...
static Value ParseFloat(Context* context) {
...
static Value IsNan(Context* context) {
...
static Value EncodeURIComponent(Context* context) {
  auto params_count = context->GetParamsSize();
  DCHECK(params_count == 1);
  std::string param = context->GetParam(0)->String()->str();
...
static int string_get_hex(std::string& param, size_t k, size_t n) {
  int c = 0, h;
  while (n-- > 0) {
...
static int hex_decode(std::string& param, size_t k) {
  int c;
...
static Value DecodeURIComponent(Context* context) {
  auto params_count = context->GetParamsSize();
  DCHECK(params_count == 1);
  std::string param = context->GetParam(0)->String()->str();
