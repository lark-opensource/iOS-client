...
std::size_t Function::AddConstNumber(double number) {
  Value v;
  if (lynx::lepus::StringConvertHelper::IsInt64Double(number)) {
...
std::size_t Function::AddConstString(
    lynx::base::scoped_refptr<StringImpl> string) {
  Value v;
  v.SetString(string);
  return AddConstValue(v);
}

std::size_t Function::AddConstRegExp(lynx::base::scoped_refptr<RegExp> regexp) {
  Value v;
  v.SetRegExp(regexp);
  return AddConstValue(v);
}
...
std::string Function::GetFunctionName() {
  ...
  lepus::Value last = const_values_.back();
  if (last.IsTable()) {
    Value name = last.Table()->GetValue(Function::kFuncName);
    if (name.IsString()) {
      function_name_ = name.String()->str();
...
Value Function::GetLineInfo() {
  base::scoped_refptr<CArray> info = CArray::Create();
  size_t len = debug_line_col_.size();
  for (size_t i = 0; i < len; i++) {
    info->push_back(Value(debug_line_col_[i]));
  }
...
int64_t Function::GetFunctionId() {
  ...
  if (const_values_.size() > 0) {
    lepus::Value last = const_values_.back();
    if (last.IsTable()) {
      Value function_id = last.Table()->GetValue(Function::kFuncId);
...
void Function::GetLineCol(int32_t index, int32_t& line, int32_t& col) {
  ...
  if (const_values_.size() > 0) {
    ...
    if (index >= 0 && last.IsTable()) {
      debug_info = last.Table()->GetValue(Function::kLineColInfo);
    }
  }
  ...
  if (debug_info.IsArray() &&
    ...
    Value number = debug_info.Array()->get(index);
    int64_t line_col = 0;
    if (number.IsInt64()) {
...
Value& Function::GetScope() {
  if (scopes_.IsNil()) {
    if (!const_values_.empty()) {
      lepus::Value last = const_values_.back();
      if (last.IsTable()) {
        Value scope = last.Table()->GetValue(Function::kScopesName);
        scopes_ = scope;
      }
    }
  }
...
std::stack<uint64_t> Function::block_scope_stack() {
  return block_scope_stack_;
}
...
std::stack<uint64_t> Function::loop_block_stack() { return loop_block_stack_; }
...
std::vector<Value> Function::GetConstValue() { return const_values_; }
...
int32_t Function::GetParamsSize() {
  ...
  if (!const_values_.empty()) {
    lepus::Value last = const_values_.back();
    if (last.IsTable()) {
      Value name = last.Table()->GetValue(Function::kParamsSize);
      if (name.IsNumber()) {
