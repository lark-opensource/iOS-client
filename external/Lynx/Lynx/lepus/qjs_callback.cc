...
std::string LEPUSGetStdStringFromAtom(LEPUSContext* ctx, LEPUSAtom prop,
                                      int32_t idx) {
  if (idx >= 0) {
...
int32_t LepusHasProperty(LEPUSContext* ctx, LEPUSValue obj, LEPUSAtom prop,
  ...
  switch (pref->tag) {
    ...
    case Value_Table: {
      ...
      return dic->Contains(LEPUSGetStdStringFromAtom(ctx, prop, idx));
    };
    default: {
      assert(false);
    } break;
...
int32_t LepusDeleteProperty(LEPUSContext* ctx, LEPUSValue this_obj,
  ...
  switch (pref->tag) {
    case Value_Table: {
      ...
      std::string key = LEPUSGetStdStringFromAtom(ctx, prop, idx);
      if (!dic->Contains(key)) {
        return true;
      }
      ret = dic->Erase(key);
  ...
  if (LEPUSValueHelper::IsLepusArray(this_obj)) {
    ...
    *prop_count = static_cast<uint32_t>(array_size);
    ...
    tab_enum = reinterpret_cast<LEPUSPropertyEnum*>(
        lepus_malloc(ctx, sizeof(LEPUSPropertyEnum) * malloc_size));
    ...
    if (!is_enum_only) {
      tab_enum[array_size - 1].atom = LEPUS_NewAtom(ctx, "length");
...
LEPUSValue LEPUSRefArrayPushCallBack(LEPUSContext* ctx, LEPUSValue this_val,
  ...
  if (array->IsConst()) {
    Context::GetFromJsContext(ctx)->ReportError(
...
int64_t LEPUSRefArrayFindCallBack(LEPUSContext* ctx, LEPUSValue this_val,
  ...
  if (dir > 0) {
    ...
    for (; from_index < array_size; ++from_index) {
      LEPUSValue op =
          array->get(static_cast<size_t>(from_index)).ToJSValue(ctx, false);
...
LEPUSValue LEPUSRefArraySlice(LEPUSContext* ctx, LEPUSValue this_val,
  ...
  for (size_t i = 0, k = start; i < count && k < array->size(); ++i, ++k) {
    LEPUS_SetPropertyInt64(ctx, ret, i, array->get(k).ToJSValue(ctx));
  }
