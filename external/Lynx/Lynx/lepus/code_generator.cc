...
#include "base/sorted_for_each.h"
...
void CodeGenerator::EnterFunction() {
  ...
  if (IsTopLevelFunction()) {
    ...
    function->function_->SetFunctionName(current_function_name_->c_str());
  }
...
void CodeGenerator::InsertVariable(const String& name, long register_id) {
...
long CodeGenerator::SearchVariable(const String& name) {
...
long CodeGenerator::SearchGlobal(const String& name) {
  return context_->global()->Search(name);
}

long CodeGenerator::SearchBuiltin(const String& name) {
  return context_->builtin()->Search(name);
}

long CodeGenerator::ManageUpvalues(const String& name) {
  lynx::base::scoped_refptr<Function> function = current_function_->function_;
  long index = function->SearchUpvalue(name.impl());
  ...
  while (!parents.empty()) {
    ...
    if (register_index >= 0) {
      long index = current->function_->AddUpvalue(name.impl(), register_index,
                                                  in_parent_vars);
      ...
      if (register_id >= 0) {
        ...
        long index = current->function_->SearchUpvalue(name.impl());
        if (index >= 0) {
          register_index = index;
          in_parent_vars = false;
          parents.pop();
  ...
  index = function->AddUpvalue(name.impl(), register_index, in_parent_vars);
  return index;
}

std::pair<long, long> CodeGenerator::ManageUpvaluesNew(const String& name) {
  lynx::base::scoped_refptr<Function> function = current_function_->function_;
  long offset = 0;
  long array_index = -1;
  for (int i = static_cast<int>(context_stack_.size() - 1); i >= 0; i--) {
    ...
    for (int32_t j = static_cast<int32_t>(parent_ids.size()) - 1; j >= 0; j--) {
      id = parent_ids[j];
      auto iter = upvalue_array.find({name.impl(), id});
...
void CodeGenerator::Visit(ReturnStatementAST* ast, void* data) {
  ...
  AstLineScope sop(current_function_->function_.Get(), ast);
  base::scoped_refptr<Function> function = current_function_->function_;
...
long CodeGenerator::GetUpvalueArrayIndex(const lynx::lepus::String& name) {
...
void CodeGenerator::Visit(LiteralAST* ast, void* data) {
  ...
  if (ast->lex_op() >= LexicalOp_Write) {
    ...
    if (ast->token().token_ == Token_Number) {
      ...
      long index = function->AddConstString(ast->token().str_.impl());
      auto instruction =
          Instruction::ABxCode(TypeOp_LoadConst, register_id, index);
      function->AddInstruction(instruction);
    } else if (ast->token().token_ == Token_RegExp) {
      long index = function->AddConstRegExp(RegExp::Create(
          ast->token().pattern_.impl(), ast->token().flags_.impl()));
      auto instruction =
          Instruction::ABxCode(TypeOp_LoadConst, register_id, index);
      function->AddInstruction(instruction);
      ...
      if (ast->scope() == LexicalScoping_Local) {
        String name = ast->token().str_.c_str();
        ...
        if (global_index >= 0) {
          ...
          function->AddInstruction(instruction);
                   ...
                   ast->token().str_ == "lynx") {
          // If targetSdkVersion >= 2.8 and ast == "lynx", generate
  ...
  if (ast->op_token().token_ == Token_And ||
    ...
    if (!support_closure_) {
      ...
      if (token == '&' || token == '|' || token == '^' || token == Token_Pow) {
        throw CompileException("target sdk version is less 1.4",
                               "'&' '+' '^' '**' is not supported",
...
void CodeGenerator::Visit(FunctionStatementAST* ast, void* data) {
  ...
  {
    current_function_name_ = ast->function_name().str_.impl();
    LineScope lsp(this, ast);
    Guard<CodeGenerator> g(this, &CodeGenerator::EnterFunction,
                           &CodeGenerator::LeaveFunction);
    {
  ...
  if (support_closure_) {
    auto function_name = ast->function_name().str_.impl();
...
bool ShouldInsertThisParam(FunctionCallAST* ast) {
  if (ast->caller()->type() == ASTType_MemberAccessor) {
    ...
    if (property->table()->type() == ASTType_Literal) {
      ...
      // for Global builtins like: String/Math should not pass [[this]]
...
void CodeGenerator::Visit(FunctionCallAST* ast, void* data) {
...

  {
    ...
    // 2. for Global builtins like: String/Math should not pass [[this]]
    if (ShouldInsertThisParam(ast)) {
...
void CodeGenerator::Visit(ObjectLiteralAST* ast, void* data) {
  ...
  base::SortedForEach(ast->property(), [this, &function, &key_reg, &value_reg,
    ...
    long key_index = function->AddConstString(it.first.impl());
    function->AddInstruction(
