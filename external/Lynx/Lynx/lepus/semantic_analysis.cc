...
void LexicalBlock::AddUpvalue(
    const base::scoped_refptr<StringImpl>& upvalue_name, uint64_t block_id) {
...
void LexicalFunction::AddUpvalue(
    const base::scoped_refptr<StringImpl>& upvalue_name, uint64_t block_id) {
  if (upvalue_array_.find({upvalue_name, block_id}) == upvalue_array_.end()) {
    upvalue_array_[{upvalue_name, block_id}] = ++upvalue_array_max_index_;
  }
...
bool SemanticAnalysis::InsertName(
    const lynx::base::scoped_refptr<StringImpl> name) {
...
LexicalScoping SemanticAnalysis::SearchName(
    const lynx::base::scoped_refptr<StringImpl> name) {
...
void SemanticAnalysis::Visit(LiteralAST* ast, void* data) {
  ...
  if (ast->token().token_ == Token_Id) {
    ast->scope() = SearchName(ast->token().str_.impl());
  }
...
void SemanticAnalysis::Visit(VariableAST* ast, void* data) {
  ...
  if (!InsertName(ast->identifier().str_.impl())) {
...
void SemanticAnalysis::Visit(FunctionStatementAST* ast, void* data) {
  if (ast->function_name().token_ != Token_EOF &&
      !InsertName(ast->function_name().str_.impl())) {
...
void SemanticAnalysis::SetSdkVersion(std::string& sdk_version) {
