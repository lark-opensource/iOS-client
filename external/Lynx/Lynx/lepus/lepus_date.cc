    ...
    switch (format[i]) {
      ...
      case 'Z': {
        ...
        tm_.tm_gmtoff *= (std::stoi(date.substr(i, 2)) * 60 +
                          std::stoi(date.substr(i + 4 - count_Z, 2)));
        tm_.tm_gmtoff *= 60;
        i = i + 6 - count_Z;
        local_flag = false;
        break;
...
std::string CDate::dateToString(Value* date, const std::string& format) {
  ...
  while (i < length) {
    switch (format[i]) {
      ...
      case 'Z': {
        ...
        int32_t utcMin = 0;
        if (time.tm_gmtoff >= 0) {
          utcHour = static_cast<int32_t>(time.tm_gmtoff / 3600);
          utcMin = static_cast<int32_t>(time.tm_gmtoff / 60 - utcHour * 60);
          snprintf(ss, kBufferSize, "+%02d", utcHour);
        } else {
          utcHour = static_cast<int32_t>(-time.tm_gmtoff / 3600);
          utcMin = static_cast<int32_t>(-time.tm_gmtoff / 60 - utcHour * 60);
        ...
        destination += ss;
        snprintf(ss, kBufferSize, "%02d", utcMin);
        destination += ss;
        i = i + count_Z + 1;
        break;
      }
...
base::scoped_refptr<CDate> CDate::ParseNumberToDate(int64_t parse_number) {
...
base::scoped_refptr<CDate> CDate::ParseStringToDate(int params_count,
