...
std::optional<Value> JSCRuntime::evaluatePreparedJavaScript(
...
std::optional<Value> JSCRuntime::evaluateJavaScript(
...
std::shared_ptr<HostObject> JSCRuntime::getHostObject(const Object& obj) {
  JSObjectRef object = JSCHelper::objectRef(obj);
  auto metadata =
      static_cast<detail::JSCHostObjectProxyBase*>(JSObjectGetPrivate(object));
  DCHECK(metadata);
...
HostFunctionType& JSCRuntime::getHostFunction(const Function& obj) {
  ...
  auto proxy = static_cast<JSCHostFunctionProxy*>(data);
...
#else
  return JSValueIsArray(ctx_->getContext(), JSCHelper::objectRef(obj));
#endif
}
...
bool JSCRuntime::isHostFunction(const Function& obj) const {
  auto cls = JSCHostFunctionProxy::getHostFunctionClass();
  return cls != nullptr &&
         JSValueIsObjectOfClass(ctx_->getContext(), JSCHelper::objectRef(obj),
                                cls);
}
...
Function JSCRuntime::createFunctionFromHostFunction(const PropNameID& name,
  ...
  return JSCHostFunctionProxy::createFunctionFromHostFunction(
