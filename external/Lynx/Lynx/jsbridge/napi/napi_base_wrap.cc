...
napi_value InstanceMethodCallbackWrapper(napi_env env,
  ...
  BridgeBaseMethodCallbackData* callbackData =
      reinterpret_cast<BridgeBaseMethodCallbackData*>(callbackInfo.Data());
  callbackInfo.SetData(callbackData->data);
  auto wrapper = callbackInfo.This().As<Napi::Object>();
  BridgeBase* instance = Napi::InstanceWrap<BridgeBase>::Unwrap(wrapper);
...
napi_value InstanceGetterCallbackWrapper(napi_env env,
                                         napi_callback_info info) {
  Napi::CallbackInfo callbackInfo(env, info);
  BridgeBaseAccessorCallbackData* callbackData =
      reinterpret_cast<BridgeBaseAccessorCallbackData*>(callbackInfo.Data());
  callbackInfo.SetData(callbackData->data);
  auto wrapper = callbackInfo.This().As<Napi::Object>();
  BridgeBase* instance = Napi::InstanceWrap<BridgeBase>::Unwrap(wrapper);
...
napi_value StaticMethodCallbackWrapper(napi_env env, napi_callback_info info) {
  Napi::CallbackInfo callbackInfo(env, info);
  BridgeBaseStaticMethodCallbackData* callbackData =
      reinterpret_cast<BridgeBaseStaticMethodCallbackData*>(
          callbackInfo.Data());
  callbackInfo.SetData(callbackData->data);
...
napi_value StaticGetterCallbackWrapper(napi_env env, napi_callback_info info) {
  Napi::CallbackInfo callbackInfo(env, info);
  BridgeBaseStaticAccessorCallbackData* callbackData =
      reinterpret_cast<BridgeBaseStaticAccessorCallbackData*>(
          callbackInfo.Data());
  callbackInfo.SetData(callbackData->data);
    ...
    if ((p.attributes & napi_static) == 0) {
      if (p.method == InstanceMethodCallbackWrapper) {
        obj.AddFinalizer(p.data, [](napi_env, void* data, void*) {
          delete static_cast<BridgeBaseMethodCallbackData*>(data);
        });
      } else if (p.getter == InstanceGetterCallbackWrapper ||
                 p.setter == InstanceSetterCallbackWrapper) {
        obj.AddFinalizer(p.data, [](napi_env, void* data, void*) {
          delete static_cast<BridgeBaseAccessorCallbackData*>(data);
        });
      }
    } else {
      if (p.method == StaticMethodCallbackWrapper) {
        obj.AddFinalizer(p.data, [](napi_env, void* data, void*) {
          delete static_cast<BridgeBaseStaticMethodCallbackData*>(data);
        });
      } else if (p.getter == StaticGetterCallbackWrapper ||
                 p.setter == StaticSetterCallbackWrapper) {
        obj.AddFinalizer(p.data, [](napi_env, void* data, void*) {
          delete static_cast<BridgeBaseStaticAccessorCallbackData*>(data);
        });
      }
    }
  }
