...
void AnimationFrameTaskHandler::DoFrame(int64_t time_stamp,
  ...
  if (post_doframe_task_) {
    post_doframe_task_->Execute(rt, time_stamp);
  }
...
void AnimationFrameTaskHandler::SetPostDoFrameTaskWithFunction(
    piper::Function func) {
  post_doframe_task_ =
      std::make_unique<FrameTask>(FrameTask(std::move(func), 0));
}

LynxApiHandler::LynxApiHandler(LynxRuntime* rt) : rt_(rt) {}

piper::Value LynxApiHandler::EnableCanvasOptimization() {
  return rt_->EnableCanvasOptimization();
}
