...
std::optional<piper::Value> JSBI::BigInt(Runtime* rt, const Value* args,
                                         size_t count,
                                         const std::string& func_name) {
  piper::Scope scope(*rt);
  if (count > 0) {
    // BigInt构造函数只支持入参为string或number
    ...
    return std::optional<piper::Value>(piper::Value(*bigint));
  }

  return piper::Value::undefined();
}

std::optional<piper::Value> JSBI::operate(Runtime* rt, const Value* args,
                                          size_t count,
                                          const std::string& func_name) {
  if (count >= 2) {
    piper::Scope scope(*rt);
    ...
    if (num1->isObject() && num2->isObject()) {
      ...
      return bigint_res ? *bigint_res : std::optional<piper::Value>();
    } else {
      return piper::Value::undefined();
    }
  }
