...
piper::Value Console::LogWithLevel(Runtime* rt, const int level,
  ...
  bool is_devtool_enabled = base::LynxEnv::GetInstance().IsDevtoolEnabled();
  if (count > 0) {
    if (is_devtool_enabled) {
      piper::Object global = rt->global();
      JSRuntimeType rt_type = rt->type();
      auto console = global.getProperty(
          *rt, rt_type == quickjs ? "lynxConsole" : "console");
    ...
    // There is no something like ConsoleDelegate can be
    // use to decouple the logic so we use code below to inject custom logic.
    // TODO(hongzhiyuan.hzy): Consider move this out in future refactor.
#if defined(MODE_HEADLESS)
    headless::EventEmitter<int64_t, std::string>::GetInstance()->EmitSync(
        rt->getRuntimeId(), "JSConsole",
        new std::pair<std::string, std::string>({func_name, msg}));
#endif
  }
  return piper::Value::undefined();
}
...
std::string Console::LogObject_(Runtime* rt, const Value* value) {
  ...
  std::string msg = LogObject_(rt, value, pre_object_vector.get(),
