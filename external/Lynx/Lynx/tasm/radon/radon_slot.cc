...
RadonSlot::RadonSlot(const lepus::String& slot_name)
    : RadonBase(kRadonSlot, "", kRadonInvalidNodeIndex), name_(slot_name) {}
...
void RadonSlot::MovePlugToComponent() {
  ...
  radon_component_->plugs_[name_] = std::move(radon_children_.back());
  ...
  radon_component_->radon_slots_helper_->DisconnectPlugElementFromParent(
      radon_component_->plugs_[name_].get());
}
...
void RadonSlot::RadonDiffChildren RADON_DIFF_ONLY(
    const std::unique_ptr<RadonBase>& old_radon_child,
    const DispatchOption& option) {
  TRACE_EVENT(LYNX_TRACE_CATEGORY,
              "RadonSlot::RadonDiffChildren in Radon Compatible",
...
RadonPlug::RadonPlug(const lepus::String& plug_name, RadonComponent* component)
    : RadonBase(kRadonPlug, "", kRadonInvalidNodeIndex), plug_name_(plug_name) {
  if (component) {
...
void RadonSlotsHelper::DiffWithPlugs(NameToPlugMap& old_plugs,
  ...
  auto& new_slots = radon_component_->slots_;
...
void RadonSlotsHelper::ReFillSlotsAfterChildrenDiff(
  ...
  auto& new_slots = radon_component_->slots_;
  auto& new_plugs = radon_component_->plugs_;
...
void RadonSlotsHelper::FillUnattachedPlugs() {
  ...
  auto& plugs = radon_component_->plugs_;
  for (auto& plug : plugs) {
    radon_component_->AddRadonPlug(plug.first, std::move(plug.second));
  }
}

void RadonSlotsHelper::RemoveAllSlots() { radon_component_->slots_.clear(); }
