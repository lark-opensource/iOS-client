...
void RadonNodeSelector::SelectImpl(
  ...
  if (is_target_node) {
    result_.push_back(static_cast<RadonNode*>(base));
...
void RadonNodeSelector::SelectInSlots(
  ...
  for (const auto& p : slots) {
    ...
    if (slot && !slot->radon_children_.empty()) {
      auto& plug = slot->radon_children_[0];
      if (!plug->radon_children_.empty()) {
        auto& slot_content = plug->radon_children_[0];
        const bool is_on_tree =
            slot_content->component() == component->component();
        if (is_on_tree) {
          SelectImpl(slot_content.get(), tokens, pos, options);
        }
      }
    }
  }
...
bool RadonNodeSelector::IsTokenSatisfied(RadonBase* base,
  ...
  switch (token.type) {
    ...
    case SelectElementToken::Type::REF_ID: {
      auto iter = node->attributes().find("react-ref");
      return iter != node->attributes().end() &&
             iter->second.first.String()->str() == token.selector_string;
    }
    ...
    case NodeSelectRoot::RootType::NODE_UNIQUE_ID: {
      if (page->proxy_) {
        auto element = page->proxy_->element_manager()->node_manager()->Get(
            root.node_unique_id);
        if (element) {
          base = static_cast<RadonNode*>(element->data_model());
        }
      }
    } break;
  }
...
void RadonNodeSelector::SelectByElementId(SelectorItem* root,
  ...
  auto element = static_cast<RadonBase*>(root)
                     ...
                     ->Get(id);
  if (element) {
    result_.push_back(static_cast<RadonNode*>(element->data_model()));
  }
}
