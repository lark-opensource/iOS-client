...
void ForEachLepusValue(const lepus::Value& value,
  ...
  switch (value.Type()) {
    case lepus::ValueType::Value_Table: {
      ...
      for (auto& pair : table) {
        auto key = lepus::Value{pair.first.impl()};
...
std::string GetTimingFlag(const lepus_value& table) {
  if (table.IsObject()) {
    lepus_value timing_flag_value = table.GetProperty("__lynx_timing_flag");
    return timing_flag_value.String()->str();
  }
  return "";
}
...
lepus::Value ConvertJSValueToLepusValue(const lepus::Value& value) {
  lepus::Value result;
  if (value.IsJSString()) {
    result = lepus::Value(value.String());
  } else if (value.IsJSBool()) {
    result = lepus::Value(value.Bool());
  } else if (value.IsJSNumber()) {
    result = lepus::Value(value.Number());
