...
#ifndef LYNX_TASM_AIR_AIR_ELEMENT_AIR_ELEMENT_H_
#define LYNX_TASM_AIR_AIR_ELEMENT_AIR_ELEMENT_H_
...
#include "tasm/air/air_element/air_element_container.h"
...
class AirElement {
 public:
  ...
  AirElement(const AirElement& node, AirPtrLookUpMap& map);
  ...
  inline ElementManager* element_manager() { return air_element_manager_; }
  ...
  inline std::shared_ptr<LayoutNode> layout_node() const {
    return layout_node_;
  }
  ...
  void SetParsedStyles(const AirCompStylesMap& parsed_styles) {
 ...
 private:
  ...
  size_t FindInsertIndex(const SharedAirElementVector& target,
                         AirElement* child);
  int GetIndexForChildLayoutNode(const std::shared_ptr<LayoutNode>& child);
  ...
  void PushToPropsBundle(const lepus::String& key, const lepus::Value& value);
...
class AirLepusRef : public base::RefCountedThreadSafeStorage {
 public:
  ...
  static base::scoped_refptr<AirLepusRef> Create(
      std::shared_ptr<AirElement> lepus_ref) {
    return base::AdoptRef(new AirLepusRef(lepus_ref));
  }

  static base::scoped_refptr<AirLepusRef> Create(AirLepusRef* ref) {
    return base::AdoptRef(new AirLepusRef(ref));
  }
...
#endif  // LYNX_TASM_AIR_AIR_ELEMENT_AIR_ELEMENT_H_
