...
int LayoutContextDarwin::CreateLayoutNode(int sign, intptr_t layout_node_ptr,
                                          const std::string& tag, PropBundle* painting_data,
  ...
  @try {
    return static_cast<int>([nodeOwner
             createNodeWithSign:sign
                  ...
                  layoutNodePtr:layout_node_ptr
        isParentInlineContainer:is_parent_inline_container]);

  } @catch (NSException* e) {
    LynxWarning(e, LYNX_ERROR_CODE_LAYOUT,
...
void LayoutContextDarwin::DestroyLayoutNodes(const std::vector<int>& ids) {
  for (int sign : ids) {
...
void LayoutContextDarwin::OnLayoutFinish() { [nodeOwner didLayoutFinished]; }
...
void LayoutContextDarwin::OnUpdateDataWithoutChange() {}
...
void LayoutContextDarwin::SetFontFaces(const CSSFontFaceTokenMap& fontFaces) {
  for (auto fontfaceIter = fontFaces.begin(); fontfaceIter != fontFaces.end(); ++fontfaceIter) {
    ...
    const CSSFontFaceAttrsMap& map = fontfaceIter->second[0].get()->getAttrMap();
