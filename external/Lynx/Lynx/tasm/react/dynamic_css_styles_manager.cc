...
// Font Size is special, it is inherited by computed value.
...
bool IsPropertySimpleInheritable(CSSPropertyID id, const CSSValue& value) {
...
DynamicCSSStylesManager::StyleUpdateFlags GetValueFlags(
    CSSPropertyID id, const CSSValue& value, bool unify_vw_vh_behavior) {
  ...
  switch (value.GetPattern()) {
    ...
    case CSSValuePattern::CALC: {
      const auto& calc_str = value.GetValue().String()->str();
...
DynamicCSSStylesManager::DynamicCSSStylesManager(
    Element* element, const DynamicCSSConfigs& configs)
...
void DynamicCSSStylesManager::UpdateFontSizeStyle(const tasm::CSSValue* value) {
  if (value && *value != font_size_) {
    ...
    if (element_->element_manager()) {
      element_->element_manager()->SetHasPatches();
    }
  }
}
...
void DynamicCSSStylesManager::AdoptStyle(CSSPropertyID css_id,
  ...
  if (new_flags != kNoUpdate) {
    ...
    ForEachFlagDo(new_flags, [&css_id, &value](ValueStorage& storage) {
      storage.insert({css_id, value});
    });
    flag_maps_.insert({css_id, new_flags});
...
void DynamicCSSStylesManager::MarkDirtyInternal() {
  if (!dirty_) {
    dirty_ = true;
    if (element_->parent()) {
      element_->parent()->StylesManager().MarkDirtyInternal();
    }
  }
}

void DynamicCSSStylesManager::UpdateWithResolvingStatus(
    const PropertiesResolvingStatus& status) {
  UpdateWithResolvingStatus(status, InheritedProps(), false, false);
}
...
void DynamicCSSStylesManager::ApplyFontSizeUpdateResolvingData(
    ...
    PropertiesResolvingStatus& next_resolving_data) {
  // high piorities
  float new_font_size = element_->font_size_;
  ...
  if (!font_size_.IsEmpty()) {
    if (font_size_need_update_ || (current_updates & font_size_flags_)) {
      ...
      if (configs_.unify_vw_vh_behavior_) {
        resolved_font_size = starlight::CSSStyleUtils::ResolveFontSize(
            font_size_, lynx_env, lynx_env.ViewportWidth(),
            lynx_env.ViewportHeight(), status.computed_font_size_,
        ...
        resolved_font_size = starlight::CSSStyleUtils::ResolveFontSize(
            ...
            status.computed_font_size_, status.page_status_.root_font_size_,
            configs);
      }
  ...
  if (base::FloatsNotEqual(new_font_size, element_->FontSize())) {
  ...
  if (!element_->parent()) {
    ...
    if (base::FloatsNotEqual(new_font_size, element_->root_font_size_)) {
  ...
  if (font_size_need_update_ || (current_updates & kUpdateEm) ||
    ...
    next_resolving_data.computed_font_size_ = element_->FontSize();
    next_resolving_data.page_status_.root_font_size_ =
        element_->root_font_size_;
  }
...
void DynamicCSSStylesManager::UpdateWithResolvingStatus(
    const PropertiesResolvingStatus& status, const InheritedProps& inherited,
    bool inherited_need_update, bool force_apply_inheritance) {
  ...
  force_apply_inheritance =
      (force_apply_inheritance || force_reapply_inheritance_);
  LynxFatal(element_, LYNX_ERROR_CODE_DOM,
            "Element is null when updating resolving status");
  auto current_updates = GetStatusChanges(status, resolving_data_);
  if (current_updates == kNoUpdate && !dirty_ && !inherited_need_update) {
    return;
  }
  if (current_updates & kUpdateFontScale) {
    element_->computed_css_style()->SetFontScale(
        status.page_status_.font_scale_);
  }
  if (current_updates & kUpdateScreenMetrics) {
    element_->computed_css_style()->SetScreenWidth(
        status.page_status_.screen_width_);
  }
  if (current_updates & kUpdateViewport) {
    element_->computed_css_style()->SetViewportWidth(
        status.page_status_.viewport_width_);
    element_->computed_css_style()->SetViewportHeight(
        status.page_status_.viewport_height_);
  }

  resolving_data_ = status;
  PropertiesResolvingStatus next_resolving_data;
  next_resolving_data.ApplyPageStatus(status);

  ApplyFontSizeUpdateResolvingData(status, current_updates,
                                   next_resolving_data);
  ApplyDirection(status, current_updates, next_resolving_data);
  auto inherited_result =
      ApplyInheritance(inherited, inherited_need_update, current_updates,
                       force_apply_inheritance);

  for (const auto& i : must_updates_) {
    if (!(i.second.second & current_updates)) {
      SetStyeToElement(i.first, i.second.first);
    }
  }
  ForEachFlagDo(current_updates,
                [this](std::map<CSSPropertyID, CSSValue>& storage) {
                  for (auto style : storage) {
                    SetStyeToElement(style.first, style.second);
                  }
                });
  ...
  for (auto* child : element_->GetChild()) {
    child->StylesManager().UpdateWithResolvingStatus(
        next_resolving_data, inherited_result.second, inherited_result.first,
        force_apply_inheritance);
  }
...
DynamicCSSStylesManager::ApplyInheritance(
  ...
  const auto& ApplyProps =
      [this, env_changes, force_apply_inheritance](
          const std::map<CSSPropertyID, InheritablePropsState>& props) {
          ...
          if (prop.second.dirty_ || is_inherited_prop_affected_by_env ||
            ...
            if (prop.second.value_.IsEmpty()) {
              ...
              if (is_style_newly_added ||
                ...
                // if property is newly added or simple inheritable or
                SetStyeToElement(prop.first, prop.second.value_);
              } else if (element_->parent()) {
                // If the property is inherited by computed value.
                const bool value_changed =
                    element_->computed_css_style()->InheritValue(
                        prop.first,
                        *(element_->parent()->computed_css_style()));
  ...
  std::map<CSSPropertyID, InheritablePropsState> new_inherit_root = props.Get();
...
void DynamicCSSStylesManager::ResetAllDirectionAwareProperty() {
  for (const auto& entry : value_storage_[kDirectionStyleType]) {
    ResetStyeToElement(entry.first);
  }
}

void DynamicCSSStylesManager::SetStyeToElement(CSSPropertyID id,
...
void DynamicCSSStylesManager::ResetStyeToElement(CSSPropertyID id) {
  CSSPropertyID trans_id = id;
  trans_id = ResolveDirectionAwareProperty(id, element_->Direction());
