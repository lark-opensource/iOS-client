...
static void MatchKey(lynx::tasm::AttributeHolder* node,
                     const std::vector<RuleData>& list, unsigned level,
                     std::vector<MatchedRule>& matched) {
  for (const auto& rule : list) {
    SelectorMatcher matcher;
    SelectorMatcher::SelectorMatchingContext matchingContext(node);
    matchingContext.selector_ = &rule.Selector();
...
static void MatchKey(
    lynx::tasm::AttributeHolder* node, const std::string& key,
    const std::unordered_map<std::string, std::vector<RuleData>>& map,
    unsigned level, std::vector<MatchedRule>& matched) {
  if (key.empty()) return;
...
void RuleSet::AddToRuleSet(
  ...
  AddStyleRule(std::make_unique<StyleRule>(std::move(selector_array), token));
}

void RuleSet::MatchStyles(lynx::tasm::AttributeHolder* node, unsigned& level,
  ...
  if (node->GetPseudoState() != lynx::tasm::kPseudoStateNone) {
    MatchKey(node, pseudo_rules_, level, output);
  }
  for (const auto& attr : node->attributes()) {
    MatchKey(node, attr.first.c_str(), attr_rules_, level, output);
  }
...
void RuleSet::AddStyleRule(const std::shared_ptr<StyleRule>& rule) {
  ...
  for (unsigned selector_index = 0; selector_index != UINT_MAX;
    ...
    ++rule_count_;
    FindBestRuleSetAndAdd(rule->SelectorAt(selector_index), rule_data);
    if (!fragment_) return;
...
bool RuleSet::FindBestRuleSetAndAdd(const LynxCSSSelector& component,
  ...
  ExtractBestSelectorValues(component, id, class_name, attr_name, attr_value,
...
const LynxCSSSelector* RuleSet::ExtractBestSelectorValues(
    const LynxCSSSelector& component, std::string& id, std::string& class_name,
    std::string& attr_name, std::string& attr_value, std::string& tag_name,
    LynxCSSSelector::PseudoType& pseudo_type) {
  const LynxCSSSelector* it = &component;
  for (; it && it->Relation() == LynxCSSSelector::kSubSelector;
       it = it->TagHistory()) {
    ExtractSelectorValues(it, id, class_name, attr_name, attr_value, tag_name,
                          pseudo_type);
  }
  if (it) {
    ExtractSelectorValues(it, id, class_name, attr_name, attr_value, tag_name,
...
void RuleSet::ExtractSelectorValues(const LynxCSSSelector* selector,
    ...
    case LynxCSSSelector::kPseudoElement:
      switch (selector->GetPseudoType()) {
        ...
        case LynxCSSSelector::kPseudoSelection:
          ...
          break;
        case LynxCSSSelector::kPseudoIs:
        case LynxCSSSelector::kPseudoWhere: {
          const LynxCSSSelectorList* selector_list = selector->SelectorList();
          DCHECK(selector_list);
          if (selector_list->HasOneSelector()) {
            ExtractSelectorValues(selector_list->First(), id, class_name,
                                  attr_name, attr_value, tag_name, pseudo_type);
          }
        } break;
        default:
          break;
