...
std::string CSSDecoder::CSSValueToString(const CSSPropertyID id,
  ...
  if (value.IsEmpty()) {
    ...
    return value.GetValue().String()->str();
  } else if (value.IsEnv()) {
    return value.AsString();
    ...
    LynxWarning(false, LYNX_ERROR_CODE_CLI,
...
std::string CSSDecoder::ToTransformType(lynx::starlight::TransformType type) {
  switch (type) {
    ...
    case TransformType::kSkewY:
      return "skewY";
    default:
      LynxWarning(false, LYNX_ERROR_CODE_ASSET, "%d transform type is invalid.",
                  (int)type);
      return "";
  }
}
...
std::string ToTransitionProperty(const lynx::tasm::CSSValue &value) {
  ...
  for (size_t i = 0; i < group->size(); i++) {
    ...
    AnimationPropertyType animation_property =
        static_cast<AnimationPropertyType>(dict->GetValue("property").Number());
    std::string property =
        CSSDecoder::ToAnimationPropertyType(animation_property);
    res += property;

    long duration = dict->GetValue("duration").Number();
    res += space;
    ...
    if (dict->Contains("timing")) {
      ...
      std::string time_func =
          TimingFunctionToString(dict->GetValue("timing").Array()->get(0));
      res += space;
      res += time_func;
    }

    if (dict->Contains("delay")) {
      long delay = dict->GetValue("delay").Number();
...
void ComputeShadowStyle(
    std::string &prop_result, const std::string &key,
    const lynx::base::scoped_refptr<lynx::lepus::Dictionary> &dict) {
  ...
  for (size_t i = 0; i < group->size(); i++) {
    ...
    if (enable) {
      ...
      if (dict->Contains("blur")) {
        ...
        ComputeShadowStyle(blur_length, "blur", dict);
        res += (blur_length + space);
      }

      if (dict->Contains("spread")) {
        std::string spread_length = "0";
        ComputeShadowStyle(spread_length, "spread", dict);
      ...
      if (dict->Contains("color")) {
        auto color = dict->GetValue("color").Number();
...
std::string CSSDecoder::ToLengthType(lynx::starlight::LengthValueType type) {
  ...
  LynxWarning(false, LYNX_ERROR_CODE_ASSET, "no such enum for type: %d",
              (int)type);
  return "";
}
...
std::string CSSDecoder::ToAnimationPropertyType(
  ...
  switch (type) {
    ...
    case AnimationPropertyType::kTransform:
      return "transform";
    case AnimationPropertyType::kLayout:
      return "layout";
...
std::string CSSDecoder::ToPositionType(PositionType type) {
  switch (type) {
...
std::string CSSDecoder::ToBoxSizingType(BoxSizingType type) {
  switch (type) {
...
std::string CSSDecoder::ToDisplayType(DisplayType type) {
  switch (type) {
...
std::string CSSDecoder::ToWhiteSpaceType(WhiteSpaceType type) {
  switch (type) {
...
std::string CSSDecoder::ToTextAlignType(TextAlignType type) {
  switch (type) {
...
std::string CSSDecoder::ToTextOverflowType(TextOverflowType type) {
  switch (type) {
...
std::string CSSDecoder::ToFontWeightType(FontWeightType type) {
  switch (type) {
...
std::string CSSDecoder::ToFlexDirectionType(FlexDirectionType type) {
  switch (type) {
...
std::string CSSDecoder::ToFlexWrapType(FlexWrapType type) {
  switch (type) {
...
std::string CSSDecoder::ToAlignContentType(AlignContentType type) {
  switch (type) {
...
std::string CSSDecoder::ToJustifyContentType(JustifyContentType type) {
  switch (type) {
...
std::string CSSDecoder::ToFontStyleType(FontStyleType type) {
  switch (type) {
...
std::string CSSDecoder::ToLinearOrientationType(LinearOrientationType type) {
  switch (type) {
...
std::string CSSDecoder::ToLinearGravityType(LinearGravityType type) {
  switch (type) {
...
std::string CSSDecoder::ToLinearLayoutGravityType(
...
std::string CSSDecoder::ToVisibilityType(VisibilityType type) {
  switch (type) {
...
std::string CSSDecoder::ToWordBreakType(WordBreakType type) {
  switch (type) {
...
std::string CSSDecoder::ToDirectionType(DirectionType type) {
  switch (type) {
...
std::string CSSDecoder::ToRelativeCenterType(RelativeCenterType type) {
  switch (type) {
...
std::string CSSDecoder::ToLinearCrossGravityType(LinearCrossGravityType type) {
  switch (type) {
