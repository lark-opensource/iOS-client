...
const std::unordered_map<std::string, CSSColor>& NameColors() {
...
bool CSSColor::ParseNamedColor(const std::string& color_str, CSSColor& color) {
  auto iter = NameColors().find(color_str);
  if (iter != NameColors().end()) {
    color = iter->second;
    return true;
  }
  return false;
...
CSSColor CSSColor::CreateFromHSLA(float h, float s, float l, float a) {
  ...
  return CSSColor{
      clamp_css_byte(css_hue_to_rgb(m1, m2, h + 1.0f / 3.0f) * 255.0f),
      clamp_css_byte(css_hue_to_rgb(m1, m2, h) * 255.0f),
      clamp_css_byte(css_hue_to_rgb(m1, m2, h - 1.0f / 3.0f) * 255.0f), a};
}

unsigned int CSSColor::Cast() {
  unsigned int color =
