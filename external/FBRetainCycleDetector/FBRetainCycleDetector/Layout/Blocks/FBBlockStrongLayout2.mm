...
static inline BOOL _IsBlockByrefStrong(struct Block_byref *aByref) {
    return (aByref->flags & BLOCK_BYREF_LAYOUT_STRONG) == BLOCK_BYREF_LAYOUT_STRONG
      && (aByref->flags & BLOCK_BYREF_HAS_COPY_DISPOSE) == BLOCK_BYREF_HAS_COPY_DISPOSE;
}

static inline const void *_Nullable _GetBlockByrefStrongContent(struct Block_byref *aByref) {
  if ((aByref->flags & BLOCK_BYREF_LAYOUT_STRONG) != BLOCK_BYREF_LAYOUT_STRONG
      || (aByref->flags & BLOCK_BYREF_HAS_COPY_DISPOSE) != BLOCK_BYREF_HAS_COPY_DISPOSE) {
    return NULL;
  }
...
/*
 识别 block 持有对象的原理
 1.https://www.jianshu.com/p/8f02158649c5
 2.Block_descriptor_3中的layout存储了block持有变量的布局
    a.如果layout < 0x1000，则捕获的对象的布局是 strong修饰的对象、__blcok 修饰的对象、weak修饰的对象、其他对象
    b.如果layout >= 0x1000，捕获的对象的类型排列的顺序不一定，但是捕获对象的类型和占用的大小在BLOCK_LAYOUT_ESCAPE所在enum中枚举出来了，例如N words 代表 N 个 sizeof(void *)
 */
