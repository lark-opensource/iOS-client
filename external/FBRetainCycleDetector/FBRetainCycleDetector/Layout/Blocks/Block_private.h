...
enum {
  ...
  BLOCK_HAS_EXTENDED_LAYOUT=(1 << 31)  // compiler block中有访问外部变量和对象
};
...
//获取 block 所有扩展的成员数据需要借助 flags 和 descriptor 中的信息
struct Block_layout {
  void *isa;
  volatile int32_t flags; // contains ref count block对象的一些特性和标志
  int32_t reserved;
  BlockInvokeFunction invoke;
  struct Block_descriptor_1 *descriptor;//指向的结构体后面，根据flags不同，会排列成不同的descriptor，BLOCK_DESCRIPTOR_2 、 BLOCK_DESCRIPTOR_3
