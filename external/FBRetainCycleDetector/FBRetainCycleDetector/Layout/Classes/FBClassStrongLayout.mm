...
static NSIndexSet *FBGetLayoutAsIndexesForDescription(NSUInteger minimumIndex, const uint8_t *layoutDescription) {
  ...
  while (*layoutDescription != '\x00') {
    int upperNibble = (*layoutDescription & 0xf0) >> 4;//非stong的个数
    int lowerNibble = *layoutDescription & 0xf;//strong的个数
...
/*
 class_getIvarLayout 描述了class 持有的变量的布局，根据此确定strong Ivar
...
NSArray<id<FBObjectReference>> *FBGetStrongReferencesForClass(Class aCls) {
  ...
  const uint8_t *fullLayout = class_getIvarLayout(aCls);//本质原理：class 的data字段的 var layout 和 weak layout
...
NSArray<id<FBObjectReference>> *FBGetClassOrObjectStrongReferences(id aClsOrObj,
                                                            NSMutableDictionary<Class, NSArray<id<FBObjectReference>> *> *layoutCache,bool isClass) {
    NSMutableArray<id<FBObjectReference>> *array = [NSMutableArray new];

    __unsafe_unretained Class previousClass = nil;
    __unsafe_unretained Class currentClass = isClass ? aClsOrObj : object_getClass(aClsOrObj);

    while (previousClass != currentClass && currentClass != nil) {
...
NSArray<id<FBObjectReference>> *FBGetObjectStrongReferences(id obj,
    ...
    return FBGetClassOrObjectStrongReferences(obj, layoutCache, false);
}
