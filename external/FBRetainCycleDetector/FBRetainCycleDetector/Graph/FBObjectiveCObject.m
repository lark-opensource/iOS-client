...
- (NSSet *)allRetainedObjects
{
  ...
  Class aCls = object_getClass(anObject);
  if (!anObject || !aCls) {
    return nil;
  }
    
  // 根据 class_getIvarLayout + class_copyIvarList 确定 strong ivar 不能找出容器类型持有的对象，需要特殊处理
      ...
      @try {
        for (id subobject in self.object) {
          ...
          if (isKeyValued && retainsValues) {
            if (!fb_safe_malloc_zone_from_ptr((__bridge void *)subobject)) {//临时尝试解决 Crash，但是也会导致 key 为字符串的漏检测
              NSString *log = [NSStringFromClass(aCls) stringByAppendingFormat:@"%@,%@",[self.namePath componentsJoinedByString:@" -> "],@"the key of container type is Incorrect!!!!!"];
              [[FBRetainCycleAlogDelegate sharedDelegate].delegate findInstanceStrongPropertyAlog:log];
              continue;
            }
