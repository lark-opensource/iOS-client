...
- (instancetype)initWithManager:(LynxLogNotificationManager *)manager
  ...
  UIColor *levelColor;
  if (_level == kLevelWarning) {
    levelColor = warnColor;
  } else if (_level == kLevelError) {
...
- (void)close {
  [_manager closeNotification:_level];
}
...
- (void)hideOnMainThread {
  [self setHidden:YES];
  [_manager hideNotificationOfIndex:_layoutIndex];
}
...
- (instancetype)initWithLogBoxManager:(LynxLogBoxManager *)manager {
  ...
  if (self) {
    ...
    [_tapSelectors setObject:@"tapWarnNotification"
                      forKey:[NSNumber numberWithInteger:kLevelWarning]];
    [_tapSelectors setObject:@"tapErrorNotification"
                      forKey:[NSNumber numberWithInteger:kLevelError]];
  }
  return self;
}
...
- (void)tapWarnNotification {
  [_manager showLogBoxWithLevel:kLevelWarning];
}

- (void)tapErrorNotification {
  [_manager showLogBoxWithLevel:kLevelError];
}
...
- (void)closeNotification:(LynxLogBoxLevel)level {
  [_manager removeLogsWithLevel:level];
