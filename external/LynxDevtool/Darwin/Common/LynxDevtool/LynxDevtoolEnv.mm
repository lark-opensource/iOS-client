...
#import <LynxMacOS/LynxError.h>
#import <LynxMacOS/LynxLog.h>
#endif

#include "LynxDebugBridge.h"
...
- (instancetype)init {
  ...
  if (self) {
     ...
     *   -> [DevtoolRuntimeManagerDarwin init]
...
- (void)initSwitchAttribute {
  ...
  _switchAttrDic = @{
    ...
    SP_KEY_ENABLE_REDBOX : @[ @YES, @NO, @YES ],
    SP_KEY_ENABLE_REDBOX_NEXT : @[ @YES, @NO, @YES ],
#if OS_IOS
    SP_KEY_ENABLE_V8 : @[ @YES, @YES, @NO ],
...
#else
    SP_KEY_ENABLE_V8 : @[ @YES, @YES, @YES ],
#endif
    SP_KEY_ENABLE_DOM_TREE : @[ @YES, @YES, @YES ],
    SP_KEY_ENABLE_PERF_MONITOR_DEBUG : @[ @YES, @NO, @NO ],
...
- (void)initSyncToNative {
  ...
  [self syncToNative:[self v8Enabled] forKey:SP_KEY_ENABLE_V8];
#if OS_IOS
...
- (void)initErrorsCanBeIgnored {
  _errorCodeDic = @{SP_KEY_ENABLE_IGNORE_ERROR_CSS : @(LynxErrorCodeCSS).stringValue};
}
...
- (void)syncToNative:(BOOL)value forKey:(NSString *)key {
  lynx::base::LynxEnv::GetInstance().SetEnv([key UTF8String], value ? true : false);
}

- (void)syncToNative:(BOOL)value
              forKey:(nonnull NSString *)key
           groupName:(nonnull NSString *)groupKey {
  lynx::base::LynxEnv::GetInstance().SetGroupedEnv([key UTF8String], value ? true : false,
                                                   [groupKey UTF8String]);
}
...
- (void)syncToNative:(nonnull NSDictionary *)groupValues forGroup:(nonnull NSString *)groupKey {
  ...
  lynx::base::LynxEnv::GetInstance().SetGroupedEnv(groupSet, [groupKey UTF8String]);
}

- (void)syncMaskToNative:(BOOL)value forKey:(NSString *)key {
  lynx::base::LynxEnv::GetInstance().SetEnvMask([key UTF8String], value ? true : false);
}
...
- (void)setV8Enabled:(BOOL)enableV8 {
  BOOL curValue = [self v8Enabled];
  [self set:enableV8 forKey:SP_KEY_ENABLE_V8];
  [self killAppIfChangeJsEngine:SP_KEY_ENABLE_V8 withValue:curValue];
}

- (BOOL)v8Enabled {
#if OS_IOS
  return [self get:SP_KEY_ENABLE_V8 withDefaultValue:NO];
#else
  return [self get:SP_KEY_ENABLE_V8 withDefaultValue:YES];
#endif
}
...
#if OS_IOS
  if ([key isEqualToString:SP_KEY_ENABLE_V8]) {
    BOOL curValue = [self v8Enabled];
    if (curValue != value) {
