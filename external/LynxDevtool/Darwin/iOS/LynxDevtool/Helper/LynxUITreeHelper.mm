...
- (CGPoint)convertPointFromScreen:(CGPoint)point ToView:(UIView*)view {
  return [[UIApplication sharedApplication].keyWindow convertPoint:point toView:view];
}
...
- (int)findNodeIdForLocationWithX:(float)x withY:(float)y fromUI:(int)uiSign {
  if (_uiOwner) {
    ...
    UIEvent* event = nil;
    CGPoint point_to_view = [self convertPointFromScreen:CGPointMake(x, y) ToView:view];
...
- (NSString*)getLynxUITree {
  NSString* res;
  if (_uiOwner) {
    LynxUI* root = (LynxUI*)[_uiOwner rootUI];
    if (root) {
...
- (NSString*)getUINodeInfo:(int)id {
  NSString* res;
  if (_uiOwner) {
    LynxUI* ui = [_uiOwner findUIBySign:(int)id];
    if (ui != NULL) {
      NSMutableDictionary* dict = [[NSMutableDictionary alloc] init];
      [dict setValue:@(ui.sign) forKey:@"id"];
