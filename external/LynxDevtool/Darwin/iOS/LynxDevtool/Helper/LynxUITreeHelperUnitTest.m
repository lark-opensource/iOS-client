...
- (void)testFindNodeIdForLocation {
  ...
  OCMStub([rootUI lynxView]).andReturn(lynxView);
  ...
  [_uiTreeHelper findNodeIdForLocationWithX:1 withY:1 fromUI:0];
  ...
  [_uiTreeHelper findNodeIdForLocationWithX:1 withY:1 fromUI:1];
  OCMVerifyAll(view);
  ui = nil;
  view = nil;

  OCMStub([_uiOwner findUIBySign:2]).andReturn(nil);
  XCTAssertEqual([_uiTreeHelper findNodeIdForLocationWithX:1 withY:1 fromUI:2], 0);
}
