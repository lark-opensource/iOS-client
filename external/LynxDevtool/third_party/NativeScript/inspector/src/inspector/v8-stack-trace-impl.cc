...
StringView V8StackTraceImpl::topNamedSourceURL() const {
  String16Builder stackTrace;
  for (size_t i = 0; i < m_frames.size(); ++i) {
    const StackFrame& frame = *m_frames[i];
    if (!frame.sourceURL().isEmpty()) {
      return toStringView(frame.sourceURL());
    }
  }
  return StringView();
}

StringView V8StackTraceImpl::topParentSourceURL() const {
  auto asyncParent = m_asyncParent.lock();
  if (asyncParent && (m_maxAsyncDepth > 0)) {
    const auto& frames = asyncParent->frames();
    if (!frames.empty()) {
      return toStringView(frames[0]->sourceURL());
    }
  }
  return StringView();
}

StringView V8StackTraceImpl::topParentNamedSourceURL() const {
  auto asyncParent = m_asyncParent.lock();
  if (asyncParent && (m_maxAsyncDepth > 0)) {
    const auto& frames = asyncParent->frames();
    for (size_t i = 0; i < frames.size(); ++i) {
      const auto& frame = frames[i];
      if (frame && !frame->sourceURL().isEmpty()) {
        return toStringView(frame->sourceURL());
      }
    }
  }
