...
lynx::tasm::CSSPropertyID InspectorCSSHelper::GetPropertyID(
    const std::string& name) {
  static std::unordered_map<std::string, lynx::tasm::CSSPropertyID>
      kPropertyNameMapping = {
#define DECLARE_PROPERTY_NAME(name, c, value) \
  {c, lynx::tasm::kPropertyID##name},
          FOREACH_ALL_PROPERTY(DECLARE_PROPERTY_NAME)
#undef DECLARE_PROPERTY_NAME
      };
  auto it = kPropertyNameMapping.find(name);
  return it != kPropertyNameMapping.end() ? it->second
                                          : lynx::tasm::kPropertyEnd;
}

const std::string& InspectorCSSHelper::GetPropertyName(
    lynx::tasm::CSSPropertyID id) {
  static const std::vector<std::string> kPropertyIDMapping = {
      "",  // start
...
#undef DECLARE_PROPERTY_ID
  ...
  if (id > lynx::tasm::kPropertyStart && id < lynx::tasm::kPropertyEnd) {
...
const std::string& InspectorCSSHelper::ToPropsName(
    lynx::tasm::CSSPropertyID id) {
...
bool InspectorCSSHelper::IsLegal(const std::string& name,
  ...
  return lynx::tasm::UnitHandler::Process(
      lynx::tasm::CSSProperty::GetPropertyID(name),
      lynx::lepus::Value(value.c_str()), output, configs);
}
