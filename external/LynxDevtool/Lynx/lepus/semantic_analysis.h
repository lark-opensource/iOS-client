...
using UpvalueArrayMap =
    std::unordered_map<std::pair<String, uint64_t>, int64_t, pair_hash>;

class LexicalFunction;
struct LexicalBlock {
  std::shared_ptr<LexicalBlock> parent_;
  uint64_t block_id_;
  std::unordered_set<lynx::lepus::String> names_;
  ...
  void AddUpvalue(const base::scoped_refptr<StringImpl>& upvalue_name,
                  uint64_t block_id);
...
class SemanticAnalysis : public Visitor {
 ...
 private:
  ...
  bool InsertName(const lynx::base::scoped_refptr<StringImpl> name);
  size_t GenerateFunctionNumber() {
    function_number_++;
    return function_number_;
  }
  int64_t GenerateBlockNumber() { return ++block_number_; }
  LexicalScoping SearchName(const lynx::base::scoped_refptr<StringImpl> name);
...
#endif  // LYNX_LEPUS_SEMANTIC_ANALYSIS_H_
