...
#ifndef LYNX_LEPUS_QJS_CALLBACK_H_
#define LYNX_LEPUS_QJS_CALLBACK_H_
...
static inline bool LEPUSAtomIsLengthProp(LEPUSContext* ctx, LEPUSAtom prop) {
  LEPUSAtom length = LEPUS_NewAtom(ctx, "length");
  bool ret = (length == prop);
  LEPUS_FreeAtom(ctx, length);
  return ret;
}
...
#endif  // LYNX_LEPUS_QJS_CALLBACK_H_
