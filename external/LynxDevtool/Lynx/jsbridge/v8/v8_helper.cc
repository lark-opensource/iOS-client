...
#if OS_ANDROID
    return piper::Value(value->BooleanValue(ctx).ToChecked());
#else
...
#if OS_ANDROID
    v8::Local<v8::String> str = value->ToString(ctx->GetIsolate());
#else
...
#if OS_ANDROID
    v8::Local<v8::Object> objRef = value->ToObject(ctx->GetIsolate());
#else
...
piper::PropNameID V8Helper::createPropNameID(v8::Local<v8::Name> name,
                                             v8::Isolate* iso) {
  if (name->IsString()) {
    v8::Local<v8::String> v8_str = name->ToString(iso);
...
std::optional<piper::Value> V8Helper::call(piper::V8Runtime* rt,
  ...
  if (isolate) {
    ...
    base::tracing::InstanceCounterTrace::JsHeapMemoryUsedTrace(
...
#if OS_ANDROID
  return createValue(
      handleScope.Escape(result.ToLocalChecked()->ToObject(isolate)), context);
#else
  return createValue(
      handleScope.Escape(
