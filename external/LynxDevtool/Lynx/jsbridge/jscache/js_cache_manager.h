...
#ifndef LYNX_JSBRIDGE_JSCACHE_JS_CACHE_MANAGER_H_
#define LYNX_JSBRIDGE_JSCACHE_JS_CACHE_MANAGER_H_
...
class AbstractJsCacheManager {
 public:
  AbstractJsCacheManager(const AbstractJsCacheManager &) = delete;
  void operator=(const AbstractJsCacheManager &) = delete;
 ...
 protected:
  ...
  virtual std::string CacheDirName() = 0;
  ...
  bool IsCoreJS(const std::string &url);

  bool IsKernelJs(const std::string &url);

  bool IsAppServiceJS(const std::string &url);

  bool IsDynamicComponentServiceJS(const std::string &url);

  bool IsCacheEnabled();

  virtual bool IsJsFileSupported(const std::string &source_url) = 0;

  std::string cache_path_;
  bool can_create_cache_ = true;

  AbstractJsCacheManager() = default;
  virtual ~AbstractJsCacheManager() = default;
};

class JsCacheManager : public AbstractJsCacheManager {
 public:
  /**
 ...
 protected:
   ...
   */
  void SaveCacheToMemory(const std::string &file_md5,
                         std::shared_ptr<Buffer> cache) {
    cache_[file_md5] = std::move(cache);
  }

  virtual bool IsJsFileSupported(const std::string &source_url) override;

  bool IsCacheEnabledForUrl(const std::string &source_url);
...
#ifdef QUICKJS_CACHE_UNITTEST
  std::thread background_thread_for_testing_;
#endif

  JsCacheManager() = default;
  virtual ~JsCacheManager() = default;
};
...
#endif  // LYNX_JSBRIDGE_JSCACHE_JS_CACHE_MANAGER_H_
