...
std::vector<CacheFileInfo> MetaData::GetExpiredCacheFileInfo(
    int64_t expired_time_seconds) const {
  std::vector<CacheFileInfo> expired_cache_files;
  int64_t now = std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::system_clock::now().time_since_epoch())
                    .count();

  ForEachUserCacheFileInfo([&](const CacheFileInfo &cache_file_info) {
    if (cache_file_info.last_accessed + expired_time_seconds < now) {
      expired_cache_files.push_back(cache_file_info);
    }
  });

  return expired_cache_files;
}

void MetaData::ForEachUserCacheFileInfo(
