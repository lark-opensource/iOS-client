...
#ifndef LYNX_TASM_GENERATOR_TTML_HOLDER_H_
#define LYNX_TASM_GENERATOR_TTML_HOLDER_H_
...
class VariableUsageRecorder {
 public:
  ...
  void MarkVariableInUse(const lepus::String &key) {
  ...
  bool IsVariableInUse(const lepus::String &key) {
    return variables_in_use_.find(key) != variables_in_use_.end();
  }

  inline const std::set<lepus::String> &variables_in_use() {
    return variables_in_use_;
  }

 private:
  std::set<lepus::String> variables_in_use_;
};
...
#endif  // LYNX_TASM_GENERATOR_TTML_HOLDER_H_
