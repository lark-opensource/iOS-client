...
#ifndef LYNX_TASM_DIFF_ALGORITHM_H_
#define LYNX_TASM_DIFF_ALGORITHM_H_
...
// https://bytedance.feishu.cn/docs/doccnpIzINQeCgcVtjm8GjJ413c
...
void FindPath(Point top_left, Point bottom_right, Compare1& compare1,
              Compare2& compare2, std::vector<Point>& result) {
  ...
  auto backward_path = std::vector<Point>{};
...
struct DiffResultBase {
 public:
  std::vector<IndexType> insertions_{};
  std::vector<IndexType> removals_{};

  DiffResultBase() = default;

  // Build the DiffResult from the Path Generated by MyersDiff
  template <typename Compare>
  explicit DiffResultBase(const std::vector<detail::Point>& path,
                          Compare compare) {
...
struct DiffResult : public DiffResultBase {
 public:
  std::vector<IndexType> update_from_{};
  std::vector<IndexType> update_to_{};
  std::vector<IndexType> move_from_{};
  std::vector<IndexType> move_to_{};
  ...
  template <typename Compare1, typename Compare2>
  DiffResult(const std::vector<detail::Point>& path, Compare1& compare1,
             Compare2& compare2) {
    for (auto i = size_t{1}; i < path.size(); ++i) {
 ...
 private:
  ...
  void DetectMoves(Compare& compare) {
    ...
    std::vector<IndexType> move_to_sorted = move_to_;
...
CHECK_RESULT std::vector<detail::Point> MyersDiffPath(
  ...
  auto res = std::vector<detail::Point>{top_left};
...
struct CachedComparator {
  ...
  bool operator()(IndexType lhs, IndexType rhs) {
    ...
    if (cached_result_.count(indices_pair)) {
      return cached_result_[indices_pair];
    }
    // cache missed
...
#endif  // LYNX_TASM_DIFF_ALGORITHM_H_
