...
#ifndef LYNX_TASM_RADON_RADON_DISPATCH_OPTION_H_
#define LYNX_TASM_RADON_RADON_DISPATCH_OPTION_H_
...
class ClassTransmitOption {
 ...
 private:
  std::unordered_set<lepus::String> removed_classes_;
  std::unordered_set<lepus::String> added_classes_;
  friend class RadonNode;
};
...
class DispatchOption {
 public:
  ...
  // When trigger style-update in ReactLynx HMR, ignore_cached_style_ will be
  // set to true
  bool ignore_cached_style_{false};
  ...
  // The timing flag carried when the component updates data by js.
  std::string timing_flag_;

  // Indicate the component is update in HMR mode, then we will only ignore
  // lifecycle of current component
  bool component_update_by_hmr_{false};
...
#endif  // LYNX_TASM_RADON_RADON_DISPATCH_OPTION_H_
