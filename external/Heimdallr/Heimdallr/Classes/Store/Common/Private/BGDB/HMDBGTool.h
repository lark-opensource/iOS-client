...
#define hmdbg_completeBlock(obj) !complete?:complete(obj);
#define hmdbg_tableNameKey @"bg_tableName"
#define hmdbg_rowid @"rowid"
#define hmdbg_uniqueKeysSelector NSSelectorFromString(@"bg_uniqueKeys")
#define hmdbg_ignoreKeysSelector NSSelectorFromString(@"bg_ignoreKeys")
#define hmdbg_unionPrimaryKeysSelector NSSelectorFromString(@"bg_unionPrimaryKeys")
...
/**
 判断并执行类方法.
 */
+(id _Nullable)executeSelector:(SEL _Nonnull)selector forClass:(__unsafe_unretained _Nonnull Class)cla;
/**
 判断并执行对象方法.
 */
+(id _Nullable)executeSelector:(SEL _Nonnull)selector forObject:(id _Nonnull)object;
/**
 根据对象获取要更新或插入的字典.
 */
