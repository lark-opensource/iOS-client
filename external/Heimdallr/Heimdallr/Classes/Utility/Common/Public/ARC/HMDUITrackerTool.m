...
static BOOL configMultipleScene;
...
+ (BOOL)sceneBasedSupport {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        configMultipleScene = [maybeDictionary hmd_dictForKey:@"UISceneConfigurations"].count > 1 || [UIApplication hmdSharedApplication].supportsMultipleScenes;
    }
...
+ (BOOL)multiScenesConfig {
    if ([self sceneBasedSupport]) return configMultipleScene;
    return NO;
}
