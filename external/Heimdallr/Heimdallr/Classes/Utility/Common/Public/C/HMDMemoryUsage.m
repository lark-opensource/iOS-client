...
u_int64_t hmd_calculateMemorySizeLevel(u_int64_t memoryByte) {
    u_int64_t memorySizeLevel = (u_int64_t)ceil(memoryByte/(HMD_MEMORY_MB*30.0));
    return memorySizeLevel;
}

u_int64_t hmd_caculateMemorySizeLevel(u_int64_t memoryByte) {
    return hmd_calculateMemorySizeLevel(memoryByte);
}

static void hmd_memory_addRunloopObserver(void) {
...
uint64_t hmd_getDeviceMemoryLimit(void) {
    ...
    if (memory_limit == 0) {
        ...
        memory_limit = atomic_load_explicit(&_physical_footprint_limit,memory_order_acquire);
        if (memory_limit == 0) {
...
int hmd_getTotalMemorySizeLevel(void) {
    ...
    return (int)hmd_calculateMemorySizeLevel(totalMemory);
}
...
/*There's no iPhone5C data on stackoverflow*/
        memoryLimit = 645;
...
/*There's no iPhone8Plus data on stackoverflow*/
        memoryLimit = 1364;
...
/*There's no iPhone11Pro data on stackoverflow*/
        memoryLimit = 2068;
...
/*There's no iPad Mini 3 data on stackoverflow*/
        memoryLimit = 696;
    } else if (strcmp(deviceModel,"iPad5,1") == 0 || strcmp(deviceModel,"iPad5,2") == 0) {
/*There's no iPad Mini 4 data on stackoverflow. The total physical memory of the device is 2GB https://arstechnica.com/gadgets/2015/09/ipad-mini-4-performance-preview-a-1-5ghz-apple-a8-with-2gb-of-ram/*/
        memoryLimit = 1383;
...
/*There's no iPad 5 data on stackoverflow*/
        memoryLimit = 1383;
...
/*There's no iPad 6 data on stackoverflow*/
        memoryLimit = 1383;
...
/*There's no iPad Mini 5 data on stackoverflow*/
        memoryLimit = 2040;
    } else if (strcmp(deviceModel,"iPad11,3") == 0 || strcmp(deviceModel,"iPad11,4") == 0) {
/*There's no iPad Air 3 data on stackoverflow*/
        memoryLimit = 2040;//iPod below
    } else if (strcmp(deviceModel,"iPod1,1") == 0) {
/*There's no iPod Touch 1 data on stackoverflow*/
        memoryLimit = 64;
    } else if (strcmp(deviceModel,"iPod2,1") == 0) {
/*There's no iPod Touch 2 data on stackoverflow*/
        memoryLimit = 64;
    } else if (strcmp(deviceModel,"iPod3,1") == 0) {
/*There's no iPod Touch 3 data on stackoverflow*/
        memoryLimit = 128;
...
/*There's no iPod Touch 6 data on stackoverflow*/
        memoryLimit = 645;
    } else if (strcmp(deviceModel,"iPod9,1") == 0) {
/*There's no iPod Touch 7 data on stackoverflow*/
        memoryLimit = 1396;
    }
    else {
        memoryLimit = (uint64_t)(.5f * physical_memory/HMD_MEMORY_MB);
