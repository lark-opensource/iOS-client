...
//  Pods
//
//  Created by 蔡腾远 on 2018/8/22.
//

#import "HMDThreadSafeArray.h"
...
@implementation HMDThreadSafeArray {
    NSMutableArray *_arr;
...
+ (void)initialize {
    if (self == [HMDThreadSafeArray class]) {
...
- (NSUInteger)indexOfObject:(id)obj inSortedRange:(NSRange)r options:(NSBinarySearchingOptions)opts usingComparator:(NSComparator CF_NOESCAPE)cmp {
...
- (void)sortUsingFunction:(NSInteger (* CF_NOESCAPE)(id, id, void *))compare context:(void *)context {
...
- (void)sortUsingComparator:(NSComparator CF_NOESCAPE)cmptr {
    LOCK([_arr sortUsingComparator:cmptr]);
}

- (void)sortWithOptions:(NSSortOptions)opts usingComparator:(NSComparator CF_NOESCAPE)cmptr {
    LOCK([_arr sortWithOptions:opts usingComparator:cmptr]);
}

- (BOOL)isEqualToArray:(NSArray *)otherArray {
    if (otherArray == self) return YES;
    if ([otherArray isKindOfClass:HMDThreadSafeArray.class]) {
        HMDThreadSafeArray *other = (id)otherArray;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:HMDThreadSafeArray.class]) {
        HMDThreadSafeArray *other = object;
        BOOL isEqual;
        pthread_mutex_lock(&globalLock);
        pthread_mutex_lock(&_lock);
        pthread_mutex_lock(&(other->_lock));
