...
({                                                                                    \
    SEL __aSEL = sel_registerName(#selector);                                         \
    id __result = HMDDynamicCall((object), __aSEL, ## __VA_ARGS__);                   \
...
({                                                                                    \
    SEL __aSEL = sel_registerName(#selector);                                         \
    Class __aClass = objc_getClass(#class);                                           \
    id __result = HMDDynamicCall(__aClass, __aSEL, ## __VA_ARGS__);                   \
    _Pragma("clang diagnostic push")                                                  \
    _Pragma("clang diagnostic ignored \"-Wunused-value\"")                            \
    __result;                                                                         \
    _Pragma("clang diagnostic pop")                                                   \
...
({                                                                                    \
    id __result;                                                                      \
    Class __expectedClass;                                                            \
    Class __objectClass;                                                              \
    id __thisObject = (object);                                                       \
    if((__thisObject) != nil &&                                                       \
    (__expectedClass = objc_getClass(#class)) != nil &&                               \
    (__objectClass = object_getClass(__thisObject)) != nil) {                         \
        do if(__objectClass == __expectedClass) {                                     \
            __result = (__thisObject); break;                                         \
        } while((__objectClass = class_getSuperclass(__objectClass)) != nil);         \
    }                                                                                 \
    __result;                                                                         \
})
...
#if ! defined DC_OB && ! defined DC_CL && ! defined DC_ET && ! defined DC_IS
...
#else
#warning DC_OB DC_CL DC_ET DC_IS already defined Dynamic Call may not work properly
#endif
