...
#if TARGET_CPU_ARM64
// int ret = syscall(SYS_proc_info, 0x9, getpid(), RUSAGE_INFO_V4, 0, (rusage_info_t *)&ru, 0);
__attribute__((always_inline)) int hmd_svc_0x80_proc_info(rusage_info_t *ru) {
    register int x0_callNum asm ("x0") = 0x9;
    register pid_t x1_pid asm ("x1") = getpid();
    register int x2_flavor asm ("x2") = RUSAGE_INFO_V4;
    register int x3_arg asm ("x3") = 0;
    register rusage_info_t *x4_buffer asm ("x4") = ru;
    register int x5_bufferSize asm ("x5") = 0;
    register int x16_syscall_number asm ("x16") = SYS_proc_info; // special syscall number store to x16
    register int x6_ret asm("x6") = 0; // store result
    __asm__ volatile(
                     "svc #0x80\n"
                     "mov x6, x0\n"
                     : "=r"(x6_ret)
                     : "r"(x0_callNum), "r"(x1_pid), "r"(x2_flavor), "r"(x3_arg), "r"(x4_buffer), "r"(x5_bufferSize), "r"(x16_syscall_number)
                     );
    return x6_ret;
}
#endif
...
bool hmd_powerlog_io_info(hmd_powerlog_io_info_data *io_info) {
    ...
    int ret = hmd_svc_0x80_proc_info((rusage_info_t *)&ru);
    ...
    */
    ...
    long long ts = hmd_powerlog_current_ts();
