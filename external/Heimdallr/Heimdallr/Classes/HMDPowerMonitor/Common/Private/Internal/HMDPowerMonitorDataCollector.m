...
{\
    if(event) {\
        ...
        HMDPowerLogInfo(@"%s event : %@",#name,event);\
        [self notifyDataChange:@#name data:event];\
    }\
}

@interface HMDPowerMonitorDataCollector()

//@property (atomic,strong) BDPowerLogNetCollector *netCollector;
@property (nonatomic,strong) NSHashTable *listeners;
@property (nonatomic,strong) HMDPowerMonitorSession *sceneSession;
...
@property (nonatomic,assign) hmd_powerlog_io_info_data lastIOInfo;
...
@implementation HMDPowerMonitorDataCollector
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.work_queue = dispatch_queue_create("hmd_powerlog_data_queue", DISPATCH_QUEUE_SERIAL);
...
- (void)_snapshotForThermal:(BOOL)init {
    ...
    if (@available(iOS 11.0, *)) {
        ...
        dict.hmd_ts = hmd_powerlog_current_ts();
        dict.hmd_sys_ts = hmd_powerlog_current_sys_ts();
        if (init) {
            [dict setValue:@(1) forKey:@"init"];
        }
        NSString *state = HMDPowerLogThermalStateName(NSProcessInfo.processInfo.thermalState);
...
- (void)_snapshotForBatteryLevel:(BOOL)init {
    HMDPowerLogPerformOnMainQueue(^{
        ...
        if (batteryLevel < 0) {
            HMDPowerLogInfo(@"battery level is invalid: %f",batteryLevel);
            return;
        }
...
- (void)_snapshotForBatteryState:(BOOL)init {
    HMDPowerLogPerformOnMainQueue(^{
        ...
        [self performOnWorkQueue:^{
            ...
            NSString *state = HMDPowerLogBatteryStateName(batteryState);
            [dict setValue:state forKey:@"state"];
...
- (void)snapshotForScene:(BOOL)init {
    HMDPowerLogPerformOnMainQueue(^{
        ...
        NSDictionary *subsceneConfig = ((BDPowerLogConfig *)[BDPowerLogManager sharedMonitor].config).subsceneConfig;
        ...
        */
        NSString *scene = [HMDTracker getLastSceneIfAvailable];
        [self performOnWorkQueue:^{
            if (!self.isRunning) {
                return;
            }
            if ([self.sceneSession.identifier isEqualToString:scene]) {
                return;
            }
            HMDPowerMonitorConfig *config = self.config;
            BOOL ignoreSceneUpdateBackgroundSession = !config.includeSceneUpdateBackgroundSession;
            BOOL enableSceneUpdateSession = !config.disableSceneUpdateSession;
            ...
            if (enableSceneUpdateSession) {
                if (self.sceneSession) {
                    if ([self.sceneSession totalTime] < minTime) {
                        [HMDPowerMonitor dropSession:self.sceneSession];
                        BDPL_DEBUG_LOG(@"[PL] Drop Scene Update Session, total time = %lld",self.sceneSession.totalTime);
                    } else {
                        [HMDPowerMonitor endSession:self.sceneSession];
                    }
                }
            }
            ...
            if (enableSceneUpdateSession) {
                self.sceneSession = [HMDPowerMonitor beginSession:@"scene_update"];
                self.sceneSession.config.ignoreBackground = ignoreSceneUpdateBackgroundSession;
                self.sceneSession.identifier = scene;
            }
        }];
    });
}
...
- (void)addGPUEvent {
    ...
    event.hmd_ts = hmd_powerlog_current_ts();
    event.hmd_sys_ts = hmd_powerlog_current_sys_ts();
    [event setValue:@(hmd_powerlog_gpu_usage()) forKey:@"gpu_usage"];
...
- (void)addIOEvent {
    hmd_powerlog_io_info_data io_info;
    if (!hmd_powerlog_io_info(&io_info)) {
        HMDPowerLogInfo(@"io usage error index : %d",self.timer_index);
        return;
    }
    ...
    #undef GET_DIFF_DATA
        ...
        event.hmd_ts = io_info.ts;
        event.hmd_sys_ts = hmd_powerlog_current_sys_ts();
        event.hmd_delta_time = delta_time;
...
- (void)timerFired {
    ...
    long long sys_ts = hmd_powerlog_current_sys_ts();
...
- (void)startTimerNoQueue {
    ...
    dispatch_source_set_timer(self.timer, DISPATCH_TIME_NOW, self.collectInterval * NSEC_PER_SEC, 0);
    WEAK_SELF
...
- (NSArray *)_filterData:(long long)start_sys_ts end:(long long)end_sys_ts array:(NSArray *)array needExtra:(BOOL)needExtra {
    ...
    [array enumerateObjectsUsingBlock:^(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.hmd_end_sys_ts < start_sys_ts) {
            lastItemBefore = obj;
        } else if (obj.hmd_start_sys_ts > end_sys_ts) {
            if (!firstItemAfter) {
                firstItemAfter = obj;
            }
        } else {
...
- (void)notifyDataChange:(NSString *)dataType data:(NSDictionary *)data {
    [self performOnWorkQueue:^{
        if (self.listeners) {
            for (id<HMDPowerMonitorDataListener> listener in self.listeners) {
                if ([listener respondsToSelector:@selector(dataChanged:data:init:)]) {
