...
@interface HDMPowerMonitorInternalSession()
{
    ...
    */
    ...
    hmd_powerlog_io_info_data _begin_io_info;
    hmd_powerlog_io_info_data _end_io_info;
...
@implementation HDMPowerMonitorInternalSession
...
- (instancetype)init {
    if (self = [super init]) {
         ...
         */
        _work_queue = dispatch_queue_create("hmd_powerlog_internal_session_queue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)addCustomEvent:(NSDictionary *)event {
    ...
    [dict setValue:@(hmd_powerlog_current_ts()) forKey:@"ts"];
...
- (void)_begin {
    ...
    _flags.begin_io_info_valid = hmd_powerlog_io_info(&_begin_io_info);
...
- (void)end {
    ...
    [HMDPowerMonitor queryDataFrom:_begin_sys_ts to:_end_sys_ts completion:^(NSDictionary * _Nonnull data) {
...
- (void)_end {
    ...
    _flags.end_io_info_valid = hmd_powerlog_io_info(&_end_io_info);
...
- (NSDictionary *)findTheLastEvent:(NSArray *)events before:(long long)before_sys_ts {
    ...
    [events enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.hmd_sys_ts <= before_sys_ts) {
            result = obj;
            *stop = YES;
        }
    }];
...
- (NSDictionary *)findTheFirstEvent:(NSArray *)events after:(long long)after_sys_ts {
    ...
    [events enumerateObjectsUsingBlock:^(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.hmd_sys_ts >= after_sys_ts) {
            result = obj;
            *stop = YES;
        }
    }];
...
- (NSDictionary *)generateThermalSummaryInfo:(NSArray *)thermalStateEvents {
    if (@available(iOS 11.0, *)) {
        ...
        [thermalStateEvents enumerateObjectsUsingBlock:^(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            long long sys_ts = obj.hmd_sys_ts;
...
- (NSString *)currentUserInterfaceStyleString {
    NSInteger interfaceStyle = [HMDPowerMonitor currentUserInterfaceStyle];
    if (interfaceStyle == 1) {
        return @"light";
    }
