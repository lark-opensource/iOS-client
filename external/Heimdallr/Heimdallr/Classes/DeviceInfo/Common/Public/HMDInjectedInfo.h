...
typedef NS_ENUM(NSInteger, HMDCarrrierPropertyType) {
    HMDCarrrierPropertyCarrierName = 0,   // [CTCarrier carrierName]
    HMDCarrrierPropertyISOCountryCode,    // [CTCarrier isoCountryCode]
    HMDCarrrierPropertyMCC,               // [CTCarrier mobileCountryCode]
    HMDCarrrierPropertyMNC                // [CTCarrier mobileNetworkCode]
};
...
typedef NSArray<NSString*> *_Nullable (^HMDCarrierMessageBlock)(HMDCarrrierPropertyType type);
...
@property (nonatomic, assign, readonly) BOOL canUploadException;
@property (nonatomic, assign, readonly) BOOL canUploadCrash;
@property (nonatomic, assign, readonly) BOOL canUploadFile;
...
//compliance
@property (atomic, copy) NSString * _Nullable scopedDeviceID; /**  */
@property (atomic, copy) NSString * _Nullable scopedUserID;
@property (nonatomic, copy) HMDCarrierMessageBlock _Nullable carrierMessageBlock;/** 注入Carrier相关信息*/

//disk control
@property (nonatomic, assign) BOOL stopWriteToDiskWhenUnhit; /**未命中采样的数据不在落盘，直接丢弃。默认为NO。*/
...
/**
 ...
 @note @p value 的取值可以是任意范围，属于给例如浮点值类型使用，比如 使用时长：0.23小时
 该值在平台只显示，但是不聚类
 */
...
/**
 ...
 @note @p value 的取值必须是 @b 枚举类型 并且枚举的选项需要低于 40 种类型，比如 iOS 大版本号：13
 该值在平台参与聚类计算，可以筛选特定类型，需要消耗计算资源
 */
...
- (void)addHitVid:(NSString *_Nonnull)vid;
