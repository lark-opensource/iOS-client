...
static FileBuffer exception_buffer      = FileBufferInvalid;
static FileBuffer exception_temp_buffer = FileBufferInvalid;

static atomic_bool tmp_finished = false;

static void mark_exception_temp_file_finish(void) {
    atomic_store_explicit(&tmp_finished,true,memory_order_release);
}

static bool is_exception_temp_file_finished(void) {
...
int prepare_exception(void) {
    
    const char *tmp_path = HMDCrashDirectory_exception_tmp_path();
    
    errno = 0;
    FileBuffer fd = hmd_file_open_buffer(tmp_path);
    if(fd == FileBufferInvalid) {
    ...
    exception_temp_buffer = fd;
    mark_exception_temp_file_finish();
    
    return 0;
}

static bool prepare_exception_file_from_temp_file_if_possible(const char * file_path) {
    
    if(!is_exception_temp_file_finished()) return false;
    
    const char *tmp_path = HMDCrashDirectory_exception_tmp_path();
    if(rename(tmp_path, file_path) != 0) return false;
    
    SDKLog("rename temp file success");
    
    if(exception_temp_buffer == FileBufferInvalid) return false;
    
    // access this file is possible
    if(access(file_path, F_OK) != 0) return false;
    
    // file descriptor is valid
    if(fcntl(exception_temp_buffer, F_GETFD) != -1) return false;
    
    exception_buffer = exception_temp_buffer;
    
    DEBUG_ASSERT(exception_buffer != FileBufferInvalid);
...
bool open_exception(void) {
    ...
    if(prepare_exception_file_from_temp_file_if_possible(file_path))
        return true;
    
    exception_buffer = hmd_file_open_buffer(file_path);
    
    DEBUG_ASSERT(exception_buffer != FileBufferInvalid);
    
    if(exception_buffer != FileBufferInvalid)
        return true;
...
void basic_info(hmdcrash_detector_context_t * _Nonnull ctx) {
    uint64_t crash_time = (uint64_t)(ctx->crash_time * 1000);
    hmd_file_begin_json_object(exception_buffer);
    
    hmd_file_write_key(exception_buffer, "exception");
    hmd_file_write_string(exception_buffer, ":");
    
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key_and_uint64(exception_buffer, "crash_time", crash_time);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "fault_address", (uint64_t)ctx->fault_address);

    
    switch (ctx->crash_type) {
        case HMDCrashTypeMachException: {
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_string(exception_buffer, "type", "MACH_Exception");
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_uint64(exception_buffer, "mach_type", ctx->mach.type);
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_uint64(exception_buffer, "mach_code", ctx->mach.code);
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_uint64(exception_buffer, "mach_subcode", ctx->mach.subcode);
        }
            break;
        case HMDCrashTypeNSException: {
            ...
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_string(exception_buffer, "type", "NSException");
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_hex(exception_buffer, "name", name);
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_hex(exception_buffer, "reason", reason);
        ...
        case HMDCrashTypeCPlusPlus: {
            ...
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_string(exception_buffer, "type", "CPP_Exception");
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_hex(exception_buffer, "name", name);
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_hex(exception_buffer, "reason", description);
        }
            break;
        case HMDCrashTypeFatalSignal: {
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_string(exception_buffer, "type", "FATAL_SIGNAL");
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_int64(exception_buffer, "signum", ctx->signal.signum);
            hmd_file_write_string(exception_buffer, ",");
            hmd_file_write_key_and_int64(exception_buffer, "sigcode", ctx->signal.sigcode);
        }
            break;
        default:
    ...
    hmd_file_end_json_object(exception_buffer);
    hmd_file_end_json_object(exception_buffer);
    hmd_file_write_string(exception_buffer, "\n");
}
...
void begin_threads(void) {
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "threads");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_array(exception_buffer);
}
...
void begin_thread(bool is_crashed, bool first_entry) {
    ...
    hmd_file_begin_json_object(exception_buffer);
    if (is_crashed) {
        hmd_file_write_key_and_bool(exception_buffer, "crashed", true);
        hmd_file_write_string(exception_buffer, ",");
    }
}

void begin_register(void) {
    hmd_file_write_key(exception_buffer, "registers");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_object(exception_buffer);
}
...
void register_info(const char *name, uintptr_t value, bool first_entry) {
    ...
    hmd_file_write_key_and_uint64(exception_buffer, name, value);
}
...
void begin_backtrace(void) {
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key(exception_buffer, "stacktrace");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_array(exception_buffer);
}
...
void backtrace_address(uintptr_t address,bool first_entry) {
    ...
    hmd_file_write_uint64(exception_buffer, address);
}
...
void end_backtrace(void) {
    hmd_file_end_json_array(exception_buffer);
}
...
void hmd_crash_begin_stack_record(void) {
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "stack_record");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_object(exception_buffer);
}

void hmd_crash_stack_record_thread_name(char *str) {
    hmd_file_write_key_and_string(exception_buffer, "thread_name", str);
}
...
void begin_runtime_info(void) {
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "runtime_info");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_object(exception_buffer);
}

void write_runtime_sel(const char *sel) {
    hmd_file_write_key_and_string(exception_buffer, "sel", sel);
    hmd_file_write_string(exception_buffer, ",");
}

void begin_crash_infos(void) {
    hmd_file_write_key(exception_buffer, "crash_infos");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_array(exception_buffer);
}
...
void write_crash_info(const char *crash_info, bool first_entry) {
    ...
    hmd_file_write_hex_string_value(exception_buffer, crash_info);
}
...
void begin_dispatch_name(void) {
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "dispatch_name");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_array(exception_buffer);
}
...
void write_dispatch_name(const char *name, bool first_entry) {
    ...
    hmd_file_write_string_value(exception_buffer, name);
}
...
void begin_pthread_name(void) {
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "pthread_name");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_array(exception_buffer);
}
...
void process_stats(void) {
    ...
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key(exception_buffer, "process_stats");
    hmd_file_write_string(exception_buffer, ":");
    hmd_file_begin_json_object(exception_buffer);
    hmd_file_write_key_and_uint64(exception_buffer, "free_bytes", extend_memory.memoryBytes.availabelMemory);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "app_used_bytes", extend_memory.memoryBytes.appMemory);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "total_bytes", extend_memory.memoryBytes.totalMemory);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "used_bytes", extend_memory.memoryBytes.usedMemory);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "total_virtual_memory", extend_memory.totalVirtualMemory);
    hmd_file_write_string(exception_buffer, ",");
    hmd_file_write_key_and_uint64(exception_buffer, "used_virtual_memory", extend_memory.virtualMemory);
    hmd_file_end_json_object(exception_buffer);
    hmd_file_end_json_object(exception_buffer);
    hmd_file_write_string(exception_buffer, "\n");
}
...
void write_storage(void) {
    struct statfs s;
    const char *path = HMDCrashDirectory_NSHomeDirectory_path();
    int ret = statfs(path, &s);
    if (ret == 0) {
        unsigned long long total = s.f_blocks * s.f_bsize;
        unsigned long long free = s.f_bavail * s.f_bsize;
        hmd_file_begin_json_object(exception_buffer);
        hmd_file_write_key(exception_buffer, "storage");
        hmd_file_write_string(exception_buffer, ":");
        hmd_file_begin_json_object(exception_buffer);
        hmd_file_write_key_and_uint64(exception_buffer, "free", free);
        hmd_file_write_string(exception_buffer, ",");
        hmd_file_write_key_and_uint64(exception_buffer, "total", total);
        hmd_file_end_json_object(exception_buffer);
        hmd_file_end_json_object(exception_buffer);
        hmd_file_write_string(exception_buffer, "\n");
    }
...
void close_exception(void) {
    hmd_file_close_buffer(exception_buffer);
    catch_finish();
}
