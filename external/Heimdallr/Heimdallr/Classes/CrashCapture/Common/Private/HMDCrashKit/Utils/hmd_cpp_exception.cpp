...
HMD_EXTERN const void * _Nullable objc_ehtype_vtable[];
...
extern "C"
{
    ...
    static void hmd_cxa_throw(void* thrown_exception, std::type_info* tinfo, void (*dest)(void*)) {
        ...
        if (std::atomic_load_explicit(&enable, std::memory_order_acquire)) {
            bool isOCException = false;
            if (tinfo) {
                struct hmd_simulate_type_info {
                    void * _Nonnull virtual_table;      // virtual table maybe PAC-signed (not exist in arm64.app)
                    void * _Nonnull hidden_name;        // the upper-most mark if it is not unique
                };
                hmd_simulate_type_info *info_content = reinterpret_cast<struct hmd_simulate_type_info *>(tinfo);
                const void * virtual_table = info_content->virtual_table;
                virtual_table = HMD_AARCH64_POINTER_PAC_STRIP(virtual_table);
                if(virtual_table == objc_ehtype_vtable + 2) {
                    isOCException = true;
                }
            }
            if (!isOCException) {//排除OCException，因为OCException有系统记录的Last Exception Backtrace
                /*
