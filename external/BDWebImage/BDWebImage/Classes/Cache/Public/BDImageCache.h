...
@property (nonatomic, copy) NSString *name; ///<缓存的名称，全部转换为小写
...
- (void)removeImageFromDiskForKey:(NSString *)key
                         callBack:(void(^)(NSString *key))callback;
...
/**
 同步取缓存，按照*type尝试缓存方式，优先尝试内存缓存，执行后*type为具体取到的缓存类型
 @param key                         当前的key值sourceKey / targetKey，sourceKey对应的是磁盘缓存，targetKey对应的是内存缓存
 @param type                       尝试查询的缓存类型，优先尝试内存缓存
 @param options                根据options对targetKey做相关操作 / 通过sourceKey得到的data做相关解码操作
 @param size                       如果有降采样，那么size为降采样后的大小
 @param decryptBlock     如果图片加密了，那么磁盘中缓存的是加密后的数据，因此需要先对data进行解密操作，如果不需要进行解密则该项为nil
 @return UIImage                返回通过内存/磁盘取到的图片
 */
- (UIImage *)imageForKey:(NSString *)key
                withType:(BDImageCacheType *)type
                 options:(BDImageRequestOptions)options
...
/**
 异步取磁盘缓存原始数据，回调在主线程
 */
- (void)imageDataForKey:(NSString *)key
              withBlock:(void(^)(NSData *imageData))block;
