...
- (void)viewDidLoad {
    ...
    _logView = [[UITextView alloc] initWithFrame:(CGRect){0, kStatusBarHeight, self.view.bounds.size.width, 0}];
//    _logView.hidden = YES;
...
#ifdef DEBUG
    [[BDImageAdapter sharedAdapter] setRecordBlock:^(NSString * _Nonnull record) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self->_logView.text = record;
...
//    self.segmentControl.frame = CGRectMake(0, kStatusBarHeight, width * 2 / 3, 44);
    ...
    [self.segmentControl setTitleTextAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"Helvetica-Bold" size:18], NSForegroundColorAttributeName:[UIColor blueColor]} forState:UIControlStateSelected];
    ...
    [self.segmentControl mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(self.view).multipliedBy(55 * _titles.count / self.view.bounds.size.width);
    }];
...
//    CGFloat pagey = self.segmentControl.frame.size.height + kStatusBarHeight;
//    self.pageViewControl.view.frame = CGRectMake(0, pagey, width, height);
    ...
    [button setImage:[UIImage imageNamed:@"invalid-name"] forState:UIControlStateNormal];
    ...
    [logButton setImage:[UIImage imageNamed:@"search"] forState:UIControlStateNormal];
    ...
    [logButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(setting.mas_left).with.offset(-10);
        make.top.equalTo(_segmentControl).with.offset(10);
        make.bottom.equalTo(_segmentControl).with.offset(-10);
        make.width.mas_equalTo(22);
    }];
...
- (void)showLog {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self->_logView.frame = (CGRect){0, kStatusBarHeight, self.view.bounds.size.width, hidden ? 200 : 0};
