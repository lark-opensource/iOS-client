...
@interface SDAdapter_Manager_BD_Tests : BaseTestCase

@end

@implementation SDAdapter_Manager_BD_Tests
...
- (void)test_03_cancel_load_image{
    ...
    NSURL *originalImageURL = [NSURL URLWithString:kTestJpegURL];
    id<SDWebImageAdapterOperationProtocol> request = [[SDWebImageAdapter sharedAdapter] loadImageWithURL:originalImageURL
                                                                                                 options:0
                                                                                                progress:nil
                                                                                               completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
...
- (void)test_14_sava_image_to_cache {
    ...
    [[SDWebImageAdapter sharedAdapter] clearDiskOnCompletion:^{
        ...
        // 异步保存
        XCTAssertNil([[SDWebImageAdapter sharedAdapter] imageFromCacheForKey:key]);
