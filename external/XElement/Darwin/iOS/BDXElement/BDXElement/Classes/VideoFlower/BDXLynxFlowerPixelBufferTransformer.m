...
@implementation BDXLynxFlowerPixelBufferTransformer
...
+ (UIImage *)bdx_imageFromCVPixelBufferRefForSystemPlayer:(CVPixelBufferRef)pixelBuffer {
  ...
  CGImageRef cgImage =
      CGImageCreate(width, height, bitPerCompoment, bitPerPixel, bytePerRow, colorSpace, bitmapInfo,
                    provider, NULL, NULL, kCGRenderingIntentDefault);
...
+ (CVPixelBufferRef)bdx_pixelBufferFromImage:(UIImage *)originImage {
  ...
  NSDictionary *optionsDic = [NSDictionary
  ...
  CVPixelBufferCreate(kCFAllocatorDefault, frameWidth, frameHeight, kCVPixelFormatType_32ARGB,
                      (__bridge CFDictionaryRef)optionsDic, &pixelBuffer);
  ...
  CGContextRef contextRef = CGBitmapContextCreate(
      pixelData, frameWidth, frameHeight, bitsPerComponent, bytesPerRow, colorSpaceRef, bitmapInfo);

  CGContextConcatCTM(contextRef, CGAffineTransformIdentity);

  CGRect rect = CGRectMake(0, 0, frameWidth, frameHeight);
