...
#import <TikTokVideoPlayerWrapper/IESVideoPlayer.h>
        ...
        if (videoModel.playUrlString.length > 0) {
            ...
            void(^block)(void) = ^(void) {
                ...
                if (localPath) {
                    ...
                    [strongSelf.player resetVideoID:videoModel.itemID ?: md5String andPlayURLs:@[url.absoluteString]];
                } else {
                    [strongSelf.player resetVideoID:videoModel.itemID ?: md5String andPlayURLs:@[videoModel.playUrlString]];
                }
            };
            
            if ([self.delegate respondsToSelector:@selector(bdx_player:fetchByResourceManager:completionHandler:)]) {
                [self.delegate bdx_player:self
                   fetchByResourceManager:[NSURL URLWithString:videoModel.playUrlString]
                    ...
                    if (localUrl) {
                        [strongSelf.player resetVideoID:videoModel.itemID ?: md5String andPlayURLs:@[localUrl.absoluteString]];
                    } else if (remoteUrl) {
                        [strongSelf.player resetVideoID:videoModel.itemID ?: md5String andPlayURLs:@[remoteUrl.absoluteString]];
                    } else {
                        block();
                    }
                }];
