...
#import "BDXCategoryNumberView.h"

@implementation BDXCategoryNumberView
...
- (Class)preferredCellClass {
    return [BDXCategoryNumberCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray arrayWithCapacity:self.titles.count];
    for (int i = 0; i < self.titles.count; i++) {
        BDXCategoryNumberCellModel *cellModel = [[BDXCategoryNumberCellModel alloc] init];
...
- (void)refreshCellModel:(BDXCategoryBaseCellModel *)cellModel index:(NSInteger)index {
    ...
    BDXCategoryNumberCellModel *myCellModel = (BDXCategoryNumberCellModel *)cellModel;
