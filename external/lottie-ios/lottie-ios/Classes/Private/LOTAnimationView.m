...
- (void)_setupWithSceneModel:(LOTComposition *)model {
  model.referencedAnimationView = self;
  if (_sceneModel) {
  ...
  _compContainer.referencedAnimationView = self;
  
  __weak typeof(self) weakSelf = self;
  void (^callback)(NSError *) = ^(NSError *error) {
    if ([weakSelf.animationDelegate respondsToSelector:@selector(animationView:didLoadResourcesWithError:)]) {
      [weakSelf.animationDelegate animationView:weakSelf didLoadResourcesWithError:error];
    }
  };
  if (_compContainer.asyncLoadingCount > 0) {
    _compContainer.asyncLoadingCallback = callback;
  } else {
    callback(nil);
  }
...
- (void)pause {
  ...
  if ([self.animationDelegate respondsToSelector:@selector(animationViewDidPause:)]) {
    [self.animationDelegate animationViewDidPause:self];
  }
}
...
{
  if ([self.animationDelegate respondsToSelector:@selector(animationViewDidStart:)]) {
    [self.animationDelegate animationViewDidStart:self];
  }
}
