...
- (void)initializeWithChildGroup:(LOTLayerGroup *)childGroup
  ...
  for (LOTLayer *layer in reversedItems) {
    ...
    if (asset.layerGroup) {
      ...
      child = compLayer;
      if (compLayer.asyncLoadingCount > 0) {
        ++self.asyncLoadingCount;
      }
    } else {
      child = [[LOTLayerContainer alloc] initWithModel:layer inLayerGroup:childGroup];
      if (child.asyncLoadingResource) {
        ++self.asyncLoadingCount;
      }
    }
    
    NSUInteger index = [childGroup.layers indexOfObject:layer];
    __weak typeof(self) weakSelf = self;
    child.asyncLoadingCallback = ^(NSError *error) {
      if (error) {
        if (ENABLE_DEBUG_LOGGING) {
          NSLog(@"-------------------- Load resource failed [%ld, %ld]: %@ --------------------", index, weakSelf.asyncLoadingCount, error);
        }
        !weakSelf.asyncLoadingCallback ?: weakSelf.asyncLoadingCallback(error);
      } else {
        if (ENABLE_DEBUG_LOGGING) {
          NSLog(@"-------------------- Load resource succeeded [%ld, %ld] --------------------", index, weakSelf.asyncLoadingCount);
        }
        --weakSelf.asyncLoadingCount;
        if (weakSelf.asyncLoadingCount == 0) {
          !weakSelf.asyncLoadingCallback ?: weakSelf.asyncLoadingCallback(nil);
        }
      }
    };
...
- (void)displayWithFrame:(NSNumber *)frame forceUpdate:(BOOL)forceUpdate {
  ...
  if ([self.referencedAnimationView.animationDelegate respondsToSelector:@selector(animationView:isDisplayingFrame:)]) {
    [self.referencedAnimationView.animationDelegate animationView:self.referencedAnimationView isDisplayingFrame:frame.floatValue];
  }
}
