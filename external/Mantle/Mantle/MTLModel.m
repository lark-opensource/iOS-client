...
// Used to cache the reflection performed in +propertyKeysOfCurrentClass.
static void *MTLModelOwnCachedPropertyKeysKey = &MTLModelOwnCachedPropertyKeysKey;
...
+ (NSSet *)propertyKeys {
    ...
    while (![cls isEqual:MTLModel.class]) {
        [keys addObjectsFromArray:[cls propertyKeysOfCurrentClass]];
        cls = cls.superclass;
    }
...
+ (NSArray *)propertyKeysOfCurrentClass {
    NSArray *cachedKeys = objc_getAssociatedObject(self, MTLModelOwnCachedPropertyKeysKey);
    if (cachedKeys != nil) return cachedKeys;
    ...
    if (properties) {
        for (unsigned i = 0; i < count; i++) {
            ...
            mtl_propertyAttributes *attributes = mtl_copyPropertyAttributes(properties[i]);
            if ([self storageBehaviorForPropertyWithKey:key] != MTLPropertyStorageNone && !attributes->ignore) {
                [keys addObject:key];
            }
            free(attributes);
        }
    ...
    objc_setAssociatedObject(self, MTLModelOwnCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
    return keys;
}
