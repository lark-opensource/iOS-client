...
mtl_propertyAttributes *mtl_copyPropertyAttributes (objc_property_t property) {
    ...
    if (typeString[0] == *(@encode(id)) && typeString[1] == '"') {
        ...
        if (className != next) {
            ...
            char trimmedClass[classNameLength + 1];
            const char *cur = className;
            char *classCur = trimmedClass;
            while (cur < next && *cur != '<')
                *classCur++ = *cur++;
            *classCur = '\0';
            attributes->objectClass = objc_getClass(trimmedClass);
            while (cur < next) {
                if (*cur =='<') {
                    ++cur;
                    char protocolName[classNameLength + 1];
                    char *protocolCur = protocolName;
                    while(cur < next && *cur != '>') {
                        *protocolCur++ = *cur++;
                    }
                    ++cur;
                    *protocolCur = '\0';
                    if (strcmp(protocolName,"Ignore") == 0) {
                        attributes->ignore = YES;
                    }
                    if (!attributes->deducedObjectClass) {
                        attributes->deducedObjectClass = objc_getClass(protocolName);
                    }
                } else {
                    break;
                }
