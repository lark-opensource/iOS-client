...
- (void)_readHTTPHeader;
{
  ...
  [self _readUntilHeaderCompleteWithCallback:^(RCTSRWebSocket *socket,  NSData *data) {
    ...
    if (!strongSelf) {
      RCTSRLog(@"websocket has been deinit");
      return;
    }
...
- (void)_initializeStreams;
{
  ...
  _outputStream.delegate = self;
    ...
        CFStringRef ip = CFDictionaryGetValue(socksConfig, CFSTR("HTTPSProxy"));
        if (ip != nil) {
            CFDictionarySetValue(socksConfig, kCFStreamPropertySOCKSProxyHost, ip);
        ...
            RCTLogInfo(@"websocketlog, socksConfig");

            CFReadStreamSetProperty((CFReadStreamRef)_inputStream, kCFStreamPropertySOCKSProxy, socksConfig);
            CFWriteStreamSetProperty((CFWriteStreamRef)_outputStream, kCFStreamPropertySOCKSProxy, socksConfig);
        }
    }
}
