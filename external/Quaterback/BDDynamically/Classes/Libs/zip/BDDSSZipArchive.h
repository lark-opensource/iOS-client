...
/**
 BBR_ZIP_LATER_VERSION_2: SSZipArchive version > 2.0
 */
 ...
 */
typedef void(^BDDSSZipArchiveProgressUpdateBlock)(int percentage, int filesProcessed, unsigned long numFiles);
...
@protocol BDDSSZipArchiveDelegate <NSObject>
@optional
...
@interface BDDSSZipArchive : NSObject {
@private
	void*           _zipFile;
	void*           _unzFile;
	...
	NSString*       _password;
	__weak id       _delegate;
    BDDSSZipArchiveProgressUpdateBlock _progressBlock;
...
/** a delegate object conforming to BDDSSZipArchiveDelegate protocol */
@property (nonatomic, weak) id<BDDSSZipArchiveDelegate> delegate;
@property (nonatomic, readonly) unsigned long numFiles;
@property (nonatomic, copy) BDDSSZipArchiveProgressUpdateBlock progressBlock;
