...
struct URLRequestContextConfig {
  // Type of HTTP cache.
  // GENERATED_JAVA_ENUM_PACKAGE: com.ttnet.org.chromium.net.impl
  enum HttpCacheType {
    // No HTTP cache.
    DISABLED,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
      // Enable HTTP-DNS.
      bool enable_http_dns,
      // Enable the verbose level log printing.
      bool enable_verbose_log,
      // TTNet home storage path. Android used only.
      const std::string& ttnet_storage_path,
      // TTNetLog file storage location.
      const std::string& net_log_user_file_path,
#endif
  ...
  bool enable_close_quic_sessions_on_ip_change = false;
#else
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Injected root certificates by user.
  std::vector<std::string> certificates;

  // Injected client's certificates and private key.
  std::vector<net::ClientCertInfo> client_cert_info;

  // Enable HTTP-DNS.
  const bool enable_http_dns;

  // Enable verbose level log printing.
  const bool enable_verbose_log;

  // Home storage path for TTNet.
  const std::string ttnet_storage_path;

  // TTNetLog file storage location path.
  const std::string net_log_user_file_path;

  // GetDomain request default value.
  std::string get_domain_default_json;

  // Bypass boe proxy host list for TTNet.
  std::string bypass_boe_json;

  // Store idc settings.
  std::string store_idc_rule_json;

  // proxy config for TTNet.
  std::string ttnet_proxy_config;

  // BOE proxy enabled for TTNet.
  bool boe_proxy_enabled{false};
#endif
...
struct URLRequestContextConfigBuilder {
  ...
  // TTNet home storage path. Android used only.
  std::string ttnet_storage_path = "";

  std::string net_log_user_file_path = "";
#endif
