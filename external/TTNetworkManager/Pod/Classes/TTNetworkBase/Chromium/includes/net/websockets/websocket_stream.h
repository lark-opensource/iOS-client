...
#if BUILDFLAG(TTNET_IMPLEMENT)
  virtual void OnQuicHandshakeStreamCreated(
      WebSocketQuicHandshakeStream* handshake_stream) = 0;
#endif
};
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    // Called before creating URLRequest and get the connection timeout.
    virtual int OnGetConnectionTimeout() const;

    virtual int OnGetRequestFlag() const;
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  virtual HttpResponseInfo GetHttpResponseInfo() const = 0;
#endif
