...
#if BUILDFLAG(TTNET_IMPLEMENT)
enum TTNetworkIsolationFlag {
  INVALID = 0,
  USE_MPA = 1 << 0,
  USE_ALTERNATIVE_NET = 1 << 1,
};
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void add_ttnet_flag(int add_flag) { ttnet_flag_ |= add_flag; }
  void remove_ttnet_flag(int del_flag) { ttnet_flag_ &= ~del_flag; }
  int get_ttnet_flag() const { return ttnet_flag_; }
#endif
...
#else
    return std::tie(top_frame_origin_, frame_origin_, opaque_and_non_transient_,
                    ttnet_flag_) ==
           std::tie(other.top_frame_origin_, other.frame_origin_,
                    other.opaque_and_non_transient_, other.ttnet_flag_);
#endif
  }
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Return the string representation of the key for session cache key.The
  // purpose of this function is to take into account that more scene
  // differentiation may be required in the future.
  std::string ToStringForSessionCache() const;
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  int ttnet_flag_{TTNetworkIsolationFlag::INVALID};
#endif
};
