...
class NET_EXPORT_PRIVATE QuicChromiumClientStream
 ...
 public:
  ...
  class NET_EXPORT_PRIVATE RawStreamDelegate {
   public:
    virtual ~RawStreamDelegate() = 0;

    virtual bool IsSkipReadingHeader() const = 0;
  };

  void SetDelegate(RawStreamDelegate* delegate) { delegate_ = delegate; }
  void SetDefaultDelegate();

 private:
  friend class RawStreamDelegate;
  RawStreamDelegate* delegate_;

  // quic::QuicSpdyStream
  bool FinishedReadingHeaders() const override;
 ...
 public:
  ...
  class NET_EXPORT_PRIVATE Handle {
   public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    quic::ConnectionCloseSource close_source() const;
    std::string error_details() const;
    void SetUnblocked();
    void SetDelegate(RawStreamDelegate* delegate);
    void SetDefaultDelegate();
    ...
    uint64_t OutStandingDataLen() const;
    uint64_t RetransmissionDataLen() const;
    uint64_t encoder_stream_sent_byte_count() const;
#endif
    bool fin_sent() const;
