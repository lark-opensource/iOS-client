...
class NET_EXPORT ClientSocketHandle {
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Try best to fill the LoadTimingInfo before the ClientSocketPool has
  // handed out the socket.
  ...
  void set_has_load_timing_info(bool value) { has_load_timing_info_ = value; }

  SocketPoolInfo& GetSocketPoolInfo() { return socket_pool_info_; }
 ...
 private:
  ...
  bool has_error_request_;

  // Timing information is set when a connection is failed, and
  // |has_error_request_| flag show this value is valid.
  LoadTimingInfo::ConnectTiming error_request_connect_timing_;
