...
class NET_EXPORT_PRIVATE URLRequestHttpJob : public URLRequestJob {
 ...
 protected:
  ...
  void TryFillResponseInfo(HttpResponseInfo* response_info) const override;
  ...
  void SetRequestDelay(base::TimeDelta delay) override;
  ...
  void SetSessionMaxRecvWindowSize(
      int32_t session_max_recv_window_size) override;
#endif
#if BUILDFLAG(ENABLE_MULTINETWORK_ON_MOBILE)
  void GetNetworkIsolationKey(
      NetworkIsolationKey& isolation_key) const override;
#endif
#endif
 ...
 private:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void DoTTNetOptimization();

  void TryRemoveTTNetHeaders();
  // Overridden from URLRequestJob:
  ...
  base::Time set_cookie_start_timestamp_;
  base::Time set_cookie_end_timestamp_;
  base::Time request_delay_start_timestamp_;
  base::Time request_delay_end_timestamp_;

  bool force_match_hpack_optimization_{false};
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_STREAM_ZSTD)
  bool is_zstd_encode_job_{false};
#endif
#endif
