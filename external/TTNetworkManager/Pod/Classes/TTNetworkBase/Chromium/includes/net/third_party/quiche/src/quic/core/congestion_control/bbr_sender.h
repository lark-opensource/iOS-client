...
class QUIC_EXPORT_PRIVATE BbrSender : public SendAlgorithmInterface {
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void UpdateQuicHostConfig(const QuicHostConfig& quic_host_config) override;
#endif
 ...
 private:
  ...
  float probe_rtt_gain_;

  // The CWND gain applied during the PROBERTT phase.
  float probe_rtt_cwnd_gain_;

  // The duration during PROBERTT phase.
  QuicTime::Delta probe_rtt_mode_ms_;

  // PROBE_RTT switch
  bool probe_rtt_disable_;
  ...
  // When slower_startup_ is 1: The gain used in STARTUP after loss has been detected.
  // 1.5 is enough to allow for 25% exogenous loss and still observe a 25% growth
  // in measured bandwidth.
  // When slower_startup_ is 2: decreases the rate in STARTUP by the total number of bytes
  // lost in STARTUP divided by CWND.
  float startup_rate_reduction_gain_;

  // Sum of bytes lost in STARTUP.
  QuicByteCount startup_bytes_lost_;

  // Disable Recovery Window
  bool disable_recovery_state_;
  ...
  float similar_min_rtt_threshold_;
  ...
  QuicTime::Delta min_rtt_expire_s_;
  int32_t startup_full_loss_count_;
  float startup_loss_threshold_;
  bool one_mss_conservation_;
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Whether STARTUP reduces the pacing rate once packet loss has been detected.
  // 0: Disable;
  // 1: Pacing_rate will reduce to startup_rate_reduction_gain_ measured bandwidth.
  // 2: Pacing rate will reduce by the total number of bytes lost in STARTUP
  //  divided by CWND.
  int32_t slower_startup_;
#else
