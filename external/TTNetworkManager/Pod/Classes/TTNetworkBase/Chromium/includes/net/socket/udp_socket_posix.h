...
#include "net/tt_net/quic/tt_quic_helper.h"
#endif
#endif
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_QUIC_RECVMMSG)
class DatagramReadIOBuffer : public IOBufferWithSize {
 public:
  explicit DatagramReadIOBuffer(int size) : IOBufferWithSize(size) {}

  void SetMsgLen(int msg_len) { msg_len_ = msg_len; }

  int GetMsgLen() const { return msg_len_; }

 protected:
  ~DatagramReadIOBuffer() override {}
  int msg_len_{0};
};

class DatagramReadBuffers
    : public base::RefCountedThreadSafe<DatagramReadBuffers> {
 public:
  DatagramReadBuffers(int length, size_t buffer_size);

  const std::vector<scoped_refptr<DatagramReadIOBuffer>>& GetData() const;

 private:
  friend class base::RefCountedThreadSafe<DatagramReadBuffers>;
  ~DatagramReadBuffers();

  std::vector<scoped_refptr<DatagramReadIOBuffer>> read_buffer_list_;
};
...
class NET_EXPORT UDPSocketPosix {
 public:
  ...
  int MultipleRead(DatagramReadBuffers* read_buffers,
                   ...
                   CompletionOnceCallback callback);
#endif
  const quic::QuicWallTime& wall_timestamp() { return wall_timestamp_; }

  uint32_t GetMulticastInterface() const { return multicast_interface_; }

  void SetBeforeConnect(bool enable_wall_timestamp, bool enable_multi_network) {
#if defined(OS_ANDROID)
    enable_wall_timestamp_ = enable_wall_timestamp;
#endif
    // |enable_multi_network_| is default true for TTNet, but should set to
    // false for TTQuicHe socket.
    enable_multi_network_ = enable_multi_network;
  }

  void SetSocketProtocol(int protocol) {
    VLOG(1) << __FUNCTION__ << " proto: " << protocol;
  ...
  void SetRecvErrMsg(bool recv) { recv_error_msg_ = recv; }
  ttnet::TTUdpSocketErrorMsg GetUdpSocketErrorMsg() const { return error_msg_; }
#endif
#if defined(OS_APPLE)
  bool IsReadCallbackEmpty() const { return read_callback_.is_null(); }
#endif
#if BUILDFLAG(ENABLE_MULTINETWORK_ON_MOBILE)
 ...
 private:
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_QUIC_RECVMMSG)
  int RecvFrom(DatagramReadBuffers* read_buffers,
               int buf_len,
               IPEndPoint* address,
               CompletionOnceCallback callback);

  int InternalMultipleRecvFromNonConnectedSocket(
      DatagramReadBuffers* read_buffers,
      int buf_len,
      IPEndPoint* address);
...
#if BUILDFLAG(TTNET_IMPLEMENT)
#if defined(OS_ANDROID)
  bool enable_wall_timestamp_{false};
#endif
  bool enable_multi_network_{true};
  quic::QuicWallTime wall_timestamp_;
  int socket_protocol_{IPPROTO_IP};

#if defined(OS_ANDROID) || defined(OS_APPLE)
  bool recv_error_msg_{false};
  ttnet::TTUdpSocketErrorMsg error_msg_;
#endif
#if BUILDFLAG(ENABLE_MULTINETWORK_ON_MOBILE)
  ...
  TTQuicHelper::Recvmmsg recvmmsg_func_{nullptr};
#endif
#endif
