...
class HttpStreamFactory::JobController
 ...
 public:
  ...
  LoadTimingInfo::RaceResult GetRaceResult() const override;

  void TryGetConnectionAttempts(ConnectionAttempts* out) const override;

  // From HttpStreamFactory::Job::Delegate.
  void OnQuicEnabled() override;

  void OnQuicStreamReady() override;

  void OnQuicStreamFailedWithoutSession() override;

  HttpStreamRequest::Delegate* delegate() override;

  // Called when race between jobs comes to a result.
  void ReportRaceResult(LoadTimingInfo::RaceResult result) const;
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  mutable LoadTimingInfo::QuicInfo::QuicConnectStatus quic_connect_;
  mutable LoadTimingInfo::RaceResult quic_attempt_;
#endif
