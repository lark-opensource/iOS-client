...
class NET_EXPORT UploadDataStream {
 public:
  ...
  bool IsEOF() const;
#if BUILDFLAG(TTNET_IMPLEMENT)
  bool TryMarkCompressFlag(const GURL& url, const std::string& header);

  bool ShouldCompress() const { return should_compress_; }

  int CompressRequestBodyBuffer(IOBuffer* input_buffer,
                                int result,
 ...
 private:
  ...
  std::unique_ptr<CompressDataStream> compress_stream_;
 ...
 public:
  ...
  int64_t GetThrottleNetSpeed() const { return throttle_net_speed_; }

 private:
  int64_t throttle_net_speed_{0L};
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  void OnReadCompleted(int result, bool sync_read = false);
#else
