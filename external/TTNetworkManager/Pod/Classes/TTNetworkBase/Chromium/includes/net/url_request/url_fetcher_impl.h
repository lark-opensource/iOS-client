...
class NET_EXPORT_PRIVATE URLFetcherImpl : public URLFetcher {
 public:
  ...
  void SetRequestTimeout(int request_timeout) override;
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_REQUEST_DELAY)
  void SetDelayTime(int delay_mills) override;
#endif
  void SetSocketTimeout(int connect_timeout,
                        int read_timeout,
                        int write_timeout) override;
  const std::string& GetRequestLog() const override;
  std::string GetRequestDetails() override;
  const LoadTimingInfo* GetLoadTimingInfo() const override;
  void SetPriority(RequestPriority priority) override;
  int GetCacheStatus() const override;
  ...
  void SetSecurityCallbackStart(base::TimeTicks timeticks) const override;
  void SetSecurityCallbackEnd(base::TimeTicks timeticks) const override;
  std::unique_ptr<base::ListValue> GetUrlChainsList() override;
  std::string GetResponseHeadersStr() override;
  std::string GetRequestHeadersJsonStr() const override;
  std::string GetResponseHeadersJsonStr() const override;
  void SaveResponseToFileAtPathByAppend(
      const base::FilePath& file_path,
      scoped_refptr<base::SequencedTaskRunner> file_task_runner) override;

  void SetThrottleNetSpeed(const int64_t bytesPerSecond) override;
  const HttpResponseInfo* GetResponseInfo() const override;
  void SetRequestTypeFlags(uint32_t type) override;
  void set_is_internal_retry_req(bool val) override;

  void FollowDeferredRedirect(
      const base::Optional<std::vector<std::string>>& removed_headers,
      const base::Optional<std::map<std::string, std::string>>&
          modified_headers,
      const std::string& redirect_url) override;
#endif
