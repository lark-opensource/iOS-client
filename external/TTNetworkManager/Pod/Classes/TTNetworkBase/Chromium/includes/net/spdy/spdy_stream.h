 ...
 public:
  ...
  class NET_EXPORT_PRIVATE Delegate {
   public:
    ...
    // Use to extract received data.
    virtual size_t OnPreviousDataReceived(IOBuffer* buf, int buf_len);
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  base::TimeTicks response_timeticks() const { return recv_first_byte_time_; }
  bool IsRemoteClosed() const { return io_state_ == STATE_HALF_CLOSED_REMOTE; }
  Delegate* delegate() const { return delegate_; }
  Delegate* ChangeDelegate(Delegate* delegate);
  bool HasPendingSendData() const { return pending_send_data_ != nullptr; }

  LoadTimingInfo::Http2Info& http2_info() { return http2_info_; }

  void FillHttp2Info(LoadTimingInfo* load_timing_info);

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  // The value of |stream_max_recv_window_size| only be default value
  // which is 6MB or a positive number greater than 6KB.
  void AdjustRecvWindowSize(int stream_max_recv_window_size);
#endif

 private:
  LoadTimingInfo::Http2Info http2_info_;
