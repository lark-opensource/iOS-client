...
class TTNET_IMPLEMENT_EXPORT NET_EXPORT URLFetcher {
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  virtual std::string GetRequestDetails();

  virtual const LoadTimingInfo* GetLoadTimingInfo() const;

  virtual void SetPriority(RequestPriority priority);

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_REQUEST_DELAY)
  virtual void SetDelayTime(int delay_mills);
#endif

  // Cancel the request.
  virtual void Cancel(int error) = 0;

  virtual void SetThrottleNetSpeed(const int64_t bytesPerSecond) = 0;

  // Return the request log content. Should be used after the request end.
  virtual const std::string& GetRequestLog() const = 0;

  // Socket level timeout control.
  virtual void SetSocketTimeout(int connect_timeout,
                                int read_timeout,
                                int write_timeout) = 0;

  // Request duration timeout control.
  virtual void SetRequestTimeout(int request_timeout) = 0;

  virtual int GetCacheStatus() const = 0;

  virtual bool WasFetchedViaProxy() const = 0;

  virtual void SetAuthCredentials(const std::string& username,
                                  const std::string& password) = 0;

  // Record time consumed by calling |addSecurityFactor| callback.
  virtual void SetSecurityCallbackStart(base::TimeTicks timeticks) const = 0;
  virtual void SetSecurityCallbackEnd(base::TimeTicks timeticks) const = 0;

  virtual std::unique_ptr<base::ListValue> GetUrlChainsList() = 0;

  virtual std::string GetResponseHeadersStr() = 0;

  virtual std::string GetRequestHeadersJsonStr() const = 0;
  virtual std::string GetResponseHeadersJsonStr() const = 0;

  // For writing file by appending in file thread
  virtual void SaveResponseToFileAtPathByAppend(
      const base::FilePath& file_path,
      scoped_refptr<base::SequencedTaskRunner> file_task_runner) = 0;

  virtual const HttpResponseInfo* GetResponseInfo() const = 0;

  virtual void SetRequestTypeFlags(uint32_t type) = 0;

  virtual void set_is_internal_retry_req(bool val) = 0;

  virtual void FollowDeferredRedirect(
      const base::Optional<std::vector<std::string>>& removed_headers,
      const base::Optional<std::map<std::string, std::string>>&
          modified_headers,
      const std::string& redirect_url) = 0;
