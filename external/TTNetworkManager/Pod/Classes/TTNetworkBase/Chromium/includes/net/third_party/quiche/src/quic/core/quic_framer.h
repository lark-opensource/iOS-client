...
#include "net/tt_net/fec/quic_fec_framework_receiver.h"
#endif
...
class QUIC_EXPORT_PRIVATE QuicFramerVisitorInterface {
 public:
  ...
  // Record ack_delay_time.
  virtual void SetAverageAckDelayTime(uint64_t) = 0;
#endif
  ...
  virtual void SetLastPacketRevived(bool last_packet_revived) {}
#endif
};
...
class QUIC_EXPORT_PRIVATE QuicFramer
    : public QuicFecFrameworkReceiver::DebugDelegate {
#else
class QUIC_EXPORT_PRIVATE QuicFramer {
#endif
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
  bool AppendFecHeader(const uint32_t source_payload_id,
                       const uint8_t type,
                       QuicDataWriter* write);
  bool AppendIetfFecHeader(const uint32_t source_payload_id,
                           const uint8_t type,
                           QuicDataWriter* writer);
  void EnableFecReceiver(QuicFecScheme scheme, QuicConnectionStats* stats);
  bool ProcessFecFrame(QuicDataReader* reader, QuicFecFrame* frame);
  bool ProcessFecHead(QuicDataReader* reader);
  ...
  void ProcessRevivedPacket();

 private:
  bool fec_receive_enable_ = false;
  QuicConnectionStats* stats_ = nullptr;
  QuicFecFrameworkReceiver fec_receiver_{this};
  // the last packets was revived from FEC scheme
  std::vector<quic::QuicFecData*> last_revived_packets_;
#endif
