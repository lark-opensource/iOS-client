 ...
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    // Get the dns request related job's info
    virtual std::string GetDnsInfoAsJsonValue() const;
    virtual int GetCacheStaleReason() const;
#endif
  };
  ...
  struct NET_EXPORT ResolveHostParameters {
    ...
    enum TTFlag {
      ...
      // If both are set, |force_use_default_network| will be preferred.
      FORCE_USE_DEFAULT_NETWORK = 1 << 1,
      FORCE_USE_CELLULAR = 1 << 2,
      FORCE_USE_WIFI = 1 << 3,
#endif
      IS_INTERNAL_RETRY_REQ = 1 << 4,
      IS_CROSS_SP_REQ = 1 << 5
    };
    ...
    // enable race stale cache for this dns request
    bool enable_race_stale_cache = false;
    // this dns request is from quic
    bool from_quic = false;
#endif
  };
  ...
  // TTNet implemented new host resolver.
  static std::unique_ptr<HostResolver> CreateTTNetWiseResolver(
      NetLog* net_log,
      base::Optional<ManagerOptions> options = base::nullopt,
      bool enable_caching = true);

  virtual void RemoveCacheEntry(const HostPortPair& host,
                                const ResolveHostParameters& parameters);

  virtual void GetCurrentDnsConfig(DnsConfig& dns_config) const;
  virtual void SetHttpDnsEnabled(bool enabled,
                                 URLRequestContextGetter* context_getter);
  virtual bool IsHttpDnsEnabled() const;
  static DnsQueryType AddressFamilyToDnsQueryType(AddressFamily address_family);
  virtual void EnablePreloadBatchHttpdns(
      const std::vector<std::string>& preload_httpdns_host_list);
  virtual void HoldOrphanDnsJob(
