...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // HttpStreamRequest::Delegate methods:
  void OnCollectQuicInfo(int64_t srtt,
                         const std::string& client_connection_id,
                         const std::string& server_connection_id,
                         const quic::QuicConnectionStats& stats) override;
  base::WeakPtr<HttpNetworkTransaction> GetWeakPtr() override;

  // HttpTransaction methods:
  void GetSocketPoolInfo(SocketPoolInfo* socket_pool_info) const override;
  const HttpResponseInfo* TryFillResponseInfo() const override;
  void TryFillLoadTimingInfo(LoadTimingInfo* load_timing_info) const override;
  void TryFillHttp2Info(LoadTimingInfo* load_timing_info) const override;
  void TryFillCompressInfo(LoadTimingInfo* load_timing_info) const override;
  void TryFillQuicInfo(LoadTimingInfo* load_timing_info) const override;
  int GetRetryAttempts() const override;
  std::vector<int> GetNetErrorListForReset() const override;

  // Inner methods:
  // Stats delay TCP time for 1.5*RTT, set RTT or none.
  void OnSetQuicDelayTcpType(LoadTimingInfo::DelayTcp type);
  void OnSetQuicDelayTcpTime(int64_t time);

  void OnQuicErrorCode(quic::QuicErrorCode error);

  // Record whether this stream is session or connection's first stream.
  void OnIsFirstStream(bool is_first_stream);
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_PROXY_SUPPORT)
  void DisallowTTNetProxyFallback() override;
  bool IsTTNetProxyFallbackAllowed() const override;
#endif
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void ResetConnectionAndRequestForResend(int error);
#else
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  int HandleIntentionallyRetry(int error);

  bool ShouldMarkInternalRetry(int error) const;

  bool ShouldIntentionallyRetryOnError(int error, int& retry_delay);

  bool ShouldRetryForceHttpdnsOnError(int error) const;

  bool ShouldRetrySkipPreferIpOnError(int error) const;

  void ReportRaceResult(LoadTimingInfo::RaceResult result);
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  base::Time send_start_timestamp_;
  base::Time send_end_timestamp_;
  base::Time read_headers_start_timestamp_;
  base::Time read_headers_end_timestamp_;

  base::Time create_stream_start_timestamp_;
  base::Time create_stream_finish_timestamp_;

  base::Time http_transaction_start_timestamp_;

  bool dns_cross_sp_action_hit_{false};
#endif
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_PROXY_SUPPORT)
  // True when the state-machine is in "fallback_zone", which means it is on
  // some state point which is after STATE_CREATE_STREAM and before
  // STATE_READ_HEADERS_COMPLETE. If the error happens out of the
  // "fallback_zone", we won't do fallback. Added for ttnet proxy fallback.
  bool is_in_ttnet_proxy_fallback_zone_{false};

  // Identify whether it is a ttnet proxy request.
  bool is_ttnet_proxy_enabled_{false};

  // When this value is true, it will do fallback to give another try if error
  // happens in "fallback zone". Added for ttnet proxy fallback.
  bool allow_ttnet_proxy_fallback_{true};

  TTNetProxyInfo ttnet_proxy_info_;

#endif
  bool has_tried_force_httpdns_{false};
  int dns_hijacked_by_error_{0};

  bool has_tried_skip_prefer_ip_{false};
  int skip_prefer_ip_by_error_{0};

  // The socket pool info is saved for the request.
  SocketPoolInfo socket_pool_info_;

  // When creating stream meets an error, keep the timing info here
  // so that we can retrieve later.
  LoadTimingInfo error_stream_timing_info_;

  // Network error which caused the transaction reset and retried.
  // Default is OK, which means no reset and resend happened.
  std::vector<int> error_for_reset_list_;

  mutable LoadTimingInfo::QuicInfo quic_info_;
