 ...
 public:
  ...
  class NET_EXPORT_PRIVATE Delegate {
   public:
    ...
    // Called when QUIC is enabled.
    virtual void OnQuicEnabled() = 0;

    // Called when QUIC stream is created successfully.
    virtual void OnQuicStreamReady() = 0;

    // Called when QUIC start to create a stream but session is closed.
    virtual void OnQuicStreamFailedWithoutSession() = 0;

    virtual HttpStreamRequest::Delegate* delegate() = 0;
#endif
  };
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  bool force_http11_{false};
  bool force_new_socket_{false};
  bool ignore_sockets_limit_{false};
#endif
