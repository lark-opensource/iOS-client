...
class NET_EXPORT HttpRequestHeaders {
 public:
  ...
  class TTNET_IMPLEMENT_EXPORT NET_EXPORT Iterator {
   public:
    ...
    // Returns true if there has a next header.
    bool HasNext() const;
#endif
  ...
  TTNET_IMPLEMENT_EXPORT bool HasHeader(const base::StringPiece& key) const {
  ...
  TTNET_IMPLEMENT_EXPORT void SetHeader(const base::StringPiece& key,
                                        const base::StringPiece& value);
  ...
  TTNET_IMPLEMENT_EXPORT void RemoveHeader(const base::StringPiece& key);
  ...
  // Headers string by form of key and values (key1=xxx&key2=xxx&key3=xxx ...).
  // Keys can be duplicate for multi values.
  std::string ToSandboxString() const;
#endif

  std::string ToJsonString() const;
  // Removes all headers that start with (case insensitive) |key|.
  void RemoveHeaderByPrefix(const base::StringPiece& key);
#endif

 private:
  HeaderVector::iterator FindHeader(const base::StringPiece& key);
  TTNET_IMPLEMENT_EXPORT HeaderVector::const_iterator FindHeader(
