...
class QUIC_EXPORT_PRIVATE QuicSession
 ...
 public:
  ...
  // Collect QUIC connection error details.
  ConnectionCloseSource close_source() const { return close_source_; }

  QuicStreamCount GetNumUsedStreams() const;

  void OnLatestPingSent() override;

  void OnLatestPingAcked() override;
 private:
  // Collect QUIC connection error details.
 ...
 public:
  void SetUpdateWindowSizeStatus(bool update) {
    has_update_window_size_ = update;
  }

  bool HasUpdateWindowSize() const { return has_update_window_size_; }

  QuicStreamOffset GetInitialWindowSize() const { return initial_window_size_; }

  void SetThrottle(size_t throttle);

  size_t GetThrottle() const { return throttle_; }

 protected:
  void SetInitialWindowSize(QuicStreamOffset size) {
    initial_window_size_ = size;
  }

  // The default max receive window size is 15MB.
  QuicStreamOffset initial_window_size_{15 * 1024 * 1024};
  bool has_update_window_size_{false};
  size_t throttle_{0};
#endif
#endif
