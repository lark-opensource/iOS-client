...
class NET_EXPORT HostCache {
 public:
#if BUILDFLAG(TTNET_IMPLEMENT)
  enum NetworkType : int { NET_UNSPECIFIED = 0, NET_CELLULAR, NET_WIFI };
#endif
  ...
  struct NET_EXPORT Key {
    ...
    auto GetTuple(const Key* key) const {
      return std::tie(key->dns_query_type, key->host_resolver_flags,
                      ...
                      key->network_isolation_key, key->secure, key->net_type,
                      key->mnc_mcc);
    }
#else
    auto GetTuple(const Key* key) const {
    ...
    bool secure = false;
#if BUILDFLAG(TTNET_IMPLEMENT)
    NetworkType net_type{NET_UNSPECIFIED};
    std::string mnc_mcc;
#endif
  };
  ...
  class NET_EXPORT Entry {
   public:
    enum Source : int {
      ...
      SOURCE_PREFER_IP,
      SOURCE_PROC,
      SOURCE_PRELOAD_BATCH_HTTPDNS,
      SOURCE_ASYNC_BATCH_HTTPDNS,
#endif
    };
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    base::Optional<AddressList>& GetMutableAddresses() { return addresses_; }
    const std::string SerializeToString() const;
    // Serialize ip list to string without ttl
    const std::string SerializeToSimpleString() const;
    static bool DeserializeFromString(const std::string& value, Entry* entry);
    bool FromHttpDnsStaleCache() const { return from_httpdns_stale_cache_; }
    void SetFromHttpDnsStaleCache(bool from) const {
      from_httpdns_stale_cache_ = from;
      if (addresses_) {
        addresses_->set_from_stale_cache(from);
      }
    }

    void SetFromMultiNetworkStaleCache(bool from) const {
      if (addresses_) {
        addresses_->set_from_stale_cache(from);
      }
    }
    void SetNetType(int net_type) const {
      if (addresses_) {
        addresses_->set_net_type(net_type);
      }
    }
#endif
   ...
   private:
    ...
    void set_network_changes(int network_changes) {
      network_changes_ = network_changes;
    }

    void set_expires(base::TimeTicks time) { expires_ = time; }
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    mutable bool from_httpdns_stale_cache_{false};
#endif
  };
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  std::unique_ptr<base::DictionaryValue> GetFileCacheLoadInfo(
      const std::string& host) const;
  void set_persistent_host_cache_wildchar_hosts(
      const std::vector<std::string>& hosts) {
    persistent_host_cache_wildchar_hosts_ = hosts;
  }
  void set_first_saving_interval(int interval) {
    first_saving_interval_ = interval;
  }

  void set_repeat_saving_interval(int interval) {
    repeat_saving_interval_ = interval;
  }

  void set_count_limit(int limit) { count_limit_ = limit; }
  void set_stale_interval(int interval) { stale_interval_ = interval; }

  void set_persistent_sync_load_wildchar_hosts(
      const std::vector<std::string>& hosts) {
    persistent_sync_load_wildchar_hosts_ = hosts;
  }
  void set_sync_load_host_cache_count_limit(int limit) {
    sync_load_host_cache_count_limit_ = limit;
  }

  void Remove(const Key& key);
  void EnablePersistentStorage(
      const std::string& storage_dir_path,
 ...
 private:
  ...
  void AsyncLoadEntriesFromFile();
  void SyncLoadEntriesFromFile();
  void ParseAndUpdateFileCache(const std::string& entries_in_string,
                               bool from_async_mode);
  bool LoadResolvingEntryKey(const std::string& key_string, Key* key);
  void SyncEntriesToFile();
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // The cache entries will be saved into file according to the
  // |first_saving_interval_| for the first time and then periodically after
  // |repeat_saving_interval_|.
  std::vector<std::string> persistent_host_cache_wildchar_hosts_;
  int first_saving_interval_{20};
  int repeat_saving_interval_{60 * 10};
  int count_limit_{100};
  int stale_interval_{60 * 30};
  std::vector<std::string> persistent_sync_load_wildchar_hosts_;
  int sync_load_host_cache_count_limit_{5};

  base::OneShotTimer first_saving_timer_;
  base::RepeatingTimer repeat_saving_timer_;

  base::FilePath storage_file_path_;
  base::FilePath sync_storage_file_path_;

  std::unique_ptr<base::ImportantFileWriter> file_writer_;
  std::unique_ptr<base::ImportantFileWriter> sync_file_writer_;

  scoped_refptr<base::SingleThreadTaskRunner> file_task_runner_;
  scoped_refptr<base::SingleThreadTaskRunner> network_task_runner_;

  bool async_file_cache_load_finished_{false};
  int64_t async_file_cache_load_size_{-1};
  int64_t async_file_cache_load_time_cost_{-1};

  bool sync_file_cache_load_finished_{false};
  int64_t sync_file_cache_load_size_{-1};
  int64_t sync_file_cache_load_time_cost_{-1};

  int64_t file_cache_load_start_time_{0};
#endif
