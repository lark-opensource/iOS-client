...
class NET_EXPORT_PRIVATE SocketPerformanceWatcher {
 public:
  ...
  // Set and Get the watching group reference for |this| watcher. NQE can base
  // on the reference value to have observation for different group.
  virtual void SetWatchingGroupReference(const std::string& reference) = 0;

  virtual std::string GetWatchingGroupReference() const = 0;
#endif
};
