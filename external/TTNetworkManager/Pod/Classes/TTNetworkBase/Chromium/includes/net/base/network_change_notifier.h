...
class NET_EXPORT NetworkChangeNotifier {
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    virtual void OnNetworkConnectedWithConnectionType(
        NetworkHandle network,
        ConnectionType connection_type,
        bool is_vpn) {}
    virtual void OnNetworkDisconnectedWithConnectionType(
        NetworkHandle network,
        ConnectionType connection_type,
        bool is_vpn) {}
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
 public:
  struct NetworkHandleWithType {
    NetworkHandle handle{kInvalidNetworkHandle};
    ConnectionType type{CONNECTION_UNKNOWN};
    bool is_vpn{false};
  };

  class NET_EXPORT RunInBackgroundObserver {
   public:
    virtual void OnRunInBackground() = 0;

   protected:
    RunInBackgroundObserver();
    virtual ~RunInBackgroundObserver();

   private:
    friend NetworkChangeNotifier;
    scoped_refptr<base::ObserverListThreadSafe<RunInBackgroundObserver>>
        observer_list_;

    DISALLOW_COPY_AND_ASSIGN(RunInBackgroundObserver);
  };

  typedef std::vector<NetworkHandleWithType> NetworkWithTypeList;

  static void GetConnectedNetworksWithType(NetworkWithTypeList* network_list);

  static bool IsBlockConnection();

  virtual void GetCurrentConnectedNetworksWithType(
      NetworkWithTypeList* network_list) const;

#if defined(OS_IOS)
  virtual std::string GetCurrentMccMnc() const {
    return "";
  }
#endif

#if defined(OS_ANDROID) || BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  static int GetForceUpdateNetworkTypeCount() {
    return force_update_network_type_count_;
  }
  static void ForceUpdateNetworkType();
#endif
  static bool IsCellularOrWifiAvailable();
  static bool IsCellular();
  static bool IsWifi();
  static std::string GetTelephonyNetworkOperator();
  static std::string GetWifiTag();

  static NetworkChangeNotifier* GetNetworkChangeNotifier();
  static void SetNetworkChangeNotifier(NetworkChangeNotifier* notifier);

  // Set the last network changed event happened time on the network thread.
  // This can just called by one module who listens the OnNetworkChange() event
  // from network thread, so anywhere on network thead can get this value by
  // calling GetNetworkChangeEventTime();
  static void SetNetworkChangeEventTimeOnNetworkThread(base::Time time);

  // Retrieve the last network changed event happened time.
  static base::Time GetNetworkChangeEventTime();
  static void AddRunInBackgroundObserver(RunInBackgroundObserver* observer);
  static void RemoveRunInBackgroundObserver(RunInBackgroundObserver* observer);
  static void NotifyObserversOfRunInBackgroundChange();
  void NotifyObserversOfRunInBackgroundImpl();
...
#if defined(OS_ANDROID) || BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  virtual void ForceUpdateNetworkTypeInfo() const;
#endif
  virtual void SetNetworkChangeTime(base::Time time);
  virtual base::Time GetLastNetworkChangeTime() const;
#if BUILDFLAG(ENABLE_NQE_AND_WIFI_TO_CELL_ON_MOBILE)
  friend class TTMultiNetworkManager;
#endif

  static void NotifyObserversOfSpecificNetworkChangeWithConnectionType(
 ...
 private:
  void NotifyObserversOfSpecificNetworkChangeWithConnectionTypeImpl(
      NetworkChangeType type,
      NetworkHandle network,
      ConnectionType connection_type,
      bool is_vpn);

  const scoped_refptr<base::ObserverListThreadSafe<RunInBackgroundObserver>>
      run_in_background_observer_list_;

  bool is_in_background_ = false;

  // Record the most recent time of the network change event.
  base::Time network_change_time_;
#if defined(OS_ANDROID) || BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  static int force_update_network_type_count_;
#endif
#endif
