...
#if BUILDFLAG(TTNET_IMPLEMENT)
  virtual void AppendConnectionAttempt(const ConnectionAttempt& attempt) {}
  virtual void SetTcpKeepAliveOptions(bool keep_alive,
                                      int keep_idle,
                                      int keep_interval,
                                      int keep_count) {}

  virtual void SetNewSocketTimeoutParam(
      const SocketTimeoutParam& socket_timeout_param) {}

  void TryFillDnsInfo(LoadTimingInfo::ConnectTiming* connect_timing) const {
    if (connect_timing) {
      connect_timing->dns_info.address_list =
          connect_timing_.dns_info.address_list;
      connect_timing->dns_info.cache_stale_reason =
          connect_timing_.dns_info.cache_stale_reason;
      connect_timing->dns_info.race_dns_stale_cache =
          connect_timing_.dns_info.race_dns_stale_cache;
    }
  }

  void SetConnectTiming(const LoadTimingInfo::ConnectTiming& connect_timing) {
    connect_timing_ = connect_timing;
  }

  virtual int32_t GetSocketBufferSize(bool send_buf) const;
