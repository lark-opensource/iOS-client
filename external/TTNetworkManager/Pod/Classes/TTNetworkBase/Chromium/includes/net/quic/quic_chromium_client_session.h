...
class NET_EXPORT_PRIVATE QuicChromiumClientSession
 ...
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    void CollectQuicInfo(bool should_reset_stats) const;

    void ReportQuicError(quic::QuicErrorCode error) const;

    void SetIsFirstStream(bool is_first_stream) const;

    void SetDelegate(base::WeakPtr<HttpNetworkTransaction> delegate);

    bool GetSSLInfo(SSLInfo* ssl_info) const;

    bool GetRemoteEndpoint(IPEndPoint* endpoint);

    base::WeakPtr<QuicChromiumClientSession> session() const;

    quic::QuicErrorCode quic_error() const { return quic_error_; }
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
                         quic::ConnectionCloseSource close_source,
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    quic::ConnectionCloseSource close_source_;
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    // Each stream could only collect QUIC stats with resetting once.
    mutable bool could_collect_quic_info_;
    base::WeakPtr<HttpNetworkTransaction> delegate_;
#if BUILDFLAG(TTQUICHE_IMPLEMENT)
   public:
    void SetHttpRequest(ttquiche::HttpRequest* http_request);

   private:
    ttquiche::HttpRequest* http_request_ = nullptr;
#endif
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
      bool show_enable_ping_without_active_dynamic_streams,
      LoadTimingInfo::ConnectTiming connect_timing,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void SetReadingYieldTime(const uint64_t yield_time) override;
  ...
  void SetSocketReadStats(uint64_t num_socket_read_called,
                          uint64_t socket_read_time) override;
#endif
  void UpdateTTNetQuicHostConfig(const quic::QuicHostConfig& quic_host_config);
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  int32_t GetMaxRecvWindowSize() const { return session_max_recv_window_size_; }
...
#if BUILDFLAG(TTQUICHE_IMPLEMENT)
  void SetSessionHandle(ttquiche::SessionHandle* session_handle);

 private:
  ttquiche::SessionHandle* session_handle_ = nullptr;
#endif
