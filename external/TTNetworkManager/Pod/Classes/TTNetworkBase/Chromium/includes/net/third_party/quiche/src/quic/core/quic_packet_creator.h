...
#include "net/tt_net/fec/quic_fec_framework_sender.h"
#endif
...
class QUIC_EXPORT_PRIVATE QuicPacketCreator
    : public QuicFecFrameworkSender::Delegate,
      public QuicFecFrameworkSender::DebugDelegate {
#else
class QUIC_EXPORT_PRIVATE QuicPacketCreator {
#endif
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
    virtual void OnFecSourceSymbolSent(uint32_t /*source_symbol_id*/) {}
    virtual void OnFecRepairSymbolSent(uint32_t /*source_symbol_id*/) {}
#endif
  };
...
#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
  void CreateAndSerializeFecFrame(TransmissionType transmission_type);
  void EnableFecSender(QuicFecScheme scheme,
                       int num_source_symbols,
                       int num_repair_symbols,
                       QuicConnectionStats* stats);

  // QuicFecFrameworkSender::Delegate
  void ConsumeFecFrame() override;
  // QuicFecFrameworkSender::DebugDelegate
...
#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
  bool fec_sender_enable_ = false;
  std::unique_ptr<QuicFecFrameworkSender> fec_sender_;
#endif
