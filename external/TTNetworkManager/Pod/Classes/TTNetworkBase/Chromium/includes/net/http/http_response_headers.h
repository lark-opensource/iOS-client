...
class NET_EXPORT HttpResponseHeaders
 ...
 public:
  ...
  TTNET_IMPLEMENT_EXPORT
  bool GetNormalizedHeader(base::StringPiece name, std::string* value) const;
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Fetch the "normalized" key and value of multiple headers which starts
  // with prefix, where all values for the header name are separated by commas.
  // See the GetNormalizedHeaders for format details.
  // Returns false if this header wasn't found.
  bool GetHeadersByPrefix(const std::string& prefix,
                          base::StringPairs& pairs) const;

  // Find the header which starts with prefix in our list (case-insensitive)
  // starting with parsed_ at index |from|.  Returns string::npos if not found.
  size_t FindHeaderByPrefix(size_t from,
                            const std::string& prefix,
                            std::string* name) const;
  std::string ToJsonString() const;
#if BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  void MockResponseCodeForTesting(int code) { response_code_ = code; }
#endif
#endif
