...
// TODO(zhangtaotao.1999)
// BUILDFLAG(TTQUICHE_IMPLEMENT)
#if BUILDFLAG(TTNET_IMPLEMENT)

#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
enum class QuicFecScheme : uint8_t;
#endif

struct QuicHostConfig {
  QuicHostConfig();
  QuicHostConfig(const QuicHostConfig& other);
  ~QuicHostConfig();
  ...
  struct FecConfig {
    FecConfig();
    FecConfig(const FecConfig& other);
    ~FecConfig();

    bool fec_enable;
    std::vector<QuicFecScheme> fec_encoder_schemes_list;
    std::vector<QuicFecScheme> fec_decoder_schemes_list;
    int fec_nss;
    int fec_nrs;
  };
  ...
  struct BbrConfig {
    BbrConfig();
    BbrConfig(const BbrConfig& other);
    ~BbrConfig();

    // BBR congestion control parameters
    // The pacing gain applied during the STARTUP phase.
    float high_gain;
    // The CWND gain applied during the STARTUP phase.
    float high_cwnd_gain;
    // The pacing gain applied during the DRAIN phase.
    float drain_gain;
    // The CWND gain applied during the PROBEBW phase.
    float cwnd_gain;
    // The pacing gain applied during the PROBERTT phase.
    float probe_rtt_gain;
    // The CWND gain applied during the PROBERTT phase.
    float probe_rtt_cwnd_gain;
    float startup_loss_threshold;
    // When slower_startup_ is 1: The gain used in STARTUP after loss has been
    // detected. 1.5 is enough to allow for 25% exogenous loss and still observe
    // a 25% growth in measured bandwidth. When slower_startup_ is 2: decreases
    // the rate in STARTUP by the total number of bytes lost in STARTUP divided
    // by CWND.
    float startup_rate_reduction_gain;
    float similar_min_rtt_threshold;

    // The duration during PROBERTT phase.
    int probe_rtt_mode_ms;
    // The minimum congestion control window
    int cwnd_min;
    // Slow down pacing rate if
    // bytes_lost_while_detecting_overshooting_ *
    // bytes_lost_multiplier_while_detecting_overshooting_ > IW.
    int bytes_lost_multiplier_while_detecting_overshooting;
    // When true, pace at 1.5x and disable packet conservation in STARTUP.
    int slower_startup;
    int min_rtt_expire_s;
    // The number of RTTs to stay in STARTUP mode.  Defaults to 3.
    int num_startup_rtts;
    int startup_full_loss_count;

    // Disable PROBE_RTT
    bool probe_rtt_disable;
    ...
    bool detect_overshooting;
    // Disable Recovery Window
    bool disable_recovery_state;
    ...
    bool drain_to_target;
    // When true, add the most recent ack aggregation measurement during
    // STARTUP.
    bool enable_ack_aggregation_during_startup;
    // When true, expire the windowed ack aggregation values in STARTUP when
    // bandwidth increases more than 25%.
    bool expire_ack_aggregation_in_startup;
    bool one_mss_conservation;
  };

  BbrConfig bbr_config;

  // Configs in class QuicSentPackageManager.
  // congestion control algorithm
  int congestion_control_type;
  // Initial congestion window size
  int icwnd_size;
  // HandShake timeout backoff flag.
  int handshake_timeout_backoff;
  // The maximum HandSake timeout.
  int handshake_timeout_max_ms;
  // The maximum TLP timeout.
  int tlp_max_ms;
  // The Maximum RTO timeout.
  int rto_max_ms;
  // Pacing send flag.
  bool disable_pacing_send;
  // The minimum RTO.
  int min_send_packet_rto_timeout;
  // The maximum amount of time to wait before sending an acknowledgement.
  int delayed_ack_time;
  bool detect_spurious_loss;

  // Configs in class QuicStreamFactory.
  // Set to store server configs on disk to support QUIC 0-RTT connect.
  int max_server_configs_stored;
  // Configs in class QuicConnection.
  int64_t ping_keepalive_interval;
  ...
  int32_t stream_recv_window_size;
  ...
  int32_t socket_receive_buffer_size;
  // Maximum time the session can be alive before crypto handshake is finished.
  int max_time_before_crypto_handshake_seconds;
  // Maximum idle time before the crypto handshake has completed.
  int max_idle_time_before_crypto_handshake_seconds;
  // Maximum time the session can be alive before 0-RTT crypto handshake is
  // finished.
  int max_time_before_0rtt_crypto_handshake_seconds;
  // Maximum idle time before 0-RTT crypto handshake has completed.
  int max_idle_time_before_0rtt_crypto_handshake_seconds;
  // Maximum packet write retry count.
  int max_packet_write_retry_count;
  // The maximum retransmittable packets could be received before ACK.
  int max_retransmittable_packets_before_ack;
  ParsedQuicVersion version;
  bool enable_packet_wall_timestamp;
  int yield_after_packets;
  // Set to keep on reading packets before reach this time.
  int yield_after_duration;
  bool enable_retry_connect_other_dns_vip;

  bool use_adaptive_reorder;
  bool remove_packet_once;
  bool retry_on_quic_errors;
  bool retry_without_alt_svc_on_quic_errors;
  bool keep_hint_when_broken_expired;
  bool enable_save_ietf_quic_to_servers_map;
  bool enable_ietf_quic_0rtt_reinvoke_fix;

  // Disable QUIC even the response has quic alt-svc.
  bool ignore_quic_alt_svc;
  // Enable ack frequency.
  bool enable_ack_frequency;

  struct QPackConfig {
    // Disable encoding header with :path key using qpack, encoding literally
    bool disable_path_qpack_header = false;
    // Disable encoding header whose key already exists in dynamic table,
    // encoding literally
    bool disable_diff_key_qpack_headers = false;
    // Disable encoding header whose key exists in disable_qpack_headers,
    // encoding literally
    std::unordered_set<std::string> disable_qpack_headers;
  };
  QPackConfig qpack_config;
};
#endif
