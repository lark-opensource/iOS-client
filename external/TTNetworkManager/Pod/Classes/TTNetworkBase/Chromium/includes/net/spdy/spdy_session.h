...
#if BUILDFLAG(TTNET_IMPLEMENT)
  TTNET_ERROR_UNSPECIFIED = -1,
  TTNET_ERROR_PROMISED_STREAM_ID_PARITY_ERROR = 59,
  TTNET_ERROR_ASSOCIATED_STREAM_ID_PARITY_ERROR = 60,
  TTNET_ERROR_PUSHED_STREAM_ID_OUT_OF_ORDER = 61,
  TTNET_ERROR_SETTINGS_ENABLE_CONNECT_PROTOCOL_ERROR = 62,
  TTNET_ERROR_HEADER_FRAME_PARSING_ERROR = 63,
  TTNET_ERROR_GOAWAY_FRAME_PARSING_ERROR = 64,
  TTNET_ERROR_HEADER_NO_STATUS_FIELD = 65,
  TTNET_ERROR_HEADER_STATUS_PARSING_ERROR = 66,
  TTNET_ERROR_REQUEST_RESPONSE_STREAM_OUT_OF_ORDER = 67,
  TTNET_ERROR_REQUEST_INVALID_PUSH_STREAM_TRAILERS = 68,
  TTNET_ERROR_GET_HEADER_FRAME_AFTER_TRAILERS = 69,
  TTNET_ERROR_GET_DATA_FRAME_BEFORE_HEADER_FRAME = 70,
  TTNET_ERROR_GET_DATA_FRAME_AFTER_TRAILERS = 71,
  TTNET_ERROR_GET_TRANSFER_ENCODING_HEADER = 72,
  // Next free value.
  NUM_SPDY_PROTOCOL_ERROR_DETAILS = 73,
#else
  // Next free value.
...
class NET_EXPORT SpdySession : public BufferedSpdyFramerVisitorInterface,
 ...
 public:
  ...
  ClientSocketHandle* GetClientSocketHandle() const {
    return client_socket_handle_.get();
  }

  const std::map<spdy::SpdyStreamId, SpdyStream*> GetActiveStreamMap() const {
    return active_streams_;
  }

  void StartPeriodPing(const base::TimeDelta& interval);

  // Drain idle timeout session when resume from backgroud.
  bool DrainSessionIfIdleTimeout(int timeout);

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  int32_t GetStreamMaxRecvWindowSize() const {
    return stream_max_recv_window_size_;
  }
  int32_t GetSessionMaxRecvWindowSize() const {
    return session_max_recv_window_size_;
  }
  // The value of |size| only be -1 or a positive number greater than 20KB.
  // If the |size| is -1, then recover the default value of received window
  // size for session and stream which are 15MB and 6MB.
  // If the session is not created yet, then |new_session| is true.
  // If the session is already there, then |new_session| is false
  // and WINDOW_UPDATE frame will be sent to peer to do flow control.
  void SetMaxRecvWindowSize(int size, bool new_session);
#endif

#if BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  void SetLastActivityTimeForTesting(int64_t time) {
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Send the PING frame at fixed interval which defined by
  // |ping_keepalive_interval_|.
  void SendPingFrameAtInterval();

  // Detail the error to all active streams with stream id's greater than
  // |stream_id|. After this function, StartGoingAway() will
  // be called.
  void DetailActiveStreams(spdy::SpdyStreamId stream_id,
                           int spdy_protocol_error,
                           int rfc_7540_error = -1);

  // Detail the error to the stream pointed to by the given iterator. Note
  // that |stream| may hold the last reference to the session.
  void DetailActiveStreamIterator(const ActiveStreamMap::iterator it,
                                  int spdy_protocol_error,
                                  int rfc_7540_error = -1);

  // Disable http2 stream priority for TNC
  bool IsH2StreamPriorityEnabled() const;
#endif
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  spdy::SettingsMap initial_settings_;
#else
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // True if spdy session initiates to detect connection availability
  // at fixed interval based on PING frame.
  bool enable_ping_keepalive_{false};

  // RFC 7540, 6.5.2: Received value for SETTINGS_HEADER_TABLE_SIZE which is 4KB
  // by default.
  uint32_t settings_header_table_size_{4096u};

  // Initial the HTTP2 PING timer to detect connection availability.
  // |ping_send_timer_| repeats interval is defined by
  // |ping_keepalive_interval_|.
  base::TimeDelta ping_keepalive_interval_;
  base::RepeatingTimer ping_send_timer_;

  bool has_added_power_monitor_observer_{false};
  // Last activity utc time for current spdy session.
