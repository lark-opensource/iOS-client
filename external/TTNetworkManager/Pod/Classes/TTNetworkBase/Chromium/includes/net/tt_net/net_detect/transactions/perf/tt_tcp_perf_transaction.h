...
#ifndef NET_TT_NET_NET_DETECT_TRANSACTIONS_PERF_TT_TCP_PERF_TRANSACTION_H_
#define NET_TT_NET_NET_DETECT_TRANSACTIONS_PERF_TT_TCP_PERF_TRANSACTION_H_
...
class TTTcpPerfTransaction : public TTBasePerfTransaction {
 public:
  TTTcpPerfTransaction(const DetectTarget& parsed_target,
                       base::WeakPtr<TTNetDetectTransactionCallback> callback);
  std::unique_ptr<BaseDetectReport> GetDetectReport() const override;

 private:
  struct PerfTcpProtocol {
    uint8_t version{0};
    uint16_t data_len{0};
    uint64_t task_id{0};
  ...
  PerfTcpProtocol protocol_;

  base::WeakPtrFactory<TTBasePerfTransaction> weak_factory_{this};
  DISALLOW_COPY_AND_ASSIGN(TTTcpPerfTransaction);
};

}  // namespace tt_detect
}  // namespace net

#endif  // NET_TT_NET_NET_DETECT_TRANSACTIONS_PERF_TT_TCP_PERF_TRANSACTION_H_
