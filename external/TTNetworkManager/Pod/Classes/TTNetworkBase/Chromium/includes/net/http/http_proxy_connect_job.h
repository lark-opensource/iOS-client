...
class NET_EXPORT_PRIVATE HttpProxySocketParams
 ...
 private:
  const bool is_ttnet_tunnel_;
  const std::map<std::string, std::string> ttnet_proxy_auth_;
#endif
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_PROXY_SUPPORT)
 private:
  void OnTTNetProxyInfoObtain(TTNetProxyInfo ttnet_proxy_info);

  OnTTNetProxyInfoCallback ttnet_proxy_info_callback_;
#endif
#endif
