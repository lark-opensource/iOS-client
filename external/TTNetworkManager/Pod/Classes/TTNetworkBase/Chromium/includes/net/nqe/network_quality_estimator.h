...
class NetLog;
#if BUILDFLAG(TTNET_IMPLEMENT) && defined(OS_ANDROID) && \
    BUILDFLAG(ENABLE_WEBSOCKETS)
class Message;
#endif
...
class TTNET_IMPLEMENT_EXPORT NET_EXPORT_PRIVATE NetworkQualityEstimator
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(NetworkQualityEstimatorTest,
                           TestEffectiveConnectionTypeCalculation);
#endif
#endif
 ...
 public:
  ...
  NetworkChangeNotifier::ConnectionType GetConnectionType() const {
    return current_network_id_.type;
  }
  ...
  void SetIsMainProcess(bool is_main_process) {
    is_main_process_ = is_main_process;
  }
  void DispatchIpcMessage(const Message& message);
#endif

#if BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  void SetRecentStartTimeForTesting(const base::TimeTicks recent_start_time);
#endif

 protected:
#if BUILDFLAG(ENABLE_NQE_AND_WIFI_TO_CELL_ON_MOBILE)
  friend class TTMultiNetworkManagerMain;
#endif

  // Forces computation of group rtt estimate. It has no observer notification,
  // but can directly get the computed value as a dictionary value in
  // OnRTTOrThroughputEstimatesComputed() callback.
  void ComputeGroupRTTEstimate();

 private:
#if defined(OS_ANDROID) && BUILDFLAG(ENABLE_WEBSOCKETS)
  void HandleSubProcHttpRtt(const Message& message);
  void HandleSubProcTransportRtt(const Message& message);
  void HandleSubProcSpdyPingRtt(const Message& message);
  void HandleMainProcConnType(const Message& message);
  void HandleMainProcRtt(const Message& message);
  bool IsMainProcAndEnableInterProc() const;
  bool IsSubProcAndEnableInterProc() const;
  ...
  base::TimeTicks recent_start_time_for_testing_ = base::TimeTicks();
#endif  // BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
