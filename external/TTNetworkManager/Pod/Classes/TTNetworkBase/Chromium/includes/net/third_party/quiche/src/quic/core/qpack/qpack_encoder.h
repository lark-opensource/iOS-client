...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void set_disable_path_qpack_header(bool disable_path_qpack_header) {
    disable_path_qpack_header_ = disable_path_qpack_header;
  }
  void set_disable_diff_key_qpack_headers(bool disable_diff_key_qpack_headers) {
    disable_diff_key_qpack_headers_ = disable_diff_key_qpack_headers;
  }
  void disable_qpack_header(const std::string& key) {
    disable_qpack_headers_.insert(key);
  }

 private:
  bool disable_path_qpack_header_ = false;
  std::unordered_set<std::string> disable_qpack_headers_;
  bool disable_diff_key_qpack_headers_ = false;
#endif
