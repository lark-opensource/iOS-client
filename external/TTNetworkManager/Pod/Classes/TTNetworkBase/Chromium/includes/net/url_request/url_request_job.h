...
class NET_EXPORT URLRequestJob {
 public:
  ...
  // finished.
  virtual void TryFillLoadTimingInfo(LoadTimingInfo* load_timing_info) const;

  // Try to get partial response infomation when the request is not normally
  // finished.
  virtual void TryFillResponseInfo(HttpResponseInfo* response_info) const;

  virtual void TryFillHttp2Info(LoadTimingInfo* load_timing_info) const;

  // Try to get partial QUIC infomation when the request is not normally
  // finished.
  virtual void TryFillQuicInfo(LoadTimingInfo* load_timing_info) const;

  virtual void TryFillCompressInfo(LoadTimingInfo* load_timing_info) const;

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_STREAM_ZSTD)
  // Try to get response zstd infomation
  virtual void TryFillSourceStreamInfo(LoadTimingInfo* load_timing_info) const;
#endif
  virtual void SetRequestDelay(base::TimeDelta delay);
  ...
  virtual int GetRetryAttempts() const;

  // Get the error code list which caused internal reset and resend. empty means
  // no reset and resend happend.
  virtual std::vector<int> GetNetErrorListForReset() const;

  // Gets the related socket pool information of the socket that the network
  // stack is currently fetching the URL from.
  virtual void GetSocketPoolInfo(SocketPoolInfo* endpoint) const;

  // Get specific http proto type (HTTP/1.1, HTTP/2, QUIC) from http header
  int GetTargetHttpProtoType() const;

  void SetThrottleMaxNetSpeed(int64_t bytesPerSecond);
  ...
  virtual void SetSessionMaxRecvWindowSize(
      int32_t session_max_recv_window_size);
#endif
#if BUILDFLAG(ENABLE_MULTINETWORK_ON_MOBILE)
  virtual void GetNetworkIsolationKey(NetworkIsolationKey& isolation_key) const;
#endif
#endif
 ...
 private:
  ...
  int read_throttle_{std::numeric_limits<int>::max()};
#endif
