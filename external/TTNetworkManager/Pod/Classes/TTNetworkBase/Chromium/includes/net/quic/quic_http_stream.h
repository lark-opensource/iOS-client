...
class NET_EXPORT_PRIVATE QuicHttpStream : public MultiplexedHttpStream {
 public:
  ...
  void TryFillQuicInfo(LoadTimingInfo* load_timing_info) override;
  void TryFillCompressInfo(LoadTimingInfo* load_timing_info) override;

 private:
  // Quic stream id. Set when the stream was closed. If |stream_| is failed to
  // be created, this takes on the default value of 0.
  quic::QuicStreamId closed_quic_stream_id_ = 0;
  // When use qpack, dynamic table sent through encoder stream.
  int64_t encoder_stream_sent_byte_count_ = 0;

  LoadTimingInfo::CompressInfo compress_info_;

 public:
#if BUILDFLAG(TTNET_IMPLEMENT_UNITTEST)
  quic::QuicErrorCode quic_error() const {
    return quic_session()->quic_error();
  }
#endif
#endif
