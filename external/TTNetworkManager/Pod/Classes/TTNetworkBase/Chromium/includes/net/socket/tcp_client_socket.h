...
// platforms.
    ...
    (!BUILDFLAG(TTNET_IMPLEMENT) || BUILDFLAG(TTNET_IMPLEMENT_UNITTEST))
...
class NET_EXPORT TCPClientSocket : public TransportClientSocket,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void AppendConnectionAttempt(const ConnectionAttempt& attempt) override;

  int32_t GetSocketBufferSize(bool send_buf) const override;

#if !defined(OS_WIN)
  void SetTcpKeepAliveOptions(bool keep_alive,
                              int keep_idle,
                              int keep_interval,
                              int keep_count) override;
  void SetNewSocketTimeoutParam(
      const SocketTimeoutParam& socket_timeout_param) override;
  void DisableSingleIPOptimization() override;
 ...
 private:
  ...
  // If the address list size is greater than two, the connect operation will
  // have two phases: 1. Quick test to connect each address with short timeout
  // to select a better one. 2. If none address meets the short timeout
  // threshold, do normal connect with the socket timeout decided by request
  // caller or OS. This flag indicates which phase current socket is in.
  bool is_quick_test_phase_;

  bool disable_single_ip_optimization_{false};

  int32_t send_buffer_size_{-1};
  int32_t recv_buffer_size_{-1};
#endif
