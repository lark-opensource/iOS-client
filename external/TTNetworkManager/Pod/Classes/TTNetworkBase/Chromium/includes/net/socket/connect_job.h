...
class NET_EXPORT_PRIVATE ConnectJob {
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    virtual void OnRaceDnsStaleCacheResolveResult(
        int rv,
        bool is_race_prefer,
        bool is_stale,
        ConnectJob* job,
        const std::vector<net::IPEndPoint>& endpoints) {}
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  virtual void SetRaceDnsStaleCacheEnabled(bool enable);

  bool GetRaceDnsStaleCacheEnabled() const {
    return race_dns_stale_cache_enabled_;
  }

  void SetRaceDnsStaleCachePrefered(bool prefer) {
    race_dns_stale_cache_prefered_ = prefer;
  }

  bool GetRaceDnsStaleCachePrefered() const {
    return race_dns_stale_cache_prefered_;
  }

  virtual void SetRaceDnsStaleCacheDropped(bool dropped);

  bool GetRaceDnsStaleCacheDropped() const {
    return race_dns_stale_cache_dropped_;
  }

  virtual void TryFillLoadTimingInfo(
      LoadTimingInfo* load_timing_info) const = 0;

  virtual void TryGetConnectionAttempts(ConnectionAttempts* out) const = 0;
  ...
  void set_is_backup_job(bool value) { is_backup_job_ = value; }
  bool is_backup_job() const { return is_backup_job_; }
 ...
 protected:
  Delegate* delegate_;
  bool race_dns_stale_cache_enabled_{false};
  bool race_dns_stale_cache_prefered_{false};
  bool race_dns_stale_cache_dropped_{false};
