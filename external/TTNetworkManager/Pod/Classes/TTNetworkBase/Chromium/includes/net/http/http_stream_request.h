 ...
 public:
  ...
  class NET_EXPORT_PRIVATE Delegate {
   public:
    ...
    virtual void DisallowTTNetProxyFallback() = 0;
    virtual bool IsTTNetProxyFallbackAllowed() const = 0;
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    // Called when QUIC stream is finished or failed, collect QUIC stats from
    // QUIC session.
    virtual void OnCollectQuicInfo(int64_t srtt,
                                   const std::string& client_connection_id,
                                   const std::string& server_connection_id,
                                   const quic::QuicConnectionStats& stats) = 0;

    virtual base::WeakPtr<HttpNetworkTransaction> GetWeakPtr() = 0;
#endif
  };
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    // Try best to return current timing information.
    virtual void TryFillLoadTimingInfo(
        LoadTimingInfo* load_timing_info) const = 0;

    // Returns the race result.
    virtual LoadTimingInfo::RaceResult GetRaceResult() const = 0;
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Try best to return current timing information.
  void TryFillLoadTimingInfo(LoadTimingInfo* load_timing_info) const;

  // Get race result from helper_.
  LoadTimingInfo::RaceResult GetRaceResult() const;

  void TryGetConnectionAttempts(ConnectionAttempts* out) const;
#endif
