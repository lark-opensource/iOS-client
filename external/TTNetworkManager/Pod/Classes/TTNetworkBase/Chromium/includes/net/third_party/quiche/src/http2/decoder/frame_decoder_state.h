...
class QUICHE_EXPORT_PRIVATE FrameDecoderState {
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_FRONTIER_SUPPORT)
    // only enable this DCHECK in http2 decode.
    if (frontier_listener_ == nullptr) {
      DCHECK(IsPaddable()) << frame_header();
    }
#else
    DCHECK(IsPaddable()) << frame_header();
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_FRONTIER_SUPPORT)
  friend class net::FrontierFrameDecoder;
#endif
 ...
 public:
  void set_frontier_listener(net::FrontierFrameDecoderListener* listener) {
    frontier_listener_ = listener;
  }
  net::FrontierFrameDecoderListener* frontier_listener() const {
    return frontier_listener_;
  }

  template <class S>
  DecodeStatus StartDecodingVarintInPayload(S* out, DecodeBuffer* db) {
    DecodeStatus status =
        structure_decoder_.Start(out, db, &remaining_payload_, true);
    if (status != DecodeStatus::kDecodeError) {
      return status;
    }
    return ReportFrameSizeError();
  ...
  template <class S>
  DecodeStatus ResumeDecodingVarintInPayload(S* out, DecodeBuffer* db) {
    DecodeStatus status = structure_decoder_.Resume(out, db, &remaining_payload_, true);
    if (status == DecodeStatus::kDecodeDone) {
      return status;
    } else if ((status == DecodeStatus::kDecodeInProgress) && (remaining_payload_ > 0)) {
      return DecodeStatus::kDecodeInProgress;
    } else {
      return ReportFrameSizeError();
    }
  ...
  DecodeStatus ReadFrontierStreamWeight(uint32_t* out, DecodeBuffer* db);

  DecodeStatus ReadFrontierPadLength(uint32_t* out, DecodeBuffer* db);

  bool SkipFrontierPadding(DecodeBuffer* db);

  http2::DecodeStatus StartDecodingFrontierFrameHeader(DecodeBuffer* db);
  http2::DecodeStatus ResumeDecodingFrontierFrameHeader(DecodeBuffer* db);

  bool SetRemainingMeta(int64_t meta_length) {
    // meta_length_size has been stripes from remaining_payload_, no longer need
    // stripes here.
    if (meta_length > remaining_payload_) {
      ReportFrameSizeError();
      remaining_meta_ = 0;
      return false;
    }

    remaining_meta_ = meta_length;
    remaining_payload_ -= meta_length;
    return true;
  }

  int64_t AvailableMeta(DecodeBuffer* db) const {
    return db->MinLengthRemaining(remaining_meta_);
  }

  void ConsumeMeta(size_t amount) { remaining_meta_ -= amount; }

  int64_t remaining_meta() const { return remaining_meta_; }

  void InitializeFrontierRemainders() {
    remaining_payload_ = frontier_frame_header_.payload_length;
    remaining_meta_ = 0;
    remaining_padding_ = 0;
  }

  int64_t remaining_payload_and_meta_and_padding() const {
    return remaining_payload_ + remaining_padding_ + remaining_meta_;
  }

  const net::FrontierFrameHeader& frontier_frame_header() const {
    return frontier_frame_header_;
  }

 private:
  enum DecodingFrontierHeaderState {
    kReadMagic,
    kStartDecodingPayloadLength,
    kResumeDecodingPayloadLength,
    kReadType,
    kReadFlags,
    kStartDecodingStreamId,
    kResumeDecodingStreamId,
    kStartDecodingMessageId,
    kResumeDecodingMessageId,
  };

  DecodingFrontierHeaderState header_state_;
  net::FrontierFrameDecoderListener* frontier_listener_ = nullptr;
  net::FrontierFrameHeader frontier_frame_header_;

  int64_t remaining_meta_;
#endif
};
