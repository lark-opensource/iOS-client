...
class NET_EXPORT WebSocketHandshakeStreamBase : public HttpStream {
 public:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    virtual std::unique_ptr<WebSocketHandshakeStreamBase> CreateQuicStream(
        std::unique_ptr<QuicChromiumClientSession::Handle> session) = 0;
#endif
  };
  ...
  virtual std::unique_ptr<WebSocketStream> Upgrade(
      const HttpResponseInfo& info) = 0;
  void TryFillHttp2Info(LoadTimingInfo* load_timing_info) override {}
