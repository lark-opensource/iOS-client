...
class NET_EXPORT HttpServerProperties
 ...
 public:
  ...
  HttpServerProperties(bool is_ttquiche_module,
                       std::unique_ptr<PrefDelegate> pref_delegate = nullptr,
                       NetLog* net_log = nullptr,
                       const base::TickClock* tick_clock = nullptr,
                       base::Clock* clock = nullptr);
#endif  // BUILDFLAG(TTQUICHE_IMPLEMENT)
  void ClearQuicAlternativeServiceInfoVector();
 ...
 private:
  ...
  void OnPrefsLoaded(std::unique_ptr<ServerInfoMap> server_info_map,
                     ...
                     std::unique_ptr<QuicServerInfoMap> quic_server_info_map,
#if BUILDFLAG(TTNET_IMPLEMENT)
                     std::unique_ptr<TTHttpServerProperties::IetfQuicSessionMap>
                         ietf_quic_session_map,
#endif
                     std::unique_ptr<BrokenAlternativeServiceList>
                         broken_alternative_service_list,
                     std::unique_ptr<RecentlyBrokenAlternativeServices>
...
#if BUILDFLAG(TTNET_IMPLEMENT)
 public:
  TTHttpServerProperties& tt_http_server_properties() {
    return tt_http_server_properties_;
  }
 private:
  friend class TTHttpServerProperties;
#if BUILDFLAG(TTQUICHE_IMPLEMENT)
  const bool is_ttquiche_module_;
#endif
  TTHttpServerProperties tt_http_server_properties_;
#endif  // BUILDFLAG(TTNET_IMPLEMENT)
