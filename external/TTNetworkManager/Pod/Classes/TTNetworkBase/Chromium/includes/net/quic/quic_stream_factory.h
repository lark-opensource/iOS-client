...
#include "net/tt_net/throttle/tt_throttle_monitor.h"
#endif

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT)
#include "net/tt_net/tudp/bis_quic_connection.h"
#endif
...
class NET_EXPORT_PRIVATE QuicStreamRequest {
 public:
#if BUILDFLAG(TTNET_IMPLEMENT)
  explicit QuicStreamRequest(
      QuicStreamFactory* factory,
      base::WeakPtr<HttpNetworkTransaction> delegate = nullptr);
#else
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT) && \
    BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
      CompletionOnceCallback callback,
      base::WeakPtr<BisQuicConnection::Delegate> bis_delegate = nullptr);
#elif BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT) && \
    !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
      CompletionOnceCallback callback,
      int session_max_recv_window_size = -1);
#elif !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT) && \
    !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
      CompletionOnceCallback callback,
      int session_max_recv_window_size = -1,
  ...
  enum TestStatus {
    NO_TESTING,
    FAIL_WITH_DEFAULT_NETWORK,
    FAIL_WITH_ERROR_CODE,
    FAIL_WITH_NULL_HANDLE,
    SUCCESS
  };

  int RequestForTesting(
      TestStatus status,
      int result,
      CompletionOnceCallback failed_on_default_network_callback,
      CompletionOnceCallback callback);
  ...
  void SetConnectionIdleTimeout(int64_t idle_timeout) {
    connection_idle_timeout_seconds_ = idle_timeout;
  }
  int64_t GetConnectionIdleTimeout() const {
    return connection_idle_timeout_seconds_;
  }
  ...
  void OnQuicErrorCode(quic::QuicErrorCode error) const;

  void CollectQuicInfoFromQuicStreamFactory();

  base::WeakPtr<HttpNetworkTransaction> delegate() const { return delegate_; }

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT)
  base::WeakPtr<BisQuicConnection::Delegate> bis_delegate() const {
    return bis_delegate_;
  }
#endif
#endif
 ...
 private:
  ...
  int64_t connection_idle_timeout_seconds_{0};

  bool is_internal_retry_req_{false};

  base::WeakPtr<HttpNetworkTransaction> delegate_;
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_TUDP_SUPPORT)
  base::WeakPtr<BisQuicConnection::Delegate> bis_delegate_;
#endif
#endif
...
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
      public ThrottleMonitor::Observer,
#endif
#if BUILDFLAG(TTNET_IMPLEMENT)
      public NetworkChangeNotifier::RunInBackgroundObserver,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    quic::ParsedQuicVersion quic_version = quic::ParsedQuicVersion::Q043();
#endif
   private:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
                      const quic::QuicHostConfig& quic_host_config,
#endif
                      IPEndPoint addr,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_THROTTLE_MONITOR)
  // ThrottleMonitor methods:
  void OnThrottleChanged(const std::set<std::string>& hosts,
                         ThrottleMonitor::StreamDirection direction,
                         const int32_t bytes_per_sec,
                         ThrottleMonitor::ThrottleLevel level) override;
  void SetMaxRecvWindowSizeForSession(const QuicSessionKey& session_key,
                                      int32_t size);

 private:
  std::map<QuicSessionKey, int32_t> session_max_recv_window_size_map_;

 public:
#endif
  // NetworkChangeNotifier::RunInBackgroundObserver methods:
  void OnRunInBackground() override;

  void OnCollectQuicInfo(const QuicSessionKey& session_key,
                         base::WeakPtr<HttpNetworkTransaction> delegate);
