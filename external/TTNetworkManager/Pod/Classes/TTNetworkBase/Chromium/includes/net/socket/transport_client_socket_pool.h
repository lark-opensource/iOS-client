...
#if BUILDFLAG(TTNET_IMPLEMENT)
    scoped_refptr<SocketParams> GetOriginSocketParams() const {
      return socket_params_;
    }
#endif
   private:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // ClientSocketPool implementation:
  void TryFillLoadTimingInfo(LoadTimingInfo* load_timing_info,
                             const ClientSocketHandle* handle) const override;

  void TryGetConnectionAttempts(
      ConnectionAttempts* out,
      const ClientSocketHandle* handle) const override;

  // NetworkChangeNotifier::RunInBackgroundObserver methods:
  void OnRunInBackground() override;

  void SetPreconnectCallback(
      PreconnectOnceCallback callback,
      const GroupId& group_id,
      const SocketTimeoutParam& socket_timeout_param) override;

  static int GetMaxSocketsPerGroup(const GroupId& group_id,
                                   int max_sockets_per_group);
  static int GetIdleSocketTimeoutGroupIdConfig(const bool used,
                                               const GroupId& group_id);
...
#endif
  void InvokePreconnectCallback(PreconnectOnceCallback callback,
                                ConnectJob* job,
                                int rv);
  void FillSocketPoolInfoWhenLeave(ClientSocketHandle* handle);

  PreconnectOnceCallback preconnect_callback_;
  GroupId preconnect_group_id_;
  SocketTimeoutParam preconnect_timeout_;
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    void OnRaceDnsStaleCacheResolveResult(
        int rv,
        bool is_race_prefer,
        bool is_stale,
        ConnectJob* job,
        const std::vector<net::IPEndPoint>& endpoints) override;

    bool WasRaceDnsStaleCachePerforming() const {
      return dns_race_stale_cache_performing_;
    }

    void SetRaceDnsStaleCacheSocketParams(scoped_refptr<SocketParams> params) {
      race_dns_stale_cache_socket_params_ = params;
    }
...
#if BUILDFLAG(TTNET_IMPLEMENT)
      return NumActiveSocketSlots() <
             TransportClientSocketPool::GetMaxSocketsPerGroup(
                 group_id_, max_sockets_per_group);
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    bool dns_race_stale_cache_performing_{false};
    bool is_dns_race_stale_cache_giveup_prefer_{false};
    bool is_dns_race_stale_cache_giveup_backup_{false};
    bool is_dns_race_stale_cache_perfer_finished_{false};
    std::vector<net::IPEndPoint> race_dns_stale_cache_prefer_job_endpoints_;

    scoped_refptr<SocketParams> race_dns_stale_cache_socket_params_;
#endif
  };
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void CancelAllConnectJobs(int error);
#else
