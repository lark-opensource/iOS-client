...
class NET_EXPORT MappedHostResolver : public HostResolver {
 public:
  ...
  std::unique_ptr<HostResolver::MdnsListener> CreateMdnsListener(
      const HostPortPair& host,
      DnsQueryType query_type) override;
  void RemoveCacheEntry(const HostPortPair& host,
                        const ResolveHostParameters& parameters) override;
