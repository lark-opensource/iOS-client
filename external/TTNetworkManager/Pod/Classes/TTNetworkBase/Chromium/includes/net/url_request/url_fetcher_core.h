...
class URLFetcherCore : public base::RefCountedThreadSafe<URLFetcherCore>,
 ...
 public:
  ...
  void SetSocketTimeout(int connect_timeout,
                        int read_timeout,
                        int write_timeout);
  ...
  bool ShouldRetryRequestForError(int error);

  void OnTransactionAboutToStart(net::URLRequest* request,
  ...
  void InformDelegateResponseRedirectReceived(
      const RedirectInfo& redirect_info,
      const HttpResponseInfo& response_info);

  // Returns the request log content. Should be used after the request end.
  const std::string& GetRequestLog();

  // Record time consumed by calling |addSecurityFactor| callback.
  void SetSecurityCallbackStart(base::TimeTicks timeticks);
  void SetSecurityCallbackEnd(base::TimeTicks timeticks);

  std::string GetRequestDetails();

  const LoadTimingInfo* GetLoadTimingInfo() const { return &loadTimingInfo_; }
  ...
  void SetDelayTime(int delay_mills);
#endif

  std::unique_ptr<base::ListValue> GetUrlChainsList();

  std::string GetResponseHeadersStr();

  std::string GetRequestHeadersJsonStr() const;
  std::string GetResponseHeadersJsonStr() const;

  void SaveResponseToFileAtPathByAppend(
      const base::FilePath& file_path,
      scoped_refptr<base::SequencedTaskRunner> file_task_runner);

  void SetThrottleNetSpeed(const int64_t bytesPerSecond);

  const HttpResponseInfo* GetResponseInfo();

  void SetRequestTypeFlags(uint32_t type);
  bool IsNon2xxResponseForDownload() const;
#endif
 ...
 private:
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  Error error_{net::OK};                // Error from the request
#else
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  int cache_entry_status_;

  base::TimeDelta request_timeout_;

  SocketTimeoutParam socket_timeout_param_;

  // Save the request log when the URLRequest is released. Should only get
  // the request log after it is finished, in this case request log content
  // should be saved in this |request_log|.
  std::string request_log_;

  std::string request_details_;

  LoadTimingInfo loadTimingInfo_;

  RequestPriority requestPriority_{DEFAULT_PRIORITY};

#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_REQUEST_DELAY)
  int delay_time_{-1};
#endif

  std::vector<std::string> redirect_list_;

  std::string response_headers_str_;

  std::string request_headers_json_str_;
  std::string response_headers_json_str_;
  int64_t throttle_net_speed_;

  HttpResponseInfo reponse_info_;
  ...
  base::Time request_start_task_post_;
  base::Time request_start_task_start_;
  base::Time request_complete_task_post_;
  base::Time request_complete_task_start_;
  bool is_not_2xx_response_{false};
#endif
