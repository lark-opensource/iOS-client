...
#if BUILDFLAG(TTNET_IMPLEMENT)
   public:
    int tt_socket_layer_flags() const { return tt_socket_layer_flags_; }

   private:
    int tt_socket_layer_flags_{0};
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_PROXY_SUPPORT)
   public:
    bool is_ttnet_tunnel() const { return is_ttnet_tunnel_; }

    std::map<std::string, std::string> get_ttnet_proxy_auth() const {
      return ttnet_proxy_auth_;
    }

   private:
    bool is_ttnet_tunnel_;
    std::map<std::string, std::string> ttnet_proxy_auth_;
#endif
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Try to retrieve current on going job of |handle|'s group timing info.
  virtual void TryFillLoadTimingInfo(
      LoadTimingInfo* load_timing_info,
      const ClientSocketHandle* handle) const = 0;

  virtual void TryGetConnectionAttempts(
      ConnectionAttempts* out,
      const ClientSocketHandle* handle) const = 0;

  typedef base::OnceCallback<void(int rv,
                                  std::unique_ptr<base::DictionaryValue>)>
      PreconnectOnceCallback;

  // Set user callback for preconnect operation. When connection trigger
  // by preconnect finished, this callback will be called.
  virtual void SetPreconnectCallback(
      PreconnectOnceCallback callback,
      const GroupId& group_id,
      const SocketTimeoutParam& socket_timeout_param) = 0;
#endif
