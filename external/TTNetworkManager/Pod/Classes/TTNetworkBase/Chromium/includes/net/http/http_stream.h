...
#if BUILDFLAG(TTNET_IMPLEMENT)
  // Gets the SocketPoolInfo with this stream's connection. This is currently
  // just implemented by HttpBasicStream.
  // TODO(jason.yc): Add implementation for other type of HttpStreams.
  virtual void GetSocketPoolInfo(SocketPoolInfo* socket_pool_info) = 0;

  // Try to fill HTTP/2 protocol error details from |SpdyStream|.
  virtual void TryFillHttp2Info(LoadTimingInfo* load_timing_info) = 0;

  // Try to fill QUIC stats from QuicChromiumClientSession.
  virtual void TryFillQuicInfo(LoadTimingInfo* load_timing_info) = 0;

  virtual void TryFillCompressInfo(LoadTimingInfo* load_timing_info) = 0;

  virtual HttpResponseInfo::ConnectionInfo GetConnectionInfo() const = 0;
#endif
