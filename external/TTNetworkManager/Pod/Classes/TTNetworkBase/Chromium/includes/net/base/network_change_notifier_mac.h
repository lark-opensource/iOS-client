...
class NetworkChangeNotifierMac: public NetworkChangeNotifier {
 ...
 public:
#if defined(OS_IOS)
  std::string GetCurrentMccMnc() const override;

  // Called on the main thread on startup, afterwards on the notifier thread.
  static ConnectionType CalculateConnectionType(std::string* mcc_mnc,
                                                SCNetworkConnectionFlags flags);

  ConnectionType CalculateConnectionTypeInternal(
      std::string* mcc_mnc,
      SCNetworkConnectionFlags flags);
#else
  static ConnectionType CalculateConnectionType(SCNetworkConnectionFlags flags);
...
#endif
 private:
  class TelephonyNetworkInfoHolder;
  std::unique_ptr<TelephonyNetworkInfoHolder> telephony_network_info_holder_;
#if defined(OS_IOS)
  std::string ios_mcc_mnc_;
#endif
#endif
