...
#if BUILDFLAG(TTNET_IMPLEMENT)
// Similar to PreconnectSocketsForHttpRequest except that the callback
// will occur when operation of connecting is finished.
int TTNetPreconnectSocketsForHttpRequest(
    ClientSocketPoolManager::SocketGroupType group_type,
    const HostPortPair& endpoint,
    int request_load_flags,
    RequestPriority request_priority,
    ...
    ClientSocketPool::PreconnectOnceCallback callback);
#endif
