...
NET_ERROR(TTNET_USER_SPECIFYING_NETWORK_DISABLED, -186)

NET_ERROR(TTNET_SPECIFIED_NETWORK_UNAVAILABLE_FOR_REQUEST, -187)

NET_ERROR(TTNET_SPECIFIED_NETWORK_UNAVAILABLE_FOR_DNS, -188)

NET_ERROR(TTNET_SPECIFIED_NETWORK_UNAVAILABLE_FOR_SOCKET, -189)

NET_ERROR(TTNET_BIND_NETWORK_FAILED, -190)

// Deprecated: return origin error code when concurrent connect fail.
// In concurrent connection, if all sockets fail, throw this error.
// NET_ERROR(TTNET_SOCKET_CONCURRENT_CONNECTION_FAIL, -191)

// The duration of request exceeds request timeout settings will throw this
// error.
NET_ERROR(TTNET_REQUEST_TIMED_OUT, -192)

//
NET_ERROR(TTNET_SOCKET_CONNECTION_TIMEOUT, -193)

//
NET_ERROR(TTNET_SOCKET_READ_TIMEOUT, -194)

//
NET_ERROR(TTNET_SOCKET_WRITE_TIMEOUT, -195)

//
NET_ERROR(TTNET_UNSUPPORTED_REDIRECT, -196)

//
NET_ERROR(TTNET_APP_UPLOAD_EXCEPTION, -197)

//
NET_ERROR(TTNET_CONNECTION_TIMED_OUT, -198)

//
NET_ERROR(TTNET_APP_TIMED_OUT, -199)

// Return this error when the connection is ready.
NET_ERROR(TTNET_FT_CONNECTION_READY, -508)

// Return this error when the client is null.
NET_ERROR(TTNET_FT_INVALID_CLIENT, -509)

// Return this error when received frame with invalid format in frontier layer.
NET_ERROR(TTNET_FT_INVALID_FRAME, -510)

// Return this error when use inputs invalid format data frame.
NET_ERROR(TTNET_FT_INVALID_MSG, -511)

// Return this error when the frontier connection is invalid.
NET_ERROR(TTNET_FT_INVALID_CONNECTION, -512)

// Return this error when the frontier stream is invalid.
NET_ERROR(TTNET_FT_INVALID_STREAM, -513)

// Return this error when the ping frame cannot be sent out.
NET_ERROR(TTNET_FT_PING_FAIL, -514)

// Return this error when the pong frame cannot be received.
NET_ERROR(TTNET_FT_PONG_TIMEOUT, -515)

// Return this error when rebuild progress fail.
NET_ERROR(TTNET_FT_REBUILD_FAIL, -516)

// Return this error when received goaway frame from peer.
NET_ERROR(TTNET_FT_RECV_GOAWAY, -517)

// Return this error when receipt of stream frame contains error code.
NET_ERROR(TTNET_FT_INVALID_STREAM_RECEIPT, -518)

// Return this error when the setting frame which implict BG/FG cannot be sent.
NET_ERROR(TTNET_FT_SETINGS_FAIL, -519)

// Return this error when frontier frame contains an invalid stream id.
NET_ERROR(TTNET_FT_INVALID_STREAM_ID, -520)

// Return this error when timeout of creating stream.
NET_ERROR(TTNET_FT_CREATE_STREAM_TIMEOUT, -527)

// Return this error when the frontier stream ID is not expected.
NET_ERROR(TTNET_FT_UNEXPECTED_STREAM_ID, -528)

// Return this error when frontier stream was reset by remote point.
NET_ERROR(TTNET_FT_STREAM_RESET, -529)

// Return this error when decode frontier header failed.
NET_ERROR(TTNET_FT_UNEXPECTED_HEADER, -530)

// Return this error when no header received from frontier side.
NET_ERROR(TTNET_FT_EMPTY_HEADER, -531)

// MD5 header needs to be added to the post request
NET_ERROR(TTNET_POST_MD5_HEADER_NOT_FOUND, -540)

// Request Be Droped By MT Sandbox
NET_ERROR(TTNET_SANDBOX_DROP, -545)

// Request Be Dropped By ThrottleMonitor.
NET_ERROR(TTNET_THROTTLE_MONITOR_DROP, -550)

// HTTP2 session idle timeout when resume from background.
NET_ERROR(HTTP2_SESSION_IDLE_TIMEOUT, -552)

// Request Be Droped By URLDispatcher Traffic Control
NET_ERROR(TTNET_TRAFFIC_CONTROL_DROP, -555)

// TTNet proxy param nullptr error.
NET_ERROR(TTNET_NULL_PROXY_PARAM, -556)

// TTNet Proxy fallback error.
NET_ERROR(TTNET_PROXY_RESET_FALLBACK, -566)

// Return this error when the BiS connection is invalid.
NET_ERROR(TTNET_BIS_CONNECTION, -570)

// Return this error when the BiS stream is invalid.
NET_ERROR(TTNET_BIS_STREAM, -571)

// Return this error when the format of URL is invalid.
NET_ERROR(TTNET_BIS_INVALID_URL, -572)

// Return this error when network change in BiS layer.
NET_ERROR(TTNET_BIS_NETWORK_CHANGED, -573)

// Return this error when queue for writing is full.
NET_ERROR(TTNET_BIS_WRITE_QUEUE_FULL, -574)

// Return this error when queue for writing is recovery.
NET_ERROR(TTNET_BIS_WRITE_QUEUE_RECOVER, -575)

// Return this error when hit connection drop command.
NET_ERROR(TTNET_BIS_HIS_DROP_COMMAND, -576)

//
NET_ERROR(TTNET_MDNS_MISSING_NAME, -580)

//
NET_ERROR(TTNET_MDNS_START_FAIL, -581)

//
NET_ERROR(TTNET_MDNS_NO_INTERFACE_INFO, -582)

// Cdn api verification
NET_ERROR(TTNET_CDN_API_VERIFICATION_FAILED, -583)
...
// Return this error when request is blocked when application run in background.
NET_ERROR(TTNET_BLOCKED_BY_BACKGROUND, -596)

// Content decoding of the zstd response body failed.
NET_ERROR(TTNET_CONTENT_ZSTD_DECODING_FAILED, -597)

// Quic recvmmsg alloc memory failed.
NET_ERROR(TTNET_QUIC_RECVMMSG_ALLOC, -598)

// TTNET DNS race failed
NET_ERROR(TTNET_DNS_RACE_FAILED, -820)
// TTNet DNS stale cache race fail
NET_ERROR(TTNET_DNS_STALE_RACE_DROPPED, -821)
#endif
