...
class NET_EXPORT URLRequestContext
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  HttpDnsContext* http_dns_context() const { return http_dns_context_.get(); }

  base::Time created_time() const { return created_time_; }
  void set_compress_native_wrapper(
      std::unique_ptr<CompressNativeWrapper> compress_native_wrapper);

  CompressNativeWrapper* compress_native_wrapper() const {
    return compress_native_wrapper_.get();
  }
  // Sets the network thread priority of this URLRequestContext. This values is
  // just used in request log. The real operation of setting priority is in
  // cronet_library_loader.cc
  void set_network_thread_priority(int priority) {
    thread_priority_ = priority;
  }
  int network_thread_priority() const { return thread_priority_; }
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_MSSDK_SUPPORT)
  int mssdk_init_mode() const { return mssdk_init_mode_; }
  void set_mssdk_init_mode(int mode) { mssdk_init_mode_ = mode; }

  int64_t mssdk_init_duration() const { return mssdk_init_duration_; }
  void set_mssdk_init_duration(int64_t duration) {
    mssdk_init_duration_ = duration;
  }

 private:
  int mssdk_init_mode_ = -1;
  int64_t mssdk_init_duration_ = -1;
 ...
 private:
  ...
  int thread_priority_ = -100;
  std::unique_ptr<CompressNativeWrapper> compress_native_wrapper_;
  std::unique_ptr<HttpDnsContext> http_dns_context_;
