...
class QUIC_EXPORT_PRIVATE QuicConnectionDebugVisitor
 ...
 public:
  ...
  virtual void OnFecSourceSymbolReceived(uint32_t source_symbol_id) {}
  virtual void OnFecRepairSymbolReceived(uint32_t repair_symbol_id) {}
  virtual void OnFecPacketRevived(uint32_t source_symbol_id) {}
#endif
};
...
class QUIC_EXPORT_PRIVATE QuicConnection
 ...
 public:
  ...
  QuicConnection(QuicConnectionId server_connection_id,
                 const QuicHostConfig& quic_host_config,
                 QuicSocketAddress initial_peer_address,
                 QuicConnectionHelperInterface* helper,
                 QuicAlarmFactory* alarm_factory,
                 QuicPacketWriter* writer,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
    if (writer_) {
      writer_->UpdateQuicHostConfig(quic_host_config_);
    }
#endif
  }
  ...
  const QuicHostConfig& quic_host_config() const {
    return quic_host_config_;
  }

#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
  // FEC
  void set_negotiated_encoder_fec_scheme(QuicFecScheme scheme);
  void set_negotiated_decoder_fec_scheme(QuicFecScheme scheme);
  ...
  // Identifies whether the last packet was revived by FEC.
  void SetLastPacketRevived(bool last_packet_revived) override;

  bool GetLastPacketRevived() const {
    return last_packet_revived_;
  }

  bool IsEncoderFecSchemeEnabled(QuicFecScheme scheme) const;
  bool IsDecoderFecSchemeEnabled(QuicFecScheme scheme) const;
  const QuicHostConfig::FecConfig& fec_config() const {
    return quic_host_config_.fec_config;
  }
#endif  // BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
#endif
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  void SetAverageAckDelayTime(uint64_t ack_delay_time_us) override;

  void SetZeroRttHit(net::LoadTimingInfo::QuicInfo::ZeroRttHit zero_rtt_hit) {
    if (stats_.zero_rtt_hit !=
        net::LoadTimingInfo::QuicInfo::ZERO_RTT_NOT_HIT_REJCHLO)
      stats_.zero_rtt_hit = zero_rtt_hit;
  }

  void SetHandshakeNegotiated(bool is_handshake_negotiated) {
    stats_.is_handshake_negotiated = is_handshake_negotiated;
  }

  void SetAveragePacketBacklogTime(uint64_t backlog_time);

  void SetPacketProcessTime(uint64_t packet_process_time);

  void SetReadingYieldTime(uint64_t yield_time);

  void SetLatestPingSentTime();

  void SetLatestPingAckedTime();

  void ResetConnectionStats();

  void UpdateQuicHostConfig(const QuicHostConfig& quic_host_config);
#if !BUILDFLAG(TTNET_IMPLEMENT_DISABLE_QUIC_RECVMMSG)
  void SetRecvmmsgEnabled(bool enabled);
  void SetSocketReadStats(uint64_t num_socket_read_called,
...
#if BUILDFLAG(TTNET_IMPLEMENT)
  QuicHostConfig quic_host_config_;

#if BUILDFLAG(TTNET_IMPLEMENT_ENABLE_FEC_SUPPORT)
  // True if the last packet was revived from FEC.
  bool last_packet_revived_ = false;
#endif
#endif
