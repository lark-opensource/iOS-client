...
namespace cbox {
    ...
    size_t split(Container &tokens, const std::string &s,
        ...
        while (--limit != 0 &&
               (curr_pos = s.find_first_of(delims, prev_pos)) != std::string::npos) {
    ...
    std::string &
    replace(std::string &s, const std::string &old_value, const std::string &new_value);
    ...
    inline std::string &replace(std::string &s, char old_value, char new_value) {
        std::replace_if(s.begin(), s.end(), std::bind(std::equal_to<char>(), _1, old_value),
                        new_value);
        return s;
    }
...
// tests if string 's' starts with the specified 'prefix' beginning a specified index 'pos'
    NLE_EXPORT_METHOD  bool starts_with(const std::string &s, const std::string &prefix, size_t pos = 0);

// tests if string 's' ends with the specified 'suffix'
    NLE_EXPORT_METHOD bool ends_with(const std::string &s, const std::string &suffix);

// convert byte array to hex string
    NLE_EXPORT_METHOD std::string to_hex_string(const void *buffer, size_t length);

    NLE_EXPORT_METHOD std::string to_hex_string(const std::string &buffer);

    NLE_EXPORT_METHOD std::string from_hex_string(const std::string &hex);

    NLE_EXPORT_METHOD bool
    from_hex_string(const char *hex, size_t hex_size, unsigned char *buffer, size_t buffer_size);
