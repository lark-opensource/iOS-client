...
rb_node_compare_addr(struct file_map_store *node, struct file_map_store *parent)
{
    ...
    if (vme_c->addr < vme_p->addr) {
        return -1;
    }
    if (vme_c->addr >= (vme_p->addr+vme_p->vme_length)) {
        return 1;
    }
    return 0;
}
...
bool
file_map_store_lookup_entry_rb(file_map_header_t mapHdr, uintptr_t factor, file_map_entry_t *vm_entry, rb_tree_type tree_type)
{
    ...
    if (tree_type == rb_tree_type_addr) {
        ...
        while (rb_entry != (struct file_map_store*)NULL) {
            ...
            if (address >= cur->addr) {
                if (address < (cur->addr + cur->vme_length)) {
                    *vm_entry = cur;
                    return true;
                }
...
file_map_enumerate_rb(file_map_header_t mapHdr, rb_tree_type tree_type) {
    if (tree_type == rb_tree_type_addr) {
        ...
        RB_FOREACH(rb_entry, rb_addr_head, &mapHdr->rb_addr_head_store) {
            file_map_entry_t       cur = VME_FOR_STORE(rb_entry, tree_type);
            ff_printf("offset: %ld~~~%ld --- in_use = %s\n", cur->vme_start, cur->vme_start+cur->vme_length, cur->in_use?"true":"false");
        }
    }else {
        file_map_store* rb_entry;
