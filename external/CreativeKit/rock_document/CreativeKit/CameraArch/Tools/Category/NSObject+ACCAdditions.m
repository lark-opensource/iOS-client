...
[NSObject acc_setInv:inv withSig:sig andArgs:args]; \
va_end(args);

@implementation NSObject (ACCAdditions)

- (id)acc_performSelectorWithArgs:(SEL)sel, ...
{
    INIT_INV(sel,sel,nil);
    [inv invoke];
    return [NSObject acc_getReturnFromInv:inv withSig:sig];
}

- (void)acc_performSelectorWithArgs:(SEL)sel afterDelay:(NSTimeInterval)delay, ...
{
    INIT_INV(delay,sel, );
    [inv retainArguments];
...
- (id)acc_performSelectorWithArgsOnMainThread:(SEL)sel waitUntilDone:(BOOL)wait, ...
{
...
#pragma clang diagnostic pop
    ...
    return wait ? [NSObject acc_getReturnFromInv:inv withSig:sig] : nil;
}

- (id)acc_performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thread waitUntilDone:(BOOL)wait, ...
{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wvarargs"
    INIT_INV(wait,sel,nil);
...
- (void)acc_performSelectorWithArgsInBackground:(SEL)sel, ...
{
    INIT_INV(sel,sel, );
    [inv retainArguments];
...
+ (id)acc_getReturnFromInv:(NSInvocation *)inv withSig:(NSMethodSignature *)sig {
...
+ (void)acc_setInv:(NSInvocation *)inv withSig:(NSMethodSignature *)sig andArgs:(va_list)args {
...
[inv setArgument:&arg atIndex:index]; \
            ...
            case_size(61) case_size(62) case_size(63) case_size(64)
                ...
                fprintf(stderr, "[%s(%d):%s]\t%s\n",[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __LINE__, __func__, [[NSString stringWithFormat:@"performSelectorWithArgs unsupported type:%s (%lu bytes)", [sig getArgumentTypeAtIndex:index],(unsigned long)size] UTF8String]);
            }
#undef case_size
...
+ (BOOL)acc_swizzleInstanceMethod:(SEL)origSelector with:(SEL)newSelector
{
...
+ (BOOL)acc_swizzleClassMethod:(SEL)origSelector with:(SEL)newSelector
{
...
{
    __block NSInteger impCount = 0;
    [self enumerateImplementationOfSelector:sel UsingBlock:^(IMP imp, BOOL *stop) {
        impCount += 1;
    }];
    return impCount;
}
...
+ (void)enumerateImplementationOfSelector:(SEL)sel UsingBlock:(void (^_Nullable)(IMP imp, BOOL *stop))block
{
    ...
    // get class's method list via runtime.
    Method *methodList = class_copyMethodList(self, &methodCount);
    NSString *targetMethodName = NSStringFromSelector(sel);
    BOOL stop = NO;
...
+ (NSString *)repeatSelectorImpOfProtocol:(NSArray *)protocols
{
    ...
    __block NSString *repeatSelStr = nil;
    [protocols enumerateObjectsUsingBlock:^(id  _Nonnull protocol, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        for (int i = 0; i < methodCount; i++) {
            ...
            NSInteger impCount = [self implementationCountOfSelector:methodDescription.name];
            if (impCount > 1) {
                repeatSelStr = NSStringFromSelector(methodDescription.name);
                free(methods);
                return;
            }
        }
