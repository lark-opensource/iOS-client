...
/*!@header HMDMachRestartableDeclaration.h
 * @abstract Mach Restartable 防护的参数
 *
 * @discussion [ 业务使用方 ] 使用请参考注释，若提及不可使用，请与 Heimdallr 开发讨论，务必严格准守
 *             [ Heimdallr 维护者 ] 该文档需要和 HMDCrashPreventMachRestartableDefinition.h 文件参数相同对应，请 double check 它们的关系
 */
...
/*!@const @p HMD_MACH_RESTARTABLE_MAX_COUNT
   @abstract max restartable range could be registered
 */
#ifndef HMD_MACH_RESTARTABLE_MAX_COUNT
#define HMD_MACH_RESTARTABLE_MAX_COUNT UINT64_C(10)
#endif

/*!
 * @const @p HMD_MACH_RESTARTABLE_OFFSET_MAX
   @abstract The maximum value @p length or @p recovery_offs can have.
 */
...
/*!@struct @p HMDMachRestartable_range
   @code 注意是当且仅当，PC 位置处于 [location, location + length - 1] 范围时，
