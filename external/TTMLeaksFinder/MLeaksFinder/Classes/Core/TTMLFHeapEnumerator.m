...
@implementation TTMLFHeapEnumerator

static void range_callback(task_t task, void *context, unsigned type, vm_range_t *ranges, unsigned rangeCount)
{
    tt_mlf_object_enumeration_block_t block = (__bridge tt_mlf_object_enumeration_block_t)context;
    if (!block) {
        return;
    }
    ...
    for (unsigned int i = 0; i < rangeCount; i++) {
        ...
        mlf_maybe_object_t *tryObject = (mlf_maybe_object_t *)range.address;
        Class tryClass = NULL;
#ifdef __arm64__
