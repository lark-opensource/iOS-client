...
void Processor::Process(protocol::ProtocolMessage &message) {
  const Json::Value &root = message.GetJsonValue();
  process(root);
}
...
std::string
Processor::wrapStopAtEntryMessage(const std::string &type,
...
void Processor::HandleAppAction(
    std::shared_ptr<protocol::RemoteDebugProtocolBodyData4Custom> custom_data) {
