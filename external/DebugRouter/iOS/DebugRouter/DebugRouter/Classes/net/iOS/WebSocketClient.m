...
- (BOOL)connect:(NSString *)url {
  ...
  self.socket = [[SRWebSocket alloc]
      initWithURLRequest:[NSURLRequest
                             requestWithURL:[NSURL URLWithString:url_]]];
  LLogInfo(@"[WS] connect %@", self.socket.url.absoluteString);
  self.socket.delegate = self;
  [self.socket
      setDelegateDispatchQueue:dispatch_queue_create("DebugRouter",
...
- (void)webSocket:(SRWebSocket *)webSocket
  ...
  if (webSocket == self.socket) {
    LLogWarn(@"[WS] onClose: code:%ld, reason:%@, wasClean:%d", (long)code,
             reason, wasClean);
    self.socket = nil;
    if (self.delegate_) {
...
- (void)reconnect {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                               (int64_t)(reconnect_time_ * NSEC_PER_SEC)),
