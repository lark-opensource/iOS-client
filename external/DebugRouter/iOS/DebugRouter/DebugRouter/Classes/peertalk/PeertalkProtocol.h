...
static const uint32_t PeertalkFrameNoTag = 0;

// Special frame type that signifies that the stream has ended.
static const uint32_t PeertalkFrameTypeEndOfStream = 0;

// NSError domain
FOUNDATION_EXPORT NSString *const PeertalkProtocolErrorDomain;

@interface PeertalkProtocol : NSObject
...
// When the stream ends, a frame of type PeertalkFrameTypeEndOfStream is received.
- (void)readFramesOverChannel:(dispatch_io_t)channel
                      onFrame:(void (^)(NSError *error, uint32_t type,
                                        uint32_t tag, uint32_t payloadSize,
                                        dispatch_block_t resumeReadingFrames))
                                  onFrame;

// Read a single frame over *channel*. A frame of type PeertalkFrameTypeEndOfStream
...
- (void)readAndDiscardDataOfSize:(size_t)size
                     overChannel:(dispatch_io_t)channel
                        callback:(void (^)(NSError *error,
                                           BOOL endOfStream))callback;

@end

@interface NSData (PeertalkProtocol)
...
@interface NSDictionary (PeertalkProtocol)
// See description of -[NSData(PeertalkProtocol) createReferencingDispatchData]
- (dispatch_data_t)createReferencingDispatchData;
@end
