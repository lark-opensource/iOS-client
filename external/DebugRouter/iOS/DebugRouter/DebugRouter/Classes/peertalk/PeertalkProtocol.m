...
static const uint32_t PeertalkProtocolVersion1 = 1;

NSString *const PeertalkProtocolErrorDomain = @"PeertalkProtocolError";

// This is what we send as the header for each frame.
typedef struct _PeertalkFrame {
...
static void _release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
    PeertalkProtocol *protocol = (__bridge_transfer id)objcobj;
    protocol->queue_ = NULL;
  }
}

@interface DispatchQueueLocalIOFrameProtocol : PeertalkProtocol
@end
@implementation DispatchQueueLocalIOFrameProtocol
- (void)setQueue:(dispatch_queue_t)queue {
}
@end

@implementation PeertalkProtocol

+ (PeertalkProtocol *)sharedProtocolForQueue:(dispatch_queue_t)queue {
  static const char currentQueueFrameProtocolKey;
  PeertalkProtocol *currentQueueFrameProtocol =
      (__bridge PeertalkProtocol *)dispatch_queue_get_specific(
          queue, &currentQueueFrameProtocolKey);
  if (!currentQueueFrameProtocol) {
    currentQueueFrameProtocol =
        [[DispatchQueueLocalIOFrameProtocol alloc] initWithDispatchQueue:NULL];
    currentQueueFrameProtocol->queue_ =
        queue; // reference, no retain, since we would create cyclic references
    dispatch_queue_set_specific(
        queue, &currentQueueFrameProtocolKey,
        (__bridge_retained void *)currentQueueFrameProtocol,
        &_release_queue_local_protocol);
    return (__bridge PeertalkProtocol *)dispatch_queue_get_specific(
        queue, &currentQueueFrameProtocolKey); // to avoid race conds
  } else {
...
- (dispatch_data_t)createDispatchDataWithFrameOfType:(uint32_t)type
                                            frameTag:(uint32_t)frameTag
                                             payload:(dispatch_data_t)payload {
  PeertalkFrame *frame =
      CFAllocatorAllocate(kCFAllocatorDefault, sizeof(PeertalkFrame), 0);
  frame->version = htonl(PeertalkProtocolVersion1);
  ...
  dispatch_data_t frameData = dispatch_data_create(
      (const void *)frame, sizeof(PeertalkFrame), queue_, ^{
...
- (void)sendFrameOfType:(uint32_t)frameType
  ...
  dispatch_data_t frame = [self createDispatchDataWithFrameOfType:frameType
                                                         frameTag:tag
                                                          payload:payload];
  dispatch_io_write(channel, 0, frame, queue_,
...
- (void)readFrameOverChannel:(dispatch_io_t)channel
                    callback:(void (^)(NSError *error, uint32_t frameType,
                                       uint32_t frameTag,
                                       uint32_t payloadSize))callback {
  __block dispatch_data_t allData = NULL;

  dispatch_io_read(
      channel, 0, sizeof(PeertalkFrame), queue_,
      ^(bool done, dispatch_data_t data, int error) {
        ...
        if (done) {
          ...
          if (dataSize == 0) {
            callback(nil, PeertalkFrameTypeEndOfStream, 0, 0);
            return;
          }

          if (!allData ||
              dispatch_data_get_size(allData) < sizeof(PeertalkFrame)) {
            callback([[NSError alloc] initWithDomain:PeertalkProtocolErrorDomain
                                                code:0
                                            userInfo:nil],
                     0, 0, 0);
            return;
          ...
          PeertalkFrame *frame = NULL;
          size_t size = 0;

          PT_PRECISE_LIFETIME dispatch_data_t contiguousData =
              dispatch_data_create_map(
                  allData, (const void **)&frame,
                  &size); // precise lifetime guarantees bytes in frame will
                          // stay valid till the end of scope
          if (!contiguousData) {
            callback([[NSError alloc] initWithDomain:NSPOSIXErrorDomain
                                                code:ENOMEM
                                            userInfo:nil],
          ...
          if (frame->version != PeertalkProtocolVersion1) {
            callback([[NSError alloc] initWithDomain:PeertalkProtocolErrorDomain
                                                code:0
                                            userInfo:nil],
                     0, 0, 0);
  ...
  dispatch_io_read(
      ...
      ^(bool done, dispatch_data_t data, int error) {
        ...
        if (done) {
          ...
          if (allData) {
            contiguousData = dispatch_data_create_map(
                allData, (const void **)&buffer, &bufferSize);
            if (!contiguousData) {
              callback([[NSError alloc] initWithDomain:NSPOSIXErrorDomain
                                                  code:ENOMEM
                                              userInfo:nil],
...
- (void)readAndDiscardDataOfSize:(size_t)size
  ...
  dispatch_io_read(
      channel, 0, size, queue_, ^(bool done, dispatch_data_t data, int error) {
        if (done && callback) {
          size_t dataSize = data ? dispatch_data_get_size(data) : 0;
          callback(error == 0
                       ? nil
...
- (void)readFramesOverChannel:(dispatch_io_t)channel
                      onFrame:(void (^)(NSError *, uint32_t, uint32_t, uint32_t,
                                        dispatch_block_t))onFrame {
  [self readFrameOverChannel:channel
                    callback:^(NSError *error, uint32_t type, uint32_t tag,
                               uint32_t payloadSize) {
                      onFrame(error, type, tag, payloadSize, ^{
                        if (type != PeertalkFrameTypeEndOfStream) {
...
@interface _PeertalkDispatchData : NSObject {
  dispatch_data_t dispatchData_;
}
@end
@implementation _PeertalkDispatchData
...
@implementation NSData (PeertalkProtocol)
...
- (dispatch_data_t)createReferencingDispatchData {
  ...
  return dispatch_data_create((const void *)self.bytes, self.length,
                              dispatch_get_main_queue(), ^{
                                // trick to have the block capture the data,
                                // thus retain/releasing
                                [self length];
                              });
}
...
+ (NSData *)dataWithContentsOfDispatchData:(dispatch_data_t)data {
  ...
  size_t bufferSize = 0;
  PT_PRECISE_LIFETIME dispatch_data_t contiguousData =
      dispatch_data_create_map(data, (const void **)&buffer, &bufferSize);
  if (!contiguousData) {
    return nil;
  }

  _PeertalkDispatchData *dispatchDataRef =
      [[_PeertalkDispatchData alloc] initWithDispatchData:contiguousData];
  NSData *newData = [NSData dataWithBytesNoCopy:(void *)buffer
                                         length:bufferSize
                                   freeWhenDone:NO];
  static const bool kDispatchDataRefKey;
  objc_setAssociatedObject(newData, (const void *)kDispatchDataRefKey,
                           dispatchDataRef, OBJC_ASSOCIATION_RETAIN);
...
@implementation NSDictionary (PeertalkProtocol)
  ...
  NSData *plistData = [NSPropertyListSerialization
      dataWithPropertyList:self
                   ...
                   options:0
                     error:&error];
  if (!plistData) {
    LLogError(@"Failed to serialize property list: %@", error);
    return nil;
  } else {
