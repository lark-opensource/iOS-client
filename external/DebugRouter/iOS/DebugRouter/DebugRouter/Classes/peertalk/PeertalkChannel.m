...
#define kDelegateFlagImplements_ioFrameChannel_shouldAcceptFrameOfType_tag_payloadSize \
  1
...
#pragma mark -
@implementation PeertalkChannel

@synthesize protocol = protocol_;

+ (PeertalkChannel *)channelWithDelegate:(id<PeertalkChannelDelegate>)delegate {
  return [[PeertalkChannel alloc] initWithProtocol:nil delegate:delegate];
}
...
- (id)initWithProtocol:(PeertalkProtocol *)protocol
  ...
  if (!(self = [super init]))
    return nil;
  protocol_ = protocol ? protocol
                       : [PeertalkProtocol
                             sharedProtocolForQueue:dispatch_get_main_queue()];
...
- (id)init {
  return [self
      initWithProtocol:[PeertalkProtocol
                           sharedProtocolForQueue:dispatch_get_main_queue()]
              delegate:nil];
}

- (BOOL)isConnected {
  return connState_ == kConnStateConnecting ||
...
- (void)setUserInfo:(id)userInfo {
  objc_setAssociatedObject(self, (const void *)&kUserInfoKey, userInfo,
...
- (void)setDispatchChannel:(dispatch_io_t)channel {
  assert(connState_ == kConnStateConnecting ||
         connState_ == kConnStateConnected || connState_ == kConnStateNone);
...
- (id<PeertalkChannelDelegate>)delegate {
  return delegate_;
}

- (void)setDelegate:(id<PeertalkChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
  ...
  if ([delegate respondsToSelector:@selector
                (ioFrameChannel:shouldAcceptFrameOfType:tag:payloadSize:)]) {
    delegateFlags_ |=
        kDelegateFlagImplements_ioFrameChannel_shouldAcceptFrameOfType_tag_payloadSize;
  }

  if (delegate_ && [delegate respondsToSelector:@selector(ioFrameChannel:
                                                         didEndWithError:)]) {
    delegateFlags_ |= kDelegateFlagImplements_ioFrameChannel_didEndWithError;
  }

  if (delegate_ &&
      [delegate respondsToSelector:@selector(ioFrameChannel:
                                        didAcceptConnection:fromAddress:)]) {
    delegateFlags_ |=
...
- (void)connectToPort:(int)port
           overUSBHub:(PeertalkUSBHub *)usbHub
  ...
  [usbHub connectToDevice:deviceID
      ...
      onEnd:^(NSError *error) {
        if (self->delegateFlags_ &
            kDelegateFlagImplements_ioFrameChannel_didEndWithError) {
...
- (void)connectToPort:(in_port_t)port
          IPv4Address:(in_addr_t)address
             callback:
                 (void (^)(NSError *error, PeertalkAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
    if (callback)
      callback([NSError errorWithDomain:NSPOSIXErrorDomain
  ...
  dispatch_io_t dispatchChannel =
      dispatch_io_create(DISPATCH_IO_STREAM, fd, protocol_.queue, ^(int error) {
        close(fd);
        if (self->delegateFlags_ &
            kDelegateFlagImplements_ioFrameChannel_didEndWithError) {
          NSError *err =
              error == 0 ? self->endError_
  ...
  if (!dispatchChannel) {
    close(fd);
    if (callback)
      callback([[NSError alloc] initWithDomain:@"PTError" code:0 userInfo:nil],
               nil);
    return;
  }
  ...
  // Success
  NSError *err = nil;
  PeertalkAddress *ptAddr = [[PeertalkAddress alloc]
      initWithSockaddr:(struct sockaddr_storage *)&addr];
...
- (void)listenOnPort:(in_port_t)port
         IPv4Address:(in_addr_t)address
            callback:(void (^)(NSError *error))callback {
  ...
  [self setDispatchSource:dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, fd,
                                                 0, protocol_.queue)];
  ...
  dispatch_source_set_cancel_handler(self->dispatchObj_source_, ^{
    // Captures *self*, effectively holding a reference to *self* until
    // cancelled.
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  dispatch_fd_t clientSocketFD =
      accept(serverSocketFD, (struct sockaddr *)&addr, &addrLen);
  ...
  if (delegateFlags_ &
      kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
    PeertalkChannel *peerChannel =
        [[PeertalkChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block PeertalkChannel *localChannelRef = self;
    dispatch_io_t dispatchChannel = dispatch_io_create(
        DISPATCH_IO_STREAM, clientSocketFD, protocol_.queue, ^(int error) {
          // Important note: This block captures *self*, thus a reference is
          // held to *self* until the fd is truly closed.
          localChannelRef = nil;

          close(clientSocketFD);

          if (peerChannel->delegateFlags_ &
              kDelegateFlagImplements_ioFrameChannel_didEndWithError) {
            NSError *err =
                error == 0 ? peerChannel->endError_
                           : [[NSError alloc] initWithDomain:NSPOSIXErrorDomain
                                                        code:error
                                                    userInfo:nil];
            [peerChannel->delegate_ ioFrameChannel:peerChannel
                                   didEndWithError:err];
    ...
    PeertalkAddress *address = [[PeertalkAddress alloc]
        initWithSockaddr:(struct sockaddr_storage *)&addr];
    [delegate_ ioFrameChannel:self
          didAcceptConnection:peerChannel
                  fromAddress:address];

    NSError *err = nil;
    if (![peerChannel startReadingFromConnectedChannel:dispatchChannel
                                                 error:&err]) {
      LLogInfo(@"startReadingFromConnectedChannel failed in accept: %@", err);
    }
  } else {
    close(clientSocketFD);
  }
...
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel
  ...
  if (connState_ != kConnStateNone && connState_ != kConnStateConnecting &&
      connState_ != kConnStateConnected) {
    if (error)
  ...
  BOOL (^handleError)(NSError *, BOOL) = ^BOOL(NSError *error, BOOL isEOS) {
    if (error) {
      // LLogError(@"Error while communicating: %@", error);
  ...
  [protocol_
      readFramesOverChannel:channel
                    onFrame:^(NSError *error, uint32_t type, uint32_t tag,
                              uint32_t payloadSize,
                              dispatch_block_t resumeReadingFrames) {
                      if (handleError(error,
                                      type == PeertalkFrameTypeEndOfStream)) {
                        return;
                      }
                      ...
                      if (accepted &&
                          (self->delegateFlags_ &
                           kDelegateFlagImplements_ioFrameChannel_shouldAcceptFrameOfType_tag_payloadSize)) {
                        accepted = [self->delegate_ ioFrameChannel:self
                      ...
                      if (payloadSize == 0) {
                        if (accepted && self->delegate_) {
                          [self->delegate_ ioFrameChannel:self
                                    didReceiveFrameOfType:type
                                                      tag:tag
                                                  payload:nil];
                        } else {
                        ...
                        if (!accepted) {
                          ...
                          [self->protocol_
                              readAndDiscardDataOfSize:payloadSize
                                           overChannel:channel
                              ...
                              readPayloadOfSize:payloadSize
                                    overChannel:channel
                                       callback:^(
                                         ...
                                         if (self->delegate_) {
                                           // dispatch_data_t can be cast to
                                           // (NSData *)
                                           [self->delegate_
                                                      ioFrameChannel:self
                                               didReceiveFrameOfType:type
...
- (void)sendFrameOfType:(uint32_t)frameType
  ...
  if (connState_ == kConnStateConnecting || connState_ == kConnStateConnected) {
    dispatch_data_t payloadCopy = dispatch_data_create(
        payload.bytes, payload.length, NULL, DISPATCH_DATA_DESTRUCTOR_DEFAULT);
    [protocol_ sendFrameOfType:frameType
                           tag:tag
                   withPayload:payloadCopy
                   overChannel:dispatchObj_channel_
                      callback:callback];
  } else if (callback) {
    callback([NSError errorWithDomain:NSPOSIXErrorDomain
                                 code:EPERM
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"<PeertalkChannel: %p (%@)%s%@>", self,
                       (connState_ == kConnStateConnecting
                            ? @"connecting"
                            : connState_ == kConnStateConnected
                                  ? @"connected"
...
#pragma mark -
@implementation PeertalkAddress
...
- (NSString *)name {
  if (sockaddr_.ss_len) {
    ...
    if (sockaddr_.ss_family == AF_INET6) {
      ...
      sin_addr =
          (const void *)&((const struct sockaddr_in6 *)&sockaddr_)->sin6_addr;
    } else {
      bufsize = INET_ADDRSTRLEN;
      sin_addr =
          (const void *)&((const struct sockaddr_in *)&sockaddr_)->sin_addr;
    }
    char *buf = CFAllocatorAllocate(kCFAllocatorDefault, bufsize + 1, 0);
    if (inet_ntop(sockaddr_.ss_family, sin_addr, buf,
                  (unsigned int)bufsize - 1) == NULL) {
