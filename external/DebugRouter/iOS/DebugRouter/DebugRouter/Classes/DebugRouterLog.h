...
#define LErrFatal(expression, errCode, ...)                                    \
  _DebugRouterErrorFatal(expression, errCode, __VA_ARGS__)
...
typedef NS_OPTIONS(NSInteger, DebugRouterLogSource) {
  DebugRouterLogSourceNaitve = 1 << 0
};

typedef void (^DebugRouterLogFunction)(DebugRouterLogLevel level,
                                       NSString *message);
...
@property(nonatomic, assign) DebugRouterLogSource acceptSource; // 默认为全部
@property(nonatomic, assign)
    NSInteger acceptRuntimeId; // -1为接收全部runtime log，默认为-1
...
DEBUGROUTER_EXTERN void
DebugRouterSetLogFunction(DebugRouterLogFunction logFunction)
...
DEBUGROUTER_EXTERN NSInteger DebugRouterAddLogObserver(
    DebugRouterLogFunction logFunction, DebugRouterLogLevel minLogLevel);
DEBUGROUTER_EXTERN NSInteger
DebugRouterAddLogObserverByModel(DebugRouterLogObserver *observer);
DEBUGROUTER_EXTERN DebugRouterLogObserver *
DebugRouterGetLogObserver(NSInteger observerId);
DEBUGROUTER_EXTERN void DebugRouterRemoveLogObserver(NSInteger observerId);
DEBUGROUTER_EXTERN NSArray<DebugRouterLogObserver *> *
DebugRouterGetLogObservers(void);

DEBUGROUTER_EXTERN void DebugRouterAddDebugLogObserver();

DEBUGROUTER_EXTERN void
DebugRouterSetMinLogLevel(DebugRouterLogLevel minLogLevel)
    __attribute__((deprecated("Use DebugRouterAddLogObserver instead.")));
DEBUGROUTER_EXTERN DebugRouterLogLevel DebugRouterGetMinLogLevel(void);

#define _DebugRouterLog(log_level, ...)                                        \
  _DebugRouterLogInternal(log_level, __VA_ARGS__)
DEBUGROUTER_EXTERN void _DebugRouterLogInternal(DebugRouterLogLevel, NSString *,
                                                ...) NS_FORMAT_FUNCTION(2, 3);

#define _DebugRouterErrorInfo(errCode, ...)                                    \
  _DebugRouterErrorInfoInternal(errCode, __VA_ARGS__)
...
DEBUGROUTER_EXTERN void _DebugRouterErrorWarningInternal(bool, NSInteger,
                                                         NSString *, ...);

#define _DebugRouterErrorFatal(expression, errCode, ...)                       \
  _DebugRouterErrorFatalInternal(expression, errCode, __VA_ARGS__)
DEBUGROUTER_EXTERN void _DebugRouterErrorFatalInternal(bool, NSInteger,
                                                       NSString *, ...);
